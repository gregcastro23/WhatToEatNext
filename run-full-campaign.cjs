...
async function main() {
  ...
  console.log(
    `  Documentation: ${config.enableDocumentation ? "ENABLED" : "DISABLED"}`,
  );
  console.log(
    `  Final Report: ${config.generateFinalReport ? "ENABLED" : "DISABLED"}`,
  );
  ...
  try {
    ...
    const reductionPercentage =
      initialAnyCount > 0
        ? ((initialAnyCount - finalAnyCount) / initialAnyCount) * 100
        : 0;
    const targetAchieved =
      reductionPercentage >= 15 && totalFixesApplied >= 250;
    ...
    console.log(
      `  Duration: ${Math.round((Date.now() - Date.now()) / 1000 / 60)} minutes`,
    );
    ...
    const finalReport = {
      ...
      achievements: [
        ...
        targetAchieved
          ? "✅ Successfully met campaign targets"
    ...
    console.log(
      `  Target Achieved: ${finalReport.targetAchieved ? "✅ YES" : "❌ NO"}`,
    );
...
`;
    ...
    console.log(
      "📄 Error report saved to: TASK_12_3_FULL_CAMPAIGN_ERROR_REPORT.md",
    );

...
class UnusedVariableCleanup {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        ...
        const importMatch = line.match(
          /import\s+(?:{([^}]+)}|([a-zA-Z_$][a-zA-Z0-9_$]*))(?:\s+as\s+([a-zA-Z_$][a-zA-Z0-9_$]*))?\s+from/,
        );
  ...
  processFile(filePath) {
    try {
      ...
      if (totalFixes > 0) {
        console.log(
          `📝 ${filePath}: ${totalFixes} unused cleanups (${importResult.fixes} imports, ${variableResult.fixes} variables)`,
        );

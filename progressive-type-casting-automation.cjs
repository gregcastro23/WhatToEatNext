...
function createProgressiveTypeCasting(content, filePath) {
  ...
  const typeCastingStrategies = [
    ...
    },
    {
      ...
      pattern: /\(([\w.]+)\s+as\s+any\)\??\.(coreMetrics|effectiveness|recommendations|compatibility)/g,
  ...
  for (const strategy of typeCastingStrategies) {
    ...
    while ((matches = strategy.pattern.exec(modifiedContent)) !== null) {
      ...
      if (strategy.condition(matches[0], context)) {
        const replacement = typeof strategy.replacement === 'function' 
...
async function processFile(filePath) {
  try {
    ...
    const { modifiedContent, fixes, appliedFixes } = createProgressiveTypeCasting(originalContent, filePath);

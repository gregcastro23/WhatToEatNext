...
function analyzeTypeCastingPatterns(content) {
  const patterns = {
    ...
    property_access: (content.match(/\([\w.]+\s+as\s+any\)\??\.\w+/g) || [])
      .length,
    method_calls: (content.match(/\([\w.]+\s+as\s+any\)\??\.\w+\(/g) || [])
      .length,
    nested_access: (
      content.match(/\([\w.]+\s+as\s+any\)\??\.\w+\??\.\w+/g) || []
    ).length,
    bracket_access: (content.match(/\([\w.]+\s+as\s+any\)\[[\w'"]+\]/g) || [])
      .length,
  };
  ...
  for (const strategy of typeCastingStrategies) {
    ...
    while ((matches = strategy.pattern.exec(modifiedContent)) !== null) {
      ...
      const endIndex = Math.min(
        ...
        matches.index + matches[0].length + 100,
      );
...
async function processFile(filePath) {
  try {
    ...
    console.log(
      `📊 Found ${analysis.total} type casting patterns:`,
      analysis.patterns,
    );

    const { modifiedContent, fixes, appliedFixes } =
      createProgressiveTypeCasting(originalContent, filePath);
...
async function main() {
  ...
  if (totalFixes > 0) {
    ...
    successfulFiles.forEach((result) => {
      console.log(
        `   • ${result.fileName}: ${result.fixes} progressive casting improvements`,
      );
    });
  }

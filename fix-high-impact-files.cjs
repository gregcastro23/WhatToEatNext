...
class HighImpactFileFixer {
  ...
  fixUnusedVariables(content, filePath) {
    ...
    while ((match = varDeclarationRegex.exec(content)) !== null) {
      ...
      if (matches === 1) {
        ...
        if (!varName.startsWith("_") && !varName.startsWith("UNUSED_")) {
          const newVarName =
            varName.startsWith("test") ||
            varName.startsWith("mock") ||
            varName.startsWith("stub")
              ? `_${varName}`
              : `UNUSED_${varName}`;
    ...
    this.log(
      `Fixed ${localFixCount} unused variable issues in ${path.basename(filePath)}`,
    );
    return fixedContent;
  }
  ...
  extractImportedItems(importStatement) {
    ...
    if (namedImportMatch) {
      const namedItems = namedImportMatch[1]
        .split(",")
        .map((item) => item.trim());
  ...
  fixExplicitAny(content, filePath) {
    ...
    this.fixedCount += localFixCount;
    this.log(
      `Fixed ${localFixCount} explicit any issues in ${path.basename(filePath)}`,
    );
    return fixedContent;
  }
  ...
  fixUnnecessaryConditions(content, filePath) {
    ...
    this.fixedCount += localFixCount;
    this.log(
      `Fixed ${localFixCount} unnecessary condition issues in ${path.basename(filePath)}`,
    );
    return fixedContent;
  }

  ...
  getWarningCount() {
    try {
      const output = execSync(
        'yarn lint --max-warnings=50000 2>&1 | grep "✖.*problems" | tail -1',
        {
          encoding: "utf8",
          stdio: "pipe",
        },
        ...
        const newLines = lines.map((line) => {
          ...
          if (line.match(/^\s*(const|let|var)\s+([a-zA-Z]\w*)\s*[=:]/)) {
            ...
            if (
              ...
              !["element", "planet", "position", "sign", "degree"].some((d) =>
                varName.toLowerCase().includes(d),
              )
            ) {
    ...
    for (const file of files.slice(0, 100)) {
      ...
      try {
        ...
        content = content.replace(
          /\blet\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=\s*([^;]+);/g,
          (match, varName, value) => {

...
class EpsilonUnknownFixer {
  ...
  async run() {
    ...
    console.log(`Unknown errors: ${initialUnknownErrors} → ${finalUnknownErrors} (${initialUnknownErrors - finalUnknownErrors} eliminated)`);
  }
  ...
  getUnknownErrorCount() {
    try {
      const output = execSync("yarn tsc --noEmit --skipLibCheck 2>&1 | grep -c \"is of type 'unknown'\"", {
        encoding: 'utf8',
        stdio: ['pipe', 'pipe', 'pipe']
      });
      return parseInt(output.trim()) || 0;
  ...
  fixIngredientRecommender(content) {
    return content
      ...
      .replace(/refreshRecommendations(?=.*is of type 'unknown')/g, 'refreshRecommendations as () => void')
  ...
  fixUnifiedCuisines(content) {
    return content
      ...
      .replace(/kalchmAnalysis\.cookingMethodInfluence(?=.*unknown)/g, '(kalchmAnalysis as any).cookingMethodInfluence')

...
for (const filePath of files) {
  ...
  const objFixes = (
    beforeObj.match(/\{\s*,\s*[a-zA-Z_$][a-zA-Z0-9_$]*\s*:/g) || []
  ).length;
  fileFixes += objFixes;
  ...
  const jestFixes = (
    beforeJest.match(/jest\?\.mock\('[^']+': any, \(: any\) =>/g) || []
  ).length;
  fileFixes += jestFixes;
  ...
  const hookFixes = (
    beforeHooks.match(/(beforeEach|afterEach)\(\(: any\) =>/g) || []
  ).length;
  fileFixes += hookFixes;
  ...
  const testFixes = (
    beforeTest.match(/(test|it)\('[^']+': any, \(: any\) =>/g) || []
  ).length;
  fileFixes += testFixes;
  ...
  const descFixes = (
    beforeDesc.match(/describe\('[^']+': any, \(: any\) =>/g) || []
  ).length;
  fileFixes += descFixes;
  ...
  if (fileFixes > 0) {
    ...
    if (processedFiles % 10 === 0) {
      console.log(
        `📝 Processed ${processedFiles} files, ${totalFixes} total fixes`,
      );
    }
  }
}

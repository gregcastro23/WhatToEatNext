...
class ConservativeFirstWave {
  ...
  async execute() {
    ...
    console.log(
      colorize("Testing one high-confidence array type replacement", "blue"),
    );
    ...
    try {
      ...
      console.log(
        colorize(
          `Initial build: ${initialBuildOk ? "OK" : "Has errors (proceeding anyway)"}`,
          initialBuildOk ? "green" : "yellow",
        ),
      );
      ...
      console.log(
        colorize(
          `✅ Applied: ${result.originalLine} → ${result.newLine}`,
          "green",
        ),
      );
      ...
      if (!buildOk) {
        ...
        console.log(
          colorize("❌ Change caused build issues, rolled back", "red"),
        );
        return false;
      }
      ...
      console.log(
        colorize(`📊 Current explicit any count: ${anyCount}`, "blue"),
      );
      ...
      console.log(
        colorize(
          "✅ One array type successfully converted: any[] → unknown[]",
          "green",
        ),
      );

...
function getFilesWithFloatingPromises() {
  try {
    const output = execSync(
      'yarn lint --format=compact 2>&1 | grep "no-floating-promises"',
      {
        encoding: "utf8",
      },
    );
...
function fixFloatingPromisesInFile(filePath) {
  ...
  try {
    ...
    content = content.replace(
      useEffectPattern,
      (match, indent, funcName, comment, closing) => {
        if (
    ...
    const standaloneAsyncPattern =
      /^(\s+)([a-zA-Z_][a-zA-Z0-9_]*)\(\);(\s*\/\/.*)?$/gm;
    content = content.replace(
      standaloneAsyncPattern,
      (match, indent, funcName, comment) => {
    ...
    if (modified) {
      ...
      console.log(
        `  - No automatic fixes applied to ${path.basename(filePath)}`,
      );
      return false;
    }
  } catch (error) {
...
function main() {
  ...
  try {
    const remainingOutput = execSync(
      'yarn lint 2>&1 | grep -c "no-floating-promises"',
      {
        encoding: "utf8",
      },
    );

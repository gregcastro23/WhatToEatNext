...
function getFilesWithFloatingPromises() {
  try {
    const output = execSync('yarn lint --format=compact 2>&1 | grep "no-floating-promises"', { encoding: 'utf8' });
...
function fixFloatingPromisesInFile(filePath) {
  ...
  try {
    ...
    const useEffectPattern = /(\s+)([a-zA-Z_][a-zA-Z0-9_]*)\(\);(\s*\/\/ [^\n]*)?(\s*}\s*,\s*\[\]?\);)/g;
    content = content.replace(useEffectPattern, (match, indent, funcName, comment, closing) => {
      if (funcName.includes('load') || funcName.includes('fetch') || funcName.includes('refresh') || funcName.includes('update')) {
        modified = true;
    ...
    content = content.replace(standaloneAsyncPattern, (match, indent, funcName, comment) => {
      ...
      if (funcName.includes('load') || funcName.includes('fetch') || funcName.includes('refresh') || 
          funcName.includes('update') || funcName.includes('init') || funcName.includes('start') ||
          funcName.includes('execute') || funcName.includes('run') || funcName.includes('process')) {
        modified = true;
    ...
    content = content.replace(topLevelPattern, (match, funcName, comment) => {
      if (filePath.includes('scripts/') && (funcName.includes('main') || funcName.includes('run') || funcName.includes('execute'))) {
        modified = true;
...
function main() {
  ...
  try {
    const remainingOutput = execSync('yarn lint 2>&1 | grep -c "no-floating-promises"', { encoding: 'utf8' });

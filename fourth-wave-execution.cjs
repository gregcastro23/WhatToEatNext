...
const FOURTH_WAVE_TARGETS = [
  {
    ...
    replace: 'private async handleCriticalFailure(error: unknown): Promise<void> {',
...
class FourthWaveExecutor {
  ...
  async execute() {
    ...
    console.log(colorize(`Targeting error handling - ${FOURTH_WAVE_TARGETS.length} error-related patterns`, 'yellow'));
    ...
    for (let i = 0; i < FOURTH_WAVE_TARGETS.length; i++) {
      ...
      console.log(colorize(`\n🔄 Processing ${i + 1}/${FOURTH_WAVE_TARGETS.length}: ${target.file}`, 'cyan'));
    ...
    console.log(`  Categories: Arrays, Properties, Signatures, Functions, Callbacks, Error Handling`);

    // Save comprehensive report
    await this.saveReport(initialAnyCount, finalCount, reduction, totalReduction, cumulativePercentage);

    if (this.results.successful > 0) {
      console.log(colorize('\n🎉 Fourth Wave Completed Successfully!', 'green'));
      console.log(colorize(`✅ ${this.results.successful} error handling patterns improved`, 'green'));
      console.log(colorize('🚀 Campaign building strong momentum - ready for fifth wave!', 'cyan'));
      return true;
    } else {
      console.log(colorize('\n⚠️ Fourth wave completed with no changes applied', 'yellow'));
      return false;
    }
  }

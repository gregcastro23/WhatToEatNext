...
class SafetyValidator {
  ...
  async validateBuildAfterBatch(modifiedFiles, includeTests = false) {
    ...
    if (!performanceValid.isValid) {
      ...
      compilationResult.compilationErrors.push(
        ...performanceValid.validationErrors,
      );
    }
    return compilationResult;
  }
    ...
    try {
      ...
      for (const [filePath, backupPath] of backupFiles.entries()) {
        ...
        try {
          ...
          rollbackErrors.push(
            `Cannot read backup file: ${backupPath} - ${error}`,
          );
          backupIntegrity = false;
          canRollback = false;
        }
  ...
  calculateSafetyScore(replacement, context) {
    ...
    if (
      replacement.validationRequired &&
    ...
    if (!isValid) {
      ...
      recommendations.push(
        "Add explanatory comments if replacement is intentional",
      );
    }
    return {
      isValid,
  ...
  validatePerformanceMetrics(metrics) {
    ...
    if (metrics.buildTime > this.safetyThresholds.maximumBuildTime) {
      ...
      recommendations.push(
        "Consider reducing batch size to improve build performance",
      );
    } else if (
      metrics.buildTime >
      this.safetyThresholds.maximumBuildTime * 0.8
    ) {
    ...
    if (memoryMB > 512) {
      ...
      recommendations.push(
        "Consider running garbage collection between batches",
      );
    }
    return {
  ...
  async validateTests(modifiedFiles) {
    try {
      ...
      const output = (0, child_process_1.execSync)(
        `${this.testCommand} --testPathPattern="${testPattern}"`,
        {
          encoding: "utf8",
          stdio: "pipe",
          timeout: this.validationTimeout,
  ...
  evaluateReplacementPatternSafety(replacement) {
    ...
    // Record replacements are generally safe
      ...
      replacement.original.includes("Record<string, any>") &&
    ...
    // Function parameter replacements are riskier
      ...
      replacement.original.includes(": any")
    ) {
      score = 0.6;

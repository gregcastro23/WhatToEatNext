...
class AnyTypeClassifier {
  constructor(rules) {
    ...
    this.patternMatchers = new Map([
      [
        ...
        this.calculateErrorHandlingConfidence.bind(this),
      ],
      [
        types_1.AnyTypeCategory.EXTERNAL_API,
        this.calculateExternalApiConfidence.bind(this),
      ],
      [
        types_1.AnyTypeCategory.TEST_MOCK,
        this.calculateTestMockConfidence.bind(this),
      ],
      [
        types_1.AnyTypeCategory.DYNAMIC_CONFIG,
        this.calculateDynamicConfigConfidence.bind(this),
      ],
      [
      ...
      [
        types_1.AnyTypeCategory.ARRAY_TYPE,
        this.calculateArrayTypeConfidence.bind(this),
      ],
      [
        types_1.AnyTypeCategory.RECORD_TYPE,
        this.calculateRecordTypeConfidence.bind(this),
      ],
    ]);
  }
  /**
  ...
  async classify(context) {
    try {
      ...
      if (
        context.hasExistingComment &&
        this.hasIntentionalDocumentation(context.existingComment)
      ) {
        return {
          isIntentional: true,
          confidence: 0.95,
      ...
      for (const [category, matcher] of this.patternMatchers) {
        ...
        score = this.applyContextualAdjustments(
          score,
          category,
          surroundingContext,
          fileTypeInfo,
  ...
  async classifyBatch(contexts) {
    ...
    for (const context of contexts) {
      try {
        ...
        console.warn(
          `Classification failed for ${context.filePath}:${context.lineNumber}`,
          error,
        );
  ...
  analyzeSurroundingCodeContext(context) {
    ...
    const hasErrorHandling =
      /try\s*\{|catch\s*\(|\.catch\s*\(|throw\s+|error|exception/i.test(
        surroundingCode,
      );
    ...
    const hasTypeAssertions = /as\s+any|<any>|\(.*\s+as\s+any\s*\)/i.test(
      surroundingCode,
    );
    ...
    const hasComplexLogic =
      (surroundingCode.match(/if\s*\(|for\s*\(|while\s*\(|switch\s*\(/g) || [])
        .length > 2 || (surroundingCode.match(/\.\w+\s*\(/g) || []).length > 3;
  ...
  analyzeFileType(filePath) {
    ...
    const isTestFile =
      ...
      pathSegments.some(
        (segment) =>
          segment === "tests" || segment === "__tests__" || segment === "test",
      );
    ...
    const isConfigFile =
      ...
      pathSegments.some(
        (segment) => segment === "config" || segment === "configs",
      );
    ...
    const isTypeDefinitionFile =
      ...
      pathSegments.some(
        (segment) => segment === "types" || segment === "@types",
      );
    ...
    const isServiceFile =
      ...
      fileName.includes(".service.") ||
      pathSegments.some(
        (segment) => segment === "services" || segment === "api",
      );
    ...
    const isComponentFile =
      ...
      pathSegments.some(
        (segment) => segment === "components" || segment === "ui",
      );
    ...
    const isUtilityFile =
      ...
      pathSegments.some(
        (segment) =>
          segment === "utils" || segment === "helpers" || segment === "lib",
      );
  ...
  categorizeFromComment(comment) {
    if (!comment) return types_1.AnyTypeCategory.LEGACY_COMPATIBILITY;
    const lowerComment = comment.toLowerCase();
  ...
  analyzeServiceLayerDomain(context) {
    ...
    if (
      externalServicePatterns.some((pattern) =>
        pattern.test(codeWithSurrounding),
      )
    ) {
      confidence = 0.85;
    ...
    // Check for service configuration (intentional)
      ...
      serviceConfigPatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.8;
      reasoning =
        "Service configuration requires flexible typing for dynamic setup options";
      isIntentional = true;
      category = types_1.AnyTypeCategory.DYNAMIC_CONFIG;
    }
    // Check for data transformation (can be more specific)
    else if (
      dataTransformPatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.75;
      reasoning =
        "Data transformation can use specific interface types for better type safety";
    ...
    // Check for service interfaces (can be more specific)
      ...
      serviceInterfacePatterns.some((pattern) =>
        pattern.test(codeWithSurrounding),
      )
    ) {
      confidence = 0.7;
    ...
    // General service context
      ...
      confidence = 0.65;
      reasoning =
        "Service layer data handling can likely use more specific types";
  ...
  analyzeIntelligenceDomain(context) {
    ...
    if (
      intelligencePatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.9;
      reasoning =
        "Intelligence systems require flexible typing for evolving analytical patterns";
      category = types_1.AnyTypeCategory.DYNAMIC_CONFIG;
    }
    // Check for dynamic analysis
    else if (
      dynamicAnalysisPatterns.some((pattern) =>
        pattern.test(codeWithSurrounding),
      )
    ) {
      confidence = 0.85;
      reasoning =
        "Dynamic analysis systems require flexible typing for adaptive behavior";
      category = types_1.AnyTypeCategory.DYNAMIC_CONFIG;
    }
    // General intelligence context
    else if (
      lowerCode.includes("intelligence") ||
      lowerCode.includes("analytics")
    ) {
      confidence = 0.75;
      reasoning =
  ...
  analyzeAstrologicalDomain(context) {
    ...
    if (
      planetaryPatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.9;
      reasoning =
    ...
    // Check for astronomy API integration
      ...
      astronomyApiPatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.85;
      reasoning =
        "Astronomy API integration requires flexible typing for varying response formats";
      category = types_1.AnyTypeCategory.EXTERNAL_API;
    }
    // Check for astrological calculations
    else if (
      calculationPatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.8;
      reasoning =
  ...
  analyzeRecipeDomain(context) {
    ...
    if (foodApiPatterns.some((pattern) => pattern.test(codeWithSurrounding))) {
      confidence = 0.85;
      reasoning =
        "Food API integration requires flexible typing for varying response structures";
      isIntentional = true;
      category = types_1.AnyTypeCategory.EXTERNAL_API;
    }
    // Check for ingredient data (can be typed more specifically)
    else if (
      ingredientPatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.8;
    ...
    // Check for recipe data (can be typed more specifically)
      ...
      recipePatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.8;
    ...
    // Check for cuisine data (can be typed more specifically)
      ...
      cuisinePatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.75;
    ...
    // General food-related context
      ...
      if (
        ...
        confidence = 0.7;
        reasoning =
          "Food-related API response or configuration may require flexible typing";
        isIntentional = true;
        category = types_1.AnyTypeCategory.EXTERNAL_API;
      } else {
        confidence = 0.7;
  ...
  analyzeCampaignDomain(context) {
    ...
    if (
      campaignConfigPatterns.some((pattern) =>
        pattern.test(codeWithSurrounding),
      )
    ) {
      confidence = 0.9;
    ...
    // Check for metrics and intelligence data
      ...
      metricsPatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.85;
      reasoning =
    ...
    // Check for campaign execution context
      ...
      executionPatterns.some((pattern) => pattern.test(codeWithSurrounding))
    ) {
      confidence = 0.8;
      reasoning =
        "Campaign execution system requires flexible typing for diverse tool integration";
      category = types_1.AnyTypeCategory.DYNAMIC_CONFIG;
    }
    // Check for tool integration
    else if (
      toolIntegrationPatterns.some((pattern) =>
        pattern.test(codeWithSurrounding),
      )
    ) {
      confidence = 0.75;
      reasoning =
        "Campaign tool integration requires flexible typing for varying tool outputs";
      category = types_1.AnyTypeCategory.EXTERNAL_API;
    }
    // General campaign context
      ...
      confidence = 0.7;
      reasoning =
        "Campaign system context often requires flexible typing for dynamic operations";
      category = types_1.AnyTypeCategory.DYNAMIC_CONFIG;
    }
    if (confidence > 0) {
      return {
  ...
  calculateErrorHandlingConfidence(context) {
    ...
    if (
      codeWithSurrounding.includes("catch") &&
      codeWithSurrounding.includes("any")
    ) {
      confidence += 0.4;
    }
  ...
  getReasoningForCategory(category, context) {
    const reasoningMap = {
      ...
      [types_1.AnyTypeCategory.TEST_MOCK]:
        "Used in test mocking - intentional for test flexibility",
      [types_1.AnyTypeCategory.DYNAMIC_CONFIG]:
      ...
      [types_1.AnyTypeCategory.ARRAY_TYPE]:
        "Simple array type - can be safely replaced with unknown[]",
      [types_1.AnyTypeCategory.RECORD_TYPE]:
        "Record type - can be safely replaced with unknown value type",
      ...
      [types_1.AnyTypeCategory.RETURN_TYPE]:
        "Function return type - may be replaceable with inferred type",
  ...
  analyzeFunctionContext(context) {
    ...
    if (
      /:\s*any\s*\{/.test(context.codeSnippet) ||
      ...
      return {
        ...
        confidence: 0.65,
        reasoning:
          "Function return type can likely be inferred or typed more specifically",
  ...
  suggestFunctionParamReplacement(context) {
    ...
    if (
      context.codeSnippet.includes("string") ||
      context.codeSnippet.includes("String")
    ) {
      return "string";
    }
    if (
      context.codeSnippet.includes("number") ||
      context.codeSnippet.includes("Number")
    ) {
      return "number";
    }
    if (
      context.codeSnippet.includes("boolean") ||
      context.codeSnippet.includes("Boolean")
    ) {
      return "boolean";
    }
  ...
  applyContextualAdjustments(
    ...
    if (
      surroundingContext.hasErrorHandling &&
      category === types_1.AnyTypeCategory.ERROR_HANDLING
    ) {
      adjustedScore *= 1.2;
    }
    if (
      surroundingContext.hasApiCalls &&
      category === types_1.AnyTypeCategory.EXTERNAL_API
    ) {
      adjustedScore *= 1.15;
    }
    if (
      surroundingContext.hasTestingCode &&
      category === types_1.AnyTypeCategory.TEST_MOCK
    ) {
      adjustedScore *= 1.1;
    }
    ...
    if (
      surroundingContext.hasComplexLogic &&
      category === types_1.AnyTypeCategory.TYPE_ASSERTION
    ) {
      adjustedScore *= 0.7;
    }
  ...
  buildDefaultContextualReasoning(
    confidence,
    surroundingContext,
    fileTypeInfo,
  ) {
    ...
    if (confidence > 0.7) {
      baseReasoning =
        "High contextual confidence - likely intentional but needs documentation";
    } else if (confidence > 0.6) {
      baseReasoning =
        "Moderate contextual confidence - may be intentional, requires review";
    } else if (confidence > 0.4) {
      baseReasoning =

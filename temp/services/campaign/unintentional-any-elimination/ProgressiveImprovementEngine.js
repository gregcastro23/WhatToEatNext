...
class ProgressiveImprovementEngine {
  ...
  adaptStrategy() {
    ...
    const averageSuccessRate =
      recentBatches.reduce(
        (sum, batch) =>
          sum +
          batch.replacementsSuccessful /
            Math.max(1, batch.replacementsAttempted),
        0,
      ) / recentBatches.length;
    const averageSafetyScore =
      recentBatches.reduce((sum, batch) => sum + batch.safetyScore, 0) /
      recentBatches.length;
    console.log(
    ...
    if (averageSafetyScore < 0.7) {
      ...
      this.adaptiveConfig.maxFilesPerBatch = Math.max(
        5,
        Math.floor(this.adaptiveConfig.maxFilesPerBatch * 0.7),
      );
  ...
  async setRealisticTargets() {
    ...
    const fileAnalysis = await this.analyzeFileComplexity(
      candidateFiles.slice(0, 20),
    );
    ...
    const milestones = [
      {
        ...
        estimatedBatches: Math.ceil(
          (candidateFiles.length * 0.1) / this.adaptiveConfig.maxFilesPerBatch,
        ),
      },
      {
        ...
        estimatedBatches: Math.ceil(
          (candidateFiles.length * 0.3) / this.adaptiveConfig.maxFilesPerBatch,
        ),
      },
      {
        ...
        estimatedBatches: Math.ceil(
          (candidateFiles.length * 0.6) / this.adaptiveConfig.maxFilesPerBatch,
        ),
      },
      {
        percentage: recommendedTarget,
        description:
          "Target achievement - complete remaining high-confidence cases",
        estimatedBatches: Math.ceil(
          (candidateFiles.length * 0.8) / this.adaptiveConfig.maxFilesPerBatch,
        ),
      },
    ];
    reasoning.push(
    ...
    reasoning.push(
      `Based on analysis of ${candidateFiles.length} files with any types`,
    );
  ...
  async monitorProgress() {
    ...
    if (this.batchHistory.length >= 5) {
      ...
      if (avgSuccessRate < 0.3) {
        ...
        recommendations.push(
          "Focus on documenting intentional any types instead of replacement",
        );
      }
      if (avgSafetyScore < 0.7) {
        needsManualIntervention = true;
        recommendations.push(
          "Safety concerns detected - pause automated processing",
        );
        recommendations.push(
      ...
      if (recentProgress < 5 && recentBatches.length >= 3) {
        ...
        recommendations.push(
          "Consider switching to documentation mode for remaining types",
        );
      }
    }
    ...
    if (currentProgress.reductionPercentage < 5) {
      recommendations.push(
        "Early stage - focus on array types (any[]) for quick wins",
      );
      recommendations.push(
        "Increase confidence threshold to 0.9 for maximum safety",
      );
    } else if (currentProgress.reductionPercentage < 10) {
      recommendations.push(
        "Good progress - expand to Record<string, any> patterns",
      );
      recommendations.push(
        "Consider enabling domain-specific analysis for better suggestions",
      );
    } else if (
      currentProgress.reductionPercentage >=
      targetInfo.recommendedTarget * 0.8
    ) {
      recommendations.push(
        "Approaching target - focus on documentation for remaining intentional types",
      );
      recommendations.push(
        "Consider manual review for complex remaining cases",
      );
    }
    return {
      currentProgress,
  ...
  analyzeSuccessRateAndAdapt() {
    ...
    if (recentBatches.length >= 3) {
      const firstHalf = recentBatches.slice(
        0,
        Math.floor(recentBatches.length / 2),
      );
      const secondHalf = recentBatches.slice(
        Math.floor(recentBatches.length / 2),
      );
      const firstHalfRate =
        firstHalf.reduce(
      ...
      if (secondHalfRate > firstHalfRate + 0.1) {
        trend = "improving";
        adaptations.push(
          "Success rate improving - consider increasing batch size",
        );
        ...
        this.adaptiveConfig.maxFilesPerBatch = Math.max(
          5,
          Math.floor(this.adaptiveConfig.maxFilesPerBatch * 0.8),
        );
        this.adaptiveConfig.confidenceThreshold = Math.min(
          0.95,
          this.adaptiveConfig.confidenceThreshold + 0.1,
    ...
    if (currentSuccessRate < 0.3) {
      adaptations.push(
        "Very low success rate - switching to documentation mode recommended",
      );
      adaptations.push("Consider manual review of remaining any types");
    } else if (currentSuccessRate < 0.5) {
      adaptations.push(
        "Low success rate - increasing confidence threshold to 0.9",
      );
      this.adaptiveConfig.confidenceThreshold = Math.min(
        0.95,
        this.adaptiveConfig.confidenceThreshold + 0.1,
    ...
    try {
      ...
      for (const filePath of filesToProcess) {
        try {
          ...
          batchMetrics.replacementsAttempted +=
            fileResult.replacementsAttempted;
          batchMetrics.replacementsSuccessful +=
            fileResult.replacementsSuccessful;
          ...
          if (
            batchMetrics.filesProcessed %
              effectiveConfig.validationFrequency ===
            0
          ) {
            console.log(
            ...
            if (currentErrorCount > initialErrorCount + 5) {
              ...
              batchMetrics.compilationErrors =
                currentErrorCount - initialErrorCount;
              batchMetrics.safetyScore = Math.max(
                0,
                1 - batchMetrics.compilationErrors / 10,
              );
              break;
            }
          }
      ...
      if (finalErrorCount > initialErrorCount) {
        ...
        batchMetrics.safetyScore = Math.max(
          0,
          1 - batchMetrics.compilationErrors / 20,
        );
      }
      batchMetrics.executionTime = Date.now() - startTime;
  ...
  async executeFullCampaign(config) {
    ...
    console.log(
      `Initial state: ${initialProgress.totalAnyTypes} any types found`,
    );
    console.log(
      `Target: ${effectiveConfig.targetReductionPercentage}% reduction`,
    );
    console.log(
    ...
    while (
      ...
      try {
        ...
        result.unintentionalTypesReplaced +=
          batchMetrics.replacementsSuccessful;
        ...
        console.log(
          `  Current batch size: ${this.adaptiveConfig.maxFilesPerBatch}`,
        );
        console.log(
  ...
  async processFile(filePath, config) {
    ...
    const classifications =
      await this.classifier.classifyBatch(anyTypeContexts);
  ...
  async findFilesWithAnyTypes() {
    try {
      ...
      console.warn(
        "Failed to find files with any types, using fallback method",
      );
  ...
  async analyzeFileComplexity(files) {
    ...
    const testFilePercentage =
      files.length > 0 ? (testFiles / files.length) * 100 : 0;
    const arrayTypePercentage =
      totalAnyTypes > 0 ? (arrayTypes / totalAnyTypes) * 100 : 0;
    const recordTypePercentage =
      totalAnyTypes > 0 ? (recordTypes / totalAnyTypes) * 100 : 0;
    const functionParamPercentage =
      totalAnyTypes > 0 ? (functionParams / totalAnyTypes) * 100 : 0;
  ...
  async getCurrentProgress() {
    ...
    for (const filePath of totalFiles.slice(0, sampleSize)) {
      try {
        const content = fs.readFileSync(filePath, "utf8");
        const lines = content.split("\n");
        totalAnyTypes += lines.filter((line) =>
          this.containsAnyType(line),
        ).length;
      } catch (error) {
        // Skip files that can't be read
      }
    ...
    const averageSuccessRate =
      totalAttempted > 0 ? totalReplacements / totalAttempted : 0;
    // Calculate reduction percentage
    const reductionPercentage =
      estimatedTotal > 0 ? (totalReplacements / estimatedTotal) * 100 : 0;

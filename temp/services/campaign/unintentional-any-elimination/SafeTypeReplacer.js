...
class SafeTypeReplacer {
  ...
  async applyReplacement(replacement, context) {
    ...
    if (context) {
      safetyValidation = this.safetyValidator.calculateSafetyScore(
        replacement,
        context,
      );
    } else {
    ...
    while (retryCount < this.maxRetries) {
      try {
        ...
        const result = await this.performAtomicReplacement(
          replacement,
          backupPath,
        );
        if (result.success) {
          ...
          if (!rollbackVerification.success) {
            ...
            return {
              ...
              failedReplacements: [replacement],
              compilationErrors: [
                `Rollback verification failed: ${rollbackVerification.error}`,
              ],
  ...
  async processBatch(replacements) {
    ...
    try {
      ...
      for (const [filePath, fileReplacements] of Array.from(
        replacementsByFile.entries(),
      )) {
        try {
          const result = await this.applyReplacementsToFile(
            filePath,
            fileReplacements,
          );
      ...
      const buildValidation =
        await this.safetyValidator.validateBuildAfterBatch(
          modifiedFiles,
          false,
        );
      ...
      const rollbackValidation =
        await this.safetyValidator.validateRollbackCapability(
      ...
      // Emergency rollback
      await this.rollbackAllFiles(backupPaths);
      throw new types_1.SafetyProtocolError(
    ...
    return [
      ...
      // Function parameter analysis and replacement
      {
        ...
        replacement: (match, context) => {
          ...
          if (paramName) {
            const inferredType = this.inferFunctionParameterType(
              context,
              paramName,
            );
            return match.replace("any", inferredType);
      ...
      // Object property type replacement
      {
        ...
        replacement: (match, context) => {
          ...
          if (propertyName) {
            const inferredType = this.inferObjectPropertyType(
              context,
              propertyName,
            );
            return match.replace("any", inferredType);
      ...
      for (const replacement of replacements) {
        try {
          ...
          if (lineIndex < 0 || lineIndex >= lines.length) {
            ...
            errors.push(
              `Invalid line number ${replacement.lineNumber} for file ${filePath}`,
            );
            continue;
          }
  ...
  async validateTypeScriptCompilation() {
    const buildResult =
      await this.safetyValidator.validateTypeScriptCompilation();
    return {
  ...
  calculateSafetyScore(replacement) {
    ...
    if (
      replacement.filePath.includes(".test.") ||
      replacement.filePath.includes("__tests__")
    ) {
    ...
    if (replacement.original === "any[]") {
      ...
      replacement.original.includes("function") ||
      replacement.original.includes("=>")
    ) {
  ...
  inferArrayElementType(context) {
    ...
    if (context.domainContext.domain === types_1.CodeDomain.RECIPE) {
      if (
        codeSnippet.includes("ingredient") ||
        codeSnippet.includes("recipe")
      ) {
        return "string";
      }
    }
  ...
  inferFunctionParameterType(context, paramName) {
    ...
    if (paramLower.includes("event") || paramLower === "e") {
      if (
        context.codeSnippet.includes("onClick") ||
        context.codeSnippet.includes("onSubmit")
      ) {
  ...
  inferReturnType(context) {
    ...
    if (allContext.includes("return ")) {
      ...
      // Boolean returns
      if (
        allContext.includes("return true") ||
        allContext.includes("return false")
      ) {
        return "boolean";
      }
      // Array returns
      ...
      // Promise returns
      if (
        allContext.includes("return Promise") ||
        allContext.includes("return new Promise")
      ) {
    ...
    if (
      codeSnippet.includes("is") ||
      codeSnippet.includes("has") ||
      codeSnippet.includes("can")
    ) {
  ...
  inferObjectPropertyType(context, propertyName) {
    ...
    if (
      ...
      propLower.includes("length") ||
      propLower.includes("size")
    ) {
      return "number";
    }
    if (
  ...
  isInErrorHandlingContext(context) {
    const { codeSnippet, surroundingLines } = context;
    const allContext = [codeSnippet, ...surroundingLines]
      .join(" ")
      .toLowerCase();
    return (
  ...
  async validateBuildSafety(modifiedFiles, includeTests = false) {
    return this.safetyValidator.validateBuildAfterBatch(
      modifiedFiles,
      includeTests,
    );
  }
  ...
  async validateRollbackSafety(originalFiles, backupFiles) {
    return this.safetyValidator.validateRollbackCapability(
      originalFiles,
      backupFiles,
    );
  }

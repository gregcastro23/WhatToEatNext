...
async function verifyIntegration() {
  console.log(
    "🔍 Verifying Unintentional Any Elimination Campaign Integration...",
  );
  try {
    // Test 1: Create campaign controller with factory function
    console.log("\n1. Testing factory function...");
    const controller = (0,
    CampaignIntegration_1.createUnintentionalAnyCampaignController)();
    ...
    const campaignConfig =
      CampaignIntegration_1.UnintentionalAnyCampaignController.createUnintentionalAnyEliminationConfig();
    console.log("✅ Campaign configuration created");
    console.log(`   Phases: ${campaignConfig.phases.length}`);
    console.log(
      `   Phase IDs: ${campaignConfig.phases.map((p) => p.id).join(", ")}`,
    );
    // Test 3: Test automation script compatibility
    console.log("\n3. Testing automation script compatibility...");
    const compatibility =
      CampaignIntegration_1.UnintentionalAnyIntegrationHelper.createAutomationScriptCompatibility();

...
console.log("🔧 Fixing malformed syntax errors (v2)...");
...
const fixMalformedSyntax = (content) => {
  ...
  // Fix malformed function calls with type assertions
  // Pattern: (c as Record<string, unknown>)?.(files as Record<string, unknown>)
  // Should be: (c as Record<string, unknown>)?.files
  fixed = fixed.replace(
    /\(([^)]+as\s+Record<string,\s*unknown>)\)\?\.\(([^)]+as\s+Record<string,\s*unknown>)\)/g,
    "($1)?.files",
  );

  // Fix malformed string literals with type assertions inside
  // Pattern: '**/*.(test as Record<string, unknown>).ts'
  // Should be: '**/*.test.ts'
  fixed = fixed.replace(
  ...
  // Fix complex malformed expressions in find calls
  // Pattern: (c as Record<string, unknown>)?.rules && (c as Record<string, (unknown>)?.rules as Record<string, unknown>)
  // Should be: (c as Record<string, unknown>)?.rules
  fixed = fixed.replace(
    /\(([^)]+as\s+Record<string,\s*unknown>)\)\?\.([\w.]+)\s+&&\s+\([^)]+as\s+Record<string,\s*\(unknown>\)\?\.([\w.]+)\s+as\s+Record<string,\s*unknown>\)/g,
    "($1)?.$2",
  );

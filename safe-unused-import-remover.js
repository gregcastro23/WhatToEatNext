...
class SafeUnusedImportRemover {
  ...
  removeSafeUnusedImports(dryRun = true) {
    ...
    console.log(
      `Requires manual review: ${analysis.requiresManualReview.length}`,
    );
  ...
  shouldPreserve(unusedImport) {
    ...
    if (
      preserveNames.some((name) =>
        importName.toLowerCase().includes(name.toLowerCase()),
      )
    ) {
      return true;
    }
  ...
  performImportRemoval(imports) {
    ...
    try {
      ...
      execSync(
        'yarn lint --fix --rule "@typescript-eslint/no-unused-vars: error"',
        {
          stdio: "inherit",
          encoding: "utf8",
        },
...
if (dryRun) {
  console.log(
    "🔍 Running in DRY RUN mode. Use --execute to actually remove imports.\n",
  );
}

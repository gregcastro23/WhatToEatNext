...
class SimpleAutoFix {
  ...
  async getInitialStats() {
    try {
      ...
      const fixableErrors = results.reduce(
        (sum, result) => sum + (result.fixableErrorCount || 0),
        0,
      );
      const fixableWarnings = results.reduce(
        (sum, result) => sum + (result.fixableWarningCount || 0),
        0,
      );
  ...
  async applyAutoFixes() {
    try {
      ...
      this.log(
        `ESLint completed with remaining issues (expected): ${error.message}`,
      );
      return true;
    }
  }
  ...
  generateReport(initialStats, finalStats) {
    ...
    if (initialStats && finalStats) {
      report.improvements = {
        ...
        fixableIssuesResolved:
          initialStats.totalFixable - finalStats.totalFixable,
      };
    ...
    fs.writeFileSync(
      "eslint-simple-autofix-report.json",
    ...
    if (initialStats && finalStats) {
      this.log(
        `Initial: ${initialStats.totalErrors} errors, ${initialStats.totalWarnings} warnings`,
      );
      this.log(
        `Final: ${finalStats.totalErrors} errors, ${finalStats.totalWarnings} warnings`,
      );

      if (report.improvements) {
        this.log(
          `Fixed: ${report.improvements.errorsFixed} errors, ${report.improvements.warningsFixed} warnings`,
        );
        this.log(
          `Auto-fixable issues resolved: ${report.improvements.fixableIssuesResolved}`,
        );
      }
    }
...
async function main() {
  ...
  try {
    ...
    if (initialStats) {
      fixer.log(
        `Initial status: ${initialStats.totalErrors} errors, ${initialStats.totalWarnings} warnings`,
      );
      fixer.log(
        `Auto-fixable issues: ${initialStats.totalFixable} (${initialStats.fixableErrors} errors, ${initialStats.fixableWarnings} warnings)`,
      );
    }
    ...
    if (finalStats) {
      fixer.log(
        `Final status: ${finalStats.totalErrors} errors, ${finalStats.totalWarnings} warnings`,
      );

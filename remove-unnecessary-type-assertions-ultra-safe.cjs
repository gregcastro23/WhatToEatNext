...
class UltraSafeTypeAssertionRemover {
  ...
  shouldPreserveAssertion(content, assertionMatch, filePath) {
    ...
    const contextBefore = content.substring(Math.max(0, matchIndex - 150), matchIndex);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (!['node_modules', '.git', 'dist', 'build', '.next', '__tests__', 'tests', 'campaign'].includes(entry.name)) {
   ...
   */
  run() {
    console.log(`🚀 Starting ultra-safe unnecessary type assertion removal${this.dryRun ? ' (DRY RUN)' : ''}...\n`);

    // Find all TypeScript files to process (excluding tests, campaign files, etc.)
    const files = this.findTypeScriptFiles('./src');
    console.log(`Found ${files.length} TypeScript files to process (excluding tests, campaign files, etc.)\n`);

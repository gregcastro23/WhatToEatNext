...
console.log('🔧 Remaining Syntax Corruption Fix Script');
console.log('==========================================');
console.log(`🔍 Mode: ${DRY_RUN ? 'DRY RUN (no changes will be made)' : 'LIVE RUN (changes will be applied)'}`);
console.log('');

// Files identified in the build output with syntax corruption
const targetFiles = [
...
// Fix patterns based on the build errors
const syntaxPatterns = [
  // Object.(entries(source)? || []) pattern
  {
    pattern: /Object\.\(entries\(([^)]+)\)\?\s*\|\|\s*\[\]\)/g,
    replacement: (match, p1) => `Object.entries(${p1} || {})`,
    description: 'Fix Object.entries with corrupted syntax'
  },
  // food.(foodNutrients || []) pattern  
  {
    pattern: /(\w+)\.\(([^)]+)\s*\|\|\s*\[\]\)/g,
    replacement: (match, p1, p2) => `(${p1}.${p2} || [])`,
    description: 'Fix property access with corrupted syntax'
  },
  // Object.values(this.(flavorProfiles)? || []) pattern
  {
    pattern: /Object\.values\(this\.\(([^)]+)\)\?\s*\|\|\s*\[\]\)/g,
    replacement: (match, p1) => `Object.values(this.${p1} || {})`,
    description: 'Fix Object.values with corrupted this access'
  },
  // Array.isArray(peak?) pattern
  {
    pattern: /Array\.isArray\((\w+)\?\)/g,
    replacement: (match, p1) => `Array.isArray(${p1})`,
    description: 'Fix Array.isArray with corrupted optional chaining'
  },
  // peak? === pattern (standalone optional chaining corruption)
  {
    pattern: /(\w+)\?\s*===\s*([^)]+)\s*\|\|\s*/g,
    replacement: (match, p1, p2) => `${p1} === ${p2} || `,
    description: 'Fix corrupted optional chaining comparison'
  }
];
...
console.log('4. If build succeeds, continue with Phase 2 TypeScript fixes'); 
...
console.log(`🔧 ${isDryRun ? '[DRY RUN] ' : ''}Reverting elementalBalance to lowercase format...`);
...
for (const file of files) {
  try {
    ...
    // Fix elementalBalance object properties to match StandardizedAlchemicalResult type
    // Type expects: { Fire: number; Earth: number; Air: number; Water: number; ... }
    
    // Pattern 1:Water: ..., Earth: ..., Air: ... }
    const elementalBalancePattern = /}]+),\s*Water:\s*([^}]+),\s*Earth:\s*([^}]+),\s*Air:\s*([^}]+)([^}]*)\}/g;
    newContent = newContent.replace(elementalBalancePattern, (match, fire, water, earth, air, rest) => {
      fileChanges++;
      return `}, Water: ${water}, Earth: ${earth}, Air: ${air}${rest} }`;
    });

    // Pattern 2: Individual property assignments within elementalBalance

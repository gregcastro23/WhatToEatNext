...
function main() {
  ...
  if (filesToProcess.length === 0) {
    console.log(
      "📁 No specific import issues found, processing TypeScript files...",
    );
  ...
  try {
    const importIssues = execSync(
      'yarn lint 2>&1 | grep -E "(import|export)" | wc -l',
      {
        encoding: "utf8",
      },
    );
    console.log(
      `📈 Remaining import-related warnings: ${parseInt(importIssues.trim())}`,
    );
  } catch (error) {

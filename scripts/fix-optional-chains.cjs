...
function getTypeScriptErrors() {
  try {
    ...
    for (const line of lines) {
      ...
      const match2801 = line.match(/^(.+?)\((\d+),(\d+)\): error TS2801: This condition will always return true/);
...
function fixOptionalChainInFile(filePath, errors, dryRun = false) {
  ...
  for (const error of fileErrors) {
    ...
    if (lineIndex >= 0 && lineIndex < lines.length) {
      ...
      if (fixed) {
        fixedCount++;
        log(`    ✓ Line ${error.line}: Fixed ${error.type} - ${fixedPatterns[fixedPatterns.length - 1]}`, 'green');
      }
    }
  }
  ...
  try {
    ...
    if (!dryRun && result.fixed > 0) {
      ...
      try {
        ...
        const remainingChainErrors = newErrors.filter(e => 
          e.type === 'TS18046' || e.type === 'TS18048' || e.type === 'TS2801'
        );
        log(`⚠️  Build still has ${remainingChainErrors.length} optional chaining errors`, 'yellow');
      }
    }

...
const ASTROLOGICAL_TIMING_TEMPLATES = {
  ...
  fire_dominant: {
    ...
    rationale:
      "Fire-dominant recipes thrive when solar and martial energies are strong, amplifying heat and transformation.",
  ...
  water_dominant: {
    ...
    rationale:
      "Water-dominant recipes benefit from lunar and venusian influences that enhance moisture and tenderness.",
    alternativePositions: [
      {
  ...
  earth_dominant: {
    ...
    rationale:
      "Earth-dominant recipes excel with venusian and saturnine energies that provide stability and richness.",
    alternativePositions: [
      {
        positions: { Venus: "Capricorn", Saturn: "Taurus", Moon: "Capricorn" },
        suitability: 0.88,
        notes:
          "Capricorn earth timing for structured, traditional preparations",
      },
    ],
  },
  ...
  air_dominant: {
    ...
    rationale:
      "Air-dominant recipes thrive with mercurial and uranian energies that promote lightness and creativity.",
    alternativePositions: [
      {
  ...
  balanced: {
    ...
    rationale:
      "Balanced recipes benefit from harmonious planetary alignments that support complex flavor interactions.",
    alternativePositions: [
      {
...
function generateAstrologicalTiming(recipe) {
  ...
  for (const [seasonName, keywords] of Object.entries(
  ...
  return {
    ...template,
    optimalLunarPhases:
      optimalLunarPhases.length > 0 ? optimalLunarPhases : undefined,
...
function determineMealType(recipe) {
  ...
  if (Array.isArray(mealType) && mealType.length > 0) {
    ...
    if (
      ["breakfast", "lunch", "dinner", "snack", "dessert"].includes(primaryType)
    ) {
      return primaryType;
    }
  }
  ...
  if (
    course.some(
      (c) =>
        c.toLowerCase().includes("appetizer") ||
        c.toLowerCase().includes("starter"),
    )
  ) {
    return "snack";
  }
...
function enhanceAllRecipes() {
  ...
  console.log(
    `Enhanced ${enhancedRecipes.length} recipes with astrological timing`,
  );
}

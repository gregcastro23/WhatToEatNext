...
class AwaitThenableFixer {
    ...
    try {
      ...
      const awaitThenableErrors = tscOutput.split("\n").filter(
        (line) =>
          ...
          (line.includes("not assignable to type") &&
      ...
      this.log(
        `Found ${filesWithErrors.size} files with potential await-thenable issues`,
      );
      return Array.from(filesWithErrors);
    } catch (error) {
      this.log(
        `Error finding await-thenable issues: ${error.message}`,
        "error",
      );
      return [];
    }
    ...
    fixes.forEach((fix) => {
      ...
      if (matches.length > 0) {
        ...
        if (validMatches.length > 0) {
          ...
          fixed = true;
          appliedFixes.push(
            `${fix.description} (${validMatches.length} occurrences)`,
          );
        }
      }
    });
  ...
  fixTestUtilityPromises(content, filePath) {
    ...
    if (this.verbose && appliedFixes.length > 0) {
      this.log(
        `  Fixed test utilities in ${path.basename(filePath)}: ${appliedFixes.join(", ")}`,
      );
    }
  ...
  async validateFixes() {
    ...
    try {
      ...
      const awaitThenableErrors = (
        tscOutput.match(/TS1308|await.*thenable/g) || []
      ).length;
...
export async function testPromiseHandling(testFn: () => Promise<any>): Promise<boolean> {
  try {
    ...
    this.log(
      `${this.dryRun ? "[DRY RUN] " : ""}Created await-thenable utility: ${utilityPath}`,
    );
    return utilityPath;
  }
  ...
  async run() {
    ...
    if (validation.awaitThenableErrors >= 0) {
      this.log(
        `   Remaining await-thenable errors: ${validation.awaitThenableErrors}`,
      );
    }

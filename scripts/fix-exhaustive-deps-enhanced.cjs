...
const FIX_PATTERNS = {
  ...
  complexExpression:
    /React Hook \w+ has a complex expression in the dependency array/,
  ...
  multipleMissing:
    /React Hook \w+ has missing dependencies: (.+)\. Either include them or remove the dependency array/,
};
...
function getExhaustiveDepsWarnings() {
  try {
    ...
    if (fs.existsSync(lintResultsPath)) {
      ...
      execSync(
        'yarn eslint --config eslint.config.cjs src --format=json --rule "react-hooks/exhaustive-deps: warn" > lint-results-deps.json',
        {
          encoding: "utf8",
          stdio: "inherit",
        },
...
function applyMissingDependencyFix(lines, warning) {
  ...
  if (!depArray) {
    log(
      `Could not find dependency array for hook at line ${hookLine + 1}`,
      "warn",
    );
    return false;
  }
  ...
  if (depArray.start.line === depArray.end.line) {
    ...
    lines.splice(
      depArray.start.line + 1,
      depArray.end.line - depArray.start.line,
    );
  }
...
function processFile(filePath, warnings) {
  ...
  warnings.forEach((warning) => {
    ...
    if (SAFETY_CONFIG.dryRun) {
      log(
        `[DRY RUN] Would fix ${warningType.type} dependency at line ${warning.line}`,
        "debug",
      );
      if (warningType.type === "missing") {
        log(
          `  Dependencies to add: ${warningType.dependencies.join(", ")}`,
          "debug",
        );
      }
      fixed++;
    ...
    switch (warningType.type) {
      ...
      case "complex":
        log(
          `Skipping complex expression at line ${warning.line} (requires manual fix)`,
          "warn",
        );

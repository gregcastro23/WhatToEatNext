...
class TS1134Processor {
  ...
  async processFile(filePath, dryRun) {
    try {
      ...
      const pattern1 = /^(\s*)(const|let|var)\s+([a-zA-Z_$][a-zA-Z0-9_$]*),\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=\s*(.+?)$/gm;
      modified = modified.replace(pattern1, (match, indent, keyword, var1, var2, value) => {
        fixCount++;
      ...
      const pattern3 = /^(\s*)(const|let|var)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*,\s*,/gm;
      modified = modified.replace(pattern3, (match, indent, keyword, varName) => {
        fixCount++;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (/^\s*(const|let|var)\s+[a-zA-Z_$][a-zA-Z0-9_$]*\s*,\s*,/.test(line)) {

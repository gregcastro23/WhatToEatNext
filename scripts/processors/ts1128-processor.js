...
class TS1128Processor {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (
          ...
          lines[i] = line.replace(
            /^(\s*)/,
            "$1// Removed misplaced expression: // ",
          );
          modified = true;
          fixCount++;
          continue;
        ...
        if (
          this.isModuleLevelExpression(lines, i) &&
        ...
        if (/^\s*[\)\]]\s*$/.test(trimmed)) {
          lines[i] = line.replace(
            /[\)\]]/,
            "// Removed orphaned closing delimiter",
          );
          modified = true;
          fixCount++;
          continue;
  ...
  isAfterTypeDeclaration(lines, closingBraceIndex) {
    ...
    for (
      let i = closingBraceIndex;
      i >= Math.max(0, closingBraceIndex - 20);
      i--
    ) {

...
class TS1442Processor {
  ...
  async processFile(filePath, dryRun) {
    try {
      ...
      const pattern1 = /^(\s*)(private|public|protected|readonly|static)?\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:\s*([^:;]+?):\s*(.+?)$/gm;
      modified = modified.replace(pattern1, (match, indent, modifier, name, type, value) => {
        fixCount++;
      ...
      const pattern2 = /^(\s*)([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:\s*([A-Z][a-zA-Z0-9_$<>[\]|&]*)\s*:\s*(.+?)$/gm;
      modified = modified.replace(pattern2, (match, indent, name, type, value) => {
        fixCount++;
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (inClass) {
          ...
          if (inClass && classDepth > 0 &&
              /^\s*(private|public|protected|readonly|static)?\s*[a-zA-Z_$][a-zA-Z0-9_$]*\s*:\s*[A-Z]/.test(line)) {
            // Check if it's an initialization (has value after type)
            const propMatch = line.match(/^(\s*(?:private|public|protected|readonly|static)?\s*)([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:\s*([A-Z][a-zA-Z0-9_$<>[\]|&]*)\s*:\s*(.+)$/);
            if (propMatch) {
              lines[i] = `${propMatch[1]}${propMatch[2]}: ${propMatch[3]} = ${propMatch[4]}`;
              fixCount++;
            }
          }
        }

...
function fixFile(filePath) {
  try {
    ...
    content = content.replace(
      objectPattern,
      (match, indent, propName, brace) => {
        // Only fix if this looks like an object property
        if (
          indent.length >= 2 &&
          !propName.match(
            /^(if|for|while|function|class|export|import|const|let|var)$/,
          )
        ) {

...
function loadAnalysisData() {
  try {
    if (!fs.existsSync(CONFIG.analysisFile)) {
      log(`Analysis file ${CONFIG.analysisFile} not found. Run analyze-type-assertions.cjs first.`, 'error');
      return null;
    }
    ...
    log(`Loaded analysis data: ${data.summary.totalAssertions} assertions across ${data.summary.filesScanned} files`);
    return data;
  } catch (error) {
...
function removeUnnecessaryAssertions(content, filePath) {
  ...
  for (const pattern of SAFE_REMOVAL_PATTERNS) {
    ...
    modifiedContent = modifiedContent.replace(pattern.pattern, (match, ...groups) => {
      ...
      const replacement = pattern.replacement.replace(/\$(\d+)/g, (_, num) => groups[parseInt(num) - 1] || '');
...
function processFile(filePath, fileAssertions) {
  try {
    ...
    log(`Processing ${path.relative(process.cwd(), filePath)} (${unnecessaryAssertions.length} unnecessary assertions)...`);
    ...
    if (CONFIG.dryRun) {
      log(`[DRY RUN] Would remove ${result.removedCount} assertions from ${path.basename(filePath)}`, 'info');
      ...
      log(`Removed ${result.removedCount} unnecessary assertions from ${path.basename(filePath)}`, 'info');
    }
...
function validateBuild() {
  ...
  try {
    ...
    if (errorCount > 500) {
      ...
      log(`TypeScript validation found ${errorCount} errors (within acceptable range) - continuing`, 'warn');
      return true;
    }
  }
}

...
const filterCuisine =
  cuisineIndex !== -1 && args[cuisineIndex + 1] ? args[cuisineIndex + 1] : null;
...
console.log(
  `Analyzing ${cuisineFiles.length} cuisine file(s)${filterCuisine ? ` (filtered: ${filterCuisine})` : ""}`,
);
console.log(
  `Ingredient mappings: ${Object.keys(INGREDIENT_ELEMENTAL_MAP).length}`,
);
      ...
      if (methodsArray.includes("{") && methodsArray.includes("name:")) {
        ...
        while ((methodMatch = methodNamePattern.exec(methodsArray)) !== null) {
          ...
          if (findMethodMatch(methodName)) {
            ...
            unmappedMethods[methodName] =
              (unmappedMethods[methodName] || 0) + 1;
          }
        }
      } else {
        ...
        let methodMatch;
        while (
          (methodMatch = stringMethodPattern.exec(methodsArray)) !== null
        ) {
          const methodName = methodMatch[1];
          totalMethods++;
          if (findMethodMatch(methodName)) {
...
const sortedIngredients = Object.entries(unmappedIngredients).sort(
  (a, b) => b[1] - a[1],
);
const sortedMethods = Object.entries(unmappedMethods).sort(
  (a, b) => b[1] - a[1],
);
...
console.log(
  `Coverage: ${mappedIngredients}/${totalIngredients} (${((mappedIngredients / totalIngredients) * 100).toFixed(1)}%)`,
);
...
console.log(
  `Coverage: ${mappedMethods}/${totalMethods} (${((mappedMethods / totalMethods) * 100).toFixed(1)}%)`,
);
...
console.log("=".repeat(80));
console.log(
  `Ingredient coverage: ${((mappedIngredients / totalIngredients) * 100).toFixed(1)}%`,
);
console.log(
  `Method coverage: ${((mappedMethods / totalMethods) * 100).toFixed(1)}%`,
);

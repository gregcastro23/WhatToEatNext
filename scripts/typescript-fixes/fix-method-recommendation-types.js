...
// Target file
const targetFile = path.resolve(__dirname, '../../src/utils/recommendation/methodRecommendation.ts');

// Type definitions to add at the top of the file
const methodTypesDefinition = `
// CookingMethodProfile interface for type safety
interface CookingMethodProfile {
  ...
  flavorProfile?: FlavorProperties;
...
// MethodRecommendationOptions interface for type safety
interface MethodRecommendationOptions {
  ingredientProperties?: ElementalProperties;
  desiredOutcome?: {
    elements?: ElementalProperties;
    flavors?: FlavorProperties;
  };
  dietaryRestrictions?: string[];
  cookingExpertise?: 'beginner' | 'intermediate' | 'advanced' | 'expert';
...
// CulturalCookingMethod interface for type safety
interface CulturalCookingMethod {
  name: string;
  category: string;
  duration: {
    min: number;
...
// ElementalProperties and FlavorProperties interfaces (if not already imported)
...
function fixFile(filePath, isDryRun) {
  ...
  try {
    ...
    if (content.includes('interface CookingMethodProfile')) {
      console.log('  CookingMethodProfile interface already exists, skipping');
      return;
    }
    ...
    const updatedContent = 
      ...
      methodTypesDefinition + 
      content.slice(insertionPoint);
    
    if (isDryRun) {
      console.log('  Would add CookingMethodProfile and related interfaces (dry run)');
    } else {
      fs.writeFileSync(filePath, updatedContent, 'utf8');
      console.log('  Added CookingMethodProfile and related interfaces');
    }
  } catch (error) {

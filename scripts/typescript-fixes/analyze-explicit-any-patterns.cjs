...
const DOMAIN_PATTERNS = {
  ASTROLOGICAL:
    /(planet|zodiac|element|lunar|celestial|astrological).*:\s*any/gi,
...
class ExplicitAnyAnalyzer {
  ...
  generateRecommendations() {
    ...
    if (this.results.byPattern.ERROR_HANDLING.count > 50) {
      recs.push({
        ...
        priority: "HIGH",
        suggestion:
          "Create standardized error types and use unknown with type guards",
    ...
    if (this.results.byPattern.TYPE_ASSERTION.count > 100) {
      recs.push({
        ...
        priority: "HIGH",
        suggestion:
          'Replace "as any" with proper interface types or progressive casting',
    ...
    if (this.results.byDomain.ASTROLOGICAL.count > 20) {
      recs.push({
        ...
        priority: "MEDIUM",
        suggestion:
          "Use ZodiacSign, Planet, Element, ElementalProperties from enhanced-astrology.d.ts",
    ...
    if (this.results.byDomain.CAMPAIGN.count > 20) {
      recs.push({
        ...
        priority: "MEDIUM",
        suggestion:
          "Use CampaignPhase, ProgressMetrics, SafetyEvent from campaign.ts",
  ...
  displayResults() {
    ...
    for (const [pattern, data] of Object.entries(this.results.byPattern)) {
      if (data.count > 0) {
        console.log(
          `${pattern.padEnd(20)} ${data.count.toString().padStart(6)} instances`,
        );
      }
    }
    ...
    for (const [domain, data] of Object.entries(this.results.byDomain)) {
      if (data.count > 0) {
        console.log(
          `${domain.padEnd(20)} ${data.count.toString().padStart(6)} instances`,
        );
      }
    }
    ...
    this.results.recommendations.forEach((rec) => {
      console.log(
        `[${rec.priority}] ${rec.type}: ${rec.suggestion} (${rec.count} instances)`,
      );
    });

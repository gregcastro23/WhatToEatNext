...
class EnterpriseAnyReplacer {
  ...
  async processFiles(files, options = {}) {
    console.log(
      `🚀 Processing ${files.length} files for enterprise any replacements...\n`,
    );
  ...
  async processFile(filePath, options) {
    ...
    try {
      ...
      modified = modified.replace(
        /\(([^)]*:\s*any[^)]*)\)/g,
        (match, params) => {
          const modifiedParams = params.replace(
            /(\w+)\s*:\s*any/g,
  ...
  inferTypeFromContext(variable, content) {
    ...
    if (match) {
      ...
      if (
        assignment.includes("getIngredient") ||
        assignment.includes("ingredient")
      ) {
        return "Ingredient";
      }
...
async function main() {
  ...
  if (options.targetFiles.length > 0) {
    ...
    if (fs.existsSync(analysisPath)) {
      ...
      console.error(
        "❌ No analysis file found. Run analyze-explicit-any-patterns.cjs first.",
      );
      process.exit(1);
    }
  }

      ...
      if (lineIndex >= 0 && lineIndex < lines.length) {
        ...
        if (chainMatch) {
          ...
          if (line.includes('const {') || line.includes('const [')) {
            ...
            const destructurePattern = new RegExp(`(const\\s+(?:{[^}]+}|\\[[^\\]]+\\])\\s*=\\s*)${expression.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`);
          ...
          // Check if it's a comparison or conditional
            ...
            const comparisonPattern = new RegExp(`\\b${expression.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`);
          ...
          // Check if it's in an array or object literal
            ...
            const literalPattern = new RegExp(`([\\[{,]\\s*)${expression.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\.${error.property}`);
            const literalMatch = line.match(literalPattern);
            if (literalMatch) {
              lines[lineIndex] = line.replace(literalMatch[0], `${literalMatch[1]}(await ${expression}).${error.property}`);
              fixed = true;
            }
          }
        }
        ...
        if (!fixed) {
          const methodChainPattern = new RegExp(`(\\w+\\([^)]*\\))(?:\\.\\w+\\([^)]*\\))*\\.${error.property}\\b`);
        ...
        if (fixed) {
          fixedInFile++;
          log(`  ✓ Line ${error.line}: Fixed advanced Promise access for ${error.property}`, 'green');
        } else {

  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (
        ...
        (line.includes("TS2339") || line.includes("TS2345"))
      ) {
    ...
    for (const error of fileErrors) {
      ...
      if (lineIndex >= 0 && lineIndex < lines.length) {
        ...
        if (
          ...
          error.message.includes("does not exist on type 'Promise<")
        ) {
            ...
            if (accessMatch) {
              ...
              if (!line.includes(`await ${varName}`)) {
                ...
                const newLine = line.replace(
                  new RegExp(`\\b${varName}\\.${property}\\b`),
                  `${awaitedVar}.${property}`,
                );
          ...
          const argPattern =
            /Argument of type 'Promise<(.+?)>' is not assignable to parameter of type '(.+?)'/;
            ...
            while ((match = funcCallPattern.exec(line)) !== null) {
              ...
              while ((varMatch = promiseVarPattern.exec(args)) !== null) {
                ...
                if (!args.includes(`await ${varName}`)) {
                  // Try adding await
                  const awaitedArgs = args.replace(
        ...
        if (fixed) {
          fixedInFile++;
          log(
            `  ✓ Line ${error.line}: Added await for Promise access`,
            "green",
          );
        } else {
          log(
            `  ⚠️  Line ${error.line}: Needs manual review - ${error.message.substring(0, 60)}...`,
            "yellow",
          );
        }
      }
...
function main() {
  ...
  try {
    ...
    if (!dryRun) {
      ...
      try {
        execSync('git stash push -m "fix-await-promises-backup"', {
          stdio: "pipe",
        });

...
function processFile(filePath, fileAssertions) {
  try {
    ...
    log(`Processing ${path.relative(process.cwd(), filePath)} (${redundantAssertions.length} redundant assertions)...`);
    ...
    if (CONFIG.dryRun) {
      ...
      log(`Removed ${result.removedCount} redundant assertions from ${path.basename(filePath)}`, 'info');
    }

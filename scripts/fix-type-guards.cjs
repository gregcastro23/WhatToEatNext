...
function getTypeErrors() {
  try {
    ...
    for (const line of lines) {
      ...
      const match18048 = line.match(/^(.+?)\((\d+),(\d+)\): error TS18048: '(.+?)' is possibly 'undefined'/);
      const match2571 = line.match(/^(.+?)\((\d+),(\d+)\): error TS2571: Object is of type 'unknown'/);
      const match2532 = line.match(/^(.+?)\((\d+),(\d+)\): error TS2532: Object is possibly 'undefined'/);
  ...
  for (const [filePath, fileErrors] of Object.entries(errorsByFile)) {
    ...
    for (const error of fileErrors) {
      ...
      if (lineIndex >= 0 && lineIndex < lines.length) {
        ...
        if (error.issue === 'unknown') {
          ...
          if (error.expression) {
            ...
            if (parts.length > 0) {
              ...
              // Replace in line
              const escapedExpression = error.expression.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
              ...
              if (match) {
                ...
                if (pattern.source.includes('\\[')) {
                  ...
                  if (!line.includes(optionalAccess)) {
                    lines[lineIndex] = line.replace(`${objName}[`, optionalAccess);
                    fixed = true;
                    break;
                  }
                  ...
                  if (!line.includes(optionalCall)) {
                    lines[lineIndex] = line.replace(`${objName}(`, optionalCall);
                    fixed = true;
                    break;
                  }
                }
        ...
        if (fixed) {
          fixedInFile++;
          log(`  ✓ Line ${error.line}: Added type guard for ${error.issue} access`, 'green');
        } else {
          log(`  ⚠️  Line ${error.line}: Needs manual review - ${error.type}`, 'yellow');
        }
      }
    }
...
function main() {
  ...
  try {
    ...
    if (!dryRun) {
      ...
      try {
        execSync('git stash push -m "fix-type-guards-backup"', { stdio: 'pipe' });

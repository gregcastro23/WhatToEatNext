...
function computeRecipeProperties(
  ingredients,
  cookingMethods,
  planetaryPositions,
) {
...
const DEMO_RECIPES = [
  {
    ...
    description:
      "Fresh Atlantic salmon fillet grilled with Mediterranean herbs and lemon",
    ...
    astrologicalTiming: {
      ...
      rationale:
        "Leo Sun provides regal fire energy, Cancer Moon enhances moisture retention, Libra Venus brings harmony to Mediterranean flavors",
  ...
  },
  {
    ...
    description:
      "Classic French slow-cooked beef stew with red wine, mushrooms, and pearl onions",
    cuisine: "French",
    ingredients: [
      {
        name: "beef chuck",
    ...
    astrologicalTiming: {
      ...
      rationale:
        "Taurus Sun and Venus provide earthy richness, Capricorn Saturn ensures proper structure through long cooking",
...
function displayRecipe(recipe) {
  ...
  console.log(
    `🏷️  Cuisine: ${recipe.cuisine} | Meal: ${recipe.mealType} | Difficulty: ${recipe.difficulty}`,
  );
  console.log(
    `⏱️  Prep: ${recipe.preparationTime}min | Cook: ${recipe.cookingTime}min`,
  );

  console.log(`\n🌟 Elemental Balance:`);
  const elements = recipe.elementalProperties;
  console.log(
    `   Fire: ${(elements.Fire * 100).toFixed(1)}% | Water: ${(elements.Water * 100).toFixed(1)}%`,
  );
  console.log(
    `   Earth: ${(elements.Earth * 100).toFixed(1)}% | Air: ${(elements.Air * 100).toFixed(1)}%`,
  );
  ...
  if (recipe.astrologicalTiming) {
    console.log(`\n🔮 Astrological Timing:`);
    console.log(
      `   Optimal: ${Object.entries(recipe.astrologicalTiming.optimalPositions)
        .slice(0, 3)
        .map(([p, s]) => `${p} in ${s}`)
        .join(", ")}`,
    );
    if (recipe.astrologicalTiming.optimalLunarPhases) {
      console.log(
        `   Lunar: ${recipe.astrologicalTiming.optimalLunarPhases.join(", ")}`,
      );
    }
    if (recipe.astrologicalTiming.optimalSeasons) {
      console.log(
        `   Seasons: ${recipe.astrologicalTiming.optimalSeasons.join(", ")}`,
      );
    }
  }
}
...
function displayComputedProperties(recipe, computed) {
  ...
  console.log(
    `   Spirit: ${alchemical.Spirit} | Essence: ${alchemical.Essence}`,
  );
  console.log(
    `   Matter: ${alchemical.Matter} | Substance: ${alchemical.Substance}`,
  );
  ...
  console.log(
    `   Heat: ${(thermo.heat * 100).toFixed(2)}% | Entropy: ${(thermo.entropy * 100).toFixed(2)}%`,
  );
  console.log(
    `   Reactivity: ${(thermo.reactivity * 100).toFixed(2)}% | Greg's Energy: ${thermo.gregsEnergy.toFixed(3)}`,
  );
  console.log(
    `   Kalchm: ${thermo.kalchm.toFixed(3)} | Monica: ${thermo.monica.toFixed(3)}`,
  );

  console.log(`\n⚡ Kinetic Properties (P=IV Circuit Model):`);
  const kinetics = computed.kineticProperties;
  console.log(
    `   Charge (Q): ${kinetics.charge.toFixed(1)} | Potential (V): ${(kinetics.potentialDifference * 1000).toFixed(1)}mV`,
  );
  console.log(
    `   Current (I): ${(kinetics.currentFlow * 1000).toFixed(1)}mA | Power (P): ${(kinetics.power * 1000000).toFixed(1)}μW`,
  );
  console.log(
    `   Force: ${kinetics.forceClassification} (${(kinetics.forceMagnitude * 1000000).toFixed(1)}μN)`,
  );
  ...
  console.log(
    `   Planetary Positions Used: ${Object.keys(computed.computationMetadata.planetaryPositionsUsed).length}`,
  );
  console.log(
    `   Cooking Methods Applied: ${computed.computationMetadata.cookingMethodsApplied.join(", ")}`,
  );
}
...
function displayValidationResults(recipe, validation) {
  ...
  console.log(
    `   Quality Score: ${validation.qualityMetrics.completenessScore}/100`,
  );
  console.log(
    `   Elemental Balance: ${(validation.qualityMetrics.elementalBalance * 100).toFixed(1)}%`,
  );
  console.log(
    `   Ingredient Coverage: ${validation.qualityMetrics.ingredientCoverage}%`,
  );
}
...
function demonstrateRecipeComputation() {
  ...
  console.log(
    "   Tier 1 (Ingredients): ✅ Elemental properties (Fire/Water/Earth/Air)",
  );
  console.log(
    "   Tier 2 (Recipes): ✅ Alchemical + Thermodynamic + Kinetic properties",
  );
  console.log(
    "   Tier 3 (Cuisines): 🔄 Statistical aggregation (future phase)",
  );
  ...
  console.log(
    "   5. Calculate thermodynamic metrics (Heat, Entropy, Reactivity, etc.)",
  );
  ...
  console.log(
    "Next steps: Cuisine-level aggregations and recommendation engine.",
  );

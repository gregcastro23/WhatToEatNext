  ...
  try {
    ...
    for (const fix of ENTITY_FIXES) {
      ...
      modifiedContent = modifiedContent.replace(
        ...
        (match, ...groups) => {
          ...
          log(
            `  ${fix.description}: ${match.trim()} → ${replacement}`,
            "debug",
          );
          fileFixCount++;
    ...
    if (modifiedContent !== originalContent) {
      if (CONFIG.dryRun) {
        ...
        log(
          `Fixed ${fileFixCount} entities in ${path.relative(process.cwd(), filePath)}`,
          "info",
        );
      }
...
function validateFixes() {
  ...
  try {
    ...
    log(
      "TypeScript validation failed - some fixes may have introduced errors",
      "warn",
    );

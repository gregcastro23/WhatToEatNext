...
const referencePatterns = [
  ...
  {
    pattern: /\bexpect_\b/g,
    replacement: "expect",
    description: "expect_ → expect",
  },
  {
    pattern: /\bString_\b/g,
    replacement: "String",
    description: "String_ → String",
  },
  {
  ...
  {
    pattern: /\b_result\b/g,
    replacement: "result",
    description: "_result → result",
  },
  {
  ...
  {
    pattern: /\.includes_\b/g,
    replacement: ".includes",
    description: ".includes_ → .includes",
  },
  ...
  {
    pattern: /\bonClick_\b/g,
    replacement: "onClick",
    description: "onClick_ → onClick",
  },
  ...
  for (const filePath of filesWithErrors) {
    ...
    for (const pattern of referencePatterns) {
      ...
      if (matches) {
        ...
        fixedInFile += count;
        log(
          `    ✓ Fixed ${count} instance(s): ${pattern.description}`,
          "green",
        );
      }
    }
      ...
      if (error.suggestion) {
        ...
        if (lineIndex >= 0 && lineIndex < lines.length) {
          ...
          if (line.match(nameRegex)) {
            ...
            fixedInFile++;
            log(
              `    ✓ Fixed: ${error.name} → ${error.suggestion} (line ${error.line})`,
              "green",
            );
          }
        }
      ...
      for (const func of drawingFunctions) {
        ...
        if (content.match(funcRegex)) {
          ...
          const definitionRegex = new RegExp(
            `function\\s+${func}\\b|const\\s+${func}\\s*=`,
          );
          if (!content.match(definitionRegex)) {
            ...
            fixedInFile++;
            log(
              `    ✓ Prefixed undefined function: ${func} → _${func}`,
              "green",
            );
          }
        }
...
function main() {
  ...
  try {
    ...
    if (!dryRun) {
      ...
      try {
        execSync('git stash push -m "fix-missing-references-backup"', {
          stdio: "pipe",
        });
    ...
    if (!dryRun && result.fixed > 0) {
      ...
      try {
        ...
        const remainingRefs = newErrors.filter(
          (e) =>
            e.type === "TS2304" || e.type === "TS2551" || e.type === "TS2552",
        );
        log(
          `⚠️  Build still has ${remainingRefs.length} reference errors`,
          "yellow",
        );
      }
    }

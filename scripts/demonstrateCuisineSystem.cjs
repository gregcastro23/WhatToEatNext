...
function demonstratePersonalizedRecommendations() {
  ...
  try {
    const { generateCuisineRecommendations, createAdvancedUserProfile } =
      loadModule("../src/utils/cuisine/cuisineRecommendationEngine.ts");
    ...
    profiles.forEach((profile) => {
      ...
      recommendations.forEach((rec, index) => {
        ...
        console.log(
          `  ${index + 1}. ${rec.cuisineName} (${scorePercent}% match)`,
        );
...
function demonstrateSignatureAnalysis(computedResults) {
  ...
  try {
    ...
    const successful = computedResults.successful.filter(
      (r) => r.properties.signatures?.length > 0,
    );
    ...
    successful.forEach((result) => {
      ...
      if (topSignature) {
        ...
        console.log(
          `  ${cuisine}: ${topSignature.property} is ${topSignature.strength}ly ${direction} than average`,
        );
...
function demonstratePlanetaryPatterns(computedResults) {
  ...
  try {
    ...
    const successful = computedResults.successful.filter(
      (r) => r.properties.planetaryPatterns?.length > 0,
    );
    ...
    successful.forEach((result) => {
      ...
      const strongestPattern = patterns.sort(
        (a, b) => b.planetaryStrength - a.planetaryStrength,
      )[0];

      if (strongestPattern) {
        const strengthPercent = (
          strongestPattern.planetaryStrength * 100
        ).toFixed(1);
        console.log(
          `  ${cuisine}: ${strongestPattern.planet} (${strengthPercent}% strength)`,
        );
        console.log(
          `    Dominant element: ${strongestPattern.dominantElement}`,
        );
        if (strongestPattern.commonSigns.length > 0) {
          console.log(
            `    Common signs: ${strongestPattern.commonSigns
              .slice(0, 2)
              .map((s) => s.sign)
              .join(", ")}`,
          );
...
function demonstrateSystemIntegration() {
  ...
  try {
    ...
    console.log(
      `📊 Cache status: ${cacheStats.totalEntries} entries, ${(cacheStats.hitRate * 100).toFixed(1)}% hit rate`,
    );
  } catch (error) {
...
function showPerformanceMetrics(computedResults) {
  ...
  console.log(
    `⏱️  Total computation time: ${(summary.computationTime / 1000).toFixed(2)}s`,
  );
  console.log(
    `🏃 Average per cuisine: ${(summary.computationTime / summary.totalCuisines).toFixed(0)}ms`,
  );
  console.log(`📚 Recipes processed: ${summary.totalRecipes}`);
  console.log(`🎯 Signatures identified: ${summary.totalSignatures}`);
  console.log(
    `📈 Success rate: ${((computedResults.successful.length / summary.totalCuisines) * 100).toFixed(1)}%`,
  );
}
...
async function main() {
  ...
  console.log("==============================");
  console.log(
    "Showcasing the complete hierarchical cuisine recommendation system",
  );
  ...
  try {
    ...
    console.log(
      "  node scripts/computeCuisineProperties.cjs    # Compute all cuisine properties",
    );
    console.log(
      "  node scripts/identifyCuisineSignatures.cjs   # Analyze signatures",
    );
    console.log(
      "  node scripts/validateCuisineSystem.cjs       # Validate system integrity",
    );
  } catch (error) {

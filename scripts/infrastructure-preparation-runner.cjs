...
class InfrastructurePreparationRunner {
  ...
  printHeader() {
    ...
    console.log(
      `${colors.blue}🚀 Preparing infrastructure for systematic linting excellence...${colors.reset}`,
    );
    console.log(
      `${colors.blue}📅 Started: ${new Date().toLocaleString()}${colors.reset}\n`,
    );
  }
  ...
  async checkPrerequisites() {
    ...
    for (const check of checks) {
      try {
        ...
        console.log(
          `   ${colors.green}✅ ${check.name}: ${version}${colors.reset}`,
        );
      } catch (error) {
        if (check.required) {
          ...
          console.log(
            `   ${colors.yellow}⚠️  ${check.name}: Not available (optional)${colors.reset}`,
          );
        }
      }
    }
  ...
  async compileInfrastructureModule() {
    console.log(
      `${colors.yellow}🔧 Compiling Infrastructure Module...${colors.reset}`,
    );
    ...
    try {
      ...
      console.log(
        `   ${colors.yellow}⚠️  TypeScript compilation has errors (proceeding anyway)${colors.reset}`,
      );
    }
  ...
  async executeInfrastructurePreparation() {
    console.log(
      `${colors.yellow}⚙️ Executing Infrastructure Preparation...${colors.reset}`,
    );
    ...
    try {
      ...
      console.log(
        `   ${colors.yellow}⚠️  Direct execution failed, running simplified validation...${colors.reset}`,
      );
  ...
  async runSimplifiedValidation() {
    ...
    result.eslintConfig.typeAwareConfig.exists =
      existsSync(typeAwareConfigPath);
    ...
    if (result.eslintConfig.fastConfig.exists) {
      try {
        ...
        execSync(
          "yarn lint:quick --max-warnings=10000 src/components/Header/Header.tsx",
          {
            cwd: this.projectRoot,
            stdio: "pipe",
            timeout: 30000,
        ...
        result.eslintConfig.fastConfig.performanceOptimized =
          result.eslintConfig.fastConfig.estimatedTime < 5000;
      } catch (error) {
        console.log(
          `   ${colors.yellow}⚠️  Fast ESLint config test failed${colors.reset}`,
        );
      }
    }
    ...
    if (result.eslintConfig.typeAwareConfig.exists) {
      try {
        ...
        result.eslintConfig.typeAwareConfig.estimatedTime =
          Date.now() - startTime;
        result.eslintConfig.typeAwareConfig.functional = true;
        result.eslintConfig.typeAwareConfig.typeCheckingEnabled = true;
      } catch (error) {
        console.log(
          `   ${colors.yellow}⚠️  Type-aware ESLint config test failed${colors.reset}`,
        );
      }
    }
    ...
    if (existsSync(packageJsonPath)) {
      ...
      result.eslintConfig.packageScripts.typeAwareLint =
        !!scripts["lint:type-aware"];
      result.eslintConfig.packageScripts.incrementalLint =
        !!scripts["lint:incremental"];
    ...
    try {
      ...
      console.log(
        `   ${colors.yellow}⚠️  Build stability check failed${colors.reset}`,
      );
    }
  ...
  generateSimplifiedRecommendations(result) {
    ...
    if (!result.eslintConfig.fastConfig.functional) {
      recommendations.push(
        "Fix fast ESLint configuration - required for development workflow",
      );
    }
    ...
    if (!result.backupSystem.gitStashAvailable) {
      recommendations.push(
        "Ensure git is properly configured for stash operations",
      );
    }
  ...
  displayResults(result) {
    ...
    console.log(
      `${statusColor}${statusIcon} Overall Readiness: ${result.overallReadiness ? "READY" : "NEEDS ATTENTION"}${colors.reset}`,
    );
    console.log(
      `${colors.blue}📈 Readiness Score: ${result.readinessScore}%${colors.reset}`,
    );
    console.log(
      `${colors.blue}⏱️  Duration: ${Math.round(duration / 1000)}s${colors.reset}\n`,
    );
    ...
    const components = [
      ...
      },
      {
        ...
        details: result.backupSystem.gitStashAvailable
          ? "Available"
          : "Not available",
      },
      {
        ...
        details: result.buildMonitoring.buildStabilityChecks
          ? "Stable"
          : "Unstable",
      },
    ];
    ...
    components.forEach((component) => {
      ...
      console.log(
        `   ${color}${icon} ${component.name}: ${component.details}${colors.reset}`,
      );
    });
  ...
  provideNextSteps(result) {
    ...
    if (result.overallReadiness) {
      console.log(
        `${colors.green}✅ Infrastructure is ready for linting excellence campaigns!${colors.reset}`,
      );
      console.log(
        `${colors.blue}📝 You can now proceed with Phase 2 tasks:${colors.reset}`,
      );
      console.log(
        `   ${colors.blue}• Task 2.1: TS1005 Syntax Error Resolution${colors.reset}`,
      );
      console.log(
        `   ${colors.blue}• Task 2.2: TS1128 Declaration Error Resolution${colors.reset}`,
      );
      console.log(
        `   ${colors.blue}• Task 2.3: TS2304/TS2339 Property and Name Resolution${colors.reset}`,
      );
      console.log(
      ...
      console.log(
        `${colors.yellow}⚠️  Infrastructure needs attention before proceeding.${colors.reset}`,
      );
      console.log(
        `${colors.blue}📝 Please address the recommendations above.${colors.reset}`,
      );
      console.log(
        `\n${colors.yellow}🔄 Re-run this script after making fixes:${colors.reset}`,
      );
      console.log(
        `${colors.yellow}   node scripts/infrastructure-preparation-runner.cjs${colors.reset}`,
      );
    }

    console.log(`\n${colors.blue}📊 View detailed reports in:${colors.reset}`);
    console.log(
      `   ${colors.blue}• .kiro/metrics/infrastructure-report.json${colors.reset}`,
    );
    console.log(
      `   ${colors.blue}• .kiro/metrics/infrastructure-report.html${colors.reset}`,
    );
  }
  ...
  handleError(error) {
    ...
    console.log(
      `   ${colors.blue}• Ensure all prerequisites are installed${colors.reset}`,
    );
    console.log(
      `   ${colors.blue}• Check that you're in the project root directory${colors.reset}`,
    );
    console.log(
      `   ${colors.blue}• Verify TypeScript and ESLint configurations exist${colors.reset}`,
    );
    console.log(
      `   ${colors.blue}• Try running 'yarn install' to update dependencies${colors.reset}`,
    );
  }
}

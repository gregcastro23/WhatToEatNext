...
function adjustForRounding(props: ElementalProperties): ElementalProperties {
  ...
  if (Math.abs(diff) < 0.01) {
    ...
    const elements = Object.entries(props) as [keyof ElementalProperties, number][];
 ...
 */
function extractElementalProperties(content: string, filePath: string): NormalizationResult[] {
 ...
 */
function updateFileWithNormalized(filePath: string, results: NormalizationResult[]): void {
...
function generateReport(results: NormalizationResult[]): void {
  ...
  console.log(`Already normalized: ${alreadyNormalized.length} (${((alreadyNormalized.length / results.length) * 100).toFixed(1)}%)`);
  console.log(`Normalized: ${normalized.length} (${((normalized.length / results.length) * 100).toFixed(1)}%)\n`);
  ...
  if (normalized.length > 0) {
    ...
    for (const result of normalized) {
      ...
      console.log(`New sum: ${(result.normalized.Fire + result.normalized.Water + result.normalized.Earth + result.normalized.Air).toFixed(3)}\n`);
    }
  }

...
def pdf_to_images(pdf_path, start_page=0, end_page=None, dpi=150):
    """Yields PIL Images for a range of pages in the PDF."""
    try:
    ...
    print(f"Rendering pages {start_page} to {end_page - 1}...")
    images = []
...
def main():
    ...
    # We open the document first to check the total number of pages
    doc = fitz.open(pdf_path)
    total_pages = len(doc)
    doc.close()
    ...
    images_to_process = pdf_to_images(pdf_path, start_page=start_page, end_page=end_page)

...
async function validateTypeScriptCompilation() {
  ...
  const result = runCommand("yarn tsc --noEmit --skipLibCheck", {
    silent: true,
  });
  ...
  const validation = {
    passed:
      errorCount === CONFIG.thresholds.typescript.errors &&
  ...
  if (validation.passed) {
    log(
      `TypeScript validation passed (${errorCount} errors, ${warningCount} warnings)`,
      "success",
    );
  } else {
    log(
      `TypeScript validation failed (${errorCount} errors, ${warningCount} warnings)`,
      "error",
    );
...
async function validateESLintAnalysis() {
  ...
  const validation = {
    passed:
      errorCount === CONFIG.thresholds.eslint.errors &&
  ...
  if (validation.passed) {
    log(
      `ESLint validation passed (${errorCount} errors, ${warningCount} warnings)`,
      "success",
    );
  } else {
    log(
      `ESLint validation failed (${errorCount} errors, ${warningCount} warnings)`,
      "error",
    );
    if (errorCount > 0) {
...
async function validateBuildProcess() {
  ...
  const validation = {
    passed:
      result.success &&
      duration <= CONFIG.thresholds.build.maxTimeSeconds * 1000,
  ...
  if (validation.passed) {
    ...
    log(
      `Build validation failed (${duration}ms, success: ${result.success})`,
      "error",
    );
    if (!result.success) {
...
async function validateDevelopmentServer() {
  ...
  const validation = {
    passed:
      serverStarted &&
      duration <= CONFIG.thresholds.performance.maxDevStartSeconds * 1000,
    serverStarted,
    duration,
    details: serverStarted
  ...
  if (validation.passed) {
    ...
    log(
      `Dev server validation failed (started: ${serverStarted}, ${duration}ms)`,
      "warning",
    );
    if (!serverStarted) {
...
async function validateTestSuite() {
  ...
  const startTime = Date.now();
  const result = runCommand("yarn test --run", {
    silent: true,
    timeout: 120000,
  });
  const duration = Date.now() - startTime;
      ...
      if (usage) {
        ...
        if (validation.diskSpace > 90) {
          ...
          results.recommendations.push(
            "Free up disk space (currently " + validation.diskSpace + "% used)",
          );
        }
      }
    }
    ...
    if (freeResult.success) {
      ...
      if (memLine) {
        ...
        if (validation.memoryUsage > 90) {
          validation.passed = false;
          results.recommendations.push(
            "High memory usage detected (" + validation.memoryUsage + "%)",
          );
        }
      }
    }
...
async function main() {
  try {
    ...
    if (passedCount === totalCount) {
      ...
      log(
        `⚠️ Partial success: ${passedCount}/${totalCount} validations passed.`,
        "warning",
      );
    } else {
      results.overall = "FAILURE";
      log(
        `❌ Recovery validation failed: ${passedCount}/${totalCount} validations passed.`,
        "error",
      );
    }

...
function validateIngredient(ingredient, ingredientKey, fileName) {
  ...
  issues.push(
    ...validateElementalProperties(ingredient, ingredientKey, fileName),
  );
  issues.push(...validateDataTypes(ingredient, ingredientKey, fileName));
  issues.push(
    ...validateRecommendedFields(ingredient, ingredientKey, fileName),
  );
...
function processFile(filePath, relativePath) {
  try {
    ...
    for (const match of ingredientKeyMatches) {
      ...
      const ingredientObjStr = objContent.substring(
        ingredientObjStart,
        ingredientObjEnd,
      );
      ...
      if (elementalsMatch) {
        ...
        if (fireMatch)
          ingredient.elementalProperties.Fire = parseFloat(fireMatch[1]);
        if (waterMatch)
          ingredient.elementalProperties.Water = parseFloat(waterMatch[1]);
        if (earthMatch)
          ingredient.elementalProperties.Earth = parseFloat(earthMatch[1]);
        if (airMatch)
          ingredient.elementalProperties.Air = parseFloat(airMatch[1]);
      }
      ...
      if (issues.length > 0) {
        const criticalOrError = issues.some(
...
function generateReport() {
  ...
  console.log(
    `⚠️  Incomplete (warnings only): ${results.incompleteIngredients}`,
  );
  ...
  console.log("-".repeat(80));
  console.log(
    `Missing Elementals:            ${results.summary.missingElementals}`,
  );
  console.log(
    `Invalid Elemental Sum:         ${results.summary.invalidElementalSum}`,
  );
  console.log(
    `Missing Required Fields:       ${results.summary.missingRequiredFields}`,
  );
  console.log(
    `Invalid Data Types:            ${results.summary.invalidDataTypes}`,
  );
  console.log(
    `Missing Recommended Fields:    ${results.summary.missingRecommendedFields}`,
  );
  ...
  if (warnings.length > 0) {
    ...
    warnings.slice(0, 10).forEach((issue, idx) => {
      console.log(
        `${idx + 1}. [${issue.type}] ${issue.file} - ${issue.ingredient}`,
      );
      console.log(`   ${issue.message}`);
    });
  }
  ...
  if (validationPassed) {
    ...
    console.log(
      `${results.incompleteIngredients} ingredients with optional improvements available`,
    );
    console.log(
    ...
    if (warnings.length > 0) {
      ...
      console.log(
        "These are recommended for maximum calculation accuracy but not required.",
      );
    }
  } else {
    ...
    console.log(
      "These MUST be fixed to ensure robust downstream calculations.",
    );

#!/usr/bin/env node
/**
 * Fetch proximate composition for curated ingredients from USDA FoodData Central.
 *
 * Emits `scripts/data/usda-composition.json`, which
 * `scripts/apply-usda-composition.mjs` writes into the ingredient data files
 * as `nutritionalProfile.composition` — water, protein, fat, carbohydrate, ash.
 *
 ...
 *
 * Usage:
 *   node scripts/fetch-usda-composition.mjs            # DEMO_KEY, rate limited
 *   FDC_API_KEY=xxx node scripts/fetch-usda-composition.mjs
 *
 * @file scripts/fetch-usda-composition.mjs
 */
...
const OUT = join(HERE, "data", "usda-composition.json");
 ...
 * `src/__tests__/data/ingredientComposition.test.ts` catches it by arithmetic,
...
function checkpoint() {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        _comment:
          "GENERATED by scripts/fetch-usda-composition.mjs from USDA FoodData Central. " +
          "`fraction` is water as a MASS FRACTION of the edible portion (g per 100 g / 100); "
          + "`companions` carries protein/fat/carbohydrate/ash in g per 100 g from the SAME record. " +
          "Applied to src/data/ingredients by scripts/apply-usda-composition.mjs.",

...
const ANY_PATTERNS = [
  ...
  // Pattern 6: Property types in interfaces/types
  {
    ...
    replacement: (match, property) => {
      ...
      } else if (property.toLowerCase().includes('name') || property.toLowerCase().includes('title')) {
        return `${property}: string;`;
      } else if (property.toLowerCase().includes('count') || property.toLowerCase().includes('amount')) {
      ...
      } else if (property.toLowerCase().includes('data') || property.toLowerCase().includes('config')) {
  ...
  try {
    ...
    ANY_PATTERNS.forEach(patternConfig => {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        const replacement = typeof patternConfig.replacement === 'function' 

...
const FIXES = [
  // 23 dried herbs
  { file: 'herbs/driedHerbs.ts', ingredients: [
  ...
  // 6 seafood
  { file: 'proteins/seafood.ts', ingredients: [
  ...
  { file: 'proteins/meat.ts', ingredient: 'chicken', profile: PROFILES.meat_chicken },
  { file: 'proteins/meat.ts', ingredient: 'pork', profile: PROFILES.meat_pork },

  // 2 oils (oils.ts)
  { file: 'oils/oils.ts', ingredients: ['palm_oil', 'almond_oil'], profile: PROFILES.oil },

  // 2 oils (seasonings/oils.ts)
  { file: 'seasonings/oils.ts', ingredients: ['ghee', 'avocado_oil'], profile: PROFILES.oil },

  // 1 citrus
  { file: 'fruits/citrus.ts', ingredient: 'mandarin', profile: PROFILES.citrus },

  // 1 spice blend
  { file: 'spices/spiceBlends.ts', ingredient: 'herbes_de_provence', profile: PROFILES.spice_blend }
];
...
console.log('🔧 Adding astrologicalProfile to all 38 incomplete ingredients...\n');

let totalFixed = 0;
let totalAttempted = 0;

for (const fix of FIXES) {
  const filePath = path.join(__dirname, '..', 'src', 'data', 'ingredients', fix.file);
...
console.log(`✅ Added astrologicalProfile to ${totalFixed} out of ${totalAttempted} ingredients`);
console.log('='.repeat(80));

...
class SyntaxRepairTool {
  ...
  repairFile(filePath: string): RepairResult {
  ...
  repairFileList(filePaths: string[]): RepairResult[] {
    return filePaths.map((filePath) => this.repairFile(filePath));
  }

  printSummary(results: RepairResult[]) {
    ...
    if (successful.length > 0) {
      ...
      successful
        ...
        .forEach((r) => {
          console.log(
            `  ${r.fixCount.toString().padStart(4)} fixes - ${r.file}`,
          );
        });
    }
...
if (require.main === module) {
  ...
  const targetPath =
    args.find((arg) => !arg.startsWith("--") && !arg.startsWith("-")) || "src";

  ...
  try {
    ...
    const filesWithErrors = results.filter((file) => {
      return file.messages.some(
        (msg) =>
          msg.severity === 2 &&
          TARGET_RULES.some((rule) => msg.ruleId?.includes(rule)),
      );
    });
...
const fixUnusedVars = (filePath, messages) => {
  ...
  const unusedVarErrors = messages
    .filter(
      (msg) => msg.ruleId?.includes("no-unused-vars") && msg.severity === 2,
    )
...
const fixFloatingPromises = (filePath, messages) => {
  ...
  const floatingPromiseErrors = messages
    .filter(
      (msg) =>
        msg.ruleId?.includes("no-floating-promises") && msg.severity === 2,
    )
    .reverse();
...
const fixAwaitThenable = (filePath, messages) => {
  ...
  const awaitThenableErrors = messages
    .filter(
      (msg) => msg.ruleId?.includes("await-thenable") && msg.severity === 2,
    )
    .reverse();
  ...
  for (const error of awaitThenableErrors) {
    if (
      error.suggestions &&
      error.suggestions[0]?.messageId === "removeAwait"
    ) {
      const lines = content.split("\n");
      const lineIndex = error.line - 1;
      const line = lines[lineIndex];
...
const fixMisusedPromises = (filePath, messages) => {
  ...
  const misusedPromiseErrors = messages
    .filter(
      (msg) =>
        msg.ruleId?.includes("no-misused-promises") && msg.severity === 2,
    )
    .reverse();

...
def generate_visual_prompt(recipe_data):
    ...
    zodiac = zodiac_data[0] if isinstance(zodiac_data, list) else zodiac_data
    ...
    lunar_phase = lunar_data[0] if isinstance(lunar_data, list) else lunar_data
    
    # Handle Season
    season_data = energy_profile.get('Season', recipe_data.get('details', {}).get('season', ['Summer']))
    season = season_data[0] if isinstance(season_data, list) else season_data
    ...
    zodiac_cue = zodiac_aesthetics.get(zodiac, zodiac_aesthetics["Aries"])
...
class NanoBananaPro:
    ...
    def generate(self, prompt, output_path="generated_image.png"):
        print(f"DEBUG: Sending prompt to Nano Banana Pro (Google Imagen/Gemini).")
        print(f"PROMPT: {prompt[:100]}...")
        
        # Simulate API call
        if not self.api_key:
            print("WARNING: No API key found for Nano Banana Pro. Simulating successful generation.")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python generate_alchemical_image.py <recipe_json_path> [output_path]")
        return

    json_path = sys.argv[1]
    output_path = sys.argv[2] if len(sys.argv) > 2 else "alchemical_dish.png"
    ...
    # 1. Synthesize Prompt
    visual_prompt = generate_visual_prompt(recipe_data)
    print(f"--- GENERATED ALCHEMICAL PROMPT ---\n{visual_prompt}\n")
    print(f"Word Count: {len(visual_prompt.split())}")

    # 2. Call Bridge
    engine = NanoBananaPro()
    engine.generate(visual_prompt, output_path)

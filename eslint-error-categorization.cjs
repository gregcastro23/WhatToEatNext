...
try {
  ...
  lines.forEach((line) => {
    // Track current file
    if (
      (line.startsWith("/Users/") && line.includes(".tsx")) ||
      line.includes(".ts")
    ) {
      currentFile = line.replace(
        "/Users/GregCastro/Desktop/WhatToEatNext/",
        "",
      );
    }
    ...
    const errorMatch = line.match(
      /^\s*\d+:\d+\s+(error|warning)\s+(.+?)\s+([a-zA-Z@/-]+)$/,
    );
    if (errorMatch) {
      ...
      if (severity === "error") {
        ...
        if (!fileErrors[currentFile])
          fileErrors[currentFile] = { errors: 0, warnings: 0 };
  ...
  console.log(
    `   Total Issues: ${parsingErrorCount + totalErrors + totalWarnings}\n`,
  );
  ...
  if (Object.keys(errorCounts).length > 0) {
    ...
    Object.entries(errorCounts)
      ...
      .forEach(([rule, count], index) => {
        console.log(
          `   ${(index + 1).toString().padStart(2)}. ${rule.padEnd(50)} ${count.toString().padStart(4)} errors`,
        );
      });
    console.log();
  }
  ...
  if (Object.keys(warningCounts).length > 0) {
    ...
    Object.entries(warningCounts)
      ...
      .forEach(([rule, count], index) => {
        console.log(
          `   ${(index + 1).toString().padStart(2)}. ${rule.padEnd(50)} ${count.toString().padStart(4)} warnings`,
        );
      });
    console.log();
  }
  ...
  if (highImpactFiles.length > 0) {
    ...
    highImpactFiles.slice(0, 15).forEach(([file, counts], index) => {
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${file.padEnd(60)} ${counts.errors.toString().padStart(3)}E ${counts.warnings.toString().padStart(3)}W`,
      );
    });
    console.log();
  }
  ...
  Object.entries(categories).forEach(([category, rules]) => {
    const categoryErrors = rules.reduce(
      (sum, rule) => sum + (errorCounts[rule] || 0),
      0,
    );
    const categoryWarnings = rules.reduce(
      (sum, rule) => sum + (warningCounts[rule] || 0),
      0,
    );

    if (category === "Parsing Errors") {
      console.log(
        `   ${category.padEnd(25)}: ${parsingErrorCount.toString().padStart(4)} errors`,
      );
    } else if (categoryErrors > 0 || categoryWarnings > 0) {
      console.log(
        `   ${category.padEnd(25)}: ${categoryErrors.toString().padStart(4)} errors, ${categoryWarnings.toString().padStart(4)} warnings`,
      );
    }
  });
  ...
  if (parsingErrorCount > 0) {
    ...
    console.log(
      '      - Backup files (with " 3", " 4" suffixes) should be excluded',
    );
  ...
  if (topError && topError[1] > 50) {
    console.log(
      `   📊 HIGH IMPACT: "${topError[0]}" rule has ${topError[1]} violations`,
    );
    console.log("      - Consider automated fixing for this rule");
  }

  const topWarning = Object.entries(warningCounts).sort(
    ([, a], [, b]) => b - a,
  )[0];
  if (topWarning && topWarning[1] > 100) {
    console.log(
      `   ⚠️  SYSTEMATIC: "${topWarning[0]}" warning has ${topWarning[1]} instances`,
    );
  ...
  if (highImpactFiles.length > 0) {
    console.log(
      `   🎯 FOCUS: ${highImpactFiles.length} high-impact files need attention`,
    );
  ...
  console.log(
    `   Total ESLint Issues: ${totalErrors + totalWarnings} (${totalErrors} errors, ${totalWarnings} warnings)`,
  );

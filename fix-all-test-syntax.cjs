...
for (const filePath of testFiles) {
  ...
  if (sigMatches > 0)
    console.log(`  ✅ Fixed ${sigMatches} function signatures`);

  // Fix malformed object literals: `{, property:` -> `{ property:`
  const objMatches = (
    content.match(/\{\s*,\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:/g) || []
  ).length;
  ...
  if (chainMatches > 0)
    console.log(`  ✅ Fixed ${chainMatches} optional chains`);
  ...
  if (semiMatches > 0)
    console.log(`  ✅ Fixed ${semiMatches} property assignments`);

  // Fix jest mock patterns
  const jestMatches = (
    content.match(/jest\?\.mock\('([^']+)': any, \(: any\) =>/g) || []
  ).length;
  content = content.replace(
    /jest\?\.mock\('([^']+)': any, \(: any\) =>/g,
    "jest.mock('$1', () =>",
  ...
  if (jestMatches > 0)
    console.log(`  ✅ Fixed ${jestMatches} jest.mock patterns`);

  // Fix test hooks
  const hookMatches = (
    content.match(/(beforeEach|afterEach)\(\(: any\) =>/g) || []
  ).length;
  content = content.replace(
    /(beforeEach|afterEach)\(\(: any\) =>/g,
    "$1(() =>",
  );
  ...
  // Fix test functions
  const testMatches = (
    content.match(/(test|it)\('([^']+)': any, \(: any\) =>/g) || []
  ).length;
  content = content.replace(
    /(test|it)\('([^']+)': any, \(: any\) =>/g,
    "$1('$2', () =>",
  ...
  // Fix describe blocks
  const descMatches = (
    content.match(/describe\('([^']+)': any, \(: any\) =>/g) || []
  ).length;
  content = content.replace(
    /describe\('([^']+)': any, \(: any\) =>/g,
    "describe('$1', () =>",
  ...
  const expectMatches = (content.match(/expect\(([^)]+) as any\)/g) || [])
    .length;
  content = content.replace(/expect\(([^)]+) as any\)/g, "expect($1)");
  fileFixes += expectMatches;
  if (expectMatches > 0)
    console.log(`  ✅ Fixed ${expectMatches} expect calls`);

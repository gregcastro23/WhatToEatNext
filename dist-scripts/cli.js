...
const progressReporter_1 = require("./progressReporter.cjs");
function execNode(cmd) {
  node_child_process_1.default.execSync(cmd, { stdio: "inherit" });
}
function tryCompiled(toolBaseName) {
  ...
  if (!here) return null;
  const candidate = node_path_1.default.join(here, `${toolBaseName}.cjs`);
  return candidate;
}
function main() {
  ...
  switch (command) {
    case "baseline": {
      ...
      const count =
        baselineIdx !== -1 && rest[baselineIdx + 1]
          ? Number(rest[baselineIdx + 1])
          : 965;
      (0, progressReporter_1.createBaselineReport)(
        "reports/unused-vars-baseline.json",
        count,
      );
      break;
    }
    case "analyze": {
      ...
      if (compiled) {
        ...
        execNode(
          `node --enable-source-maps --loader ts-node/esm ${node_path_1.default.posix.join("src", "scripts", "unused-vars", "analyzeUnusedVariables.ts")} --out ${out}`,
        );
      }
      break;
    }
    case "batch": {
      ...
      const maxBatch =
        maxBatchIdx !== -1 && rest[maxBatchIdx + 1]
          ? rest[maxBatchIdx + 1]
          : "15";
      const maxCrit =
        maxCritIdx !== -1 && rest[maxCritIdx + 1] ? rest[maxCritIdx + 1] : "8";
      ...
      if (compiled) {
        ...
        execNode(
          `node --enable-source-maps --loader ts-node/esm ${node_path_1.default.posix.join("src", "scripts", "unused-vars", "batchEliminateUnused.ts")} --in ${inPath} ${dry} --max-batch ${maxBatch} --max-batch-critical ${maxCrit}`,
        );
      }
      break;
    }
    case "progress": {
      ...
      (0, progressReporter_1.updateProgress)({});
      break;
    }
    default: {
      // eslint-disable-next-line no-console
      console.log(
        "Usage: yarn unused-vars <baseline|analyze|batch|progress> [options]",
      );
    }
  }
}

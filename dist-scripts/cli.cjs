...
function main() {
    ...
    switch (command) {
        case 'baseline': {
            ...
            (0, progressReporter_1.createBaselineReport)('reports/unused-vars-baseline.json', count);
            break;
        }
        case 'analyze': {
            ...
            if (compiled) {
                ...
                execNode(`node --enable-source-maps --loader ts-node/esm ${node_path_1.default.posix.join('src', 'scripts', 'unused-vars', 'analyzeUnusedVariables.ts')} --out ${out}`);
            }
            break;
        }
        case 'batch': {
            ...
            if (compiled) {
                ...
                execNode(`node --enable-source-maps --loader ts-node/esm ${node_path_1.default.posix.join('src', 'scripts', 'unused-vars', 'batchEliminateUnused.ts')} --in ${inPath} ${dry} --max-batch ${maxBatch} --max-batch-critical ${maxCrit}`);
            }
            break;
        }
        case 'progress': {

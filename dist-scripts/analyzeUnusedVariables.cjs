...
const domainPreservation_1 = require("./domainPreservation.cjs");
function parseArgs(argv) {
  ...
  const maxFiles =
    maxIndex !== -1 && argv[maxIndex + 1]
      ? Number(argv[maxIndex + 1])
      : undefined;
  return { outPath, maxFiles };
}
async function collectUnusedVariables(maxFiles) {
  const outputFile = node_path_1.default.resolve(
    process.cwd(),
    "temp-lint.json",
  );
  ...
  const limited =
    typeof maxFiles === "number"
      ? results.slice(0, Math.max(0, maxFiles))
      : results;
  const findings = [];
  for (const res of limited) {
    ...
    for (const msg of res.messages) {
      ...
      const decision = (0, domainPreservation_1.decidePreservation)(
        variableName,
        filePath,
      );
      findings.push({

...
// Function to get real zodiac sign modality
function getZodiacModality(sign: string): string {
  ...
  return modalityMap[sign.toLowerCase()] || 'Cardinal';
}
...
// Function to calculate real dignity effect based on planetary dignity
function calculateDignityEffect(planet: string, sign: string): Record<string, number> {
  const dignity = getPlanetaryDignity(planet, sign);
  const element = getZodiacElement(sign);
  
  const effect: Record<string, number> = { Fire: 0, Water: 0, Air: 0, Earth: 0 };
  
  if (dignity !== 0) {
    // Apply dignity effect to the sign's element
    effect[element] = dignity;
    
    // Apply additional effects for major dignities
    if (Math.abs(dignity) >= 2) {
      // Major dignity affects multiple elements
      ...
      const planetElementList = planetElements[planet] || [element];
      planetElementList.forEach(planetElement => {
        effect[planetElement] = (effect[planetElement] || 0) + dignity;
      });
    }
  }
...
function alchemize(planetaryPositions: Record<string, PlanetaryPosition>): StandardizedAlchemicalResult {
  ...
  for (const [planet, position] of Object.entries(planetaryPositions)) {
    ...
    if (alchemy) {
      ...
      log(`  Alchemy: Spirit=${(alchemy.Spirit * dignityMultiplier).toFixed(2)}, Essence=${(alchemy.Essence * dignityMultiplier).toFixed(2)}, Matter=${(alchemy.Matter * dignityMultiplier).toFixed(2)}, Substance=${(alchemy.Substance * dignityMultiplier).toFixed(2)}`, 'gray');
    }
  ...
  log('\n📊 Final Totals:', 'bright');
  log(`  Alchemy: Spirit=${totals.Spirit.toFixed(2)}, Essence=${totals.Essence.toFixed(2)}, Matter=${totals.Matter.toFixed(2)}, Substance=${totals.Substance.toFixed(2)}`, 'yellow');
  log(`  Elements: Fire=${totals.Fire.toFixed(2)}, Water=${totals.Water.toFixed(2)}, Air=${totals.Air.toFixed(2)}, Earth=${totals.Earth.toFixed(2)}`, 'yellow');
  ...
  return {
    ...
    metadata: {
      source: 'alchemize',
      dominantElement,
      dominantModality: getZodiacModality(planetaryPositions['Sun']?.sign || 'aries'),
...
async function demonstrateAlchemize() {
  log('🔮 Demonstrating Alchemize Function with Extracted Planetary Positions', 'bright');
  ...
  try {
    ...
    Object.entries(planetaryPositions).forEach(([planet, position]) => {
      log(`   ${planet}: ${position.sign} ${position.degree}°${position.minute || 0}' ${position.isRetrograde ? '(R)' : ''}`, 'cyan');
    });
    ...
    // Debug: Print the entire result object
    log('\n🐞 Debug: Raw alchemize result:', 'magenta');
    ...
    if (!result || typeof result !== 'object') {
      log('❌ Error: Result is not an object', 'red');
      return;
    }
    if (!result.elementalProperties) {
      log('❌ Warning: elementalProperties is missing from result', 'red');
      return;
    }
    if (!result.thermodynamicProperties) {
      log('❌ Warning: thermodynamicProperties is missing from result', 'red');
      return;
    }
    if (typeof result.kalchm === 'undefined' || typeof result.monica === 'undefined') {
      log('❌ Warning: kalchm or monica is missing from result', 'red');
    }
    ...
    log(`   Entropy: ${result.thermodynamicProperties.entropy.toFixed(4)}`, 'yellow');
    log(`   Reactivity: ${result.thermodynamicProperties.reactivity.toFixed(4)}`, 'yellow');
    log(`   Greg's Energy: ${result.thermodynamicProperties.gregsEnergy.toFixed(4)}`, 'yellow');
    ...
    log('\n✅ Alchemize demonstration completed successfully!', 'green');

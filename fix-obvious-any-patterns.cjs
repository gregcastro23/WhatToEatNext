    ...
    for (const { pattern, replacement, description } of replacements) {
      if (typeof replacement === "function") {
        ...
        const changeCount =
          ...
          (newContent.match(pattern) || []).length;
        if (changeCount > 0) {
          content = newContent;
          fixes += changeCount;
...
function generateExplicitAnyReport() {
  try {
    ...
    lines.forEach((line) => {
      ...
      if (match) {
        ...
        if (line.includes("jest.MockedFunction")) {
          patternStats["Jest Mock Functions"] =
            (patternStats["Jest Mock Functions"] || 0) + 1;
        } else if (line.includes("Record<string, any>")) {
          patternStats["Record Types"] =
            (patternStats["Record Types"] || 0) + 1;
        } else if (line.includes("any[]")) {
          patternStats["Array Types"] = (patternStats["Array Types"] || 0) + 1;
        } else if (line.includes(": any =")) {
          patternStats["Variable Assignments"] =
            (patternStats["Variable Assignments"] || 0) + 1;
        } else if (line.includes(": any)")) {
          patternStats["Function Parameters"] =
            (patternStats["Function Parameters"] || 0) + 1;
        } else {
    ...
    console.log("\n💡 Recommendations:");
    console.log(
      "  - Many issues are in test files where any types may be appropriate",
    );
    console.log("  - Jest mock functions could use more specific typing");
    console.log(
      "  - Record<string, any> types are often correct for dynamic data",
    );

...
try {
  ...
  results.forEach((file) => {
    if (file.messages && file.messages.length > 0) {
      const parseErrors = file.messages.filter(
        (msg) => msg.message && msg.message.includes("Parsing error"),
      );
  ...
  const sorted = Array.from(filesWithParseErrors.entries()).sort(
  ...
  console.log(
    `Total errors: ${sorted.reduce((sum, [, data]) => sum + data.count, 0)}\n`,
  );

  // Show files with 1-5 errors (easy targets)
  const easyFiles = sorted.filter(
    ([, data]) => data.count >= 1 && data.count <= 5,
  );

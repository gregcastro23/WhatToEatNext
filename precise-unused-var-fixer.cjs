...
class PreciseUnusedVarFixer {
  ...
  async fixUnusedVariables() {
    ...
    const lintOutput = execSync(
      'yarn lint 2>&1 | grep -E "(no-unused-vars|@typescript-eslint/no-unused-vars)"',
      {
        encoding: "utf8",
      },
    );
    ...
    this.log(
      `Found unused variables in ${Object.keys(fileWarnings).length} files`,
    );
        ...
        for (const { line, varName } of variables) {
          ...
          if (lineIndex >= 0 && lineIndex < lines.length) {
            ...
            // Pattern 2: Variable declarations
              ...
              originalLine.includes(`const ${varName}`) ||
              originalLine.includes(`let ${varName}`)
            ) {
              lines[lineIndex] = originalLine.replace(
                new RegExp(`\\b${varName}\\b`, "g"),
                `_${varName}`,
    ...
    this.log(
      `Fixed ${this.fixedCount} unused variables in ${this.processedFiles} files`,
    );
  }
  ...
  async execute() {
    ...
    const reductionPercent =
      initialWarnings > 0
        ? ((reduction / initialWarnings) * 100).toFixed(1)
        : 0;

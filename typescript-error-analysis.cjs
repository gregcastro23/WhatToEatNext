...
const ERROR_DEFINITIONS = {
  TS18046: {
    ...
    description:
      'Variable is of type "unknown" and needs type assertion or narrowing',
    severity: "high",
  ...
  TS2571: {
    ...
    description:
      'Object is of type "unknown" and cannot be used without type assertion',
    severity: "high",
    category: "type-safety",
...
class TypeScriptErrorAnalyzer {
  ...
  async collectErrors() {
    try {
      ...
      const errorLines = detailedOutput
        .trim()
        .split("\n")
        .filter((line) => line.trim());
      ...
      console.warn(
        "Warning: Could not collect all error details:",
        error.message,
      );
    }
  }
    ...
    for (const [errorCode, count] of Object.entries(
      ...
      if (definition) {
        ...
        severityBreakdown[severity] =
          (severityBreakdown[severity] || 0) + count;
        ...
        categoryBreakdown["unknown"] =
          (categoryBreakdown["unknown"] || 0) + count;
        severityBreakdown["unknown"] =
          (severityBreakdown["unknown"] || 0) + count;
      }
    }
  ...
  async identifyHighImpactFiles() {
    ...
    for (const fileInfo of this.analysis.highImpactFiles) {
      const fileErrors = this.errors.filter(
        (error) => error.file === fileInfo.file,
      );
      const errorTypes = {};
      ...
      fileInfo.dominantErrorType = Object.entries(errorTypes).sort(
        ([, a], [, b]) => b - a,
      )[0]?.[0];
    }
  }
  ...
  async generateRecommendations() {
    ...
    if (this.analysis.criticalErrors.length > 0) {
      recommendations.push({
        ...
        actions: this.analysis.criticalErrors.map(
          (error) =>
    ...
    if (this.analysis.highImpactFiles.length > 0) {
      recommendations.push({
        ...
        actions: this.analysis.highImpactFiles
          .slice(0, 10)
          .map(
            (file) =>
    ...
    const automationCandidates = Object.entries(
      this.analysis.errorBreakdown,
    ).filter(([code, count]) => {
      ...
      return (
        definition &&
        count > 50 &&
        definition.fixStrategy.includes("Type assertion")
      );
    });
    ...
    if (automationCandidates.length > 0) {
      recommendations.push({
        ...
        actions: automationCandidates.map(
          ([code, count]) =>
  ...
  async generateReport() {
    ...
    // Write JSON report
    fs.writeFileSync(
      "typescript-error-analysis-report.json",
...
${report.highImpactFiles
  .slice(0, 10)
  .map(
    (file, index) =>
      `${index + 1}. **${file.file}**: ${file.errorCount} errors (${file.dominantErrorType})`,
  )
  .join("\n")}
  ...
  displaySummary(report) {
    ...
    Object.entries(report.errorBreakdown)
      ...
      .forEach(([code, count]) => {
        const def = ERROR_DEFINITIONS[code];
        console.log(
          `   ${code}: ${count} errors - ${def ? def.name : "Unknown"}`,
        );
      });

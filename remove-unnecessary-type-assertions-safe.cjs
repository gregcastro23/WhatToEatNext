...
class SafeUnnecessaryTypeAssertionRemover {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (!['node_modules', '.git', 'dist', 'build', '.next', '__tests__', 'tests'].includes(entry.name)) {
   ...
   */
  run() {
    console.log(`🚀 Starting safe unnecessary type assertion removal${this.dryRun ? ' (DRY RUN)' : ''}...\n`);

    // Find all TypeScript files to process (excluding tests and campaign files)
    const files = this.findTypeScriptFiles('./src');
    console.log(`Found ${files.length} TypeScript files to process (excluding tests and campaign files)\n`);

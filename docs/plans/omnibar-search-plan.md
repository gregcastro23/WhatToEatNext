...
| **2 — API** | `GET /api/search` + Zod schema (`searchSchemas.ts`, text-only recipe rows) + route tests (handler called directly; jest blocks network) | Warm p95 < 150 ms (measured p95 6.7 ms, p99 23.5 ms over 937 real queries; cold build ≈ 0.5 s). Cache headers asserted; 400/429/503 no-store. `bun run verify` green |
| **2.5 — Catalog and dossiers** *(added 2026-09-23)* | **Union catalog**: `allIngredients` ∪ unified, keyed by slug, src/data winning field by field (≈1,010; brings in eggs). **Readable slug dossiers** `/ingredients/<slug>`: name- and key-style URLs 308 to the slug, no substring fallback. **SSR** metadata, body and JSON-LD. **Slugs in the sitemap**. The dossier's "used in" list comes from the live reverse index. Hero hrefs switch to slugs | `dossierHrefs.test.ts` known list → empty; `egg` has a hero; a crawl of every sitemap slug renders its own ingredient with a real title |
| **3 — Omnibar UI** | **Inline header search** with a results dropdown on desktop; **full-screen sheet on mobile** from the header search icon; ⌘K focuses the bar. **Smart Enter** (exact top result → open it, else `/search`). **Minimal `/search?q=`** (SSR, noindex). Text-only recipe rows. Merge rule: an exact local nav match (`pantry`) suppresses a server correction or hero | `/` First Load stays ≤ 220 kB. Component tests (typing "spinich" shows the correction, the hero and the recipes; keyboard walk). Screenshots from the preview |

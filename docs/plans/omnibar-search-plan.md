...
| **4 — Actions and deep links** | Hero actions: **Cook with this** (`/recipe-builder?ingredients=` prefill), **Add to pantry** (existing local pantry; account sync is a separate follow-up), **Show pairings** (complementary list, linked where a name resolves). `/sauces?focus=`. ~~Add to cart~~ dropped | Built: action chips under the hero (←/→ from the hero; ↵ or click); pairings open in place, cards linked to their dossiers (2,336 of 4,208 pairing names are exact card names, 55.5%), other names search; the builder queues the card with its own facts after its saved queue loads; `/sauces?focus=` renders the sauce server-side, so Smart Enter now opens an exact sauce. The same actions on `/search` and the dossier. Each action verified in a local production build and the preview |
| **5 — Multi-ingredient and intent** | `spinach eggs feta` coverage ranking. Chips for derived diet, parsed time and season. Planetary and quality filters. Elemental queries only if the per-category distinct-vector check passes | Built (`lib/search/intent*.ts`, `coverage.ts`, `recipeDiet.ts`, `authoredFacts.ts`): **several ingredients** rank recipes by how many they use ("uses 2 of 3 · missing feta"; together = at least two), and the named ingredients are always listed. **Diet** is derived per recipe with the canonical classifier from every line and every card it names (worst verdict wins; each side of an "or" counts): 381 vegan and 629 vegetarian of 1,063 live recipes. An independent regex witness found carbonara (guanciale) and Lasagna al Forno (Bolognese ragù) in vegetarian results; 5 measured classifier terms fixed them (0 disagreements). Gluten-, dairy- and nut-free are shown "not verified" (10, 0 and 1 cards attest). **Time and meal:** the live catalog stores prep 30 + cook 30 and category "main" on 1,063 of 1,063 recipes, so both come from the static twin (Phase 0 bridge, 1,063 of 1,063); the HSCA generator's fill-in 15 is not a time, leaving 566 with a stated time. "Quick" = 30 min or less. **Season, planet, quality** filter ingredient cards (a season list names the season; year-round cards are left out). Regional names suggest a cuisine (`oaxacan` → Mexican). **Elemental: not built.** The check failed: 23 of 52 category × element rankings are template-dominated, and "earth grains" has 2 distinct vectors in its top 10 (`elementalCheck.test.ts` reruns it). Every chip carries its basis, spelled out on `/search` |
...
| `spinach eggs` | Recipes containing both *(Phase 5 ✅: 5 recipes, Spinach Pasta first)* | "uses 2 of 2" label |

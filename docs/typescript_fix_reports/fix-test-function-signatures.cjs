...
class TestFunctionSignatureFixer {
  ...
  extractTestFilesFromOutput(output) {
    const errorLines = output.split('\n').filter(line =>
      line.includes('error TS1005') && line.includes('.test.')
    );
    const files = new Set();
  ...
  fixTestFunctionSignatures(content) {
    ...
    const testPattern1 = /test\s*\(\s*'([^']+)'\s*:\s*any\s*,\s*async\s*\(\s*\)\s*=>/g;
    const matches1 = [...fixedContent.matchAll(testPattern1)];
    if (matches1.length > 0) {
      fixedContent = fixedContent.replace(testPattern1, "test('$1', async () =>");
      fixes += matches1.length;
      console.log(`    Fixed ${matches1.length} test function signatures with ': any, async()'`);
    }
    ...
    if (matches2.length > 0) {
      ...
      console.log(`    Fixed ${matches2.length} test function signatures with ': any, ()'`);
    }

    // Pattern 3: describe('description': any, () => {
    // Fix to: describe('description', () => {
    const describePattern = /describe\s*\(\s*'([^']+)'\s*:\s*any\s*,\s*\(\s*\)\s*=>/g;
    const matches3 = [...fixedContent.matchAll(describePattern)];
    if (matches3.length > 0) {
      fixedContent = fixedContent.replace(describePattern, "describe('$1', () =>");
      fixes += matches3.length;
      console.log(`    Fixed ${matches3.length} describe function signatures with ': any, ()'`);
    }
    ...
    if (matches4.length > 0) {
      ...
      console.log(`    Fixed ${matches4.length} it function signatures with ': any, async()'`);
    }

    // Pattern 5: beforeEach/afterEach with malformed signatures
    const beforeEachPattern = /beforeEach\s*\(\s*:\s*any\s*,\s*async\s*\(\s*\)\s*=>/g;
    const matches5 = [...fixedContent.matchAll(beforeEachPattern)];
    if (matches5.length > 0) {
      fixedContent = fixedContent.replace(beforeEachPattern, "beforeEach(async () =>");
      fixes += matches5.length;
      console.log(`    Fixed ${matches5.length} beforeEach function signatures`);
    }
  ...
  processFile(filePath) {
    try {
      ...
      // Fix patterns
      const { content: fixedContent, fixes } = this.fixTestFunctionSignatures(originalContent);
  ...
  async repair() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if ((i + 1) % 3 === 0) {
        console.log(`  🔍 Build validation checkpoint at file ${i + 1}/${files.length}`);

...
class TargetedWarningFixer {
  constructor() {
    ...
    this.preservePatterns = {
      astrological: /\b(planet|degree|sign|longitude|position|transit|retrograde|aspect|house|element|zodiac|astro|lunar|solar|mercury|venus|mars|jupiter|saturn|uranus|neptune|pluto)\b/i,
      campaign: /\b(metrics|progress|safety|campaign|intelligence|system|enterprise|validation|analyzer|fixer|processor)\b/i,
      test: /\b(mock|stub|test|expect|describe|it|beforeEach|afterEach|jest|UNUSED_|spec|fixture)\b/i,
      elemental: /\b(fire|water|earth|air|elemental|harmony|compatibility|balance|alchemy|alchemical)\b/i,
      debug: /\b(debug|console|log|error|warn|info|trace|performance|monitoring)\b/i
    };
  }
  ...
  async fixConsoleStatements() {
    ...
    try {
      ...
      for (const filePath of consoleFiles.slice(0, 100)) { // Process first 100 files
      ...
      this.log(`✅ Fixed console statements in ${fixedFiles} files (${fixedStatements} statements)`);
  ...
  async fixConsoleStatementsInFile(filePath) {
    try {
      ...
      const isDebugFile = /\b(debug|test|spec|\.test\.|\.spec\.|setupMemoryManagement|memoryOptimization)\b/i.test(filePath);
      ...
      if (isDebugFile || isConfigFile) {
        ...
        const consolePattern = /^(\s*)(console\.(log|warn|error|info|debug|trace))/gm;
        newContent = newContent.replace(consolePattern, (match, indent, consoleCall) => {
          fixCount++;
          modified = true;
      ...
      this.errors.push(`Error processing console statements in ${filePath}: ${error.message}`);
  ...
  shouldPreserveConsole(args, content) {
    ...
    return preserveKeywords.some(keyword => args.includes(keyword)) ||
           Object.values(this.preservePatterns).some(pattern => pattern.test(args));
  }

  async fixExplicitAnyWarnings() {
    this.log('🔧 Step 2: Fixing explicit any warnings (2791 issues) - Conservative approach');
    ...
    try {
      ...
      for (const filePath of anyFiles.slice(0, 50)) { // Process first 50 files conservatively
      ...
      this.log(`✅ Fixed explicit any types in ${fixedFiles} files (${fixedAnyTypes} types)`);
  ...
  async fixExplicitAnyInFile(filePath) {
    try {
      ...
      this.errors.push(`Error processing explicit any in ${filePath}: ${error.message}`);
      return { modified: false, count: 0 };
    }
  }

  containsDomainLogic(content) {
    return Object.values(this.preservePatterns).some(pattern => pattern.test(content));
  }
  ...
  async fixUnusedVariableWarnings() {
    ...
    try {
      ...
      for (const filePath of unusedVarFiles.slice(0, 75)) { // Process first 75 files
      ...
      this.log(`✅ Fixed unused variables in ${fixedFiles} files (${fixedVariables} variables)`);
  ...
  async fixUnusedVariablesInFile(filePath) {
    try {
      ...
      const unusedParamPattern = /function\s+[a-zA-Z_$][a-zA-Z0-9_$]*\s*\(([^)]+)\)/g;
      newContent = newContent.replace(unusedParamPattern, (match, params) => {
        const paramList = params.split(',').map(p => p.trim());
        const modifiedParams = paramList.map(param => {
          const paramName = param.split(':')[0].trim();
          if (!paramName.startsWith('_') && !this.isParamUsed(paramName, content)) {
            fixCount++;
            modified = true;
      ...
      this.errors.push(`Error processing unused variables in ${filePath}: ${error.message}`);
      return { modified: false, count: 0 };
    }
  }
  ...
  async generateReport() {
    ...
    fs.writeFileSync('targeted-warning-fixes-report.json', JSON.stringify(report, null, 2));

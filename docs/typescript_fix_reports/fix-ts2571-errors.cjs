...
function getTS2571Errors() {
  try {
    const output = execSync('yarn tsc --noEmit --skipLibCheck 2>&1 | grep "TS2571"', {
      encoding: 'utf8',
      stdio: 'pipe'
    });
    ...
    if (lineIndex >= 0 && lineIndex < lines.length) {
      ...
      lineErrors.forEach(error => {
        if (error.message.includes("Object is of type 'unknown'")) {
          ...
          line = line.replace(/Object\.(keys|values|entries)\(([^)]+)\)/g, (match, method, obj) => {
            return `Object.${method}(${obj} as Record<string, unknown>)`;
          });

          // Pattern 2: for...in loops with unknown objects
          line = line.replace(/for\s*\(\s*const\s+(\w+)\s+in\s+([^)]+)\)/g, (match, key, obj) => {
...
function main() {
  ...
  for (const filePath of sortedFiles.slice(0, 15)) { // Process top 15 files

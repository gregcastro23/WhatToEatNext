...
function fixMainPageIntegrationTest() {
  ...
  const mockComponentPattern = /const\s+(\w+)\s*=\s*\(\)\s*=>\s*<div[^>]*>.*?<\/div>/g;
  ...
  content = content.replace(
    ...
    (match, componentName, props) => {
      ...
      const componentDefPattern = new RegExp(`const ${componentName}\\s*=\\s*\\(\\)\\s*=>`);
...
function fixCampaignTestUtils() {
  ...
  if (!content.includes('type SafetyLevel') && !content.includes('interface SafetyLevel')) {
  ...
  content = content.replace(
    ...
    (match, obj, prop) => {
      ...
      const matchingLines = lines.filter(line =>
        line.includes(match) &&
        (line.includes('unknown') || content.includes(`${obj} is of type 'unknown'`))
      );

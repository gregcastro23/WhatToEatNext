...
async function runPhase(phase) {
  ...
  if (!DRY_RUN) {
    ...
    if (actualResult.code !== 0) {
      console.error(
        `❌ Phase ${phase.id} execution failed:`,
        actualResult.stderr,
      );
      return false;
    }
...
async function main() {
  ...
  const phasesToRun = PHASE
    ? phases.filter((p) => p.id === parseInt(PHASE))
    : phases;
  ...
  if (!DRY_RUN && successfulPhases === phasesToRun.length) {
    ...
    if (finalErrors >= 0 && initialErrors >= 0) {
      ...
      console.log(
        `Reduction rate: ${((reduction / initialErrors) * 100).toFixed(1)}%`,
      );
    }
    ...
    if (!SKIP_BUILD) {
      ...
      console.log(
        `Final build status: ${finalBuild ? "✅ SUCCESS" : "❌ FAILED"}`,
      );
    }
  }

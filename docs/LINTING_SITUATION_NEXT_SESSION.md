...
After wave4 (2026-07-05), that production-code scan finds about `1326` explicit-any-like matches. The largest clusters are:
...
Remaining targets needing a genuine attempt: `lib/monitoring/prometheus-metrics.ts`, `menuPlanner/nutritionalCalculator.ts` (correctly deferred, live-page-adjacent), `kineticsFoodMatcher.ts`, `dynamicImport.ts`, `databaseCleanup.ts`, `cuisine/signatureIdentificationEngine.ts`, `types/guards.ts`, `types/alchemy.ts`, `services/AstrologizeApiCache.ts`, `hooks/usePlanetaryKinetics.ts`, plus wave3's originally-skipped `PlanetaryHourCard.tsx` (worth a proper look — sole importer is a pure pass-through wrapper, likely low risk despite the initial defer).

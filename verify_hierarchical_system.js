...
function verifyLevel1Ingredients() {
  ...
  try {
    ...
    const normalizedSum = Object.values(avgElementals).reduce(
      (s, v) => s + v,
      0,
    );
    if (Math.abs(normalizedSum - 1.0) > 0.01) {
      errors.push(
        `Ingredient aggregation: Result doesn't sum to 1.0 (sum: ${normalizedSum})`,
      );
    } else {
      results.push(
        `✅ Ingredient aggregation: Valid normalized result (sum: ${normalizedSum.toFixed(3)})`,
      );
    }
  } catch (error) {
  ...
  try {
    ...
    Object.entries(mockAlchemical).forEach(([prop, value]) => {
      if (value < 0) {
        ...
        errors.push(
          `Alchemical property ${prop}: Unreasonably high value ${value}`,
        );
      } else {
        results.push(
          `✅ Alchemical property ${prop}: Valid value ${value.toFixed(2)}`,
        );
      }
    });
    ...
    results.push(
      "✅ Recipe computation: Mock thermodynamic properties generated",
    );
    ...
    const missingPlanets = requiredPlanets.filter(
      (planet) => !TEST_PLANETARY_POSITIONS[planet],
    );
...
function verifyLevel3Cuisines() {
  ...
  try {
    ...
    results.push(
      `✅ Signature identification: ${mockSignatures.length} signatures identified`,
    );
    ...
    results.push(
      `✅ Planetary pattern analysis: ${mockPlanetaryPatterns.length} patterns identified`,
    );

    mockPlanetaryPatterns.forEach((pattern) => {
      results.push(
        `   ${pattern.planet}: ${pattern.planetaryStrength.toFixed(2)} strength, dominant element: ${pattern.dominantElement}`,
      );
    });
    ...
    if (mockRecommendations.length > 0) {
      results.push(
        `✅ Recommendation engine: ${mockRecommendations.length} recommendations generated`,
      );
      results.push(
        `   Top recommendation: ${mockRecommendations[0].cuisineName} (${Math.round(mockRecommendations[0].compatibilityScore * 100)}% match)`,
      );
    } else {
...
async function main() {
  ...
  console.log(
    `   Performance: ${summary.performance.averageTimePerTest.toFixed(2)}ms per test`,
  );

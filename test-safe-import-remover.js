...
try {
  ...
  const beforeCount = execSync('yarn lint --format=compact 2>&1 | grep -E "@typescript-eslint/no-unused-vars" | wc -l', {
    encoding: 'utf8'
  }).trim();
  ...
  if (fs.existsSync(removerPath)) {
    ...
    const hasSafetyProtocols = content.includes('astrologicalPatterns') && content.includes('campaignSystemPatterns');
  ...
  const totalIssues = execSync('yarn lint --format=compact 2>&1 | grep -E "(Warning|Error)" | wc -l', {
    encoding: 'utf8'
  }).trim();

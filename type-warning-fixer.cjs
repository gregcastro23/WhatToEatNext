...
class TypeWarningFixer {
  ...
  async fixTypeWarnings() {
    ...
    try {
      ...
      console.log(
        `📁 Found ${filesWithAnyTypes.length} files with explicit any types\n`,
      );
      ...
      for (let i = 0; i < filesWithAnyTypes.length; i += batchSize) {
        ...
        console.log(
          `🔄 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(filesWithAnyTypes.length / batchSize)}`,
        );
  ...
  async processBatch(files) {
    for (const filePath of files) {
      ...
      try {
        ...
        if (isSensitive) {
          console.log(
            `    ⚠️  Sensitive file detected, using conservative approach`,
          );
        }
        ...
        if (result.modified) {
          ...
          console.log(
            `    🛡️  Preserved: ${result.preserved} any types (domain-specific)`,
          );
    ...
    for (const [pattern, replacement] of Object.entries(
      ...
      if (matches) {
        ...
        if (isSensitive && matches.length > 5) {
          console.log(
            `    ⚠️  Too many replacements (${matches.length}) in sensitive file, skipping pattern: ${pattern}`,
          );
          continue;
        }
  ...
  shouldPreserveAnyTypes(content, filePath) {
    ...
    const shouldPreserve =
      preservePatterns.length > 0 && this.isSensitiveFile(filePath);
  ...
  generateReport() {
    ...
    if (this.replacedTypes.length > 0) {
      ...
      for (const replacement of this.replacedTypes) {
        ...
        replacementCounts[key] =
          (replacementCounts[key] || 0) + replacement.count;
      }
    ...
    fs.writeFileSync(
      "type-warning-fixing-report.json",
  ...
  checkProgress() {
    try {
      ...
      console.log(
        `   Estimated reduction: ${2676 - parseInt(newCount)} warnings fixed`,
      );
    } catch (error) {

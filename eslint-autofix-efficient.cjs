...
class EfficientAutoFixProcessor {
  ...
  async getSourceFiles() {
    try {
      ...
      const output = execSync(
        'find src -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" | head -100',
        {
          encoding: "utf8",
        },
      );
  ...
  async checkFileFixable(filePath) {
    try {
      ...
      const output = execSync(
        `yarn lint --fix-dry-run --format=json "${filePath}" 2>/dev/null`,
        {
          encoding: "utf8",
      ...
      if (
        results.length > 0 &&
        results[0].output &&
        results[0].output !== results[0].source
      ) {
        return {
  ...
  async applyFixToFile(filePath) {
    try {
      ...
      try {
        ...
        this.fixedFiles++;
        this.log(
          `✅ Successfully fixed: ${path.relative(process.cwd(), filePath)}`,
        );
        return true;
      } catch (compileError) {
        ...
        fs.unlinkSync(backupPath);
        this.log(
          `❌ Fix caused compilation error, restored: ${path.relative(process.cwd(), filePath)}`,
        );
        return false;
      }
    } catch (error) {
  ...
  async processFiles() {
    ...
    for (let i = 0; i < sourceFiles.length; i++) {
      ...
      if (i % 10 === 0) {
        ...
        this.log(
          `Progress: ${progress}% (${i}/${sourceFiles.length} files checked)`,
        );
      }
      ...
      if (fixableInfo) {
        fixableCount++;
        this.log(
          `Found fixable issues in: ${path.relative(process.cwd(), filePath)} (${fixableInfo.fixableErrorCount} errors, ${fixableInfo.fixableWarningCount} warnings)`,
        );
  ...
  async getLintingStats() {
    try {
      ...
      const warningOutput = execSync(
        'yarn lint --format=json 2>/dev/null | jq "[.[] | .warningCount] | add" || echo "0"',
        {
          encoding: "utf8",
        },
      );
  ...
  generateReport() {
    ...
    fs.writeFileSync(
      "eslint-autofix-efficient-report.json",
...
async function main() {
  ...
  try {
    ...
    processor.log(
      `Initial linting status: ${initialStats.errors} TypeScript errors, ${initialStats.warnings} ESLint warnings`,
    );
    ...
    processor.log(
      `Final linting status: ${finalStats.errors} TypeScript errors, ${finalStats.warnings} ESLint warnings`,
    );

...
function analyzeFileForLegitimateAny(content, filePath) {
  ...
  const isTestFile = fileName.includes('.test.') || fileName.includes('.spec.') || filePath.includes('__tests__');
  ...
  for (const match of anyMatches) {
    ...
    const analysis = analyzeLegitimacy(line, context, filePath, isTestFile, isTypeFile, isConfigFile);
  ...
  for (const pattern of LEGITIMATE_ANY_PATTERNS) {
    if (pattern.pattern.test(line) || pattern.pattern.test(context)) {
      ...
      if (pattern.confidence === 'medium') {
        analysis.suggestions.push('Consider creating specific interface if usage patterns are predictable');
...
async function documentFileAnyTypes(filePath) {
  try {
    ...
    console.log(`🔍 Found ${legitimateMatches.length} legitimate any types (${needsDoc.length} need documentation)`);
...
async function main() {
  ...
  const result = execSync('yarn lint --format=unix 2>/dev/null | grep "@typescript-eslint/no-explicit-any" | cut -d: -f1 | sort | uniq', { encoding: 'utf8' });
  const filesWithAny = result.trim().split('\n').filter(f => f);
  ...
  if (successfulResults.length > 0) {
    ...
    console.log(`📈 Documentation rate: ${((totalDocumented / startingCount) * 100).toFixed(1)}% of total explicit-any issues`);

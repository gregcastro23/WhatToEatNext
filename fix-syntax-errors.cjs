...
function getFilesWithSyntaxErrors() {
  try {
    const output = execSync(
      "yarn tsc --noEmit --skipLibCheck 2>&1 | grep \"TS1003\" | cut -d'(' -f1 | sort | uniq",
      {
        encoding: "utf8",
        stdio: "pipe",
      },
...
function fixSyntaxErrors(content) {
  ...
  fixed = fixed.replace(
    /\(([^)]+) as Record<string, unknown>\)\.([^)]+) as Record<string, unknown>\)/g,
  ...
  fixed = fixed.replace(
    /\) as Record<string, unknown>\)\.([a-zA-Z_$][a-zA-Z0-9_$]*)/g,
...
function main() {
  ...
  try {
    const remainingErrors = execSync(
      'yarn tsc --noEmit --skipLibCheck 2>&1 | grep "TS1003" | wc -l',
      {
        encoding: "utf8",
        stdio: "pipe",
      },

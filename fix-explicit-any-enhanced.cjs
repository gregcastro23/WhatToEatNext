...
async function classifyAnyTypes(filePath) {
  ...
  try {
    ...
    log(
      "debug",
      `Classified ${classifications.length} any types in ${filePath}`,
    );

    return classifications;
  } catch (error) {
    log("warn", `Classification failed for ${filePath}, using fallback`, {
      error: error.message,
    });
    return classifyWithFallback(filePath);
  }
...
function classifyWithFallback(filePath) {
  try {
    ...
    log("error", `Fallback classification failed for ${filePath}`, {
      error: error.message,
    });
    return [];
  }
...
function applySafeReplacements(filePath, classifications) {
  try {
    ...
    const safeReplacements = classifications.filter(
      (c) =>
        !c.isIntentional &&
        c.confidence >= 0.8 &&
        c.suggestedReplacement &&
        ["array_type", "record_type", "variable_assignment"].includes(
          c.category,
        ),
    );
    ...
    log(
      "debug",
      `Found ${safeReplacements.length} safe replacements in ${filePath}`,
    );
    ...
    if (fixes > 0) {
      if (CONFIG.dryRun) {
        log(
          "info",
          `DRY RUN: Would apply ${fixes} fixes to ${filePath}`,
          appliedFixes,
        );
    ...
    log("error", `Error applying replacements to ${filePath}`, {
      error: error.message,
    });
...
function getFilesWithExplicitAny(maxFiles = CONFIG.maxFiles) {
  try {
    ...
    log("error", "Error getting files with explicit-any warnings", {
      error: error.message,
    });
    return [];
  }
...
async function processBatch(files) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const replacementResult = applySafeReplacements(
        file.path,
        classifications,
      );
  ...
  results.improvements = {
    typeScriptErrorReduction:
      results.initialMetrics.typeScriptErrors -
      results.finalMetrics.typeScriptErrors,
    explicitAnyReduction:
      results.initialMetrics.explicitAnyWarnings -
      results.finalMetrics.explicitAnyWarnings,
  };
...
async function main() {
  ...
  try {
    ...
    const checkpoint = createSafetyCheckpoint(
      "Enhanced explicit-any fixes start",
    );
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      if (CONFIG.safetyLevel === "MAXIMUM" && !CONFIG.dryRun) {
        try {
          ...
          log(
            "error",
            `Build validation failed after batch ${batchIndex + 1} - stopping`,
          );
          break;
        }
      }
    ...
    const reportPath = path.join(
      CONFIG.backupDir,
      `enhanced-fix-report-${Date.now()}.json`,
    );

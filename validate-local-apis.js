      ...
      if (response.data) {
        ...
        if (name.includes('Astrologize')) {
          ...
          if (response.data.success && response.data.alchemicalResult) {
            ...
            console.log(`   🌍 Planetary positions: ${Object.keys(response.data.planetaryPositions || {}).length} planets`);
          } else {
...
async function main() {
  ...
  results.push(await testLocalAPI('Astrologize POST', '/api/astrologize', 'POST', TEST_PAYLOADS.astrologize));
  results.push(await testLocalAPI('Alchemize POST', '/api/alchemize', 'POST', TEST_PAYLOADS.alchemize));

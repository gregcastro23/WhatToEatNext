...
const createTempFixScript = () => {
  const originalFixScript = fs.readFileSync(
    path.join(__dirname, "fix-elemental-logic.js"),
    "utf8",
  );
  ...
  fs.writeFileSync(
    path.join(TEST_DIR, "temp-fix-script.js"),
    modifiedFixScript,
  );
...
function testFixedFile(filename, patterns) {
  ...
  for (const pattern of patterns) {
    if (pattern.type === "contains") {
      ...
      if (content.includes(pattern.value)) {
        console.log(
          `❌ FAILED: ${filename} should NOT contain: ${pattern.value}`,
        );
        passed = false;
      } else {
        console.log(
          `✅ PASSED: ${filename} does not contain: ${pattern.value}`,
        );
      }
    }
  }
...
try {
  ...
  const comparisonTests = [
    {
      type: "not_contains",
      value: "// Opposing elements with low compatibility",
    },
    {
      type: "not_contains",
      value: "return 0.2; // Opposing elements have low compatibility",
    },
    {
      type: "contains",
      value: "All different elements have good compatibility",
    },
  ];
  const comparisonPassed = testFixedFile(
    "elementComparison.ts",
    comparisonTests,
  );
  ...
  const pairsTests = [
    {
      type: "not_contains",
      value: "const fireWater = properties.Fire + properties.Water;",
    },
    {
      type: "not_contains",
      value: "const earthAir = properties.Earth + properties.Air;",
    },
  ];

...
const modifiedTsConfig = {
  ...
  include: [
    ...originalTsConfig.include.filter(
      (pattern) => !pattern.includes("jest-dom"),
    ),
    fileToCheck,
  ],
};
...
console.log(
  `🔍 Checking TypeScript in ${fileToCheck} while ignoring jest-dom...`,
);
...
tsc.on("close", (code) => {
  ...
  if (code === 0) {
    ...
    if (!errors.includes("testing-library__jest-dom")) {
      ...
      console.log(
        "💡 The file has other TypeScript errors that need to be fixed.",
      );
    } else {
  ...
  process.exit(
    code === 0 ? 0 : errors.includes("testing-library__jest-dom") ? 1 : 0,
  );
});

"use strict";
// Test script for K_alchm and Monica Constant calculations
Object.defineProperty(exports, "__esModule", { value: true });
const alchemicalEngine_1 = require("./calculations/alchemicalEngine");
// Sample values
const Spirit = 4;
const Essence = 7;
const Matter = 6;
const Substance = 2;
// Sample thermodynamic values
const Fire = 1.0;
const Water = 0.6;
const Air = 0.6;
const Earth = 0.7;
// Calculate denominator values
const denominator1 = Substance + Essence + Matter + Water + Air + Earth;
const denominator2 = Essence + Matter + Earth + Water;
const denominator3 = Matter + Earth;
// Calculate thermodynamic properties
const heat = (Math.pow(Spirit, 2) + Math.pow(Fire, 2)) / Math.pow(denominator1, 2);
const entropy = (Math.pow(Spirit, 2) + Math.pow(Substance, 2) + Math.pow(Fire, 2) + Math.pow(Air, 2)) / Math.pow(denominator2, 2);
const reactivity = (Math.pow(Spirit, 2) + Math.pow(Substance, 2) + Math.pow(Essence, 2) + Math.pow(Fire, 2) + Math.pow(Air, 2) + Math.pow(Water, 2)) / Math.pow(denominator3, 2);
const energy = heat - (reactivity * entropy);
// Calculate K_alchm and Monica Constant
const kAlchm = (0, alchemicalEngine_1.calculateKAlchm)(Spirit, Essence, Matter, Substance);
const monicaConstant = (0, alchemicalEngine_1.calculateMonicaConstant)(energy, reactivity, kAlchm);
// Log the results
console.log('Input values:');
console.log("Spirit: ".concat(Spirit));
console.log("Essence: ".concat(Essence));
console.log("Matter: ".concat(Matter));
console.log("Substance: ".concat(Substance));
console.log("Fire: ".concat(Fire));
console.log("Water": ".concat(Water));
console.log("Air: ".concat(Air));
console.log("Earth": ".concat(Earth));
console.log('\nIntermediate calculations:');
console.log("Heat: ".concat(heat));
console.log("Entropy: ".concat(entropy));
console.log("Reactivity: ".concat(reactivity));
console.log("Energy: ".concat(energy));
console.log('\nFinal results:');
console.log("K_alchm: ".concat(kAlchm));
console.log("Monica Constant: ".concat(monicaConstant));

...
const mockGetEnhancedPlanetaryPositions = getEnhancedPlanetaryPositions as jest.MockedFunction<typeof getEnhancedPlanetaryPositions>;
...
describe('Cross-Backend Planetary Rectification Integration', () => {
  ...
  describe('Basic Rectification Functionality', () => {
    test('should perform basic rectification with Planetary Agents sync', async () => {
      ...
      expect(result.rectification_report.discrepancies_found).toBeGreaterThanOrEqual(0);
      expect(result.planetary_agents_sync_status).toMatch(/synced|partial|failed/);
      expect(result.rectification_report.rectification_duration_ms).toBeGreaterThan(0);
    });
  ...
  describe('Caching and Performance', () => {
    test('should use cached results within TTL', async () => {
      ...
      // First call
      const result1 = await rectificationService.rectifyPlanetaryPositions(testDate);

      // Second call (should use cache)
      const result2 = await rectificationService.rectifyPlanetaryPositions(testDate);
  ...
  describe('Health Monitoring', () => {
    ...
    test('should report critical when VSOP87 system fails', async () => {
      mockGetEnhancedPlanetaryPositions.mockRejectedValue(new Error('VSOP87 failure'));
  ...
  describe('Emergency Rectification', () => {
    test('should perform emergency rectification bypassing cache', async () => {
      ...
      const emergencyResult = await rectificationService.emergencyRectification(testDate);

      expect(emergencyResult.success).toBe(true);
      expect(emergencyResult.rectification_report.authoritative_source).toBe('planetary_agents_authoritative');
      expect(emergencyResult.planetary_agents_sync_status).toMatch(/synced|partial/);
    });
  ...
  describe('Accuracy Validation', () => {
    test('should detect and correct position discrepancies > 0.01°', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.rectification_report.discrepancies_found).toBeGreaterThan(0);
      expect(result.rectification_report.corrections_applied).toBeGreaterThan(0);
  ...
  describe('Performance Benchmarks', () => {
    test('should complete rectification within performance targets', async () => {
      ...
      expect(result.rectification_report.rectification_duration_ms).toBeLessThan(500);
    });

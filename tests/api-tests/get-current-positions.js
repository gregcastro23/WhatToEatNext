...
async function getCurrentPlanetaryPositions() {
  try {
    ...
    Object.entries(positions).forEach(([planet, position]) => {
      console.log(`  ${planet}: ${position.sign} ${position.degree}°${position.minute}' (${position.exactLongitude.toFixed(2)}°)${position.isRetrograde ? ' (R)' : ''}`);
    });
    ...
    Object.entries(positions).forEach(([planet, position]) => {
      console.log(`    ${planet}: { sign: '${position.sign}', degree: ${position.degree}, minute: ${position.minute}, exactLongitude: ${position.exactLongitude.toFixed(2)}, isRetrograde: ${position.isRetrograde} },`);
    });

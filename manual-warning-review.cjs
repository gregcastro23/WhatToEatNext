...
for (const [category, info] of Object.entries(categories)) {
  try {
    const count = parseInt(execSync(info.command, { encoding: 'utf8' }).trim()) || 0;
...
if (results.UNUSED_VARIABLES?.count > 200) {
  recommendations.push({
    ...
    description: 'Many may be intentionally preserved for astrological/campaign logic'
  });
}
...
console.log(`Estimated effort: ${totalManualReview > 1000 ? 'HIGH' : totalManualReview > 500 ? 'MEDIUM' : 'LOW'}`);
console.log();
...
fs.writeFileSync('manual-warning-review-report.json', JSON.stringify(reportData, null, 2));

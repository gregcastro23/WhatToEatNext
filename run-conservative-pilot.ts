...
} from "./src/services/campaign/unintentional-any-elimination/types";
...
async function main() {
  ...
  console.log(
    `   Target success rate: ${(config.targetSuccessRate * 100).toFixed(1)}%`,
  );
  ...
  console.log(
    `   Real-time validation: ${config.realTimeValidation ? "Enabled" : "Disabled"}`,
  );
  console.log("");
  ...
  try {
    ...
    console.log(
      `Execution time: ${Math.round((result.pilotEndTime.getTime() - result.pilotStartTime.getTime()) / 1000)}s`,
    );
    console.log("");
    ...
    console.log(
      `   Successful replacements: ${result.totalSuccessfulReplacements}`,
    );
    ...
    if (result.batchResults.length > 0) {
      ...
      result.batchResults.forEach((batch) => {
        ...
        console.log(
          `   Batch ${batch.batchNumber}: ${batch.successfulReplacements}/${batch.casesProcessed} successful (${duration}s) ${batch.buildStable ? "✅" : "❌"}`,
        );
      });
      console.log("");
    }
    ...
    console.log(
      `   Rollbacks performed: ${result.safetyMetrics.rollbacksPerformed}`,
    );
    console.log(`   Batch failures: ${result.safetyMetrics.batchFailures}`);
    console.log(
      `   Compilation errors: ${result.safetyMetrics.compilationErrors}`,
    );
    console.log(
      `   Safety protocol activations: ${result.safetyMetrics.safetyProtocolActivations}`,
    );
    console.log("");
    ...
    const batchSizeValid = result.batchResults.every(
      (batch) =>
        batch.casesProcessed >= config.minFilesPerBatch &&
        batch.casesProcessed <= config.maxFilesPerBatch,
    );
    console.log(
      `   ✓ Limited batch processing (10-15 files): ${batchSizeValid ? "✅ PASS" : "❌ FAIL"}`,
    );

    // Requirement: Focus on array types and simple Record types
    const focusedOnTargetTypes =
      config.focusCategories.includes(AnyTypeCategory.ARRAY_TYPE) &&
      config.focusCategories.includes(AnyTypeCategory.RECORD_TYPE);
    console.log(
      `   ✓ Focus on array/Record types: ${focusedOnTargetTypes ? "✅ PASS" : "❌ FAIL"}`,
    );
    ...
    console.log(
      `   ✓ Real-time validation enabled: ${realTimeValidationEnabled ? "✅ PASS" : "❌ FAIL"}`,
    );

    // Requirement: Collect success rate metrics and safety protocol effectiveness
    const metricsCollected =
      result.successRate !== undefined && result.safetyScore !== undefined;
    console.log(
      `   ✓ Success rate & safety metrics: ${metricsCollected ? "✅ PASS" : "❌ FAIL"}`,
    );
    ...
    console.log(
      `   ✓ Campaign infrastructure integration: ${integrationValid ? "✅ PASS" : "❌ FAIL"}`,
    );

    // Requirement: Target >80% successful replacements with zero build failures
    const targetAchieved =
      result.successRate >= config.targetSuccessRate &&
      result.buildFailures === 0;
    console.log(
      `   ✓ Target >80% success, 0 build failures: ${targetAchieved ? "✅ PASS" : "❌ FAIL"}`,
    );
    ...
    console.log(
      `   Status: ${allRequirementsMet ? "✅ ALL REQUIREMENTS MET" : "⚠️ SOME REQUIREMENTS NOT MET"}`,
    );
    ...
    if (result.success && allRequirementsMet) {
      ...
      console.log(
        "   📋 Review results and adjust strategy before full campaign",
      );
    }

    console.log("");
    console.log(
      "📄 Detailed reports available at: .kiro/campaign-reports/conservative-pilot/",
    );

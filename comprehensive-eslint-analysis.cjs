...
try {
  ...
  const output = execSync(
    'yarn lint src --ignore-pattern "**/*\\ *" --format=compact 2>&1',
    {
      encoding: "utf8",
  ...
  if (Object.keys(errorCounts).length > 0) {
    ...
    sortedErrors.forEach(([rule, count], index) => {
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${rule.padEnd(55)} ${count.toString().padStart(4)} errors`,
      );
    });
    console.log();
  }
  ...
  if (Object.keys(warningCounts).length > 0) {
    ...
    sortedWarnings.forEach(([rule, count], index) => {
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${rule.padEnd(55)} ${count.toString().padStart(4)} warnings`,
      );
    });
    console.log();
  }
  ...
  if (highImpactFiles.length > 0) {
    ...
    highImpactFiles.slice(0, 20).forEach(([file, stats], index) => {
      ...
      console.log(
        `   ${(index + 1).toString().padStart(2)}. ${file.padEnd(70)} ${stats.errors.toString().padStart(3)}E ${stats.warnings.toString().padStart(3)}W (${total})`,
      );
    });
    console.log();
  }
  ...
  Object.entries(categories).forEach(([category, rules]) => {
    ...
    if (categoryErrors > 0 || categoryWarnings > 0) {
      console.log(
        `   ${category.padEnd(30)}: ${categoryErrors.toString().padStart(4)} errors, ${categoryWarnings.toString().padStart(4)} warnings`,
      );
    }
  });
  ...
  if (topWarning) {
    console.log(
      `   ⚠️  Most Common Warning: "${topWarning[0]}" (${topWarning[1]} instances)`,
    );
  ...
  if (highImpactFiles.length > 0) {
    console.log(
      `   🎯 ${highImpactFiles.length} high-impact files need attention`,
    );
    console.log(
      "      - Focus on test files with many floating promise errors",
    );
  }
  ...
  console.log(
    `   ✅ Identified ${Object.keys(errorCounts).length} different error types`,
  );
  console.log(
    `   ✅ Identified ${Object.keys(warningCounts).length} different warning types`,
  );
  console.log(
  ...
  try {
    ...
    const basicCount = execSync(
      'yarn lint src --ignore-pattern "**/*\\ *" 2>&1 | grep -c "Error\\|Warning"',
      {
        encoding: "utf8",
      },
    );

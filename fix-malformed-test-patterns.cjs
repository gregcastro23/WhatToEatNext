...
function fixMalformedPatterns(content) {
  ...
  fixed = fixed.replace(
    /jest\.fn\(\)\s+as\s+any\.mockReturnValue\(([^)]+)\)/g,
    "jest.fn().mockReturnValue($1)",
  );
  fixed = fixed.replace(
    /jest\.fn\(\)\s+as\s+any\.mockResolvedValue\(([^)]+)\)/g,
    "jest.fn().mockResolvedValue($1)",
  );
  fixed = fixed.replace(
    /jest\.fn\(\)\s+as\s+any\.mockRejectedValue\(([^)]+)\)/g,
    "jest.fn().mockRejectedValue($1)",
  );
  fixed = fixed.replace(
    /jest\.fn\(\)\s+as\s+any\.mockImplementation\(([^)]+)\)/g,
  ...
  fixed = fixed.replace(
    /expect\s*\(\s*([^)]+)\s*\)\s*\.\s*([^(]+)\s*\(\s*([^)]*)\s*\)\s*\.\s*([^(]+)\s*\(/g,
    "expect($1).$2($3).$4(",
  );

  // Fix malformed import statements
  fixed = fixed.replace(
    /import\s*\{\s*([^}]+)\s*\}\s*from\s*['"]([^'"]+)['"]\s*as\s*any/g,
    "import { $1 } from '$2'",
  );

  // Fix malformed export statements
  fixed = fixed.replace(
    /export\s*\{\s*([^}]+)\s*\}\s*as\s*any/g,
    "export { $1 }",
  );

  // Fix malformed function declarations
  fixed = fixed.replace(
    /function\s+([^(]+)\s*\(\s*([^)]*)\s*\)\s*:\s*any\s*\{/g,
    "function $1($2): any {",
  );

  // Fix malformed class declarations
  fixed = fixed.replace(
    /class\s+([^{]+)\s*\{\s*([^}]*)\s*\}\s*as\s*any/g,
    "class $1 { $2 }",
  );

  // Fix malformed interface declarations
  fixed = fixed.replace(
    /interface\s+([^{]+)\s*\{\s*([^}]*)\s*\}\s*as\s*any/g,
    "interface $1 { $2 }",
  );

  // Fix malformed type declarations
  fixed = fixed.replace(
    /type\s+([^=]+)\s*=\s*([^;]+);\s*as\s*any/g,
    "type $1 = $2;",
  );

  // Fix malformed variable declarations
  fixed = fixed.replace(
    /const\s+([^=]+)\s*=\s*([^;]+);\s*as\s*any/g,
    "const $1 = $2 as any;",
  );
  fixed = fixed.replace(
    /let\s+([^=]+)\s*=\s*([^;]+);\s*as\s*any/g,
    "let $1 = $2 as any;",
  );
  fixed = fixed.replace(
    /var\s+([^=]+)\s*=\s*([^;]+);\s*as\s*any/g,
    "var $1 = $2 as any;",
  );
  ...
  fixed = fixed.replace(
    /catch\s*\(\s*([^)]+)\s*\)\s*as\s*any\s*\{/g,
    "catch ($1) {",
  );

  // Fix malformed for loops
  fixed = fixed.replace(
    /for\s*\(\s*([^)]+)\s*\)\s*as\s*any\s*\{/g,
    "for ($1) {",
  );

  // Fix malformed while loops
  fixed = fixed.replace(
    /while\s*\(\s*([^)]+)\s*\)\s*as\s*any\s*\{/g,
    "while ($1) {",
  );
...
function main() {
  ...
  try {
    const errorOutput = execSync(
      'npx tsc --noEmit --skipLibCheck 2>&1 | grep -E "TS1005|TS1135|TS1011|TS1128" | wc -l',
      { encoding: "utf8" },
    );

...
(function () {
  console.log(
    "[ClickFix] Initializing click handler diagnostic and fix module...",
  );
  ...
  if (Event.prototype.stopPropagation !== originalStopProp) {
    console.warn(
      "[ClickFix] Event.prototype.stopPropagation has been overridden",
    );
  ...
  if (Event.prototype.preventDefault !== originalPreventDefault) {
    console.warn(
      "[ClickFix] Event.prototype.preventDefault has been overridden",
    );
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      if (element && !element.__hasClickListener) {
        console.log(
          "[ClickFix] Found expandable element without working click handler",
          element,
        );
        missedClicks.push({
          element: element,
    ...
    true,
  ); // Use capture phase to detect all clicks
  ...
  window.setTimeout(function () {
    ...
    try {
      ...
      const originalCreateEventHandle =
        ReactDOM.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED?.Events
          ?.createEventHandle;
      if (originalCreateEventHandle) {
        ReactDOM.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.Events.createEventHandle =
          function () {
    ...
    expandableElements.forEach((element) => {
      ...
      element.addEventListener(
        ...
        function (event) {
          ...
          if (componentProps) {
            const toggleProps = Object.entries(componentProps).find(
              ([key]) =>
            ...
            if (toggleProps && typeof toggleProps[1] === "function") {
              try {
                toggleProps[1](event);
                console.log(
                  "[ClickFix] Called component toggle function:",
                  toggleProps[0],
                );
              } catch (error) {
                console.error(
                  "[ClickFix] Error calling component toggle function:",
                  error,
                );
              }
            }
  ...
  window.ClickFix = {
    ...
    fixCuisineCards: function () {
      document
        .querySelectorAll('.cuisineCard, [class*="cuisine"]')
        .forEach((el) => {
          el.setAttribute("data-expandable", "true");
    ...
    fixRecipeCards: function () {
      document
        .querySelectorAll('.recipe-card, [id*="recipe-"]')
        .forEach((el) => {
          el.setAttribute("data-expandable", "true");
          el.style.cursor = "pointer";
    ...
    fixIngredientCards: function () {
      document
        .querySelectorAll('.ingredientCard, [class*="ingredient"]')
        .forEach((el) => {
          el.setAttribute("data-expandable", "true");
          el.style.cursor = "pointer";
    ...
    fixMethodCards: function () {
      document
        .querySelectorAll('.method-card, [class*="method"]')
        .forEach((el) => {
          el.setAttribute("data-expandable", "true");
          el.style.cursor = "pointer";
  ...
  console.log(
    "[ClickFix] Module initialized. Call window.ClickFix.diagnose() for diagnostics.",
  );
  console.log(
    "[ClickFix] Call window.ClickFix.forceFix() to manually apply fixes.",
  );
  console.log(
    "[ClickFix] Call window.ClickFix.fixAllComponents() to fix all component types.",
  );
})();

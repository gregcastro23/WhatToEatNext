...
function validatePackageJson() {
  ...
  try {
    ...
    if (packageJson.engines?.node) {
      log(
        `  ✅ Node.js version specified: ${packageJson.engines.node}`,
        "green",
      );
      score++;
    } else {
...
function validateTypeScriptConfig() {
  ...
  try {
    ...
    if (
      ...
      log(
        `  ⚠️  Target is ${tsconfig.compilerOptions?.target}, consider es2022+`,
        "yellow",
      );
    }
    ...
    if (tsconfig.compilerOptions?.module === "esnext") {
      ...
      log(
        `  ❌ Module should be esnext, currently: ${tsconfig.compilerOptions?.module}`,
        "red",
      );
    }
...
function testESModuleImports() {
  ...
  return {
    score: workingImports,
    total: totalTests,
    passed: workingImports >= totalTests * 0.8,
  };
}
...
function validateBuildSystem() {
  ...
  try {
    ...
    log(
      "  ⚠️  Next.js build test failed (may need dev dependencies)",
      "yellow",
    );
...
function generateReport(results) {
  ...
  results.forEach(({ name, result }) => {
    ...
    log(
      `${status} ${name}: ${result.score}/${result.total} (${percentage}%)`,
      color,
    );
  ...
  if (allPassed && overallPercentage >= 90) {
    log(
      "\n🎉 EXCELLENT! Your project is fully configured for ES modules!",
      "green",
    );

...
class TS1005TestSignatureFixer {
  ...
  async fixTestSignatures(filePath) {
    try {
      ...
      const testPattern = /(test\s*\(\s*[^:]+):\s*any\s*,\s*(async\s*\(\s*\)\s*=>)/g;
      ...
      const itPattern = /(it\s*\(\s*[^:]+):\s*any\s*,\s*(async\s*\(\s*\)\s*=>)/g;
      ...
      const describePattern = /(describe\s*\(\s*[^:]+):\s*any\s*,\s*(\(\s*\)\s*=>)/g;
      ...
      if (fixesApplied > 0 && content !== originalContent) {
        ...
        console.log(`   ✅ ${path.basename(filePath)}: ${fixesApplied} test signature fixes applied`);
      }

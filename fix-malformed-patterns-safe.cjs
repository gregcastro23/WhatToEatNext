...
function countTs1128() {
  try {
    ...
    const out = execSync(
      'yarn tsc --noEmit --skipLibCheck 2>&1 | grep -c "error TS1128" || echo "0"',
...
console.log(
  isDryRun
    ? "🎯 Malformed Patterns Safe Fixer (DRY RUN)"
    : "🎯 Malformed Patterns Safe Fixer (LIVE)",
);
...
const limited =
  typeof limitArg === "number" ? candidates.slice(0, limitArg) : candidates;
console.log(
  `🔍 Found ${candidates.length} candidate files, processing ${limited.length}`,
);
...
for (const filePath of limited) {
  ...
  if (isDryRun) {
    ...
    const beforeCount = (
      original.match(/sig,\s*n:|severit,\s*y:|\[ke,\s*y:/g) || []
    ).length;
    const afterCount = (
      updated.match(/sig,\s*n:|severit,\s*y:|\[ke,\s*y:/g) || []
    ).length;
    const delta = beforeCount - afterCount;
    totalChanges += delta > 0 ? delta : 0;
    console.log(
      `   📝 Would fix ${delta > 0 ? delta : 0} occurrence(s) in ${path.relative(root, filePath)}`,
    );
  } else {
    ...
    console.log(
      `   ✅ Fixed ${fixedCount} occurrence(s) in ${path.relative(root, filePath)}`,
    );
  }
}
...
if (isDryRun) {
  console.log("\n============================================================");
  console.log(
    `📈 DRY RUN complete. Total would-fix occurrences: ${totalChanges}`,
  );

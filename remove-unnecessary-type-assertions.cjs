...
class UnnecessaryTypeAssertionRemover {
  ...
  loadAnalysisData() {
    try {
      ...
      if (fs.existsSync(analysisPath)) {
        ...
        console.log(`📊 Loaded analysis data: ${this.analysisData.assertions.length} assertions analyzed`);
        console.log(`🎯 Target: ${this.analysisData.summary.categories.unnecessary} unnecessary assertions`);
      }
    } catch (error) {
      console.warn('⚠️  Could not load analysis data, using pattern-based approach');
    }
  }
  ...
  shouldProcessFile(filePath) {
    ...
    if (this.analysisData) {
      ...
      const hasUnnecessaryAssertions = this.analysisData.assertions.some(
        assertion => assertion.file === normalizedPath &&
  ...
  shouldPreserveAssertion(content, assertionMatch, filePath) {
    ...
    if (matchLine > 0) {
      ...
      if (prevLine && (prevLine.includes('//') || prevLine.includes('/*'))) {
        ...
        if (prevLine.toLowerCase().includes('necessary') ||
      ...
      for (const { pattern, replacement, description, category } of patterns) {
        ...
        for (const match of matches) {
          ...
          if (this.shouldPreserveAssertion(content, match[0], filePath)) {
            ...
            console.log(`  Skipped: ${description} - "${match[0]}" in ${path.basename(filePath)} (preserved)`);
            continue;
          }
          ...
          console.log(`  ${this.dryRun ? 'Would remove' : 'Removed'}: ${description} - "${match[0]}" in ${path.basename(filePath)}`);
        }
      }
  ...
  generateReport() {
    ...
    if (this.analysisData) {
      ...
      console.log(`Total unnecessary assertions found: ${unnecessaryAssertions.length}`);
      ...
      console.log(`Files with unnecessary assertions: ${Object.keys(fileGroups).length}`);
   ...
   */
  run() {
    console.log(`🚀 Starting unnecessary type assertion removal${this.dryRun ? ' (DRY RUN)' : ''}...\n`);

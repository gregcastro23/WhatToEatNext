...
_Rev 5 · 2026-06-04 · prod = `master` (Vercel auto-deploy) + Railway Postgres_
...
### 2026-06-04
- [x] **Onboarding 0%-completion fix + admin dashboard honesty** ([#507](https://github.com/gregcastro23/WhatToEatNext/pull/507)) — root-caused the `/admin` "3 signups / 0 onboarded / 0%" DEGRADED panel to a dual-write gap: the `/onboarding` page posts to `/api/agent-forge/ignite`, which only patched the `users.profile` JSONB, but the app + funnel read the normalized `user_profiles` columns (via `rowToUserWithProfile`). Ignite now persists through `userDatabase.updateUserProfile()` (dual-writes both) + a handler regression test. Also de-fabricated 5 `/admin/dashboard` panels (real per-route traffic, real mint/burn, honest empty/not-wired). **Not** a `session.user.id` JWT issue — the callbacks already propagate it. Gates green; **PR open** (review pending — not yet on `master`). Remaining dashboard placeholders → see the 🧩 subsection below.
...
### Admin dashboard — remaining placeholders / not-wired panels 🤖
_From the [#507](https://github.com/gregcastro23/WhatToEatNext/pull/507) `/admin/dashboard` honesty audit. #507 already fixed the egregious fabrications — APIHeatmap's "1.3M requests" + seeded heatmap → **real** per-route traffic; hardcoded "184" cuisines; CosmicYield's seeded mint/burn sparkline; RecipeQuality's seeded SEMS bars + mock recipe fallback; SEMSDistribution's sample SEMS + fake Monica/Kalchm constants. These remaining panels still render placeholder / empty / honestly-"not wired" data. **Not tech-week-blocking** — `/admin/dashboard` is operator-only and already on the "avoid on the projector" list._
- [ ] **`SubdomainMatrix`** (`src/app/admin/_dashboard/extras.tsx:24`) — per-route `latency` ("44ms"…) + `err` (0.01…) are **hardcoded**, and the `tele` fallback is a mock (badged "mock"). Real latency/err are now in the payload (`apiRoutes` ← `summarizeAllPaths()`, added in #507) — map route→`PathSummary`, or drop the two faked columns. ⏱S
- [ ] **`PractitionerGeo`** (`extras.tsx:676`) — empty placeholder ("roll up by city / region when we have a sample"). Birth charts already store coords in `user_profiles.birth_data`; aggregate into regions server-side and populate `practitionerGeo`. ⏱M
- [ ] **Billing / `CostBurndown`** (`extras.tsx:889` + `panels.tsx:978` "BILLING ANALYTICS · NOT WIRED") — no real cost source. Wire Vercel/Railway usage + Stripe (note `commerce.mrr` is already live). ⏱M–L (external APIs)
- [ ] **`ModerationQueue`** (`panels.tsx:1344`, "OFFLINE EVAL · NOT WIRED") — no moderation/eval telemetry exists; wire when there's a real queue, else keep the honest-empty state. ⏱M
- [ ] **`SEMSDistribution`** (`src/app/admin/_dashboard/sky.tsx`) — now an honest "NOT WIRED" panel (#507). Real wiring needs a per-recipe thermodynamic-score table captured during ingestion. Do **not** fabricate — alchemical calc is the proprietary path. ⏱L
- [ ] **Cuisines KPI** (`Dashboard.tsx`) — shows "—" since #507 (was a hardcoded "184"; real ≈ 16). Add a server-side `stats.totalCuisines` (count the static catalog in the dashboard route) + render it. ⏱S

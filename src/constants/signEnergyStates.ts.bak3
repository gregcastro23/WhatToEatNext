...
export const ENERGY_STATES: Record<ZodiacSign, { baseEnergy: number, currentEnergy: number }> = {
    ...
    sagittarius: { baseEnergy: BASE_SIGN_ENERGIES.sagittarius, currentEnergy: BASE_SIGN_ENERGIES.sagittarius },
    capricorn: { baseEnergy: BASE_SIGN_ENERGIES.capricorn, currentEnergy: BASE_SIGN_ENERGIES.capricorn },
    ...
    return ZODIAC_SIGNS.map(sign => {
        ...
        PLANETARY_RULERSHIPS[sign].forEach(planet => {
            ...
            if (planetPosition) {
                ...
                const positionStrength = typeof planetPosition.degree === 'number' 
        ...
        const currentEnergy = modifierValues.length > 0

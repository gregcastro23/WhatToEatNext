...
export const DEFAULT_ASTROLOGICAL_STATE = {
  ...
  planetaryPositions: {
    ...
    Moon: { sign: "taurus", degree: 8 },
    Mercury: { sign: "gemini", degree: 10 },
    Venus: { sign: "libra", degree: 12 },
    Mars: { sign: "aries", degree: 20 },
    Jupiter: { sign: "sagittarius", degree: 5 },
    Saturn: { sign: "capricorn", degree: 18 },
    Uranus: { sign: "aquarius", degree: 25 },
 ...
 */
export function getDefaultPlanetaryPositions(): Record<
  string,
  CelestialPosition
> {
  ...
  for (const key in defaults) {
    if (key in values) {
      ...
      if (
        ...
        errors.push(
          `Type mismatch for ${key}: expected ${defaultType}, got ${valueType}`,
        );
      }
    }
  }

...
export const getZodiacBoost = (zodiacSign: string, item: unknown): number => {
  ...
  const zodiacBoost = itemData.zodiacInfluences?.includes(normalizedSign)
    ? 0.3
    : 0;
  ...
  const seasonalBoost =
    _calculateSeasonalAlignment(normalizedSign, item) * 0.15;
...
const _calculateSeasonalAlignment = (
  zodiacSign: string,
  item: unknown,
): number => {
...
export const _getFlavorBoost = (
  ...
  return Object.entries(elementBoost).reduce(
    (acc, [element, boost]) =>
      acc + (ingredientData.elementalProperties?.[element] || 0) * (boost || 0),
    0,
  );
};
 ...
 */
export const _getNutritionalSynergy = (
  _planet: Planet,
  _ingredient: unknown,
): string[] =>
 ...
 */
export const _formatelementalState = (
  ...
  const validEntries = Object.entries(elements)
    ...
    .map(
      ([elem, val]) => `${elem} ${Math.round(((val as number) || 0) * 100)}%`,
    )
    .join(" · ");

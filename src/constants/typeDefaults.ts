 ...
 */
export const EmptyAlchemicalProperties: AlchemicalPropertiesType = {
  Spirit: 0,
  Essence: 0,
  Matter: 0,
  Substance: 0
 ...
 */
export const DefaultAlchemicalProperties: AlchemicalPropertiesType = {
  Spirit: 0.25,
  Essence: 0.25,
  Matter: 0.25,
  Substance: 0.25
 ...
 */
export const EmptyElementalProperties: ElementalPropertiesType = {
  Fire: 0,
  Water: 0,
  Earth: 0,
  Air: 0
 ...
 */
export const DefaultElementalProperties: ElementalPropertiesType = {
  Fire: 0.25,
  Water: 0.25,
  Earth: 0.25,
  Air: 0.25
 ...
 */
export const DefaultCompleteAlchemicalResult: CompleteAlchemicalResultType = {
 ...
 */
export const EmptyPlanetaryPositions: PlanetaryPositionsType = {} as const;
 ...
 */
export const DefaultZodiacSign: ZodiacSignType = 'aries' as const;
 ...
 */
export const DefaultIngredientMapping: IngredientMappingType = {
 ...
 */
export const DefaultAstrologicalState: AstrologicalStateType = {
 ...
 */
export const createErrorResponse = <T>(error: string): ServiceResponseType<T> => ({
  success: false,
  error,
 ...
 */
export const createSuccessResponse = <T>(data: T): ServiceResponseType<T> => ({
  success: true,
  data,
  timestamp: new Date().toISOString()
});
...
export const createSafeElementalProperties = (
  ...
  const fire = Number.isFinite(properties.Fire) ? Math.max(0, properties.Fire!) : 0.25;
  const water = Number.isFinite(properties.Water) ? Math.max(0, properties.Water!) : 0.25;
  const earth = Number.isFinite(properties.Earth) ? Math.max(0, properties.Earth!) : 0.25;
  const air = Number.isFinite(properties.Air) ? Math.max(0, properties.Air!) : 0.25;
...
export const createSafeAlchemicalProperties = (
  ...
  const spirit = Number.isFinite(properties.Spirit) ? Math.max(0, properties.Spirit!) : 0.25;
  const essence = Number.isFinite(properties.Essence) ? Math.max(0, properties.Essence!) : 0.25;
  const matter = Number.isFinite(properties.Matter) ? Math.max(0, properties.Matter!) : 0.25;
  const substance = Number.isFinite(properties.Substance) ? Math.max(0, properties.Substance!) : 0.25;
 ...
 */
export const createSafeThermodynamicMetrics = (
  metrics: Partial<ThermodynamicMetricsType>
): ThermodynamicMetricsType => {
  return {
    heat: Number.isFinite(metrics.heat) ? Math.max(0.1, Math.min(1.0, metrics.heat!)) : 0.5,
    entropy: Number.isFinite(metrics.entropy) ? Math.max(0.1, Math.min(1.0, metrics.entropy!)) : 0.5,
    reactivity: Number.isFinite(metrics.reactivity) ? Math.max(0.1, Math.min(1.0, metrics.reactivity!)) : 0.5,
    gregsEnergy: Number.isFinite(metrics.gregsEnergy) ? Math.max(0.1, Math.min(1.0, metrics.gregsEnergy!)) : 0.5,
    kalchm: Number.isFinite(metrics.kalchm) ? Math.max(0.1, metrics.kalchm!) : 1.0,
    monica: Number.isFinite(metrics.monica) ? Math.max(0.1, metrics.monica!) : 1.0
  };
};
 ...
 */
export const validateZodiacSign = (sign: string): ZodiacSignType => {
  const validSigns: ZodiacSignType[] = [
    'aries', 'taurus', 'gemini', 'cancer', 'leo', 'virgo',
    'libra', 'scorpio', 'sagittarius', 'capricorn', 'aquarius', 'pisces'
  ];
  
  const normalizedSign = sign.toLowerCase() as ZodiacSignType;
 ...
 */
export const validateLunarPhase = (phase: string): LunarPhaseType => {

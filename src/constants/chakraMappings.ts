...
export const ZODIAC_CHAKRA_MAPPINGS: Record<ZodiacSign, Chakra[]> = ZODIAC_SIGNS.reduce((mappings, sign) => {
    mappings[sign] = CHAKRAS.filter(chakra => 
        CHAKRA_ZODIAC_MAPPINGS[chakra].includes(sign)
    );
    return mappings;
...
export function calculateChakraEnergies(signEnergyStates: Record<ZodiacSign, number>): Record<Chakra, number> {
    ...
    CHAKRAS.forEach(chakra => {
        ...
        if (relatedSigns.length > 0) {
            const totalEnergy = relatedSigns.reduce((sum, sign) => 
                sum + (signEnergyStates[sign] || 0), 0);

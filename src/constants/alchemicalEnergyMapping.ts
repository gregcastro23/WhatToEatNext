...
export const _planetElementMap = (
  isDaytime: boolean,
...
export const planetPropertyMap = (
  isDaytime: boolean,
): Record<string, string> => ({
...
export function calculateThermodynamicProperties(
  ...
  try {
    ...
    const entropy =
      0.6 * alchemicalProps.Substance + 0.4 * alchemicalProps.Essence;

    // Reactivity is related to Fire/Spirit and Water/Essence
    // High Spirit and Essence values increase reactivity
    const reactivity =
      0.5 * alchemicalProps.Spirit + 0.5 * alchemicalProps.Essence;

import { _VALID_SEASONS as VALID_SEASONS } from "@/constants/seasonalConstants";
...
export function validateRecipe(recipe: Recipe): boolean {
  ...
  if (recipe.season) {
    ...
    const isValidSeason = seasons.every((season: string) => {
      ...
      return VALID_SEASONS.map((s) => s.toLowerCase()).includes(
        normalizedSeason,
      );
    });
  ...
  return recipe.ingredients.every((ing: RecipeIngredient) =>
...
export function validateIngredient(ingredient: RecipeIngredient): boolean {
  ...
  if (ingredient.seasonality) {
    ...
    const isValidSeasonality = ingredient.seasonality.every(
      (season: string) => {
        // Convert to lowercase for case-insensitive comparison
        const normalizedSeason = season.toLowerCase();
        return VALID_SEASONS.map((s) => s.toLowerCase()).includes(
  ...
  if (ingredient.elementalProperties) {
    const sum = Object.values(ingredient.elementalProperties).reduce(
      (acc: number, _val: number) => acc + _val,
      0,
    );

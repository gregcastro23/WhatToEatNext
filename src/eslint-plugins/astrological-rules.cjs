  ...
  create(context) {
    ...
    return {
      ...
      UpdateExpression(node) {
        if (node.argument.type === 'Identifier' && PROTECTED_CONSTANTS.includes(node.argument.name)) {
          context.report({
            node,
  ...
  create(context) {
    ...
    return {
      ObjectExpression(node) {
        ...
        const elementNames = properties.map(prop => 
          prop.key.type === 'Identifier' ? prop.key.name : null
        ).filter(Boolean);
        
        // Check for missing elements
        const missingElements = REQUIRED_ELEMENTS.filter(element => !elementNames.includes(element));
...
const requireTransitDateValidation = {
  ...
  create(context) {
    ...
    const isAstrologicalFile = filename.includes('calculations') || 
    ...
    return {
      ...
      'Program:exit'() {
        ...
        if (isAstrologicalFile && hasValidationImport && !hasValidationCall) {
          context.report({
            node: context.getSourceCode().ast,
            message: 'Consider adding transit date validation calls for planetary position accuracy'
          });
        }
      }
    };

...
module.exports = {
  rules: {
    ...
    'declaration-expected': createProcessorRule('TS1128', [
      ...
      {
        regex: /^(\s*)}\s*,\s*$/gm,
        message: 'Orphaned closing brace with comma - likely from removed object property'
      }
    ]),
    ...
    'quote-escaping': createProcessorRule('Quote Escaping', [
      {
        regex: /'[^']*'[a-z]/gi,
        message: 'Potential apostrophe escaping issue - check for unescaped quotes in strings'
      },
      {
        regex: /\u2018|\u2019|\u201C|\u201D/g,

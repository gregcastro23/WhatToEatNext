/**
 * Script to update fruit ingredients with nutritional information
 * Uses USDA FoodData Central API to fetch and add nutritional data to existing ingredients
 *
 * Run with: yarn ts-node src/scripts/updateFruits.ts
 */
...
const PROGRESS_FILE = path.resolve(CACHE_DIR, 'fruits_progress.json');
...
// Default elemental properties for fruits
...
/**
 * Main function to process all fruit ingredient files
 * @returns {Promise<void>}
 */
async function updateFruits() {
  // console.log(`Processing fruits category`);

  // Get all fruit ingredient files
...
export const fruits = fixIngredientMappings(rawFruits) as Record<string, IngredientMapping>;
`;

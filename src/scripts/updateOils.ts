/**
 * Script to update oil ingredients with nutritional information
 * Uses USDA FoodData Central API to fetch and add nutritional data to existing ingredients
 *
 * Run with: yarn ts-node src/scripts/updateOils.ts
 */
...
const PROGRESS_FILE = path.resolve(CACHE_DIR, 'oils_progress.json');
...
// Default elemental properties for oils
...
/**
 * Main function to process all oil ingredient files
 * @returns {Promise<void>}
 */
async function updateOils() {
  // console.log(`Processing oils category`);

  // Get all oil ingredient files
...
export const oils = fixIngredientMappings(rawOils) as Record<string, IngredientMapping>;
`;

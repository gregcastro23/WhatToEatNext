...
class UnusedVariableAnalyzer {
  ...
  initializePreservationPatterns() {
    return {
      ...
      astrological: {
        patterns: [/\b(astrology|astrological|celestial|cosmic)\b/i],
      ...
      campaign: {
        ...
        reason:
          "Campaign system variable - preserved for monitoring and intelligence features",
        confidence: 0.85,
      },
      culinary: {
        patterns: [
        ...
        reason:
          "Culinary domain variable - preserved for recipe and ingredient systems",
        confidence: 0.8,
      },
      test: {
        patterns: [
        ...
        reason:
          "Test infrastructure variable - preserved for testing framework",
        confidence: 0.75,
      },
      service: {
        patterns: [
  ...
  getUnusedVariables() {
    try {
      ...
      try {
        ...
        console.warn(
          "Failed to parse lint JSON output, falling back to text parsing",
        );
  ...
  parseTextLintOutput() {
    try {
      ...
      try {
        ...
        console.error(
          "Could not read unused-vars-raw.txt file:",
          readError.message,
        );
        return [];
      }

      const lines = lintOutput
        .trim()
        .split("\n")
      ...
      lines.forEach((line) => {
        ...
        const match = line.match(
          /^(.+?):(\d+):(\d+):\s+(.+?)\s+\[Error\/@typescript-eslint\/no-unused-vars\]/,
        );
        if (match) {
      ...
      console.log(
        `📊 Found ${unusedVars.length} unused variable warnings (text parsing)`,
      );
      return unusedVars;
    } catch (error) {
  ...
  checkPreservationPatterns(variableName, filePath, fileContent = "") {
    ...
    const astrologicalResult =
      this.astrologicalDetector.detectAstrologicalDomain(
        variableName,
        filePath,
        fileContent,
      );
    ...
    for (const [domain, config] of Object.entries(this.preservationPatterns)) {
      ...
      const pathMatch = config.patterns.some((pattern) =>
      ...
      const contentMatch =
        fileContent &&
        config.patterns.some((pattern) => pattern.test(fileContent));
  ...
  analyzeVariable(unusedVar) {
    ...
    const preservation = this.checkPreservationPatterns(
      variableName,
      unusedVar.filePath,
      fileContent,
    );
  ...
  generateCategoryStats(results) {
    ...
    results.forEach((result) => {
      // File type stats
      stats.byFileType[result.fileType] =
        (stats.byFileType[result.fileType] || 0) + 1;

      // Domain stats
      stats.byDomain[result.preservation.domain] =
        (stats.byDomain[result.preservation.domain] || 0) + 1;
      ...
      stats.byEliminationStrategy[result.eliminationStrategy.method] =
        (stats.byEliminationStrategy[result.eliminationStrategy.method] || 0) +
        1;
    ...
    stats.preservationSummary.preservationRate = (
      (stats.preservationSummary.preserved / stats.total) *
      100
    ).toFixed(1);
  ...
  generateRecommendations(results, stats) {
    ...
    const highConfidenceEliminations = results.filter(
      (r) =>
        !r.preservation.shouldPreserve &&
        r.eliminationStrategy.confidence > 0.8,
    );
  ...
  generateBatchProcessingPlan(results) {
    const eliminationCandidates = results.filter(
      (r) => !r.preservation.shouldPreserve,
    );
    const batches = {};
    ...
    Object.values(batches).forEach((batch) => {
      const totalConfidence = batch.variables.reduce(
        (sum, v) => sum + v.eliminationStrategy.confidence,
        0,
      );
      batch.averageConfidence = (
        totalConfidence / batch.variables.length
      ).toFixed(3);
    });
  ...
  getProcessingOrder(batches) {
    return Object.values(batches)
      .sort((a, b) => {
        ...
        return (
          parseFloat(b.averageConfidence) - parseFloat(a.averageConfidence)
        );
      })
  ...
  printSummary(report) {
    ...
    report.batchProcessingPlan.processingOrder.forEach((batchName, index) => {
      const batch = report.batchProcessingPlan.batches.find(
        (b) => b.name === batchName,
      );
      console.log(
        `     ${index + 1}. ${batchName} (${batch.totalCount} variables, confidence: ${batch.averageConfidence})`,
      );
    });
  ...
  async analyze() {
    ...
    const results = unusedVars.map((unusedVar) =>
      this.analyzeVariable(unusedVar),
    );
    ...
    const outputPath = path.join(
      process.cwd(),
      "unused-variables-analysis-report.json",
    );

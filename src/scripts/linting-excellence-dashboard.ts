...
class LintingExcellenceDashboardCLI {
  ...
  private async runValidation(options: CLIOptions): Promise<void> {
    ...
    if (result.regressionAnalysis.detected) {
      ...
      console.log("-".repeat(30));
      console.log(
        `Severity: ${result.regressionAnalysis.severity.toUpperCase()}`,
      );
      console.log(
    ...
    if (result.metrics.parserErrors > 0) {
      ...
      console.log(
        '   Run: yarn lint:fix --rule "@typescript-eslint/no-explicit-any"',
      );
        ...
        void (async () => {
          try {
            ...
            if (
              ...
              if (result.regressionAnalysis.detected) {
                console.log(
                  `   🔴 Regression: ${result.regressionAnalysis.severity}`,
                );
              }
            } else if (options.verbose) {
              console.log(
                `✅ [${new Date().toISOString()}] All systems normal`,
              );
            }
    ...
    const procedures = [
      {
        ...
        action: () => {
          try {
            execSync("rm -rf .eslintcache .eslint-ts-cache/", {
              stdio: "pipe",
            });
            return true;
          } catch {
      ...
      },
      {
        ...
        action: () => {
          try {
            execSync("yarn lint --print-config src/index.ts > /dev/null", {
              stdio: "pipe",
            });
            return true;
          } catch {

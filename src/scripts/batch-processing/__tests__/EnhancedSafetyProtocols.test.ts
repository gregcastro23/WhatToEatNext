...
describe('EnhancedSafetyProtocols', () => {
  ...
  beforeEach(() => {
    ...
    const config: Partial<HighImpactFileConfig> = {
  ...
  describe('Risk Assessment', () => {
    test('should assess core calculation files as critical risk', () => {
      const assessment = safetyProtocols.assessFileRisk(
    ...
    test('should detect astrological calculations in file content', () => {
      mockFs.readFileSync.mockReturnValue('const planetary = calculatePlanetaryPositions();');
    ...
    test('should detect campaign system logic', () => {
      mockFs.readFileSync.mockReturnValue('const metrics = campaignProgress.getMetrics();');
  ...
  describe('Manual Review Workflow', () => {
    test('should create manual review request for high-risk files', () => {
      const assessment: FileRiskAssessment = {
      ...
      const reviewRequest = safetyProtocols.createManualReviewRequest(assessment);
    ...
    test('should track pending manual reviews', () => {
      ...
      const pendingReviews = safetyProtocols.getPendingManualReviews();
    ...
    test('should approve manual reviews', () => {
      ...
      safetyProtocols.createManualReviewRequest(assessment);
      const success = safetyProtocols.approveManualReview('/project/src/test.ts', 'Looks good');
    ...
    test('should reject manual reviews', () => {
      ...
      safetyProtocols.createManualReviewRequest(assessment);
      const success = safetyProtocols.rejectManualReview('/project/src/test.ts', 'Too risky');
  ...
  describe('Enhanced Validation', () => {
    test('should perform TypeScript compilation validation', async () => {
      const result = await safetyProtocols.performEnhancedValidation(
        '/project/src/test.ts',
    ...
    test('should perform service layer validation', async () => {
      mockFs.readFileSync.mockReturnValue('export const api = "/api/test";');
    ...
    test('should detect missing API definitions in service files', async () => {
      mockFs.readFileSync.mockReturnValue('const api = "/api/test"; // No export');
    ...
    test('should validate core calculations', async () => {
      mockFs.readFileSync.mockReturnValue(`
        ...
        const Earth = 0.5;
        const Air = 0.3;
        function calculateElemental() { return Fire + Water; }
      `);
  ...
  describe('Configuration Options', () => {
    test('should respect disabled manual review option', () => {
      const configWithoutReview: Partial<HighImpactFileConfig> = {
        requireManualReview: false
      };

      const protocolsNoReview = new EnhancedSafetyProtocols(configWithoutReview);
      const assessment = protocolsNoReview.assessFileRisk(
        '/project/src/calculations/planetary.ts',
        25 // High variable count
      );
    ...
    test('should respect disabled enhanced validation option', () => {
      const configWithoutValidation: Partial<HighImpactFileConfig> = {
        enhancedValidation: false
      };

      const protocolsNoValidation = new EnhancedSafetyProtocols(configWithoutValidation);
      const assessment = protocolsNoValidation.assessFileRisk(
        '/project/src/services/api.ts',
        10
      );
    ...
    test('should respect custom variable threshold', () => {
      const configCustomThreshold: Partial<HighImpactFileConfig> = {
        maxVariablesAutoProcess: 10 // Lower threshold
      };

      const protocolsCustom = new EnhancedSafetyProtocols(configCustomThreshold);
      const assessment = protocolsCustom.assessFileRisk(
        '/project/src/utils/helper.ts',
    ...
    test('should respect custom batch sizes', () => {
      const configCustomBatch: Partial<HighImpactFileConfig> = {
        criticalFileBatchSize: 3,
        serviceLayerBatchSize: 6
      };

      const protocolsCustom = new EnhancedSafetyProtocols(configCustomBatch);

      const criticalAssessment = protocolsCustom.assessFileRisk(
        '/project/src/calculations/planetary.ts',
        10
      );
      expect(criticalAssessment.recommendedBatchSize).toBe(3);

      const serviceAssessment = protocolsCustom.assessFileRisk(
        '/project/src/services/api.ts',
        10
      );

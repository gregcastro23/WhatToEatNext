...
describe('SafeBatchProcessor', () => {
  ...
  describe('Batch Creation', () => {
    test('should create batches respecting maximum batch size', async () => {
      const largeFileSet: FileProcessingInfo[] = Array.from({ length: 50 }, (_, i) => ({
      ...
      const results = await processor.processBatches(largeFileSet);
    ...
    test('should use smaller batch sizes for critical files', async () => {
      const criticalFiles: FileProcessingInfo[] = Array.from({ length: 20 }, (_, i) => ({
      ...
      const results = await processor.processBatches(criticalFiles);
    ...
    test('should sort files by risk level', async () => {
      ...
      const results = await processor.processBatches(mixedFiles);
  ...
  describe('Safety Protocols', () => {
    test('should create git stash before processing', async () => {
      ...
      const results = await processor.processBatches([mockFiles[0]]);
    ...
    test('should create safety checkpoints', async () => {
      ...
      const checkpoints = processor.getSafetyCheckpoints();
  ...
  describe('Error Handling', () => {
    ...
    test('should stop processing on batch failure', async () => {
      const multipleFiles = [mockFiles[0], mockFiles[1], mockFiles[2]];
      ...
      const results = await processor.processBatches(multipleFiles);
      ...
      const failedIndex = results.indexOf(failedBatch!);
  ...
  describe('Configuration Options', () => {
    test('should respect disabled validation option', async () => {
      const configWithoutValidation: Partial<BatchProcessingConfig> = {
        validateAfterEachBatch: false
      };

      const processorNoValidation = new SafeBatchProcessor(configWithoutValidation);
      const results = await processorNoValidation.processBatches([mockFiles[0]]);
    ...
    test('should respect disabled rollback option', async () => {
      const configWithoutRollback: Partial<BatchProcessingConfig> = {
        autoRollbackOnError: false
      };

      const processorNoRollback = new SafeBatchProcessor(configWithoutRollback);
      ...
      const results = await processorNoRollback.processBatches([mockFiles[0]]);
    ...
    test('should respect disabled git stash option', async () => {
      const configWithoutStash: Partial<BatchProcessingConfig> = {
        createGitStash: false
      };

      const processorNoStash = new SafeBatchProcessor(configWithoutStash);
      const results = await processorNoStash.processBatches([mockFiles[0]]);
  ...
  describe('Logging', () => {
    test('should respect log level configuration', () => {
      const debugProcessor = new SafeBatchProcessor({ logLevel: 'debug' });
      const errorProcessor = new SafeBatchProcessor({ logLevel: 'error' });

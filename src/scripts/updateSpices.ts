/**
 * Script to update spice ingredients with nutritional information
 * Uses USDA FoodData Central API to fetch and add nutritional data to existing ingredients
 *
 * Run with: yarn ts-node src/scripts/updateSpices.ts
 */
...
const PROGRESS_FILE = path.resolve(CACHE_DIR, 'spices_progress.json');
...
// Default elemental properties for spices
...
/**
 * Main function to process all spice ingredient files
 * @returns {Promise<void>}
 */
async function updateSpices() {
  // console.log(`Processing spices category`);

  // Get all spice ingredient files
...
export const spices = fixIngredientMappings(rawSpices) as Record<string, IngredientMapping>;
`;

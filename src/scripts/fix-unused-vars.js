...
try {
    ...
    eslintResult[0].messages.forEach((message) => {
      if (message.ruleId === "@typescript-eslint/no-unused-vars") {
        ...
        try {
          ...
          console.warn(
            `Could not extract variable name from message: ${message.message}`,
          );
        }
      }
    });
  ...
  if (unusedVars.length === 0) {
    console.log(
      "No unused variables found, or all unused variables already start with underscore",
    );
    // Clean up backup
  ...
  try {
    ...
    if (remainingErrors === 0) {
      ...
      console.log(
        `There are still ${remainingErrors} unused variable errors remaining.`,
      );

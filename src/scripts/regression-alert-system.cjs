...
class RegressionAlertSystem {
  ...
  loadBaselines() {
    try {
      if (fs.existsSync(REGRESSION_CONFIG.files.baselines)) {
        return JSON.parse(
          fs.readFileSync(REGRESSION_CONFIG.files.baselines, "utf8"),
        );
      }
    } catch (error) {
  ...
  loadRegressions() {
    try {
      if (fs.existsSync(REGRESSION_CONFIG.files.regressions)) {
        return JSON.parse(
          fs.readFileSync(REGRESSION_CONFIG.files.regressions, "utf8"),
        );
      }
    } catch (error) {
  ...
  loadHistory() {
    try {
      if (fs.existsSync(REGRESSION_CONFIG.files.history)) {
        return JSON.parse(
          fs.readFileSync(REGRESSION_CONFIG.files.history, "utf8"),
        );
      }
    } catch (error) {
  ...
  saveBaselines() {
    try {
      fs.writeFileSync(
        REGRESSION_CONFIG.files.baselines,
  ...
  saveRegressions() {
    try {
      fs.writeFileSync(
        REGRESSION_CONFIG.files.regressions,
        JSON.stringify(this.regressions, null, 2),
      );
    } catch (error) {
  ...
  saveHistory() {
    try {
      fs.writeFileSync(
        REGRESSION_CONFIG.files.history,
  ...
  calculateRegressionSeverity(baseline, current, metric) {
    ...
    const minIncrease =
      REGRESSION_CONFIG.regression.minimumIncrease[metric] || 1;
    if (increase < minIncrease) return null;

    // Determine severity based on percentage increase
    if (
      percentageIncrease >= REGRESSION_CONFIG.regression.thresholds.critical
    ) {
      return "CRITICAL";
    } else if (
      percentageIncrease >= REGRESSION_CONFIG.regression.thresholds.major
    ) {
      return "MAJOR";
    } else if (
      percentageIncrease >= REGRESSION_CONFIG.regression.thresholds.moderate
    ) {
      return "MODERATE";
    } else if (
      percentageIncrease >= REGRESSION_CONFIG.regression.thresholds.minor
    ) {
      return "MINOR";
    }
  ...
  detectRegressions(currentMetrics) {
    ...
    if (tsSeverity) {
      regressions.push({
        ...
        percentage: (
          ((currentMetrics.typeScriptErrors - baseline.typeScriptErrors) /
            baseline.typeScriptErrors) *
          100
        ).toFixed(1),
    ...
    if (eslintErrorsSeverity) {
      regressions.push({
        ...
        percentage: (
          ((currentMetrics.eslintErrors - baseline.eslintErrors) /
            baseline.eslintErrors) *
          100
        ).toFixed(1),
    ...
    if (eslintWarningsSeverity) {
      regressions.push({
        ...
        percentage: (
          ((currentMetrics.eslintWarnings - baseline.eslintWarnings) /
            baseline.eslintWarnings) *
          100
        ).toFixed(1),
    ...
    if (baseline.lintDuration && currentMetrics.lintDuration) {
      ...
      if (perfPercentage >= 50 && perfIncrease >= 10) {
        // 50% increase and at least 10 seconds
        regressions.push({
  ...
  sendAlert(regression) {
    ...
    // File alert
    this.log(
      `REGRESSION [${regression.severity}] ${regression.type}: $) {regression.message}`,
    );
...
${this.getRecommendedActions(regression)}
    ...
    switch (regression.type) {
      case "typescript":
        ...
        if (regression.severity === "CRITICAL") {
          actions.push(
            "- Consider triggering emergency campaign: `yarn lint:campaign:start emergency`",
          );
        }
        break;
      ...
      case "eslint-errors":
        ...
        if (
          ...
          actions.push(
            "- Consider triggering cleanup campaign: `yarn lint:campaign:start standard`",
          );
        }
        break;
      ...
      case "performance":
        ...
        actions.push(
          "- Check for large file additions or complex rule changes",
        );
          ...
          attachments: [
            {
              ...
              fields: [
                ...
                {
                  title: "Increase",
                  value: `+${regression.increase} (+${regression.percentage}%)`,
                  short: true,
                },
  ...
  executeRegressionResponse(regression) {
    ...
    if (regression.severity === "CRITICAL") {
      this.log(
        `Executing critical regression response for $) {regression.type}`,
      );

      if (
        regression.type === "typescript" ||
        regression.type === "eslint-errors"
      ) {
        // Trigger emergency campaign if available
        try {
          if (fs.existsSync("src/scripts/linting-campaign-cli.cjs")) {
            this.log(
              "Triggering emergency campaign due to critical regression",
            );
  ...
  cleanupOldRegressions() {
    ...
    cutoffDate.setDate(
      cutoffDate.getDate() - REGRESSION_CONFIG.alerts.retentionDays,
    );
    const cutoffISO = cutoffDate.toISOString();

    // Move old active regressions to resolved
    const oldRegressions = this.regressions.active.filter(
      (r) => r.timestamp < cutoffISO,
    );
    this.regressions.resolved.push(
      ...oldRegressions.map((r) => ({
        ...r,
        resolvedAt: new Date().toISOString(),
        status: "auto-resolved",
      })),
    );
    this.regressions.active = this.regressions.active.filter(
      (r) => r.timestamp >= cutoffISO,
    );

    // Limit active regressions
    if (
      this.regressions.active.length > REGRESSION_CONFIG.alerts.maxActiveAlerts
    ) {
      const excess = this.regressions.active.splice(
        0,
        this.regressions.active.length -
          REGRESSION_CONFIG.alerts.maxActiveAlerts,
      );
      this.regressions.resolved.push(
        ...excess.map((r) => ({
          ...r,
    ...
    this.history.entries = this.history.entries.filter(
      (entry) => entry.detectedAt >= cutoffISO,
    );
  }
  ...
  generateRegressionReport() {
    ...
    if (this.baselines.established) {
      ...
      console.log(
        `  TypeScript Errors: $) {this.baselines.metrics.typeScriptErrors}`,
      );
      console.log(`  ESLint Errors: $) {this.baselines.metrics.eslintErrors}`);
      console.log(
        `  ESLint Warnings: $) {this.baselines.metrics.eslintWarnings}`,
      );
      console.log(
        `  Lint Duration: $) {this.baselines.metrics.lintDuration}s\n`,
      );
    } else {
      console.log("📈 Baseline Status: NOT ESTABLISHED");
      console.log(
        "  Run with --establish-baseline to set current state as baseline\n",
      );
    }
    ...
    if (this.regressions.active.length > 0) {
      ...
      this.regressions.active.forEach((regression) => {
        console.log(
          `  [${regression.severity}] $) {regression.type}: $) {regression.message}`,
        );
        console.log(
          `    Increase: +${regression.increase} (+$) {regression.percentage}%)`,
        );
    ...
    if (recentHistory.length > 0) {
      ...
      recentHistory.forEach((entry) => {
        console.log(
          `  ${entry.detectedAt}: [${entry.severity}] $) {entry.type} (+$) {entry.percentage}%)`,
        );
      });
      console.log("");
    }
    ...
    const criticalCount = this.history.entries.filter(
      (e) => e.severity === "CRITICAL",
    ).length;
    const majorCount = this.history.entries.filter(
      (e) => e.severity === "MAJOR",
    ).length;
...
Environment Variables:
  REGRESSION_WEBHOOK_URL - Webhook URL for alerts (optional)
    ...
    default:
      ...
      console.error(
        'Run "node regression-alert-system.cjs help" for usage information',
      );
      process.exit(1);
  }
}

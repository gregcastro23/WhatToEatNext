    ...
    try {
      ...
      for (const [source, importInfo] of imports) {
        if (importInfo.lines.length > 1) {
          ...
          const adjustedIndex = firstLineIndex - linesToRemove.filter(idx => idx < firstLineIndex).length;
    ...
    try {
      ...
      for (const importLine of importLines) {
        ...
        if (match) {
          ...
          if (source.startsWith('node:') || ['fs', 'path', 'crypto', 'util', 'child_process'].includes(source)) {
            categories.builtin.push(importLine);
          } else if (source.startsWith('@/') || source.startsWith('@components/') || source.startsWith('@utils/')) {

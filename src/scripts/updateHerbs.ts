...
const DEFAULT_ELEMENTAL_PROPERTIES = { Air: 0.4, Fire: 0.3, Earth: 0.2, Water: 0.1 };
...
async function processIngredientFile(filePath) {
  ...
  try {
    ...
    const metadataFields = [
      'waxing', 'full', 'purpurea', 'angustifolia', 'pallida', 'harvesting', 
      'energetics', 'chemical_constituents', 'traditional_uses', 'tincture', 
      'decoction', 'infusion', 'glycerite', 'salve', 'powder', 'growing_season',
      'flowering_period', 'actions', 'systems_affected', 'name', 'parts_used',
      'method', 'dosage', 'applications', 'conditions', 'protocols'
    ];
    
    // First update existing ingredients
    console.log(`Found ${ingredientMatches.length} potential ingredients in file`);
    
    let actualIngredientsCount = 0;
    ...
    for (const match of ingredientMatches) {
      ...
      if (metadataFields.includes(ingredientName.toLowerCase()) || 

...
// Default elemental properties for herbs
...
function transformUSDADataToNutritionalProfile(food) {
  ...
  const vitaminIds = new Set([
    1106, 1107, 1108, 1109, 1114, 1120, 1162, 1165, 1166, 1167, 1175, 1176,
    1177, 1178, 1180, 1183, 1184, 1185, 1187,
  ]);
...
async function findIngredientMatch(ingredientName) {
  ...
  try {
    ...
    for (const food of searchResults.foods) {
      ...
      const wordDiff =
        food.description.split(' ').length - cleanName.split(' ').length;
      score -= wordDiff * 5;
    ...
    if (bestMatch && bestMatch.fdcId) {
      // console.log(`Found best match: ${bestMatch.description} (${bestMatch.fdcId})`);

      // Cache the ingredient data using the search result
      // This avoids making another API call for now
      progress.processedIngredients[ingredientKey] = bestMatch;
      saveProgress();
...
async function processIngredientFile(filePath) {
  ...
  try {
    ...
    // Extract the main ingredient objects from the file
    // Look for the pattern: 'ingredient_name': { ... object properties ... },
    ...
    // Filter out ingredients that are likely metadata fields
    ...
    // console.log(`Found ${ingredientMatches.length} potential ingredients in file`);

    const actualIngredientsCount = 0;
    ...
    for (const match of ingredientMatches) {
      ...
      if (
        metadataFields.includes(ingredientName.toLowerCase()) ||
        ingredientName.includes('_') ||
        ingredientObj.includes('"name":') || // Skip objects that are clearly not top-level ingredients
        ingredientObj.includes("'name':")
      ) {
        // console.log(`  Skipping metadata field: ${ingredientName}`);
        continue;
      }

      actualIngredientsCount++;
    ...
    // console.log(`Found ${actualIngredientsCount} actual ingredients in file`);
    ...
    if (COMMON_INGREDIENTS.length > 0) {
      ...
      if (lastCloseBracePos === -1) {
        ...
        for (const newIngredient of COMMON_INGREDIENTS) {
          ...
          // Get the underscored version of the ingredient name for the key
...
/**
 * Main function to process all herb ingredient files
 * @returns {Promise<void>}
 */
async function updateHerbs() {
  // console.log(`Processing herbs category`);

  // Get all herb ingredient files
...
export const herbs = fixIngredientMappings(rawHerbs) as Record<string, IngredientMapping>;
`;

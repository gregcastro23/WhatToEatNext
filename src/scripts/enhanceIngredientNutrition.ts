/**
 * Script to enhance ingredients with nutritional information
 * Uses USDA FoodData Central API to fetch and add nutritional data to existing ingredients
 *
 * Run with: yarn ts-node src/scripts/enhanceIngredientNutrition.ts [category]
 * Example: yarn ts-node src/scripts/enhanceIngredientNutrition.ts vegetables
 */
...
async function makeApiRequest(url, cacheKey) {
  ...
  const retries = 0;
  const maxRetries = 5;
  const waitTime = 5000; // Start with 5 seconds
...
function transformUSDADataToNutritionalProfile(food) {
  ...
  food.foodNutrients.forEach((nutrient) => {
    ...
    const id =
      nutrient.nutrientId ||
      (nutrient.nutrient && nutrient.nutrient.id) ||
      nutrient.number ||
      '';
  ...
  const calories =
    (nutrients['1008'] && nutrients['1008'].amount) ||
    (nutrients['208'] && nutrients['208'].amount) ||
    0; // Energy (kcal)
  const protein =
    (nutrients['1003'] && nutrients['1003'].amount) ||
    (nutrients['203'] && nutrients['203'].amount) ||
    0; // Protein (g)
  const carbs =
    (nutrients['1005'] && nutrients['1005'].amount) ||
    (nutrients['205'] && nutrients['205'].amount) ||
    0; // Carbohydrate (g)
  const fat =
    (nutrients['1004'] && nutrients['1004'].amount) ||
    (nutrients['204'] && nutrients['204'].amount) ||
    0; // Total lipid/fat (g)
  const fiber =
    (nutrients['1079'] && nutrients['1079'].amount) ||
    (nutrients['291'] && nutrients['291'].amount) ||
    0; // Fiber, total dietary (g)
  const sugar =
    (nutrients['1063'] && nutrients['1063'].amount) ||
    (nutrients['269'] && nutrients['269'].amount) ||
    0; // Sugars, total (g)
  ...
  if (
    (nutrients['1106'] && nutrients['1106'].amount > 0) ||
    (nutrients['320'] && nutrients['320'].amount > 0)
  )
    vitamins.push('a');
  if (
    (nutrients['1162'] && nutrients['1162'].amount > 0) ||
    (nutrients['401'] && nutrients['401'].amount > 0)
  ) {
    vitamins.push('c');
    antioxidants.push('vitamin c');
  }
  if (
    (nutrients['1114'] && nutrients['1114'].amount > 0) ||
    (nutrients['328'] && nutrients['328'].amount > 0)
  )
    vitamins.push('d');
  if (
    (nutrients['1109'] && nutrients['1109'].amount > 0) ||
    (nutrients['323'] && nutrients['323'].amount > 0)
  ) {
    vitamins.push('e');
    antioxidants.push('vitamin e');
  }
  if (
    (nutrients['1185'] && nutrients['1185'].amount > 0) ||
    (nutrients['430'] && nutrients['430'].amount > 0)
  )
    vitamins.push('k');
  if (
    (nutrients['1165'] && nutrients['1165'].amount > 0) ||
    (nutrients['404'] && nutrients['404'].amount > 0)
  )
    vitamins.push('b1');
  if (
    (nutrients['1166'] && nutrients['1166'].amount > 0) ||
    (nutrients['405'] && nutrients['405'].amount > 0)
  )
    vitamins.push('b2');
  if (
    (nutrients['1167'] && nutrients['1167'].amount > 0) ||
    (nutrients['406'] && nutrients['406'].amount > 0)
  )
    vitamins.push('b3');
  if (
    (nutrients['1175'] && nutrients['1175'].amount > 0) ||
    (nutrients['415'] && nutrients['415'].amount > 0)
  )
    vitamins.push('b6');
  if (
    (nutrients['1178'] && nutrients['1178'].amount > 0) ||
    (nutrients['418'] && nutrients['418'].amount > 0)
  )
    vitamins.push('b12');
  if (
    (nutrients['1177'] && nutrients['1177'].amount > 0) ||
    (nutrients['417'] && nutrients['417'].amount > 0)
  )
    vitamins.push('folate');

  // Check for minerals - fix the comparison logic
  if (
    (nutrients['1087'] && nutrients['1087'].amount > 0) ||
    (nutrients['301'] && nutrients['301'].amount > 0)
  )
    minerals.push('calcium');
  if (
    (nutrients['1089'] && nutrients['1089'].amount > 0) ||
    (nutrients['303'] && nutrients['303'].amount > 0)
  )
    minerals.push('iron');
  if (
    (nutrients['1090'] && nutrients['1090'].amount > 0) ||
    (nutrients['304'] && nutrients['304'].amount > 0)
  )
    minerals.push('magnesium');
  if (
    (nutrients['1091'] && nutrients['1091'].amount > 0) ||
    (nutrients['305'] && nutrients['305'].amount > 0)
  )
    minerals.push('phosphorus');
  if (
    (nutrients['1092'] && nutrients['1092'].amount > 0) ||
    (nutrients['306'] && nutrients['306'].amount > 0)
  )
    minerals.push('potassium');
  if (
    (nutrients['1093'] && nutrients['1093'].amount > 0) ||
    (nutrients['307'] && nutrients['307'].amount > 0)
  )
    minerals.push('sodium');
  if (
    (nutrients['1095'] && nutrients['1095'].amount > 0) ||
    (nutrients['309'] && nutrients['309'].amount > 0)
  )
    minerals.push('zinc');
  if (
    (nutrients['1098'] && nutrients['1098'].amount > 0) ||
    (nutrients['312'] && nutrients['312'].amount > 0)
  )
    minerals.push('copper');
  if (
    (nutrients['1101'] && nutrients['1101'].amount > 0) ||
    (nutrients['315'] && nutrients['315'].amount > 0)
  )
    minerals.push('manganese');
  if (
    (nutrients['1103'] && nutrients['1103'].amount > 0) ||
    (nutrients['317'] && nutrients['317'].amount > 0)
  )
    minerals.push('selenium');
...
async function findIngredientMatch(ingredientName) {
  ...
  try {
    ...
    if (!cookingMethods.some((method) => query.includes(method))) {
      ...
      if (
        freshFoods.some((type) => query.includes(type)) &&
        !query.includes('oil') &&
        !query.includes('processed')
      ) {
        query += ', raw';
      }
    }
    ...
    const searchUrl = `${USDA_API_BASE}/foods/search?query=${encodeURIComponent(
      query
...
async function processIngredientFile(filePath, category) {
  ...
  try {
    ...
    for (const match of ingredientMatches) {
      ...
      vitamins: [${nutritionalProfile.vitamins
        .map((v) => `'${v}'`)
        .join(', ')}],
      minerals: [${nutritionalProfile.minerals
        .map((m) => `'${m}'`)
        .join(', ')}]`;

        if (
          nutritionalProfile.antioxidants &&
          nutritionalProfile.antioxidants.length > 0
        ) {
          nutritionStr += `,
      antioxidants: [${nutritionalProfile.antioxidants
        .map((a) => `'${a}'`)
        .join(', ')}]`;
        }
        ...
        if (ingredientObj.includes('nutritionalProfile')) {
          ...
          const updatedIngredient = ingredientObj.replace(
            existingNutritionRegex,
            nutritionStr
          );
        ...
        // Otherwise insert it before the closing brace of the ingredient object
          ...
          const position =
            modifiedContent.indexOf(ingredientObj) + ingredientObj.length - 1;
          const updatedIngredient =
            ingredientObj.slice(0, -1) + ',\n    ' + nutritionStr + '\n  }';
          modifiedContent = modifiedContent.replace(
            ingredientObj,
            updatedIngredient
          );
    ...
    if (COMMON_INGREDIENTS_BY_CATEGORY[category]) {
      ...
      if (lastCloseBracePos === -1) {
        ...
        for (const newIngredient of COMMON_INGREDIENTS_BY_CATEGORY[category]) {
          ...
          const ingredientKey = newIngredient
            .toLowerCase()
            .replace(/\s+/g, '_');
      ...
      antioxidants: [${nutritionalProfile.antioxidants
        ...
        .join(', ')}]`;
            ...
            const capitalizedName = newIngredient
              .split(' ')
  ...
  '${ingredientKey}': {
    ...
    ${nutritionStr}
            ...
            modifiedContent =
              modifiedContent.slice(0, lastCloseBracePos) +
              newIngredientStr +
...
async function enhanceIngredients() {
  ...
  if (!targetCategory || !ALL_CATEGORIES.includes(targetCategory)) {
    ...
    // console.log('Example: yarn ts-node src/scripts/enhanceIngredientNutrition.ts vegetables');
    return;
  }
...
const raw${capitalizeFirstLetter(
        targetCategory
      )}: Record<string, Partial<IngredientMapping>> = {
  // Placeholder for new ingredients
};

export const ${targetCategory} = fixIngredientMappings(raw${capitalizeFirstLetter(
        targetCategory
...
// Run the script
enhanceIngredients().catch((error) => {

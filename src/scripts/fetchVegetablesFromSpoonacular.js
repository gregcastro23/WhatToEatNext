...
const servicesPath = path.join(rootDir, 'src/services/SpoonacularService.js');
const { SpoonacularService } = require(servicesPath);
...
async function fetchVegetableData() {
  ...
  const categories = {
    leafyGreens: ['swiss chard', 'arugula', 'endive', 'radicchio', 'watercress', 'mustard greens', 'collard greens'],
    rootVegetables: ['turnip', 'daikon radish', 'parsnip', 'rutabaga', 'celeriac'],
    otherVegetables: ['asparagus', 'artichoke', 'cucumber', 'okra', 'zucchini', 'peas', 'green beans', 'celery', 'fennel', 'kohlrabi', 'bok choy']
  };
  ...
  for (const [key, data] of Object.entries(vegetablesData)) {
    ...
    const vegetableName = vegetableData.name ? vegetableData.name.toLowerCase() : key;
    ...
    } else if (categories.rootVegetables.some(v => vegetableName.includes(v))) {
...
`;
  ...
  const indexPath = path.resolve(__dirname, '../../src/data/ingredients/vegetables/index.ts');
  if (Object.keys(categorizedVegetables.otherVegetables).length > 0) {

...
function documentIntentionalAny() {
  ...
  try {
    ...
    // Process each file (limit to 30 files for safety)
    const filesToProcess = Object.entries(fileGroups).slice(0, 30);

    for (const [filePath, locations] of filesToProcess) {
      try {
        let content = fs.readFileSync(filePath, 'utf8');
...
`;
  ...
  async generateFinalReport(totalReductions, totalDocumented) {
...
module.exports = { FullCampaignExecutor };

...
class ConfidenceScoringAnalyzer {
  ...
  analyzeEliminationCandidates(results) {
    ...
    return {
      ...
      highestConfidence: eliminationCandidates.reduce(
        (max, candidate) =>
          candidate.eliminationStrategy.confidence > max
            ? candidate.eliminationStrategy.confidence
            : max,
        0,
      ),
      averageConfidence:
        eliminationCandidates.length > 0
          ? eliminationCandidates.reduce(
              (sum, c) => sum + c.eliminationStrategy.confidence,
              0,
            ) / eliminationCandidates.length
          : 0,
    };
  }
  ...
  analyzePreservationConfidence(results) {
    const preservedVariables = results.filter(
      (r) => r.preservation.shouldPreserve,
    );
    ...
    return {
      ...
      overallAverageConfidence:
        preservedVariables.reduce(
          (sum, v) => sum + v.preservation.confidence,
          0,
  ...
  generateConfidenceRecommendations(distribution, elimination, preservation) {
    ...
    if (veryHighConfidence.count > 0) {
      const eliminationCandidates = veryHighConfidence.variables.filter(
        (v) => !v.shouldPreserve,
      );
    ...
    if (highConfidence.count > 0) {
      const eliminationCandidates = highConfidence.variables.filter(
        (v) => !v.shouldPreserve,
      );
      if (eliminationCandidates.length > 0) {
        recommendations.push({
          ...
          action:
            "Review manually before processing with enhanced safety protocols",
          variables: eliminationCandidates.map((v) => `${v.name} in ${v.file}`),
        });
      }
    }
    ...
    Object.entries(preservation.byDomain).forEach(([domain, data]) => {
      if (data.count > 10 && data.averageConfidence > 0.8) {
        recommendations.push({
          ...
          action:
            "Consider transformation into active features rather than elimination",
        });
      }
    });
    ...
    const lowConfidenceEliminations = Object.entries(distribution)
      ...
      .reduce(
        (total, [, data]) =>
          total + data.variables.filter((v) => !v.shouldPreserve).length,
        0,
      );
  ...
  printConfidenceSummary(confidenceReport) {
    ...
    Object.entries(confidenceReport.confidenceDistribution).forEach(
      ([range, data]) => {
        ...
        if (data.count > 0 && data.count <= 5) {
          data.variables.forEach((v) => {
            ...
            console.log(
              `     - ${v.name} (${v.confidence.toFixed(3)}) [${status}] - ${v.domain}`,
            );
          });
        }
      },
    ...
    console.log(
      `   Total Elimination Candidates: ${confidenceReport.eliminationAnalysis.total}`,
    );
    console.log(
      `   Highest Confidence: ${confidenceReport.eliminationAnalysis.highestConfidence.toFixed(3)}`,
    );
    console.log(
      `   Average Confidence: ${confidenceReport.eliminationAnalysis.averageConfidence.toFixed(3)}`,
    );

    console.log(`\n🛡️  Preservation Analysis:`);
    console.log(
      `   Total Preserved Variables: ${confidenceReport.preservationAnalysis.total}`,
    );
    console.log(
      `   Overall Average Preservation Confidence: ${confidenceReport.preservationAnalysis.overallAverageConfidence.toFixed(3)}`,
    );
    ...
    Object.entries(confidenceReport.preservationAnalysis.byDomain)
      ...
      .forEach(([domain, data]) => {
        console.log(
          `     ${domain}: ${data.count} variables (avg confidence: ${data.averageConfidence.toFixed(3)})`,
        );
      });
  ...
  async analyze() {
    ...
    const outputPath = path.join(
      process.cwd(),
      "confidence-scoring-analysis.json",
    );

...
const COMMON_INGREDIENTS = [
  // Vegetables
  'spinach, raw', 'kale, raw', 'broccoli, raw', 'carrot, raw', 'bell pepper, red, raw',
  'tomato, raw', 'cucumber, raw', 'onion, raw', 'garlic, raw', 'potato, raw',
  
  // Fruits
  'apple, raw', 'banana, raw', 'orange, raw', 'strawberry, raw', 'blueberry, raw',
  'avocado, raw', 'lemon, raw', 'lime, raw', 'grapefruit, raw', 'pineapple, raw',
  
  // Proteins
  'chicken breast, raw', 'beef, ground, raw', 'salmon, raw', 'tuna, raw', 'pork, raw',
  'lamb, raw', 'egg, raw', 'tofu, raw', 'tempeh, raw', 'seitan, raw',
  
  // Dairy & Alternatives
  'milk, whole', 'yogurt, plain', 'cheese, cheddar', 'butter, unsalted', 'cream, heavy',
  'almond milk, unsweetened', 'coconut milk', 'oat milk', 'soy milk, unsweetened',
  
  // Grains
  'rice, white, raw', 'rice, brown, raw', 'quinoa, raw', 'oats, raw', 'barley, raw',
  'wheat flour', 'cornmeal', 'bread, whole wheat', 'pasta, raw',
  
  // Nuts & Seeds
  'almonds, raw', 'walnuts, raw', 'cashews, raw', 'peanuts, raw', 'sunflower seeds, raw',
  'flaxseed, raw', 'chia seeds, raw', 'pumpkin seeds, raw',
  
  // Legumes
  'lentils, raw', 'chickpeas, raw', 'black beans, raw', 'kidney beans, raw', 'pinto beans, raw',
  
  // Herbs & Spices
  'basil, fresh', 'oregano, dried', 'rosemary, dried', 'thyme, dried', 'cilantro, fresh',
  'cumin, ground', 'cinnamon, ground', 'turmeric, ground', 'black pepper, ground', 'salt',
  
  // Oils & Fats
  'olive oil', 'coconut oil', 'avocado oil', 'sesame oil', 'flaxseed oil',
  
  // Other
  'honey', 'maple syrup', 'sugar', 'vinegar, apple cider', 'soy sauce',
  'mustard', 'ketchup', 'mayonnaise', 'hot sauce', 'tomato sauce'
];
 ...
 */
function transformUSDADataToNutritionalProfile(food: USDAFood): NutritionalProfile {
...
async function generateNutritionalDataFile(ingredientsDir: string) {
  ...
  const ingredientsToFetch = ingredientNames.length > 0 ? ingredientNames : COMMON_INGREDIENTS;
...
async function discoverIngredients(ingredientsDir: string): Promise<string[]> {
  try {
    ...
    const categories = fs.readdirSync(ingredientsDir, { withFileTypes: true })
    ...
    for (const category of categories) {
      ...
      const files = fs.readdirSync(categoryPath)
        .filter(file => file.endsWith('.ts') && !file.endsWith('.d.ts') && file !== 'index.ts' && file !== 'types.ts');
      ...
      for (const file of files) {
        ...
        if (nameMatches) {
          nameMatches.forEach(match => {
            const nameValue = match.match(/['"]name['"]\s*:\s*['"]([^'"]+)['"]/);
          ...
          exportMatches.forEach(match => {
            ...
            if (keys) {
              keys.forEach(key => {
                ...
                if (cleanKey && !cleanKey.includes('name') && !cleanKey.includes('category')) {

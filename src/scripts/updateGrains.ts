/**
 * Script to update grain ingredients with nutritional information
 * Uses USDA FoodData Central API to fetch and add nutritional data to existing ingredients
 *
 * Run with: yarn ts-node src/scripts/updateGrains.ts
 */
...
const PROGRESS_FILE = path.resolve(CACHE_DIR, 'grains_progress.json');
...
// Default elemental properties for grains
...
/**
 * Main function to process all grain ingredient files
 * @returns {Promise<void>}
 */
async function updateGrains() {
  // console.log(`Processing grains category`);

  // Get all grain ingredient files
...
export const grains = fixIngredientMappings(rawGrains) as Record<string, IngredientMapping>;
`;

...
class React19NextJS15Validator {
  ...
  addResult(category, test, passed, details, errors) {
...
`;
    ...
    const hasReactInScopeError = result1.output.includes(
...
`;
    ...
    const hasFragmentError = result2.output.includes(
      "react/react-in-jsx-scope",
    );
...
`;
    ...
    const hasDefaultExportError = result1.output.includes(
...
`;
    ...
    const hasSuspenseError =
      result1.output.includes("error") && !result1.success;
    ...
    this.addResult(
      ...
      !hasSuspenseError,
      hasSuspenseError
        ? "ESLint has issues with Suspense"
        : "Suspense working correctly",
...
`;
    ...
    const hasTransitionError =
      result2.output.includes("error") && !result2.success;
    ...
    this.addResult(
      ...
      !hasTransitionError,
      hasTransitionError
        ? "ESLint has issues with transitions"
...
`;
    ...
    const hasExhaustiveDepsWarning = result1.output.includes(
    ...
    this.addResult(
      ...
      !hasExhaustiveDepsWarning
        ? ["No exhaustive-deps warning detected"]
        : undefined,
    );
...
`;
    ...
    const hasRulesOfHooksError = result2.output.includes(
    ...
    try {
      ...
      if (fs.existsSync(eslintConfigPath)) {
        ...
        const reactSettings = eslintConfig.find(
          (config) => config.settings?.react?.version,
        );

        const hasCorrectReactVersion =
          reactSettings?.settings?.react?.version === "19.1.0";
        ...
        this.addResult(
          ...
          !hasCorrectReactVersion
            ? ["ESLint React version not set to 19.1.0"]
            : undefined,
        );

        // Check modern JSX transform rules
        const reactRules = eslintConfig.find(
          (config) => config.rules && config.rules["react/react-in-jsx-scope"],
        );
        ...
        const hasEnhancedHooksRules =
          hooksConfig?.rules?.["react-hooks/exhaustive-deps"]?.[1]
            ?.additionalHooks;
        ...
        this.addResult(
          ...
          !hasEnhancedHooksRules
            ? ["Enhanced hooks rules not configured"]
            : undefined,
        );
      }
    } catch (error) {
  ...
  async validate() {
    console.log(
      "🚀 Starting React 19 and Next.js 15 Compatibility Validation\n",
    );
  ...
  generateReport() {
    ...
    if (passedTests === totalTests) {
      console.log(
        "🎉 All React 19 and Next.js 15 compatibility validations passed!",
      );
    } else {
      console.log(

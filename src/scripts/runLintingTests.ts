    ...
    try {
      ...
      return {
        ...
        errors: this.extractErrors(
          (error as any).stdout || (error as any).message,
        ),
...
`;
  ...
  private displaySummary(report: TestSuiteReport): void {
    ...
    if (report.failedTests === 0) {
      console.log(
        "🎉 ALL TESTS PASSED! Linting system is ready for production.",
      );
    } else {
      console.log(
  ...
  private async checkAstrologicalRules(): Promise<{
    ...
    try {
      const rulesPath = path.resolve(
        __dirname,
        "../eslint-plugins/astrological-rules.cjs",
      );
  ...
  private async checkPerformanceSettings(): Promise<{
    ...
    try {
      ...
      const perfConfig = config.find(
        (c: unknown) =>
          (c as any).settings && (c as any).settings["import/cache"],
      );
      const issues: string[] = [];

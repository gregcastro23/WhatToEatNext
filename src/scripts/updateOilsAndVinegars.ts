...
const VINEGARS_DIR = path.resolve(
  process.cwd(),
...
const DEFAULT_OIL_ELEMENTAL_PROPERTIES = {
  Fire: 0.3,
  Earth: 0.3,
  Water: 0.2,
  Air: 0.2,
};
const DEFAULT_VINEGAR_ELEMENTAL_PROPERTIES = {
  Water: 0.4,
  Air: 0.3,
  Earth: 0.2,
  Fire: 0.1,
  ...
  try {
    ...
    if (category === 'oils') {
      for (const [oilName, nutritionData] of Object.entries(oilNutritionData)) {
        ...
        if (
          updatedContent.includes(`'${safeName}'`) ||
          updatedContent.includes(`"${safeName}"`)
        ) {
          ...
          if (updatedContent.includes('nutritionalProfile')) {
            ...
            updatedContent = updatedContent.replace(
              propertyInsertRegex,
              `$1,\n    nutritionalProfile: ${JSON.stringify(
                nutritionData,
                null,
                2
              )}`
      ...
      for (const [vinegarName, nutritionData] of Object.entries(
        vinegarNutritionData
      )) {
...
async function updateOilsAndVinegars() {
  ...
  const SEASONINGS_DIR = path.resolve(
    process.cwd(),

...
const EXPORT_CONST_REGEX =
  /export\s+const\s+(\w+)\s*:\s*Record<string,\s*(\w+)>\s*=\s*{/;
...
function fixIngredientFile(filePath) {
  ...
  const hasUtilsImport = importPaths.some((path) =>
    path.includes('elementalUtils')
  );
  ...
  if (!hasUtilsImport) {
    ...
    if (lastImport) {
      ...
      // Removed duplicate:       // Removed duplicate:       content = `import { fixIngredientMappings } from '@/utils/elementalUtils';\n${content}`;
    }
  }
  ...
  content = content.replace(
    ...
    }: Record<string, Partial<${mappingType}>> = {`
  );
  ...
  if (lastBraceIndex !== -1) {
    content =
      content.slice(0, lastBraceIndex + 1) +
      `\n\n// Fix the ingredient mappings to ensure they have all required properties` +
      `\nexport const ${variableName}: Record<string, ${mappingType}> = fixIngredientMappings(raw${
        variableName.charAt(0).toUpperCase() + variableName.slice(1)
      });\n` +

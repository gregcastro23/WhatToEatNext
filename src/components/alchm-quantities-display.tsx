...
import { Flame, Droplets, Wind, Mountain, Coins, RefreshCw } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-orange-300">Current Quantities</h3>
        <button
          onClick={fetchData}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-gradient-to-br from-red-900/30 to-orange-900/20 border border-red-500/30 rounded-lg">
          ...
          <div className="text-2xl font-bold mb-2 text-red-300">{quantities.Spirit.toFixed(1)}</div>
          <div className="h-2 bg-gray-900/30 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-red-500 to-orange-500 rounded-full transition-all duration-500"
              style={{ width: `${(quantities.Spirit / 10) * 100}%` }}
            />
          </div>
        </div>
        ...
        <div className="p-4 bg-gradient-to-br from-blue-900/30 to-cyan-900/20 border border-blue-500/30 rounded-lg">
          ...
          <div className="text-2xl font-bold mb-2 text-blue-300">{quantities.Essence.toFixed(1)}</div>
          <div className="h-2 bg-gray-900/30 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full transition-all duration-500"
              style={{ width: `${(quantities.Essence / 10) * 100}%` }}
            />
          </div>
        </div>
        ...
        <div className="p-4 bg-gradient-to-br from-green-900/30 to-emerald-900/20 border border-green-500/30 rounded-lg">
          ...
          <div className="text-2xl font-bold mb-2 text-green-300">{quantities.Matter.toFixed(1)}</div>
          <div className="h-2 bg-gray-900/30 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-green-500 to-emerald-500 rounded-full transition-all duration-500"
              style={{ width: `${(quantities.Matter / 10) * 100}%` }}
            />
          </div>
        </div>
        ...
        <div className="p-4 bg-gradient-to-br from-purple-900/30 to-violet-900/20 border border-purple-500/30 rounded-lg">
          ...
          <div className="text-2xl font-bold mb-2 text-purple-300">
            {quantities.Substance.toFixed(1)}
          </div>
          <div className="h-2 bg-gray-900/30 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-purple-500 to-violet-500 rounded-full transition-all duration-500"
              style={{ width: `${(quantities.Substance / 10) * 100}%` }}
            />
          </div>
        </div>
      </div>
      ...
      <div className="p-6 bg-gradient-to-r from-amber-900/30 to-yellow-900/20 border border-amber-500/30 rounded-lg">
        ...
        <div className="text-3xl font-bold text-amber-300">{quantities.ANumber.toFixed(1)}</div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
        <div className="p-3 bg-cyan-900/20 border border-cyan-500/30 rounded-lg text-center">
          ...
          <div className="text-lg text-cyan-300">{data.heat.toFixed(4)}</div>
        </div>
        <div className="p-3 bg-orange-900/20 border border-orange-500/30 rounded-lg text-center">
          ...
          <div className="text-lg text-orange-300">{data.entropy.toFixed(4)}</div>
        </div>
        <div className="p-3 bg-purple-900/20 border border-purple-500/30 rounded-lg text-center">
          ...
          <div className="text-lg text-purple-300">{data.reactivity.toFixed(4)}</div>
        </div>

...
import @/lib  from 'ChakraAlchemyService ';
...
const PlanetaryHoursDisplay: React.FC<PlanetaryHoursDisplayProps> = ({
  compact = false,
}) => {
  const [currentHour, setCurrentHour] = useState<CapitalizedPlanet | null>(
    null
  );
  const [currentDay, setCurrentDay] = useState<CapitalizedPlanet | null>(null);
  const [currentMinute, setCurrentMinute] = useState<CapitalizedPlanet | null>(
    null
  );
  const [allHours, setAllHours] = useState<Map<number, CapitalizedPlanet>>(
    new Map()
  );
  ...
  const [alchemicalProperty, setAlchemicalProperty] = useState<string | null>(
    null
  );
  ...
  if (compact) {
    return (
      <div className="bg-opacity-30 bg-purple-900 rounded-md">
            ...
            <div className="flex items-center">
              ...
              <span
                className={`ml-1.5 text-sm font-medium ${
                  currentHour ? getPlanetColor(currentHour) : ''
                }`}
              >
        ...
        {showDetails && (
          <div className="px-3 pb-2 space-y-2 border-t border-purple-700 / (30 || 1)">
            {/* Day info */}
            <div className="flex items-center justify-between pt-2">
              ...
              <div
                className={`flex items-center ${
                  currentDay ? getPlanetColor(currentDay) : ''
                }`}
              >
            ...
            <div className="flex items-center justify-between">
              ...
              <div
                className={`flex items-center ${
                  currentMinute ? getPlanetColor(currentMinute) : ''
                }`}
              >
            ...
            <div className="flex items-center">
              ...
              <div className="flex flex-wrap gap-1">
                {associatedChakras.map((chakra, index) => (
                  <span
                    key={index}
                    className="text-xs px-2 py-0.5 bg-opacity-30 bg-purple-700 rounded-full text-purple-200"
                  >
                    {chakra}
                  </span>
                ))}
          ...
          {Array.from({ length: 24 }, (_, i) => i).map((hour) => {
            ...
            return (
              ...
              >
                ...
                {planet && (
                  <div
                    className={`flex items-center ${getPlanetColor(planet)}`}
                  >

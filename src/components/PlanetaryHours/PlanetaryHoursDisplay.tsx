...
import { planetElementMap, planetPropertyMap } from '@/constants/alchemicalEnergyMapping';
import {
...
const PlanetaryHoursDisplay: React.FC<PlanetaryHoursDisplayProps> = ({ compact = false }) => {
        ...
        if (isCapitalizedPlanet(hourInfo.planet)) {
          ...
          if (isValidPlanetName(planetLowerCase)) {
            ...
            if (property && ENERGY_STATE_CHAKRA_MAPPING[property]) {
              ...
              const chakraNames = associatedChakraKeys.map(key => getChakraDisplayName(key));
  ...
  if (compact) {
    return (
      <div className='rounded-md bg-purple-900 bg-opacity-30'>
        <div
        ...
        >
          <div className='flex items-center space-x-2'>
            ...
            <div className='flex items-center'>
              ...
              <span
                className={`ml-1.5 text-sm font-medium ${currentHour ? getPlanetColor(currentHour) : ''}`}
              >
        ...
        {showDetails && (
          <div className='space-y-2 border-t border-purple-700/30 px-3 pb-2'>
            {/* Day info */}
            ...
            <div className='flex items-center justify-between'>
              <div className='flex items-center'>
                <Timer className='mr-2 h-4 w-4 text-gray-300' />
                <span className='text-sm text-gray-300'>Minute:</span>
              </div>
              <div
                className={`flex items-center ${currentMinute ? getPlanetColor(currentMinute) : ''}`}
              >
            ...
            {alchemicalProperty && (
              <div className='flex items-center justify-between'>
                ...
                <span className='rounded-full bg-blue-700 bg-opacity-30 px-2 py-0.5 text-xs text-blue-200'>
                  {alchemicalProperty}
                </span>
              </div>
            )}
  ...
  return (
    <div className='rounded-lg bg-purple-900 bg-opacity-20 p-4'>
      <h3 className='mb-3 text-lg font-semibold text-white'>Planetary Influences</h3>

      <div className='space-y-2'>
        {/* Day Display */}
        <div className='rounded-md bg-purple-900 bg-opacity-30 p-3'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center'>
            ...
            <div
              className={`flex items-center font-medium ${currentDay ? getPlanetColor(currentDay) : ''}`}
            >
              {currentDay && getPlanetIcon(currentDay)}
        ...
        <div className='rounded-md bg-purple-900 bg-opacity-30 p-3'>
          <div className='flex items-center justify-between'>
            ...
            <div
              className={`flex items-center font-medium ${currentHour ? getPlanetColor(currentHour) : ''}`}
            >
        ...
        <div className='rounded-md bg-purple-900 bg-opacity-30 p-3'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center'>
              <Timer className='mr-2 h-5 w-5 text-gray-300' />
              <span className='text-gray-300'>Planetary Minute:</span>
            </div>
            <div
              className={`flex items-center font-medium ${currentMinute ? getPlanetColor(currentMinute) : ''}`}
            >
              {currentMinute && getPlanetIcon(currentMinute)}
        ...
        {alchemicalProperty && (
          <div className='mt-2 border-t border-gray-700 pt-2'>
            <div className='flex items-center'>
              <span className='mr-2 text-xs text-gray-400'>Alchemical Energy:</span>
              <span className='rounded-full bg-blue-700 bg-opacity-30 px-2 py-0.5 text-xs text-blue-200'>
                {alchemicalProperty}
              </span>
            </div>
        ...
        {associatedChakras.length > 0 && (
          <div className='mt-2 border-t border-gray-700 pt-2'>
            <div className='flex items-center'>
              <span className='mr-2 text-xs text-gray-400'>Influences Chakras:</span>
              <div className='flex flex-wrap gap-1'>
                {associatedChakras.map((chakra, index) => (
                  <span
                    key={index}
                    className='rounded-full bg-purple-700 bg-opacity-30 px-2 py-0.5 text-xs text-purple-200'
                  >
                    {chakra}
                  </span>
                ))}
      ...
      <div className='mt-4'>
        <h4 className='text-md mb-2 font-semibold text-white'>Daily Hours</h4>
            ...
            return (
              <div
                ...
                className={`flex items-center justify-between rounded-md p-2 ${
                  isCurrentHour ? 'bg-purple-700 bg-opacity-50' : 'bg-gray-800 bg-opacity-20'
                }`}
              >

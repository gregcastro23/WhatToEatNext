...
export default function PlanetaryPowerWidget({
  location = { lat: 40.7128, lon: -74.006 }, // Default NYC
  className = "",
  compact = false,
  ...
  useEffect(() => {
    if (kinetics?.data.base.power) {
      ...
      const currentHourData = kinetics.data.base.power.find(
  ...
  if (compact) {
    return (
      <div
        className={`inline-flex items-center gap-2 px-3 py-1 bg-white/90 backdrop-blur rounded-full shadow-sm ${className}`}
      >
        ...
        <div className="flex flex-col">
          <span
            className={`text-xs font-semibold ${getPowerLevelColor(currentPowerLevel)}`}
          >
        ...
        <>
          ...
          <div className="mb-3">
            <div className="flex justify-between text-xs mb-1">
              ...
              <span
                className={`font-semibold ${getPowerLevelColor(currentPowerLevel)}`}
              >
                {(currentPowerLevel * 100).toFixed(0)}%
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className={`h-full rounded-full transition-all duration-500 ${
                  currentPowerLevel >= 0.8
                    ? "bg-green-500"
                    : currentPowerLevel >= 0.6
                      ? "bg-yellow-500"
          ...
          <div className="mb-3">
            ...
            <div
              className={`h-1 rounded-full bg-gradient-to-r ${getElementColor(dominantElement)}`}
            />
          </div>
                    ...
                    <div className="relative h-8">
                      <div className="absolute bottom-0 left-0 right-0 mx-auto w-6">
                        <div
                          ...
                          style={{
                            height: `${Math.min(100, (value / 5) * 100)}%`,
                          }}
                        />
                      </div>
                    </div>

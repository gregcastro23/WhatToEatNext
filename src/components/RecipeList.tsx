...
interface Recipe {
  ...
  instructions?: string[];
  preparationSteps?: string[]; // Alternative field name in data
  mealType?: string;
  season?: string[];
  ...
  cookingMethods?: string[]; // Alternative field name in data
...
export default function RecipeList({ cuisineFilter }: RecipeListProps = {}) {
  ...
  // Calculate current planetary hour on mount and when time changes
  useEffect(() => {
    const calculatePlanetaryHour = () => {
      const now = new Date();
      const dayOfWeek = now.getDay(); // 0 = Sunday, 1 = Monday, etc.
      
      // Ruling planets of days in Chaldean order
      const dayRulers = ['Sun', 'Moon', 'Mars', 'Mercury', 'Jupiter', 'Venus', 'Saturn'];
      const planetaryOrder = ['Saturn', 'Jupiter', 'Mars', 'Sun', 'Venus', 'Mercury', 'Moon']; // Chaldean order
      
      // Get sunrise and sunset times (approximate)
      const date = now.toISOString().split('T')[0];
      const sunrise = new Date(`${date}T06:00:00`); // Approximation
      const sunset = new Date(`${date}T18:00:00`);   // Approximation
      
      // Calculate day and night lengths in hours
      ...
      if (now >= sunrise && now < sunset) {
        ...
        hourOfDay = Math.floor(hoursSinceSunrise / planetaryHourLength);
      } else {
        // Night hour
        const hoursSinceSunset = now < sunrise 
          ? (now.getTime() + 24*60*60*1000 - sunset.getTime()) / (1000 * 60 * 60)
          : (now.getTime() - sunset.getTime()) / (1000 * 60 * 60);
        const planetaryHourLength = nightLength / 12;
        hourOfDay = Math.floor(hoursSinceSunset / planetaryHourLength) + 12;
      }
      
      // Calculate the planetary hour
      const dayRuler = dayRulers[dayOfWeek];
      const startingPlanetIndex = planetaryOrder.indexOf(dayRuler);
      const hourRulerIndex = (startingPlanetIndex + hourOfDay) % 7;
      const planetaryHour = planetaryOrder[hourRulerIndex];
      
      return planetaryHour;
    };
    
    const hourPlanet = calculatePlanetaryHour();
    setCurrentPlanetaryHour(hourPlanet);
  ...
  }, [cuisineFilter, currentPlanetaryAlignment, currentZodiac, activePlanets, isDaytime, currentPlanetaryHour]);
    ...
    return recipeList.map(recipe => {
      ...
      if (recipe.season && currentZodiac) {
        ...
        const zodiacToSeason: Record<string, string> = {
          ...
          // Fall/Autumn signs
          'libra': 'autumn',
          'libra': 'fall', // Handle both spellings
          'scorpio': 'autumn',
          'scorpio': 'fall',
          'sagittarius': 'autumn',
          'sagittarius': 'fall',
      ...
      if (currentPlanetaryHour && recipe.astrologicalInfluences) {
        ...
        const hourKeywords = planetaryInfluenceMap[currentPlanetaryHour] || [];
        ...
        const matchingInfluences = recipe.astrologicalInfluences.filter(influence => 
          ...
          influence.toLowerCase().includes(currentPlanetaryHour.toLowerCase())
        );
      ...
      if (recipe.astrologicalInfluences && recipe.astrologicalInfluences.length > 0) {
        ...
        const matchingInfluences = activePlanets?.filter(planet => 
      ...
      if (currentPlanetaryAlignment && recipe.astrologicalInfluences) {
        ...
        const planetaryPositions = [
          { planet: 'Sun', sign: currentPlanetaryAlignment.sun?.sign, degree: currentPlanetaryAlignment.sun?.degree },
          { planet: 'Moon', sign: currentPlanetaryAlignment.moon?.sign, degree: currentPlanetaryAlignment.moon?.degree },
          { planet: 'Mercury', sign: currentPlanetaryAlignment.mercury?.sign, degree: currentPlanetaryAlignment.mercury?.degree },
          { planet: 'Venus', sign: currentPlanetaryAlignment.venus?.sign, degree: currentPlanetaryAlignment.venus?.degree },
          { planet: 'Mars', sign: currentPlanetaryAlignment.mars?.sign, degree: currentPlanetaryAlignment.mars?.degree },
          { planet: 'Jupiter', sign: currentPlanetaryAlignment.jupiter?.sign, degree: currentPlanetaryAlignment.jupiter?.degree },
          { planet: 'Saturn', sign: currentPlanetaryAlignment.saturn?.sign, degree: currentPlanetaryAlignment.saturn?.degree }
        ...
        planetaryPositions.forEach(planet => {
          ...
          const matchesPlanetSign = recipe.astrologicalInfluences.some(influence => 
            influence.toLowerCase().includes(planet.sign!.toLowerCase())
          );
      ...
      <div className="mb-6 p-4 bg-indigo-50 rounded-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          ...
          <div className="flex items-center">
            ...
            <span>{currentPlanetaryHour || 'Calculating...'}</span>
          </div>
          <div className="flex items-center">
                ...
                <div className="mb-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="flex items-center">
                      ...
                      <span>Season: {recipe.season ? recipe.season.join(', ') : 'All'}</span>
      </div>
                ...
                {recipe.astrologicalInfluences && recipe.astrologicalInfluences.length > 0 && (
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {recipe.astrologicalInfluences.map((influence, idx) => (

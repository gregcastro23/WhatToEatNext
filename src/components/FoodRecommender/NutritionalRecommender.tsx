...
const NutritionalRecommender: React.FC<NutritionalRecommenderProps> = ({
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4">
      <h2 className="text-xl font-semibold mb-4">Nutritional Recommendations</h2>
                ...
                {filter.nutritional.vitamins.map(vitamin => (
                  ...
                  >
                    ...
                    <button 
                      ...
                      onClick={() => {
                        const updatedVitamins = filter.nutritional?.vitamins?.filter(v => v !== vitamin) || [];
                        handleNutritionalFilterChange({ vitamins: updatedVitamins });
                      }}
                    >
                      ×
                    </button>
                ...
                {filter.nutritional.minerals.map(mineral => (
                  ...
                  >
                    ...
                    <button 
                      ...
                      onClick={() => {
                        const updatedMinerals = filter.nutritional?.minerals?.filter(m => m !== mineral) || [];
                        handleNutritionalFilterChange({ minerals: updatedMinerals });
                      }}
                    >
                      ×
                    </button>
        ...
        {categoriesToDisplay.length === 0 ? (
          ...
          categoriesToDisplay.map((category) => (
            <div key={category} className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {recommendations[category].map((ingredient: IngredientMapping) => (
                  <IngredientCard 
                    key={ingredient.name} 
                    ...
                    isSpoonacularLoading={isSpoonacularLoading[ingredient.name] || false}
                    onLoadSpoonacularData={() => fetchEnhancedNutritionData(ingredient.name)}
                    isExpanded={expandedIngredient === ingredient.name}
                    onToggleExpand={(name) => setExpandedIngredient(prev => prev === name ? null : name)}
                  />
                ))}
              </div>
            </div>
        ...
        <div className="grid grid-cols-4 gap-1 mb-3">
          {calories && (
            <div className="bg-gray-50 p-1 rounded text-center">
              ...
              <div className="font-medium">{Math.round(calories.amount)}</div>
            </div>
          )}
          {protein && (
            <div className="bg-gray-50 p-1 rounded text-center">
              <div className="text-xs text-gray-500">Protein</div>
              <div className="font-medium">{Math.round(protein.amount)}g</div>
            </div>
          )}
          {carbs && (
            <div className="bg-gray-50 p-1 rounded text-center">
              <div className="text-xs text-gray-500">Carbs</div>
              <div className="font-medium">{Math.round(carbs.amount)}g</div>
            </div>
          )}
          {fat && (
            <div className="bg-gray-50 p-1 rounded text-center">
              <div className="text-xs text-gray-500">Fat</div>
              <div className="font-medium">{Math.round(fat.amount)}g</div>
            </div>
          )}
        </div>

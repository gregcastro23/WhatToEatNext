...
const FoodBalanceTracker: React.FC<FoodBalanceTrackerProps> = ({
  ...
  const [mealType, setMealType] = useState<'all' | 'breakfast' | 'lunch' | 'dinner' | 'snack'>('breakfast');
  
  // Elemental feature states
  const [showRecommendations, setShowRecommendations] = useState<boolean>(false);
  const [missingElements, setMissingElements] = useState<ElementalRecommendation[]>([]);
  ...
  // Handle ingredient/dish selection
  const handleIngredientSelect = (item: Ingredient | Dish) => {
    if (showCuisineSelection && selectedCuisine && 'mealType' in item) {
      ...
      const propertyEntries = dish.properties ? 
        dish.properties.filter(prop => 
          // Only include valid FoodProperty values
          ['hot', 'cold', 'wet', 'dry', 'light', 'heavy', 'spicy', 'mild', 'fresh', 
           'preserved', 'sweet', 'sour', 'bitter', 'umami', 'balanced', 'creamy', 
           'neutral', 'aromatic', 'crispy', 'grilled', 'layered', 'comforting', 
           'savory', 'numbing', 'refreshing', 'hearty', 'tangy', 'rich', 'complex', 
           'mild-spicy', 'earthy'].includes(prop)
        ) as FoodProperty[] : 
        [];
      
      // Ensure elementalProperties is never an empty object
      const elementalProps = dish.elementalProperties || defaultElementalProperties;
      ...
      setEntries([...entries, {
        ...
        category: 'category' in dish && typeof dish.category === 'string' ? dish.category : 'dish',
  ...
  useEffect(() => {
    if (showElementalFeatures && entries.length > 0) {
      ...
      Object.entries(propertyBalance).forEach(([element, value]) => {
        const numericValue = typeof value === 'object' && value && 'count' in value 
          ? value.count 
        ...
        if (numericValue < 0.2) {
          ...
          const recommendedIngredients = ingredients
            .filter(ing => {
              const elementValue = ing.elementalProperties[element as keyof ElementalProperties];
              return elementValue && elementValue > 0.4; // Only ingredients strong in this element
            })
            .sort((a, b) => {
              const aValue = a.elementalProperties[element as keyof ElementalProperties] || 0;
              const bValue = b.elementalProperties[element as keyof ElementalProperties] || 0;
    ...
    <div className="card w-full max-w-4xl shadow-sm">
        ...
        {showCuisineSelection && (
          <div className="flex gap-2">
            <button
              ...
              className={`btn btn-sm ${selectedView === 'today' ? 'btn-primary' : 'btn-outline'}`}
            >
              Today
            </button>
            <button
              onClick={() => setSelectedView('week')}
              className={`btn btn-sm ${selectedView === 'week' ? 'btn-primary' : 'btn-outline'}`}
            >
              Week
            </button>
          </div>
      ...
      <div className="card-content space-y-6 p-4">
        ...
        <div className="relative">
          <div className="flex items-center">
            ...
            <input
              ...
              placeholder={showCuisineSelection && selectedCuisine ? "Search dishes..." : "Search ingredients..."}
              ...
              {searchResults.map(item => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-500 mt-1 flex justify-between">
                    <span>{`${item.nutrition?.calories || 0} cal | ${item.nutrition?.protein || 0}g protein`}</span>
                  ...
                  {item.elementalProperties && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {Object.entries(item.elementalProperties).map(([element, value]) => {
                        if (value > 0.2) {
                          const Icon = ElementIcons[element as keyof typeof ElementIcons] || Flame;
                          return (
                            <span 
                              key={element} 
                  ...
                  {item.properties && item.properties.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {item.properties.map(prop => (
                        <span key={prop} className="inline-flex items-center px-2 py-1 text-xs rounded bg-gray-100">
                          {prop}
                        </span>
                      ))}
                    </div>
                  ...
                  <div>
                    <div className="font-medium flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-500">
                        {new Date(entry.timeAdded || entry.timeEaten).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                      </span>
                    </div>
                    
                    {/* Show elemental properties if available */}
                    {entry.elementalProperties && Object.keys(entry.elementalProperties).length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {Object.entries(entry.elementalProperties).map(([element, value]) => {
          ...
          <div className="pt-4 border-t">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {Object.keys(nutritionTargets).map(nutrient => {
                ...
                const statusColor = 
                  summary.percentage < 50 ? 'bg-blue-100 text-blue-800' :
                  summary.percentage <= 90 ? 'bg-green-100 text-green-800' :
                  summary.percentage <= 110 ? 'bg-yellow-100 text-yellow-800' : 
                  ...
                  <div key={nutrient} className="p-3 border rounded-md">
                    ...
                    <div className="flex justify-between items-center mt-2">
                      <div className="text-lg font-bold">
                        {summary.total.toFixed(0)}<span className="text-sm ml-1 font-normal">{summary.unit}</span>
                      </div>
                    ...
                    <div className="mt-2 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                      <div 
                        className={`h-full ${
                          summary.percentage < 50 ? 'bg-blue-500' :
                          summary.percentage <= 90 ? 'bg-green-500' :
                          summary.percentage <= 110 ? 'bg-yellow-500' : 
                          'bg-red-500'
                        }`}
        ...
        {showCuisineSelection && recommendations.length > 0 && (
          <div className="pt-4">
            <h3 className="text-lg font-medium mb-3">Recommended Next Dishes</h3>
          ...
          <div className="pt-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {Object.entries(propertyBalance).map(([element, value]) => {
                ...
                const ElementIcon = ElementIcons[element as keyof typeof ElementIcons] || Flame;
                const elementLevel = 
                  numericValue < 0.2 ? 'Low' :
                  numericValue < 0.4 ? 'Moderate' :
                  numericValue < 0.7 ? 'Balanced' : 'High';
                
                const statusColor = 
                  numericValue < 0.2 ? 'bg-red-100 text-red-800' :
                  numericValue < 0.4 ? 'bg-yellow-100 text-yellow-800' :
                  numericValue < 0.7 ? 'bg-green-100 text-green-800' : 
                  'bg-blue-100 text-blue-800';
                
                return (
                  <div key={element} className="p-3 border rounded-md flex items-center gap-3">
                    ...
                    <div className="text-lg font-semibold">{(numericValue * 100).toFixed(0)}%</div>
                  </div>
                );
              })}
            </div>
            ...
            {showRecommendations && missingElements.length > 0 && (
              <div className="mt-4 p-4 border rounded-md bg-gray-50">
                <h4 className="font-medium mb-3">Recommended Ingredients to Balance Elements</h4>
                <div className="space-y-3">
                  {missingElements.map(rec => {
                    const ElementIcon = ElementIcons[rec.element] || Flame;
                    return (
                      <div key={rec.element} className="p-3 border rounded-md bg-white">
        ...
        {showElementalFeatures && entries.length > 0 && (
          <div className="pt-4 border-t">
            <h3 className="text-lg font-medium mb-3">Elemental Combinations & Effects</h3>
            <div className="p-4 border rounded-md bg-gray-50">
                ...
                {Object.entries(propertyBalance)
                  ...
                  .map(([element, value], index, arr) => {
                    if (index < arr.length - 1) {
                      ...
                      return (
                        <li key={`${element}-${nextElement}`} className="p-3 border rounded-md bg-white">
                          <div className="font-medium flex items-center">
                            <span className="flex items-center gap-1">
                              {ElementIcons[element as keyof typeof ElementIcons] && 
                                React.createElement(ElementIcons[element as keyof typeof ElementIcons], { size: 16, className: "text-primary" })}
                              {element}
                            </span>
                            <span className="mx-2">+</span>
                            <span className="flex items-center gap-1">
                              {ElementIcons[nextElement as keyof typeof ElementIcons] && 
                                React.createElement(ElementIcons[nextElement as keyof typeof ElementIcons], { size: 16, className: "text-primary" })}
                              {nextElement}
                            </span>
                          </div>
                          <p className="text-sm text-gray-700 mt-1">
                            {getElementalCombinationEffect(element, nextElement)}
                          </p>
                        </li>
                      );
                    }
              ...
              {Object.entries(propertyBalance)
                ...
                <p className="text-center text-gray-500 py-3">
                  Add more diverse ingredients to see elemental combination effects.
                </p>
              )}
            </div>
          </div>
...
function getElementalCombinationEffect(element1: string, element2: string): string {
  const combinations: Record<string, Record<string, string>> = {
    "Fire": {
      "Water": "Creates balance between energy and emotion. Good for digestive health.",
      "Air": "Enhances creativity and mental clarity. Supports respiratory system.",
      "Earth": "Provides grounding energy while maintaining vitality. Supports metabolism."
    },
    "Water": {
      ...
      "Earth": "Creates deep emotional stability. Supports kidney and bladder health."
    },
    "Air": {

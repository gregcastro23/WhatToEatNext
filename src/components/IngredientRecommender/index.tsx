...
export default function IngredientRecommender() {
  ...
  const [selectedIngredient, setSelectedIngredient] =
    useState<ExtendedIngredientRecommendation | null>(null);
  ...
  const getElementGradient = (element: string) => {
    switch (element) {
      ...
      case 'Water':
        return 'rgba(107, 181, 255, 0.05)';
      case 'Earth':
        return 'rgba(107, 255, 142, 0.05)';
      case 'Air':
        return 'rgba(217, 179, 255, 0.05)';
      default:
        return 'rgba(229, 231, 235, 0.05)';
    }
  };
  ...
  const getElementBorderColor = (element: string) => {
    switch (element) {
      ...
      case 'Water':
        return 'rgba(107, 181, 255, 0.6)';
      case 'Earth':
        return 'rgba(107, 255, 142, 0.6)';
      case 'Air':
        return 'rgba(217, 179, 255, 0.6)';
      default:
  ...
  const getUniqueRecommendations = (
    ...
    spiceVariationMap.forEach((variations, spiceKey) => {
      ...
      if (variations.length > 0) {
        ...
        if (variations.length > 2 && spiceKey !== 'cumin') {
          // Avoid multiple cumins
  ...
  const _categorizeIngredient = (ingredient: Ingredient | UnifiedIngredient): string => {
    ...
    if (ingredient.category) {
      ...
      if (
        ['protein', 'meat', 'egg', 'dairy', 'plant_based', 'seafood', 'poultry'].includes(_category)
      ) {
        return 'proteins';
      }
    ...
    if (
      knownSpices.some(spice => name.includes(spice)) &&
    ...
    // Check for fruits
    if (knownFruits.some(fruit => name.includes(fruit)) || name.includes('fruit')) {
      return 'fruits';
    }
    ...
    if (knownVegetables.some(vegetable => name.includes(vegetable)) || name.includes('vegetable')) {
      return 'vegetables';
    }
    ...
    // Check for grains
    if (knownGrains.some(grain => name.includes(grain)) || name.includes('grain')) {
      return 'grains';
    }
    ...
    // Check for proteins
    if (knownProteins.some(protein => name.includes(protein)) || name.includes('protein')) {
      return 'proteins';
    }
    ...
    if (ingredient.elementalProperties) {
      ...
      const dominantElement = Object.entries(ingredient.elementalProperties).sort(
        ([, a], [, b]) => {
  ...
  const combinedCategorizedRecommendations = useMemo(() => {
    ...
    if (foodRecommendations && foodRecommendations.length > 0) {
      foodRecommendations.forEach(ingredient => {
        ...
        // For other ingredients, use explicit category if available
          ...
          const _category = getNormalizedCategory(
            ingredient as unknown as Ingredient | UnifiedIngredient,
          );
    ...
    Object.entries(astroRecommendations).forEach(([_category, items]) => {
      (items ?? []).forEach(item => {
        const normalizedCategory = getNormalizedCategory(
          item as unknown as Ingredient | UnifiedIngredient,
        );
        const targetCategory =
          normalizedCategory === 'other'
            ? determineCategory(String(item.name))
            : normalizedCategory;
    ...
    if (!categories.oils || categories.oils.length < 3) {
      const existingOilNames = new Set(
        (categories.oils || []).map(oil => oil?.name?.toLowerCase()),
      );
    ...
    Object.keys(categories).forEach(category => {
      ...
      categories[category] = categories[category].sort((a, b) => b.matchScore - a.matchScore);

      // Then apply the unique filter if needed
      if (category === 'spices' || category === 'seasonings') {
        categories[category] = getUniqueRecommendations(
          categories[category] as IngredientRecommendation[],
        );
      } else {
        ...
        categories[category].forEach(item => {
          const normalizedName = String(item.name).toLowerCase().trim();
          if (
            !uniqueMap.has(normalizedName) ||
            item.matchScore > (uniqueMap.get(normalizedName)?.matchScore || 0)
          ) {
            uniqueMap.set(normalizedName, item);
          }
        });
      ...
      <div className={styles.ingredientDetails}>
        ...
        {Boolean(itemData.description) && (
          <div className={styles.detailSection}>
            <h4 className={styles.detailTitle}>About {String(itemData.name)}:</h4>
            <p
              style={{
                ...
                .map(([element, value]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={styles.progressValue}>{Math.round(numericValue * 100)}%</div>
                    </div>
                  );
                })}
            </div>
        ...
        {Boolean((item as ExtendedIngredientRecommendation).culinaryApplications) &&
          Object.keys((item as ExtendedIngredientRecommendation).culinaryApplications || {})
            .length > 0 && (
            <div className={styles.detailSection}>
              <h4 className={styles.detailTitle}>
              ...
              >
                {Object.entries(
                  (item as ExtendedIngredientRecommendation).culinaryApplications || {},
                ).map(([method, details]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {detailsData.notes &&
                      Array.isArray(detailsData.notes) &&
                      detailsData.notes.length > 0 ? (
                        <div
                          style={{
                            fontSize: '0.85rem',
                            color: '#4b5563',
                      ...
                      {detailsData.techniques &&
                      Array.isArray(detailsData.techniques) &&
                      detailsData.techniques.length > 0 ? (
                        <div
                          style={{
                            fontSize: '0.8rem',
                            color: '#6b7280',
          ...
          <div className={styles.detailSection}>
            ...
            >
              {Array.isArray((item as ExtendedIngredientRecommendation).pairings)
                ? ((item as ExtendedIngredientRecommendation).pairings as string[])
                    .slice(0, 10)
                    .map(pairing => (
                      <span
                        key={pairing}
                ...
                : Object.keys(
                    ((item as ExtendedIngredientRecommendation).pairings as unknown as Record<
                      string,
                      unknown
                    >) || {},
                  )
        ...
        {(item as ExtendedIngredientRecommendation).nutritionalHighlights &&
          Object.keys((item as ExtendedIngredientRecommendation).nutritionalHighlights || {})
            .length > 0 && (
            <div className={styles.detailSection}>
              <h4 className={styles.detailTitle}>
        ...
        {(item as Record<string, unknown>).astrologicalProfile && (
          <div className={styles.detailSection}>
            ...
            >
              {((item as Record<string, unknown>).astrologicalProfile as Record<string, unknown>)
                .rulingPlanets ? (
                <div
                  style={{
                    fontSize: '0.85rem',
                ...
                >
                  <span style={{ fontWeight: '600', width: '140px', color: '#4b5563' }}>
                    Planetary influence:{' '}
                  </span>
                  <span style={{ fontWeight: '500', color: '#1f2937' }}>
                    {(
                      (
                        (item as Record<string, unknown>).astrologicalProfile as Record<
                          string,
                          unknown
                        >
                    ...
                    {(
                      (
                        (item as Record<string, unknown>).astrologicalProfile as Record<
                          ...
                          unknown
                        >
                      ).favorableZodiac as string[]
                    )
                      .map((sign: string) => sign.charAt(0).toUpperCase() + sign.slice(1))
                      .join(', ')}
                  </span>
                </div>
              ) : null}
  ...
  const renderIngredientCard = (item: unknown) => {
    ...
    const dominantElement =
      Object.entries((itemData.elementalProperties as Record<string, unknown>) || {}).sort(
        ([, a], [, b]) => {
      ...
      >
        <div className={styles.cardContent}>
          <div className={styles.nameRow}>
            ...
            <h3 className={styles.ingredientName}>{String(itemData.name)}</h3>
          </div>
  ...
  return (
    <div className={styles.container} data-time-of-day={isDaytime ? 'day' : 'night'}>
      <div className={styles.innerContainer}>
        <h1 className={styles.title}>Celestial Ingredient Recommendations</h1>
        ...
        {showAll ? (
            ...
            {ORDERED_CATEGORIES.filter(category =>
              ...
              <div key={category}>
                ...
                >
                  ...
                  <span style={{ marginLeft: 'auto' }}>{expanded[category] ? '▼' : '►'}</span>
                </div>

                {expanded[category] && (
                  <div className={styles.categoryGrid}>
                    {combinedCategorizedRecommendations[category]
                      .slice(0, CATEGORY_DISPLAY_COUNTS[category] || 8)
                      .map(item => {
          ...
          <>
            ...
            <div style={{ textAlign: 'center', marginTop: '1rem' }}>
              <button className={styles.showAllButton} onClick={toggleShowAll}>
                {showAll ? (
                  <>
                    <ChevronUp size={16} />
                    Show less
                  ...
                  <>
                    <ChevronDown size={16} />
                    View all{' '}
                    {Object.values(combinedCategorizedRecommendations).reduce(
        ...
        <div className={styles.filters}>
          ...
          <div className={styles.categoryFilters}>
            <div className={styles.categoryFilterRow}>
              {ORDERED_CATEGORIES.slice(0, 3).map(
                category =>
                  Object.keys(combinedCategorizedRecommendations).includes(category) && (
                    <button
                      key={category}
            ...
            <div className={styles.categoryFilterRow}>
              {ORDERED_CATEGORIES.slice(3, 6).map(
                category =>
                  Object.keys(combinedCategorizedRecommendations).includes(category) && (
                    <button
                      key={category}
            ...
            <div className={styles.categoryFilterRow}>
              {ORDERED_CATEGORIES.slice(6).map(
                category =>
                  Object.keys(combinedCategorizedRecommendations).includes(category) && (
                    <button
                      key={category}

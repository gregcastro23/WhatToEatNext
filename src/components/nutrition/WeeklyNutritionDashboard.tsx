...
import { WeeklyNutritionResult, ComplianceSeverity, getComplianceSeverity } from '@/types/nutrition';
...
export function WeeklyNutritionDashboard({
  ...
  const totalMacroCalories = 
    ...
    (weeklyTotals.carbs * 4) +
    (weeklyTotals.fat * 9);
  
  const macroPercentages = {
    protein: totalMacroCalories > 0 ? (weeklyTotals.protein * 4 / totalMacroCalories) * 100 : 33,
    carbs: totalMacroCalories > 0 ? (weeklyTotals.carbs * 4 / totalMacroCalories) * 100 : 33,
    fat: totalMacroCalories > 0 ? (weeklyTotals.fat * 9 / totalMacroCalories) * 100 : 33
  };
      ...
      <div className={styles.compactView}>
          ...
          <div className={styles.macroPills}>
            <div className={styles.pill}>
              ...
              <span className={styles.pillValue}>
                ...
                <span className={styles.pillTarget}>/ {Math.round(weeklyGoals.calories)}</span>
              </span>
            </div>
            <div className={styles.pill}>
              ...
              <span className={styles.pillValue}>
                ...
                <span className={styles.pillTarget}>/ {Math.round(weeklyGoals.protein)}g</span>
              </span>
            </div>
            <div className={styles.pill}>
              ...
              <span className={styles.pillValue}>
                ...
                <span className={styles.pillTarget}>/ {Math.round(weeklyGoals.carbs)}g</span>
              </span>
            </div>
            <div className={styles.pill}>
              ...
              <span className={styles.pillValue}>
                ...
                <span className={styles.pillTarget}>/ {Math.round(weeklyGoals.fat)}g</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className={styles.compactRight}>
          ...
          {weeklyCompliance.deficiencies.length > 0 && (
            <div className={styles.deficiencyAlert}>
              ...
              <span className={styles.alertText}>
                {weeklyCompliance.deficiencies.length} nutrient{weeklyCompliance.deficiencies.length > 1 ? 's' : ''} below target
              </span>
            </div>
          )}
          ...
          <button
            ...
            aria-label={isExpanded ? "Collapse nutrition details" : "Expand nutrition details"}
          >
            {isExpanded ? (
              <>
        ...
        <div className={styles.expandedView}>
          <div className={styles.expandedGrid}>
            ...
            <div className={styles.expandedSection}>
              <h3 className={styles.sectionTitle}>Macronutrient Distribution</h3>
              <MacroSummary
                totals={{
            ...
            <div className={styles.expandedSection}>
              ...
              <div className={styles.varietyStats}>
                <div className={styles.varietyStat}>
                  <span className={styles.varietyLabel}>Unique Ingredients</span>
                  <span className={styles.varietyValue}>{variety.uniqueIngredients}</span>
                </div>
                <div className={styles.varietyStat}>
                  <span className={styles.varietyLabel}>Unique Recipes</span>
                  <span className={styles.varietyValue}>{variety.uniqueRecipes}</span>
                </div>
                <div className={styles.varietyStat}>
          ...
          {weeklyCompliance.deficiencies.length > 0 && (
            <div className={styles.suggestions}>
              <h4 className={styles.suggestionsTitle}>💡 Nutrition Recommendations</h4>
              <ul className={styles.suggestionsList}>
                {weeklyCompliance.deficiencies.slice(0, 5).map((def, idx) => (
                  <li key={idx} className={styles.suggestionItem}>
                    ...
                    Currently averaging {Math.round(def.averageDaily)}{getNutrientUnit(def.nutrient)}/day 
                    (target: {Math.round(def.targetDaily)}{getNutrientUnit(def.nutrient)})
                  </li>
                ))}
              </ul>
            </div>
...
function formatNutrientName(nutrient: string): string {
  ...
  return nameMap[nutrient] || nutrient.charAt(0).toUpperCase() + nutrient.slice(1);
}

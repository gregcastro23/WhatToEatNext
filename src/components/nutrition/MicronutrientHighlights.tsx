...
import { NutritionalSummary, NutritionalTargets, ComplianceDeficiency } from '@/types/nutrition';
...
const KEY_MICRONUTRIENTS = [
  'vitaminC', 'vitaminD', 'calcium', 'iron', 'magnesium', 'potassium', 'zinc', 'folate'
];
...
const formatNutrientName = (nutrient: string): string => {
  ...
  return nameMap[nutrient] || nutrient.replace(/([A-Z])/g, ' $1').replace(/^./, (str) => str.toUpperCase());
};
...
export function MicronutrientHighlights({ totals, goals, deficiencies }: MicronutrientHighlightsProps) {
  ...
  return (
    <div className={styles.micronutrientHighlights}>
      {displayedMicros.length === 0 ? (
        <p className={styles.noData}>No key micronutrient data available or goals set.</p>
      ) : (
        <ul className={styles.micronutrientList}>
          {displayedMicros.map((micro, index) => (
            <li key={index} className={`${styles.micronutrientItem} ${micro.isDeficient ? styles.deficient : ''}`}>
              ...
              <div className={styles.nutrientProgress}>
                ...
                <span className={styles.nutrientValue}>
                  {micro.total}{micro.unit} <span className={styles.nutrientTarget}>/ {micro.goal}{micro.unit}</span>
                </span>
              </div>
              {micro.isDeficient && (
                <span className={styles.deficiencyIndicator} title="Below target">⚠️</span>
              )}
            </li>
          ))}
        </ul>

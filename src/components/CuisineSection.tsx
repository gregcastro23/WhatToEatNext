...
export const CuisineSection: React.FC<CuisineSectionProps> = ({
  ...
  if (!(cuisineRecipes || []).length) {
    ...
    const isSpecialCase =
      cuisineString.toLowerCase() === 'african' || cuisineString.toLowerCase() === 'american';
    if (isSpecialCase) {
  ...
  const _hasRegionalVariants = (cuisineRecipes || []).some(r => {
    const recipeData = r as Record<string, unknown>;
    return (
      recipeData.regionalCuisine &&
      (recipeData.cuisine as string).toLowerCase() === cuisine.toLowerCase()
    );
  });
  const _regionalVariantNames = [
    ...new Set(
      cuisineRecipes
        .filter(r => {
          const recipeData = r as Record<string, unknown>;
          return (
            recipeData.regionalCuisine &&
            String(recipeData.cuisine || '').toLowerCase() === String(cuisine || '').toLowerCase()
          );
        })
        .map(r => {
          const recipeData = r as Record<string, unknown>;

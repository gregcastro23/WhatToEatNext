...
const AlchemicalRecommendationsView: React.FC<AlchemicalRecommendationsProps> = ({
  ...
  const cookingMethodsArray = useMemo(() => Object.entries(cookingMethods).map(([key, method]) => {
      ...
      const { elementalEffect: declaredEffect } = method as any;
      let elementalEffect;
      if (declaredEffect) {
        elementalEffect = declaredEffect;
      } else {
  ...
  const cuisinesArray = useMemo(() => {
    ...
    return Object.entries(cuisines).map(([key, cuisine]) => {
      ...
      const { elementalState: declaredState } = cuisine;
      let elementalState;
      if (declaredState) {
        elementalState = declaredState;
      } else {

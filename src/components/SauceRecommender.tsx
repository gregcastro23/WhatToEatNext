      ...
      // Add sauce recommendations based on protein
        ...
        sauceRecommender.forProtein[protein].forEach((sauceName: string) => {
          // Find the sauce data in the traditionalSauces
            Object.entries(cuisineData.traditionalSauces).forEach(([id, sauceData]: [string, unknown]) => {
              ...
              >
                ...
                {isExpanded && (
                  <div className="px-3 py-2 border-t border-gray-100">
                    ...
                    {Array.isArray((sauce.preparationSteps as unknown) || (sauce.procedure as unknown) || (sauce.instructions as unknown)) ? (
                          <ol className="pl-4 list-decimal">
                            {((sauce.preparationSteps as string[]) || (sauce.procedure as string[]) || (sauce.instructions as string[])).map((step: string, i: number) => (
                              <li key={i}>{step}</li>
                            ))}
                          </ol>
                        ) : (
                      <p>{(sauce.preparationSteps as string) || (sauce.procedure as string) || (sauce.instructions as string)}</p>
                        )}
                      </div>
                    )}

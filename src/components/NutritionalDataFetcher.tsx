  ...
  return (
    <div className="text-center py-12 px-4 border border-dashed border-gray-300 rounded-lg bg-gray-50">
      ...
      <p className="mt-1 text-sm text-gray-500">
        Enter a food name in the search box above to see nutritional information
...
function ZodiacRecommendations({
  dominantElement,
}: {
  ...
  return (
    <div className="mt-6 p-4 border border-purple-200 bg-purple-50 rounded">
      ...
      <p className="text-sm text-purple-700 mb-3">
        This food's dominant {dominantElement} element makes it especially
        beneficial for:
      </p>
...
export default function NutritionalDataFetcher() {
  ...
  const [nutritionalData, setNutritionalData] =
    useState<NutritionalProfile | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!data) {
        setError(
          `No nutritional data found for "${searchTerm}". Try a different food name.`
        );
      ...
      setError(
        'Error fetching nutritional data. Please try again or try a different food name.'
      );
      setNutritionalData(null);
      setElementalData(null);
    } finally {
      ...
      {!isLoading && nutritionalData && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-4 border rounded bg-white shadow">
            ...
            <p>
              <strong>Calories:</strong> {nutritionalData.calories.toFixed(1)}
            </p>
            ...
            {nutritionalData.vitamins &&
              Object.keys(nutritionalData.vitamins).length > 0 && (
                <>
                  <h3 className="text-lg font-semibold mt-4 mb-2">Vitamins</h3>

                  {/* Calculate if we have any non-zero values */}
                  {Object.values(nutritionalData.vitamins).some(
                    (v) => v > 0
                  ) ? (
                    <>
                      <ul className="pl-5 list-disc">
                        {Object.entries(nutritionalData.vitamins).map(
                          ([vitamin, value]) => {
                      ...
                      <p className="text-xs text-gray-500 mt-2">
                        ...
                        indicates good source.
                      </p>
                    </>
                  ) : (
                    <div className="bg-amber-50 p-3 rounded text-amber-800 text-sm">
                      <p>
                        <span className="font-medium">Note:</span> Detailed
                        vitamin data is not available for this food in the USDA
                        database.
                      </p>
                      <p className="mt-1 text-xs">
                        Try searching for a similar food or a less processed
            ...
            {nutritionalData.minerals &&
              Object.keys(nutritionalData.minerals).length > 0 && (
                <>
                  <h3 className="text-lg font-semibold mt-4 mb-2">Minerals</h3>

                  {/* Calculate if we have any non-zero values */}
                  {Object.values(nutritionalData.minerals).some(
                    (v) => v > 0
                  ) ? (
                    <>
                      <ul className="pl-5 list-disc">
                    ...
                    <div className="bg-amber-50 p-3 rounded text-amber-800 text-sm">
                      <p>
                        <span className="font-medium">Note:</span> Detailed
                        mineral data is not available for this food in the USDA
                        database.
                      </p>
                      <p className="mt-1 text-xs">
                        Try searching for a similar food or a less processed
            ...
            <div className="p-4 border rounded bg-white shadow">
              ...
              <div className="space-y-3">
                <div className="flex items-center">
                  ...
                  <span className="ml-2">
                    {(elementalData.Fire * 100).toFixed(1)}%
                  </span>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <span className="ml-2">
                    {(elementalData.Water * 100).toFixed(1)}%
                  </span>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <span className="ml-2">
                    {(elementalData.Earth * 100).toFixed(1)}%
                  </span>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <span className="ml-2">
                    {(elementalData.Air * 100).toFixed(1)}%
                  </span>
                </div>
              </div>
              ...
              <div className="mt-6">
                ...
                <p className="text-xl">
                  {
                    Object.entries(elementalData).sort(
                      ([, a], [, b]) => b - a
                    )[0][0]
                  }
                </p>

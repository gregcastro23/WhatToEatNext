          ...
          if (hasElementalProps && ingredient.elementalProperties) {
            try {
              ...
              if (elementFilter && ingredient.elementalProperties) {
                const elementValue = (ingredient.elementalProperties as Record<string, unknown>)[
                  elementFilter
                ];
              ...
              if (ingredient.nutritionalProfile) {
                ...
                const macros =
                  ((nutrition as Record<string, unknown>).macros as Record<string, unknown>) || {};

                // Calculate protein density (protein per calorie)
                const proteinDensity = calories > 0 ? Number(macros.protein || 0) / calories : 0;

                // Calculate fiber density (fiber per calorie)
                const fiberDensity = calories > 0 ? Number(macros.fiber || 0) / calories : 0;

                // Calculate vitamin/mineral richness
                const nutritionData = nutrition as Record<string, unknown>;
                const vitaminCount = Object.keys(
                  (nutritionData.vitamins as Record<string, unknown>) || {},
                ).length;
                const mineralCount = Object.keys(
                  (nutritionData.minerals as Record<string, unknown>) || {},
                ).length;
                ...
                const phytonutrientScore =
                  Object.keys((nutritionData.phytonutrients as Record<string, unknown>) || {})
                    .length / 10; // Normalized to ~0-1 range

                // Calculate macronutrient balance based on ratios
                const totalMacros =
                  Number(macros.protein || 0) + Number(macros.carbs || 0) + Number(macros.fat || 0);
            ...
            {items.map((ingredient, index) => {
              ...
              if (ingredient.nutritionalProfile) {
                ...
                const proteinDensity =
                  calories > 0
                    ? Number((macros as Record<string, unknown>).protein || 0) / calories
                    : 0;

                // Calculate fiber density (fiber per calorie)
                const fiberDensity =
                  calories > 0
                    ? Number((macros as Record<string, unknown>).fiber || 0) / calories
                    : 0;

                // Calculate vitamin/mineral richness
                const vitaminCount = Object.keys(
                  ((nutrition as Record<string, unknown>).vitamins as Record<string, unknown>) ||
                    {},
                ).length;
                const mineralCount = Object.keys(
                  ((nutrition as Record<string, unknown>).minerals as Record<string, unknown>) ||
                    {},
                ).length;
                const micronutrientScore = (vitaminCount + mineralCount) / 20; // Normalized to ~0-1 range
                ...
                const phytonutrientScore =
                  Object.keys(
                    ((nutrition as Record<string, unknown>).phytonutrients as Record<
                      string,
                      unknown
                    >) || {},
                  ).length / 10; // Normalized to ~0-1 range
                ...
                // Calculate macronutrient balance based on ratios
                const totalMacros =
                  Number((macros as Record<string, unknown>).protein || 0) +
                  Number((macros as Record<string, unknown>).carbs || 0) +
                  Number((macros as Record<string, unknown>).fat || 0);
                let macroBalanceScore = 0.5;

                if (totalMacros > 0) {
                  const proteinRatio =
                    Number((macros as Record<string, unknown>).protein || 0) / totalMacros;
                  const carbsRatio =
                    Number((macros as Record<string, unknown>).carbs || 0) / totalMacros;
                  const fatRatio =
                    Number((macros as Record<string, unknown>).fat || 0) / totalMacros;
                ...
                >
                  ...
                  <div className='elemental-properties'>
                    {Object.entries(ingredient.elementalProperties || {}).map(
                      ([element, value]) => (
                        <div key={element} data-testid={`${element.toLowerCase()}-value`}>
                          {element}: {(value * 100).toFixed(0)}%
                        </div>
                      ),
                    )}
                  </div>
                  <div className='harmony-score'>Match: {displayScore}%</div>
                </div>
              );
            })}
          </div>

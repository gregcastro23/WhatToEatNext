...
const CuisineGroup: React.FC<Props> = ({ recipes, elementalState }) => {
  ...
  const renderScoreBadge = (score: number) => {
    ...
    return (
      <span
        className={`text-sm ${getMatchScoreClass(score)} flex items-center rounded px-2 py-1 transition-all duration-300 hover:scale-105`}
        title={tooltipText}
      >
        <span>{score}%</span>
  ...
  const organizedRecipes = useMemo(() => {
    ...
    const recipesByCuisine: Record<string, Array<Recipe & { matchScore: number }>> = Object.values(
      CUISINE_TYPES,
    ).reduce(
      (acc, cuisine) => {
        acc[cuisine] = [];
      ...
      {Object.entries(organizedRecipes).map(([cuisine, cuisineRecipes]) => (
        <div key={cuisine} className='cuisine-section'>
          <div className='mb-4 flex items-center justify-between'>
            ...
            <span className='text-sm text-gray-600'>{cuisineRecipes.length} recipes available</span>
          </div>
          ...
          {cuisineRecipes.length > 0 ? (
            <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3'>
              {cuisineRecipes.slice(0, displayCount).map((recipe, index) => (
                <div
                  ...
                  className='recipe-card rounded-lg border p-4 transition-shadow hover:shadow-lg'
                >
            ...
            <div className='rounded border py-4 text-center text-gray-500'>

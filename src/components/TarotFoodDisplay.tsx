...
import @/utils  from 'tarotMappings ';
import {
  Flame,
  Droplets,
  Mountain,
...
export default function TarotFoodDisplay({
  onTarotLoaded,
}: TarotFoodDisplayProps) {
  ...
  if (error)
    return <div className="text-red-400 mb-4">Tarot unavailable: {error}</div>;
  if (!tarotCards)
    return (
  ...
  return (
    <div className="mb-6 mt-2">
      <h3 className="text-lg font-semibold mb-3">
        Tarot Correspondence for Today
      </h3>
      ...
      <div className="flex items-center mb-3 text-xs text-purple-300">
        ...
        {hasSunPosition(currentPlanetaryAlignment) && (
          <span className="ml-3">
            • Sun: {currentPlanetaryAlignment.sun.sign}{' '}
            {Math.floor(currentPlanetaryAlignment.sun.degree || 0)}°
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div
          className={`rounded-lg p-4 bg-opacity-10 ${getElementColor(element)}`}
        >
          <div className="flex justify-between items-start">
            <div>
              <h4 className="font-bold text-white text-lg drop-shadow-md">
                {tarotCards.minorCard?.name || 'Minor Arcana'}
              </h4>
          ...
          <div className="mt-4 text-sm">
            <div className="italic font-medium text-white bg-black bg-opacity-30 p-2 rounded-md">
              {tarotCards.minorCard?.keywords?.join(', ') ||
                'No keywords available'}
            </div>
            <p className="mt-2 text-sm opacity-90 bg-black bg-opacity-20 p-2 rounded-md text-white">
              This card influences your ingredient selections by enhancing their{' '}
              {element.toLowerCase()} properties.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-lg p-4 bg-purple-900 bg-opacity-30 shadow-lg">
          <div className="flex justify-between items-start">
            <div>
              <h4 className="font-bold text-white text-lg drop-shadow-md">
                {tarotCards.majorCard?.name || 'Major Arcana'}
              </h4>
              <div className="flex items-center mt-1 bg-black bg-opacity-20 rounded px-2 py-1 inline-block">
                <span className="text-yellow-300 mr-1">✧</span>
                <span className="text-sm font-medium text-white">
                  {tarotCards.majorCard?.planet || 'Unknown Planet'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 text-sm">
            <div className="italic font-medium text-white bg-black bg-opacity-30 p-2 rounded-md">
              {tarotCards.majorCard?.keywords?.join(', ') ||
                'No keywords available'}
            </div>
            <p className="mt-2 text-sm opacity-90 bg-black bg-opacity-20 p-2 rounded-md text-white">
              This card heightens the influence of{' '}
              {tarotCards.majorCard?.planet || 'planetary'} energies on today's
              recommended foods.
            </p>
          </div>
        </div>
      ...
      {tarotCards.planetaryCards &&
        Object.keys(tarotCards.planetaryCards).length > 0 && (
          <div className="mt-4">
            <h4 className="text-sm font-semibold mb-2">
              Active Planetary Influences
            </h4>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
              {Object.entries(tarotCards.planetaryCards).map(
                ([planet, card]) => (
                  <div
                    key={planet}
                    className="rounded-lg p-2 bg-gray-800 bg-opacity-40 text-xs"
                  >
                    ...
                    <div className="flex items-center mt-1">
                      ...
                      <span className="ml-1 text-xs text-gray-500">
                        {Math.round((card.energy || 0.5) * 100)}%
                      </span>
                    </div>
                  </div>
                )

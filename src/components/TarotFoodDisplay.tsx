...
export default function TarotFoodDisplay({ onTarotLoaded }: TarotFoodDisplayProps) {
  ...
  const { currentPlanetaryAlignment, isLoading: astroLoading } = useAstrologicalState();
  ...
  const element = SUIT_TO_ELEMENT[suit] || 'Unknown';
  const token = SUIT_TO_TOKEN[suit] || 'Quantum';

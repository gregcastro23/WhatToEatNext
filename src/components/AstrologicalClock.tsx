...
function _isValidPosition(pos: unknown): boolean {
  ...
  return Boolean(
    pos && typeof sign === 'string' && typeof degree === 'number' && degree >= 0 && degree < 30,
  );
}
...
const AstrologicalClock: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className='rounded-lg bg-white p-4 shadow-md'>
        <h2 className='mb-2 text-lg font-semibold'>Astrological Clock</h2>
  ...
  const processedPositions: Record<string, ClockPlanetaryPosition> = React.useMemo(() => {
    ...
    Object.entries(planetaryPositions).forEach(([planet, position]) => {
      if (position && typeof position === 'object') {
        const posData = position as unknown as {
          sign?: string;
          degree?: number;
          exactLongitude?: number;
          isRetrograde?: boolean;
        ...
        const degree =
          typeof posData.degree === 'number'
            ? posData.degree
            : typeof posData.exactLongitude === 'number'
              ? posData.exactLongitude % 30
              : 0;
        ...
        processed[planet] = {
          ...
          degree: degree,
          dignity:
            (dignity as unknown as { type?: string })?.type ||
            (dignity?.strength !== undefined
              ? `${(dignity as unknown as { type?: string })?.type || 'Neutral'}`
              : undefined),
          dignityValue: dignity?.strength || 0,
          dignityDescription:
            (dignity as unknown as { description?: string })?.description ||
            `${dignity?.strength > 0 ? 'Favorable' : dignity?.strength < 0 ? 'Challenging' : 'Neutral'} placement`,
  ...
  return (
    <div className='rounded-lg bg-white p-4 shadow-md'>
      ...
      {showLegend && (
        <div className='mb-4 rounded-md bg-gray-50 p-2 text-sm'>
          <h3 className='mb-1 font-medium'>Symbol Legend</h3>
      ...
      <div className='mt-3 text-sm'>
        <p>
          Ascendant: {ZODIAC_SYMBOLS[chartData.ascendant || 'Libra']}{' '}
          {chartData.ascendant || 'Not available'}
        </p>
        <p className='mt-1 text-xs text-gray-500'>Showing current planetary positions</p>
      ...
      {Object.keys(processedPositions).length > 0 && (
        <div className='mt-4 rounded-lg border bg-gradient-to-r from-indigo-50 to-purple-50 p-3'>
          <h3 className='mb-2 text-sm font-semibold text-indigo-700'>
            🌟 Live Planetary Positions
          </h3>
          <div className='grid grid-cols-1 gap-2 text-xs sm:grid-cols-2'>
            {Object.entries(processedPositions)
              .slice(0, 6)
              .map(([planet, pos]) => (
                <div
                  key={planet}
                ...
                >
                  <div className='flex items-center space-x-2'>
                    ...
                    <div>
                      ...
                      {pos.isRetrograde && <span className='ml-1 text-xs text-red-500'>℞</span>}
                    </div>
                  </div>
                  <div className='text-right'>
                    ...
                    {pos.dignity && (
                      ...
                      >
                        {pos.dignity} ({(pos.dignityValue || 0) > 0 ? '+' : ''}
                        {pos.dignityValue?.toFixed(1)})
                      </div>
                    )}
                  </div>
                </div>
          ...
          <p className='mt-2 text-xs text-gray-500'>
            ℞ = Retrograde • Dignity values: Exaltation/Domicile (+), Detriment/Fall (-), Neutral
            (0)
          </p>
        </div>
      )}
            ...
            <table className='w-full text-xs'>
              ...
              <tbody>
                {Object.entries(chartData.planets).map(([planet, data]) => {
                  // Apply surgical type casting with variable extraction
                  const planetData = data as unknown as {
                    sign?: ZodiacSign;
                    degree?: number;
                    isRetrograde?: boolean;
                  };
                  ...
                  const dignity = isNode
                    ? { type: 'N/A', strength: 0 }
                    : getPlanetaryDignity(planet, sign as ZodiacSign | undefined);
                  ...
                  return (
                    <tr
                      key={planet}
                      className={`border-t border-gray-100 ${isNode ? 'bg-gray-50' : ''}`}
                    >
                      ...
                      <td className={`px-2 py-1 ${dignityClass}`}>{dignity.type}</td>
                    </tr>
                  );
                })}
              </tbody>

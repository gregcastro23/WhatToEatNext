...
export function PlanetInfoModal({ planetName, isOpen, onClose }: PlanetInfoModalProps) {
  ...
  const processPlanetInfo = useCallback(() => {
    if (isOpen && planetName && Object.keys(planetaryPositions).length > 0) {
      ...
      try {
        log.info(`Processing planet info for ${planetName}:`, {
          planetData: planetaryPositions[planetName.toLowerCase()],
        });
        ...
        setError(
          `Error loading planet information: ${err instanceof Error ? err.message : String(err)}`,
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50'>
      <div className='max-h-[90vh] w-full max-w-lg overflow-auto rounded-lg bg-white shadow-xl dark:bg-gray-800'>
        <div className='sticky top-0 flex items-center justify-between border-b border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800'>
          <h2 className='text-xl font-bold'>
            {loading
              ...
              : error
                ? 'Error'
                : `${planetInfo?.name} in ${planetInfo?.sign}`}
          </h2>
          <button
            onClick={onClose}
          ...
          {loading ? (
            ...
            <div className='space-y-6'>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <div>
                  <h3 className='text-lg font-semibold'>{planetInfo.name}</h3>
                  <p className='capitalize'>
                    {planetInfo.sign} {planetInfo.degree.toFixed(1)}°
                  </p>
                ...
                <div>
                  <div className='text-right'>
                    <span className='inline-block rounded-full bg-purple-100 px-2 py-1 text-xs font-medium capitalize text-purple-800 dark:bg-purple-900 dark:text-purple-200'>
              ...
              <div className='border-t border-gray-200 pt-4 dark:border-gray-700'>
                <h3 className='mb-2 font-semibold'>Major Arcana</h3>
                <div className='flex items-center space-x-3'>
                  <div className='flex h-20 w-14 items-center justify-center rounded bg-indigo-100 text-2xl dark:bg-indigo-900'>
                    {getTarotSymbol(planetInfo.tarotCard.name)}
                  </div>
                  <div>
                    <p className='font-medium'>{planetInfo.tarotCard.name}</p>
                    <p className='text-sm text-gray-600 dark:text-gray-300'>
                      Element: {planetInfo.tarotCard.element}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {planetInfo.aspects.length > 0 && (
                <div className='border-t border-gray-200 pt-4 dark:border-gray-700'>
                  <h3 className='mb-2 font-semibold'>Aspects</h3>
                  <ul className='space-y-2'>
                    {planetInfo.aspects.map((aspect, idx) => (
                      <li key={idx} className='flex items-start'>
                        <span
                          className={`mr-2 mt-1.5 inline-block h-2 w-2 rounded-full ${getAspectColor(aspect.type)}`}
                        ></span>
                        <div>
                          <p className='font-medium capitalize'>
                            {aspect.type} to{' '}
                            {aspect.planet.charAt(0).toUpperCase() + aspect.planet.slice(1)}
              ...
              {/* Elemental Influence */}
              <div className='border-t border-gray-200 pt-4 dark:border-gray-700'>
                <h3 className='mb-2 font-semibold'>Elemental Influence</h3>
                <div className='grid grid-cols-4 gap-2'>
                  <div className='text-center'>
                    ...
                    <p className='mt-1 text-xs'>Fire</p>
                  </div>
                  <div className='text-center'>
                    <div className='h-4 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700'>
                      <div
                  ...
                  <div className='text-center'>
                    ...
                    <p className='mt-1 text-xs'>Air</p>
                  </div>
                  <div className='text-center'>
                    <div className='h-4 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700'>
                      <div
                    ...
                    <p className='mt-1 text-xs'>Earth</p>
                  </div>
                </div>
              </div>
              ...
              {/* Token Influence */}
              <div className='border-t border-gray-200 pt-4 dark:border-gray-700'>
                <h3 className='mb-2 font-semibold'>Token Influence</h3>
                <div className='grid grid-cols-4 gap-2'>
                  <div className='text-center'>
                    <div className='h-4 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700'>
                      <div
                    ...
                    <p className='mt-1 text-xs'>Spirit</p>
                  </div>
                  <div className='text-center'>
                    <div className='h-4 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700'>
                      <div
                    ...
                    <p className='mt-1 text-xs'>Essence</p>
                  </div>
                  <div className='text-center'>
                    <div className='h-4 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700'>
                      <div
                    ...
                    <p className='mt-1 text-xs'>Matter</p>
                  </div>
                  <div className='text-center'>
                    <div className='h-4 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700'>
                      <div
                    ...
                    <p className='mt-1 text-xs'>Substance</p>
                  </div>
                </div>
              </div>
            </div>

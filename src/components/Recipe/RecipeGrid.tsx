// src / (components || 1)/recipe / (RecipeGrid || 1).tsx

import { useState, useMemo } from 'react';
import RecipeCard from './RecipeCard';
import @/data  from 'seasons ';
import {
  SlidersHorizontal,
  Flame,
  Droplets,
...
export const RecipeGrid: React.FC<RecipeGridProps> = ({
  ...
  const [elementalFilter, setElementalFilter] =
  ...
  let getElementalScore = (
    ...
    let standardizedRecipe = recipeElementalService.standardizeRecipe(recipe);
    return element === 'all'
      ? 1
    ...
    return standardizedRecipes
      ...
      .sort((a, b) => {
        switch (sortBy) {
          ...
          case 'traditional':
            return (
              ((b as any).traditional || 0) - ((a as any).traditional || 0)
            );
          case 'elemental':
            return (
              getElementalScore(b, elementalFilter) -
              getElementalScore(a, elementalFilter)
            );
          case 'seasonal':
      ...
      <div className="mb-6 flex flex-wrap gap-4 justify-between items-center">
        <div className="flex gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white rounded-lg shadow hover:bg-gray-50"
            >
              {elementalFilter === 'Fire' && (
                <Flame className="h-4 w-4 text-red-500" />
              )}
              {elementalFilter === 'Water' && (
                <Droplets className="h-4 w-4 text-blue-500" />
              )}
              {elementalFilter === 'Air' && (
                <Wind className="h-4 w-4 text-sky-500" />
              )}
              {elementalFilter === 'Earth' && (
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`p-2 rounded ${
              viewMode === 'compact' ? 'bg-blue-100' : 'bg-gray-100'
            }`}
            title="Compact view"
          >
            ...
            >
              <RecipeCard
                ...
                elementalHighlight={
                  elementalFilter === 'all'
                    ? 'none'
                    : (elementalFilter as RecipeCardElementalFilter)
                }

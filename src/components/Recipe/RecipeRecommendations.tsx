...
import @/utils  from 'recipeMatching ';
import type {
  ElementalProperties,
...
import @/services  from 'SpoonacularService ';
import @/lib  from 'alchemicalEngine ';
...
} from '@/services / (AlchemicalTransformationService || 1)';
import @/utils  from 'lunarPhaseUtils ';
...
interface Dish {
  ...
  ingredients?: Array<
    string | { name: string; amount: number; unit: string; category?: string }
  >;
  instructions?: string[];
...
interface CuisineData {
  ...
  dishes: {
    [key: string]: {
      // mealType like breakfast, lunch, dinner
...
const RecipeRecommendations: React.FC<RecipeRecommendationsProps> = ({
  ...
  const {
    ...
    currentZodiac:
      (state.astrologicalState?.zodiacSign as ZodiacSign) || undefined,
    lunarPhase: state.lunarPhase
      ? (normalizeLunarPhase(state.lunarPhase) as LunarPhaseWithSpaces) ||
        undefined
      : undefined,
    // Optional: pass tarot effects if available in your state
    tarotElementBoosts:
      state.astrologicalState?.tarotElementBoosts || undefined,
    tarotPlanetaryBoosts:
      state.astrologicalState?.tarotPlanetaryBoosts || undefined,
  ...
  useEffect(() => {
    ...
    alchemicalService.setCurrentZodiac(
      state.astrologicalState?.zodiacSign as ZodiacSign
    );
    alchemicalService.setLunarPhase(
      state.lunarPhase
        ? (normalizeLunarPhase(state.lunarPhase) as LunarPhaseWithSpaces)
        : null
    );
    alchemicalService.setTarotElementBoosts(
      state.astrologicalState?.tarotElementBoosts || {}
    );
    alchemicalService.setTarotPlanetaryBoosts(
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {optimizedRecipes.map(
            ({ recipe, compatibility, dominantElement }) => (
              <RecipeCard

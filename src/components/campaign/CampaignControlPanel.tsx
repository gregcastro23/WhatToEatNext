...
export const CampaignControlPanel: React.FC<CampaignControlPanelProps> = ({
  ...
  const [safetyLevel, setSafetyLevel] = useState<'conservative' | 'standard' | 'aggressive'>(
    'standard',
  );
  ...
  return (
    <div className='quick-action-card'>
      ...
      <div className='action-footer'>
        ...
        <button className='execute-btn' onClick={onExecute} disabled={!action.enabled}>
          Execute
        </button>
      </div>
    </div>
  ...
  return (
    <div className='campaign-status-card'>
      <div className='campaign-header'>
        ...
        <div className='status-badge' style={{ backgroundColor: getStatusColor(campaign.status) }}>
      ...
      {campaign.currentPhase && <p className='current-phase'>Phase: {campaign.currentPhase}</p>}
      ...
      <div className='campaign-controls'>
        {campaign.status === 'running' && (
          <>
            <button onClick={() => onControl(campaign.campaignId, 'pause')}>Pause</button>
            <button onClick={() => onControl(campaign.campaignId, 'stop')}>Stop</button>
          </>
        )}
        {campaign.status === 'paused' && (
          <>
            <button onClick={() => onControl(campaign.campaignId, 'resume')}>Resume</button>
            <button onClick={() => onControl(campaign.campaignId, 'stop')}>Stop</button>
          </>
        )}
      </div>

      <div className='last-update'>Updated: {campaign.lastUpdate.toLocaleTimeString()}</div>
    </div>
  );
};
...
const CampaignResultCard: React.FC<{
  ...
  return (
    <div className='campaign-result-card'>
      ...
      <div className='result-timestamp'>{result.completedAt.toLocaleString()}</div>
    </div>
  );
};

...
function planetaryPositionsToAlchemicalValues(positions: { [key: string]: any }) {
  ...
  const planetaryAlchemy: {
    [key: string]: { Spirit: number; Essence: number; Matter: number; Substance: number };
  } = {
...
export default function RealtimeAlchemicalCalculator() {
  const { positions, loading, error, lastUpdated, source, refresh, isRealtime, isConnected } =
  ...
  return (
    <div className='space-y-6'>
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-sm opacity-75'>
              {source?.includes('astrologize')
                ? '✨ Using Real Astrologize Calculations'
          ...
          <div className='text-right'>
            ...
            <p className='text-sm opacity-75'>{isRealtime ? 'Real-time data' : 'Cached data'}</p>
            {lastUpdated && (
              <p className='text-xs opacity-60'>Updated: {lastUpdated.toLocaleTimeString()}</p>
            )}
            <button
              onClick={refresh}
              className='mt-1 rounded bg-white bg-opacity-20 px-2 py-1 text-xs hover:bg-opacity-30'
            >
              Refresh
            </button>
          </div>
      ...
      <div className='rounded-lg bg-white p-6 shadow-md'>
        <h3 className='mb-4 text-lg font-semibold'>Current Planetary Positions</h3>
        <div className='grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5'>
          {Object.entries(positions || {}).map(([planet, position]) => (
            <div key={planet} className='rounded bg-gray-50 p-3 text-center'>
              <div className='font-semibold text-gray-800'>{planet}</div>
              <div className='text-sm capitalize text-gray-600'>{position.sign}</div>
              <div className='text-xs text-gray-500'>
                {position.degree}°{position.minute || 0}'
              </div>
              {position.isRetrograde && <div className='text-xs text-red-500'>Retrograde</div>}
            </div>
          ))}
        </div>
        <div className='mt-4 text-sm text-gray-500'>Data source: {source || 'Unknown'}</div>
      </div>
      ...
      <div className='grid gap-6 md:grid-cols-2'>
        {/* Elemental Breakdown */}
        <div className='rounded-lg bg-white p-6 shadow-md'>
          <h3 className='mb-4 text-lg font-semibold'>Alchemical Components</h3>
        ...
        <div className='rounded-lg bg-white p-6 shadow-md'>
          <h3 className='mb-4 text-lg font-semibold'>Thermodynamic Metrics</h3>
          <div className='space-y-3'>
            <div className='flex justify-between'>
          ...
          <div className='mt-6 rounded bg-gray-50 p-4'>
            <h4 className='mb-2 font-semibold'>Advanced Constants</h4>
      ...
      <div className='rounded-lg bg-gradient-to-r from-indigo-50 to-purple-50 p-6'>
        <h3 className='mb-4 text-lg font-semibold'>Alchemical Interpretation</h3>
        <div className='space-y-2 text-gray-700'>
          <p>
            <strong>Dominant Alchemical Quality:</strong>{' '}
            {Math.max(Spirit, Essence, Matter, Substance) === Spirit
              ? 'Spirit (Active Principle)'
              : Math.max(Spirit, Essence, Matter, Substance) === Essence
                ? 'Essence (Vital Force)'
                : Math.max(Spirit, Essence, Matter, Substance) === Matter
                  ? 'Matter (Physical Form)'
                  : 'Substance (Structural Foundation)'}
          </p>
          <p>
            <strong>Elemental Emphasis:</strong>{' '}
            {Math.max(Fire, Water, Air, Earth) === Fire
              ? 'Fire (Creative Energy)'
              : Math.max(Fire, Water, Air, Earth) === Water
                ? 'Water (Emotional Flow)'
                : Math.max(Fire, Water, Air, Earth) === Air
                  ? 'Air (Mental Activity)'
          ...
          <p>
            ...
            {gregsEnergy > 0
              ? gregsEnergy > 0.1
                ? 'High Energy - Dynamic Phase'
                : 'Moderate Energy - Balanced Phase'

...
export default function PlanetaryFoodRecommendations({
  ...
  const currentPlanetaryRecommendations = useMemo(() => {
    ...
    const mapping = PLANETARY_FOOD_MAPPINGS.find(
      (m) => m.planet === planetName,
    );
    ...
    return (
      <div className={`bg-white rounded-lg shadow-lg p-6 ${className}`}>
        ...
        <div className="text-gray-500">
          Loading celestial recommendations...
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg p-6 ${className}`}>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {currentPlanetaryRecommendations.ingredients
            .slice(0, 8)
            .map((ingredient) => (
              <span
                key={ingredient}

...
export default function CelestialDisplay() {
  ...
  let drawZodiacWheel = (
    ...
    for (let i = 0; i < 12; i++) {
      ...
      let y1 = centerY + (radius - 10) * Math.sin(startAngle);
      let x2 = centerX + radius * Math.cos(startAngle);
      let y2 = centerY + radius * Math.sin(startAngle);
  ...
  let drawLunarNode = (
    ...
    ctx.textBaseline = 'middle';
    ctx.fillText(
      nodeType === 'northNode' ? 'N.Node' : 'S.Node',
      labelX,
      labelY
    );
  };
          ...
          <div className="flex flex-col text-xs text-gray-400">
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-0.5">
              <div className="flex items-center gap-1">
                <div
                  className={`w-2 h-2 rounded-full ${ELEMENT_COLORS['Fire']}`}
                ></div>
                <span>Fire</span>
              </div>
              <div className="flex items-center gap-1">
                <div
                  className={`w-2 h-2 rounded-full ${ELEMENT_COLORS['Earth']}`}
                ></div>
                <span>Earth</span>
              </div>
              <div className="flex items-center gap-1">
                <div
                  className={`w-2 h-2 rounded-full ${ELEMENT_COLORS['Air']}`}
                ></div>
                <span>Air</span>
              </div>
              <div className="flex items-center gap-1">
                <div
                  className={`w-2 h-2 rounded-full ${ELEMENT_COLORS['Water']}`}
                ></div>
                <span>Water</span>
              </div>
            </div>

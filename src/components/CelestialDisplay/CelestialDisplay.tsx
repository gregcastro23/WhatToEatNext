...
import { Droplets, Flame, Mountain, Star, Wind } from 'lucide-react';
...
export default function CelestialDisplay() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      _drawZodiacWheel(ctx, centerX, centerY, radius);
      ...
      const planets = Object.entries(alignment)
        ...
        .sort((a, b) => {
          ...
          return (
            (order[a[0] as keyof typeof order] || 99) - (order[b[0] as keyof typeof order] || 99)
          );
        });
  ...
  const _drawCelestialBody = (
    ...
    ctx.textBaseline = 'middle';
    ctx.fillText(
      PLANET_SYMBOLS[planet as keyof typeof PLANET_SYMBOLS] || planet.charAt(0).toUpperCase(),
      x,
      y,
    );
  ...
  return (
    <div className='relative overflow-hidden rounded-lg bg-gray-900 bg-opacity-75 p-4 shadow-md'>
      ...
      <div className='relative aspect-square'>
        ...
        ></canvas>
        <div className='absolute bottom-1 left-1 right-1 flex justify-between rounded bg-gray-900 bg-opacity-90 p-2'>
          <div className='flex flex-col space-y-1 text-xs'>
            <div className='flex items-center text-gray-200'>
              <span className='mr-1 font-medium'>Current Sign:</span>
              <span className='capitalize text-yellow-400'>{state.currentZodiac}</span>
            </div>
            <div className='flex flex-wrap gap-x-3'>
              <div className='flex items-center text-sm text-gray-400'>
                <span className='mr-1 text-yellow-500'>{PLANET_SYMBOLS.sun}</span>
                <span className='capitalize'>
                  {(planetaryAlignment as PlanetaryAlignment).Sun?.sign}
                </span>
              </div>
              <div className='flex items-center text-sm text-gray-400'>
                <span className='mr-1 text-gray-300'>{PLANET_SYMBOLS.moon}</span>
                <span className='capitalize'>
                  {(planetaryAlignment as PlanetaryAlignment).Moon?.sign}
                </span>
              </div>
              <div className='flex items-center text-sm text-gray-400'>
                <span className='mr-1 text-orange-500'>{PLANET_SYMBOLS.northNode}</span>
                <span className='capitalize'>
          ...
          <div className='flex flex-col text-xs text-gray-400'>
            <span className='mb-1 text-xs text-gray-300'>Elements:</span>
            <div className='grid grid-cols-2 gap-x-3 gap-y-0.5'>
              <div className='flex items-center gap-1'>
                <div className={`h-2 w-2 rounded-full ${ELEMENT_COLORS['Fire']}`}></div>
                <span>Fire</span>
              </div>
              <div className='flex items-center gap-1'>
                <div className={`h-2 w-2 rounded-full ${ELEMENT_COLORS['Earth']}`}></div>
                <span>Earth</span>
              </div>
              <div className='flex items-center gap-1'>
                <div className={`h-2 w-2 rounded-full ${ELEMENT_COLORS['Air']}`}></div>
                <span>Air</span>
              </div>
              <div className='flex items-center gap-1'>
                <div className={`h-2 w-2 rounded-full ${ELEMENT_COLORS['Water']}`}></div>
                <span>Water</span>
              </div>
            </div>
          </div>

...
import {
  safeImportAndExecute,
  safeImportFunction,
} from "@/utils/(dynamicImport || 1)";
...
let getLunarPhaseDescription = (phase: string): string => {
  const descriptions: Record<string, string> = {
    ...
    waxing_crescent:
      'Building momentum, gathering resources, and taking initial steps.',
...
const MoonDisplay: React.FC = () => {
  ...
  let northNode = useMemo(() => {
    ...
    let node = planetaryPositions?.northnode || planetaryPositions?.northNode;
  ...
  useEffect(() => {
    ...
    let interval = setInterval(() => calculateTimes(), 30 * 60 * 1000);
      ...
      try {
        ...
        if (calculatePhase && getPhaseName && getIllumination) {
          ...
          if (phaseName === 'waning_crescent' && illuminationPct < 1) {
            correctedIllumination = Math.max(
              1,
              Math.min(25, illuminationPct || 12)
            );
          }
        ...
        throw new Error(
          'Failed to calculate lunar phase. Please check the implementation.'
        );
      }
    };
  ...
  useEffect(() => {
    ...
    if (planetaryPositions.moon && planetaryPositions.moon.sign) {
      ...
      debugLog(
        'Moon position available from planetary alignment:',
        planetaryPositions.moon
      );
    }
    ...
    <div className="bg-gray-900 bg-opacity-90 rounded-lg p-4 shadow-lg border border-indigo-800">
      ...
      <div className="flex mb-2">
        ...
        <div>
          <p className="font-medium capitalize">
            {moonPhase.phase.replace(/_ / (g || 1), ' ')}
          </p>
          <p className="text-sm text-gray-300">
            {moon && moon.sign
              ? `Moon in ${capitalizeFirstLetter(moon.sign)} ${formatDegree(
                  moon.degree
                )}`
              : 'Loading...'}
      ...
      {expanded && (
        <div className="mt-4 border-t border-gray-700 pt-4">
          <p className="text-sm text-gray-300 mb-3">{moonPhase.description}</p>
              ...
              <div className="bg-gray-800 rounded p-3">
                ...
                <div className="font-medium">
                  {northNode && northNode.sign
                    ? `${capitalizeFirstLetter(northNode.sign)} ${formatDegree(
                        northNode.degree
                      )}`
                    : 'Calculating...'}
              ...
              <div className="bg-gray-800 rounded p-3">
                ...
                <div className="font-medium">
                  {southNode && southNode.sign
                    ? `${capitalizeFirstLetter(southNode.sign)} ${formatDegree(
                        southNode.degree
                      )}`
                    : 'Calculating...'}

...
async function getRecommendations(
  ...
  const astroStateData = {
    ...
    activePlanets: planetaryPositions
      ? Object.keys(planetaryPositions as unknown as ElementalProperties)
      : [
          'Sun',
          'Moon',
          'Mercury',
  ...
  return await getIngredientRecommendations(
    astroStateData as unknown as ElementalProperties & {
      timestamp: Date;
...
export default function IngredientRecommendations({
  ...
  const getElementIcon = (element: string) => {
    switch (element) {
      case 'Fire':
        return <Flame className={`${styles.tagIcon} ${styles.fire}`} size={16} />;
      case 'Water':
        return <Droplets className={`${styles.tagIcon} ${styles.water}`} size={16} />;
      case 'Earth':
        return <Mountain className={`${styles.tagIcon} ${styles.earth}`} size={16} />;
      case 'Air':
  ...
  const _renderIngredientDetails = (ingredient: Ingredient) => {
    ...
    const matchPercentage = !isNaN(score) ? `${Math.round(score * 100)}%` : '50%';
        ...
        <div className='flavor-profile'>
          ...
          {ingredientData.sensoryProfile && (
            <div className={styles.sensoryHighlights}>
              {Object.entries(
                ((ingredientData.sensoryProfile as unknown as ElementalProperties)
                  .taste as unknown as ElementalProperties) || {},
              )
  ...
  const renderCompactIngredientCard = (ingredient: IngredientRecommendation) => {
    ...
    const dominantElement = Object.entries(elementalProps).sort((a, b) => b[1] - a[1])[0][0];
    ...
    return (
      ...
      >
        <div className={styles.compactCardHeader}>
          ...
          {matchPercentage && <span className={styles.compactScore}>{matchPercentage}</span>}
        </div>
        ...
        <div className={styles.compactFooter}>
          ...
          {ingredient.modality && (
            <span
              className={`${styles.modalityBadge} ${styles[ingredient.modality.toLowerCase()]}`}
            >
              {ingredient.modality}
            </span>
          )}
  ...
  return (
    <div className={styles.container}>
      ...
      {Object.entries(recommendations).length > 0 ? (
        ...
        <div className={styles.noResults}>No ingredients found matching your filters.</div>
      )}
    </div>
  );
}

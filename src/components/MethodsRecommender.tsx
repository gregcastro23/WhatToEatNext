    ...
    if (!loading && currentPlanetaryAlignment) {
      ...
      const methodsWithScores = Object.entries(allCookingMethods)
        .map(([methodName, methodData]) => {
          ...
          const safeMethodData =
            methodData && typeof methodData === 'object'
              ? {
                  element: (typeof methodDataObj.element === 'string'
                    ? methodDataObj.element
                    : 'Fire') as Element,
                  intensity:
                    typeof methodDataObj.intensity === 'number' ? methodDataObj.intensity : 0.5,
                  effect: (typeof methodDataObj.effect === 'string'
                    ? methodDataObj.effect
                    : 'enhance') as 'enhance' | 'transmute' | 'diminish' | 'balance',
                  applicableTo: Array.isArray(methodDataObj.applicableTo)
                    ? methodDataObj.applicableTo
                    : ['all'],
                  ...methodDataObj,
                }
              : {
          ...
          return {
            ...
            description:
              typeof methodInfoData.description === 'string'
                ? methodInfoData.description
                : 'A cooking method that transforms food with heat, moisture, or chemical processes.',
            score: adjustedScore,
            elementalEffect: methodInfoData.elementalEffect || methodInfoData.elementalProperties,
            duration: methodInfoData.duration,
            suitable_for: Array.isArray(methodInfoData.suitable_for)
              ? methodInfoData.suitable_for
              : [],
    ...
    return (
      <div className={styles['cooking-methods-container']}>
        <div className={styles['cooking-methods-header']}>
          ...
          <div className={styles['top-recommendation']}>Analyzing celestial energies...</div>
        </div>
      </div>
    );
  }
            ...
            >
              <div className={styles['method-header']}>
                ...
                <div className={styles['method-score']}>
                  <div
                    ...
                    style={{
                      width: `${Math.round((methodScoresRef.current[method.id] || method.score) * 100)}%`,
                    }}
                  />
                  <span>
                    {Math.round((methodScoresRef.current[method.id] || method.score) * 100)}%
                  </span>
                </div>
              </div>
              ...
              {method.elementalEffect && (
                <div className={styles['elemental-balance']}>
                  {(method.elementalEffect.Fire || 0) > 0.2 && (
                    <div
                      className={`${styles.element} ${styles.fire}`}
                      title={`Fire: ${Math.round((method.elementalEffect.Fire || 0) * 100)}%`}
                    >
                      <Flame size={14} />
                  ...
                  {(method.elementalEffect.Water || 0) > 0.2 && (
                    <div
                      className={`${styles.element} ${styles.water}`}
                      title={`Water: ${Math.round((method.elementalEffect.Water || 0) * 100)}%`}
                    >
                      <Droplets size={14} />
                  ...
                  {(method.elementalEffect.Air || 0) > 0.2 && (
                    <div
                      className={`${styles.element} ${styles.air}`}
                      title={`Air: ${Math.round((method.elementalEffect.Air || 0) * 100)}%`}
                    >
                      <Wind size={14} />
                  ...
                  {(method.elementalEffect.Earth || 0) > 0.2 && (
                    <div
                      className={`${styles.element} ${styles.earth}`}
                      title={`Earth: ${Math.round((method.elementalEffect.Earth || 0) * 100)}%`}
                    >
                      <Mountain size={14} />
              ...
              {selectedMethodId === method.id && (
                <div className={styles['expanded-details']}>
                  ...
                  <div className='mb-3 rounded-lg border-l-2 border-purple-500 bg-gray-50 p-3 shadow-sm'>
                    <h4 className='mb-2 flex items-center text-sm font-semibold text-gray-700'>
                      <Info size={14} className='mr-1 text-purple-500' />
                    ...
                    <div className='grid grid-cols-1 gap-2 md:grid-cols-2'>
                      {technicalTips.map((tip, index) => (
                        <div
                          key={index}
                          className='rounded border-l-2 border-purple-400 bg-white p-2 text-xs text-gray-600'
                        >
                          {tip}
                        </div>
                      ))}
                  ...
                  <div className='rounded-lg border-l-2 border-blue-500 bg-blue-50 p-3 shadow-sm'>
                    ...
                    <div className='grid grid-cols-1 gap-2 md:grid-cols-2'>
                      {idealIngredients.map((ingredient, index) => (
                        <div
                          key={index}
                          className='rounded border-l-2 border-blue-400 bg-white p-2 text-xs text-gray-600'
                        >
                          {ingredient}
                        </div>
                      ))}

...
function CuisineSelector({
    ...
    if (sortBy === 'alchemical' && Object.keys(planetaryPositions).length > 0) {
      ...
      sorted.sort((a, b) => {
        ...
        if ('gregsEnergy' in a && 'gregsEnergy' in b) {
          return (
            ((b as CuisineData).gregsEnergy as number) - ((a as CuisineData).gregsEnergy as number)
          );
        }
  ...
  const getCuisineModality = (cuisine: CuisineData): Modality => {
    ...
    return determineModalityFromElements(
      (cuisine.elementalState as any) ||
        (cuisine.elementalProperties as any) || {
          Fire: 0.25,
          Water: 0.25,
          Earth: 0.25,
          Air: 0.25,
      ...
      if (zodiacFilter !== 'all') {
        ...
        if (zodiacFilter !== 'all' && !zodiacInfluences.includes(zodiacFilter)) {
          ...
          if ('planetaryDignities' in cuisine) {
            const hasPlanetaryMatch = Object.values(
              (cuisine as CuisineData).planetaryDignities || {},
            ).some(dignity =>
              (dignity as PlanetaryDignityDetails).favorableZodiacSigns?.includes(zodiacFilter),
            );
      ...
      {currentZodiac && (
        <div className='current-influences mb-4 rounded-lg bg-blue-50 p-3'>
          <p>
            ...
            {currentLunarPhase && <span> - {currentLunarPhase.split(/(?=[A-Z])/).join(' ')}</span>}
          </p>
        </div>
      )}
        ...
        {filteredCuisines.map(cuisine => {
          ...
          const isZodiacFavorable =
            ...
            ((cuisine as CuisineData).zodiacInfluences?.includes(currentZodiac) ||
              Object.values((cuisine as CuisineData).planetaryDignities || {}).some(dignity =>
                (dignity as PlanetaryDignityDetails).favorableZodiacSigns?.includes(currentZodiac),
              ));
          ...
          return (
            ...
            >
              ...
              <div className='cuisine-modality mt-2 flex items-center justify-between'>
                ...
                {'gregsEnergy' in cuisine && (
                  <span className='alchemical-score text-sm'>
                    Compatibility:{' '}
                    {Math.round(((cuisine as CuisineData).gregsEnergy as number) * 100)}%
                  </span>
                )}
              </div>
              ...
              {(cuisine as CuisineData).zodiacInfluences &&
                ((cuisine as CuisineData).zodiacInfluences?.length ?? 0) > 0 && (
                  <div className='zodiac-influences mt-2 text-xs'>
                    <span>Zodiac influences: </span>
                    {((cuisine as CuisineData).zodiacInfluences as string[]).map(

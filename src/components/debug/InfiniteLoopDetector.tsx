    ...
    if (timeElapsed > 100) {
      if (timeElapsed < timeWindow && renderCount.current > threshold) {
        if (!loopDetectedRef.current) {
          ...
          if ((window as Record<string, Record<string, string>>).__REACT_DEVTOOLS_GLOBAL_HOOK__) {
            (window as Record<string, Record<string, string>>).__REACT_DEVTOOLS_GLOBAL_HOOK__.__INFINITE_LOOP_DETECTED__ = {
              componentStack: componentStackRef.current,
              rendersPerSecond: (renderCount.current / timeElapsed) * 1000,

...
import { Recipe } from "@/types/(recipe || 1)";
import {
  ChakraEnergies,
  ElementalProperties,
  Planet,
...
import { KeyCardChakraMapping } from "@/types/(chakra || 1)";
 ...
 */
function getElementalChakraMap(): Record<
  keyof ElementalProperties,
  Array<keyof ChakraEnergies>
> {
  return {
      ...
      .map((recipe) => {
        ...
        if (recipe.astrologicalAffinities?.planets) {
          if (recipe.astrologicalAffinities.planets.includes(planetaryHour)) {
            ...
            const hourChakras =
              this.chakraService.getChakrasByPlanet(planetaryHour);
            ...
            const overlap = hourChakras.filter((chakra) =>
              recipeChakras.includes(chakra)
            ).length;

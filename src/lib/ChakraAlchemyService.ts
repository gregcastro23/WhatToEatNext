...
export class ChakraAlchemyService {
  ...
  public getCardsForChakra(chakra: ChakraPosition): KeyCardChakraMapping[] {
    return _KEY_CARD_CHAKRA_MAPPINGS.filter(
      (card) => card.chakraPosition === chakra,
    );
  }
   ...
   */
  public energyStatesToChakraEnergies(
    ...
    Object.entries(_CHAKRAS).forEach(([position, chakra]) => {
      ...
      if (key) {
        const { primaryEnergyState } = chakra as {
          primaryEnergyState?: keyof EnergyStateProperties;
        };
  ...
  private addZodiacInfluence(
    ...
    Object.entries(this.chakraEnergyStateMappings).forEach(
      ([chakraPos, mapping]) => {
        ...
        const hasMatchingElement = elements.some((elem) =>
          chakraElements.includes(elem),
        );
        ...
        if (
          (chakraPos === "root" || chakraPos === "throat") &&

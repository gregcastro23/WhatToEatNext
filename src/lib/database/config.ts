...
export function validateDatabaseConfig(): { valid: boolean; errors: string[] } {
  ...
  if (!databaseConfig.databaseUrl) {
    if (!databaseConfig.host) errors.push('DB_HOST is required when DATABASE_URL is not provided');
    if (!databaseConfig.database) errors.push('DB_NAME is required when DATABASE_URL is not provided');
    if (!databaseConfig.user) errors.push('DB_USER is required when DATABASE_URL is not provided');
  ...
  if (databaseConfig.maxConnections < 1 || databaseConfig.maxConnections > 100) {

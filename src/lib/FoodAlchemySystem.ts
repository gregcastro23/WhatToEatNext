...
export class FoodAlchemySystem {
  ...
  private generateRecommendations(
    ...
    if (dayElements && hourElements) {
      ...
      if (hourElement === 'Fire') {
        recommendations.push(
          `${food.food} is best utilized in the current ${
            isDaytime ? 'day' : 'night'
        ...
        recommendations.push(
          `During these ${
            isDaytime ? 'day' : 'night'
          } hours, focus on bringing out ${food.food}'s aromatic qualities.`
        );
      } else if (hourElement === 'Air') {
        recommendations.push(
          `The current ${
            isDaytime ? 'day' : 'night'
          } hours favor highlighting ${food.food}'s delicate flavors.`
        );
      ...
      if (state.planetaryPositions) {
        ...
        if (
          ...
          if (dayDignity && dayDignity > 0 && food.planet === planetaryDay) {
            recommendations.push(
              ...
              } in ${daySign}, strengthening ${food.food}'s properties.`
            );
          } else if (
            dayDignity &&
            dayDignity < 0 &&
            ...
            recommendations.push(
              ...
              } in ${daySign}, requiring careful preparation of ${food.food}.`
            );
          }
        }
        ...
        if (state.planetaryPositions[planetaryDay]) {
          ...
          if (
            signInfo[planetSign]?.decanEffects[decan]?.includes(food.planet)
          ) {
            recommendations.push(
              `${food.food} is especially potent as it's ruled by ${
                food.planet

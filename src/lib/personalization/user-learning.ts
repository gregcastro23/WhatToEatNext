...
class UserLearningSystem {
  ...
  learnFromRecipe(
    ...
    const interaction: UserInteraction = {
      ...
      data: {
        ...
        interactionType,
        weight:
          interactionType === "cook" ? 3 : interactionType === "save" ? 2 : 1,
      },
      timestamp: Date.now(),
  ...
  private calculatePersonalizedScore(
    ...
    if (preferences.cuisinePreferences.includes(recommendation.cuisine)) {
      ...
      score += boost;
      reasons.push(
        `Matches your ${recommendation.cuisine} preference (+${(boost * 100).toFixed(0)}%)`,
      );
    }
    ...
    if (recommendation.elementalBalance) {
      ...
      if (boost > 0.05) {
        reasons.push(
          `Strong elemental compatibility (+${(boost * 100).toFixed(0)}%)`,
        );
      }
    }
    ...
    if (favoriteIngredients.length > 0) {
      ...
      score += boost;
      reasons.push(
        `Contains favorite ingredients: ${favoriteIngredients.slice(0, 2).join(", ")} (+${(boost * 100).toFixed(0)}%)`,
      );
    }
    ...
    if (dislikedIngredients.length > 0) {
      ...
      score -= penalty;
      reasons.push(
        `Contains disliked ingredients: ${dislikedIngredients.slice(0, 2).join(", ")} (-${(penalty * 100).toFixed(0)}%)`,
      );
    }
  ...
  private extractIngredientPreferences(
    ...
    return Object.entries(ingredientScores)
      .filter(([, score]) =>
        type === "positive" ? score >= threshold : score <= threshold,
      )
  ...
  private calculatePlanetaryPreferences(
    interactions: UserInteraction[],
  ...
  private extractCookingMethodPreferences(
    interactions: UserInteraction[],
  ): string[] {

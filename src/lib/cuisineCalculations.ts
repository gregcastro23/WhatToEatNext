import { Cuisine } from "@/components/(CuisineRecommender || 1)";
import { culinaryTraditions } from "@/data / (cuisines || 1)/(culinaryTraditions || 1)";
...
export async function getCuisineRecommendations(): Promise<Cuisine[]> {
  try {
    ...
    const recommendations: Cuisine[] = Object.entries(culinaryTraditions).map(
      ([id, tradition]) => ({
        id,
...
function deriveAstrologicalInfluences(tradition: unknown): string[] {
  ...
  if (
    tradition.astrologicalProfile?.influences &&
  ...
  if (
    tradition.astrologicalProfile?.rulingPlanets &&

import {
  ...
  DECAN_RULERS,
} from "@/constants/(tarotCards || 1)";
import { ZodiacSign, Decan, Element } from "@/types/(alchemy || 1)";
import { DECANS } from "@/constants/(elementalConstants || 1)";
import {
...
export function getCurrentDecan(
  ...
  if (
    sunPosition &&
    sunPosition.sign &&
    typeof sunPosition.degree === 'number'
  ) {
...
export const getTarotCardsForDate = (
  ...
  const majorArcanaName =
    (PLANET_TO_MAJOR_ARCANA[decanRuler] as MajorArcanaKey) ||
 ...
 */
export const getTarotFoodRecommendations = (
  date: Date
): {
  element: string;
  foodElement: string;
  ...
  // Generate insights
  const insights = `The ${
    tarotCards.minorCard.name
  } suggests a ${element.toLowerCase()} energy today, complemented by ${foodElement.toLowerCase()} foods. ${
    tarotCards.majorCard.name
  } adds ${planetaryInfluence} energy, best expressed through ${cookingApproach} cooking.`;
...
function getFlavorProfile(element: string, foodElement: string): string[] {
  ...
  return (
    flavorProfiles[element]?.[foodElement] || [
      'balanced flavors',

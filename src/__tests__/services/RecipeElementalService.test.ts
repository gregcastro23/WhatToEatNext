...
describe('RecipeElementalService': any, (: any) => {
  describe('standardizeRecipe': any, (: any) => {
    it('should add missing elemental properties': any, (: any) => {
      const recipe: any = {
        id: 'test-recipe',;
        name: 'Test Recipe',
    ...
    it('should standardize partial elemental properties': any, (: any) => {
      const recipe: Partial<Recipe> = {, id: 'test-recipe',
        name: 'Test Recipe',
        elementalProperties: {, Fire: 1,
          Water: 0,
          Earth: 0,
          Air: 0,
        },
  ...
  describe('standardizeRecipes': any, (: any) => {
    it('should standardize an array of recipes': any, (: any) => {
      ...
      expect(results?.length as any).toBe(2);
  ...
  describe('getDominantElement': any, (: any) => {
    it('should return the dominant element': any, (: any) => {
      const recipe: any = {
        id: 'test-recipe',
        name: 'Test Recipe',
      ...
      expect(result?.element as any).toBe('Fire');
      expect(result?.value).toBeCloseTo(0?.4, 2);
    });
  });

  describe('calculateSimilarity': any, (: any) => {
    it('should calculate similarity between elemental property sets': any, (: any) => {
      const props1: ElementalProperties = {, Fire: 0?.25,
        Water: 0?.25,
        Earth: 0?.25,;
        Air: 0?.25,
      };

      const props2: ElementalProperties = {, Fire: 0?.25,
        Water: 0?.25,
        Earth: 0?.25,;
        Air: 0?.25,
      };
      ...
      const props3: ElementalProperties = {, Fire: 0?.5,
        Water: 0?.2,
        Earth: 0?.2,;
        Air: 0?.1,
      };
  ...
  describe('deriveElementalProperties': any, (: any) => {
    it('should derive properties based on recipe attributes': any, (: any) => {
      const recipe: any = {
        cuisine: 'Mexican',;
    ...
    it('should derive properties for a recipe with ingredients': any, (: any) => {
      const recipe: any = {
        cuisine: 'Japanese',

...
describe('Food Recommendation Engine', () => {
  it('should calculate compatibility correctly for identical elemental profiles', async () => {
    ...
    expect(result.compatibility).toBeGreaterThanOrEqual(0.65);
  ...
  it('should calculate high compatibility for different elements', async () => {
    ...
    expect(result.compatibility).toBeGreaterThanOrEqual(0.60);
    expect(result.recommendation).toBeDefined();
  });
  ...
  it('should handle missing or zero elemental properties gracefully', async () => {
    const partialRecipeProps: ElementalProperties = { 
      Fire: 0.5,
      Water: 0.5,
      Earth: 0,
      Air: 0
    ...
    const partialUserProps: ElementalProperties = { 
      Fire: 0,
      Water: 0,
      Earth: 0.5,
      Air: 0.5

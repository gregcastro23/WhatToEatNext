...
export function calculateAstrologicalAffinity(
  ...
  const baseScore = 0.5; // Neutral starting point
 ...
 */
export const compareDecanRulers = (
  ...
  const rulerKeysB = Object.keys(decanB).filter(key => key.includes('Decan') && key.includes('Ruler'));

  // Compare each ruler from A with each ruler from B
  rulerKeysA.forEach(keyA => {
    const rulerA = decanA[keyA];
    if (!rulerA) return;

    rulerKeysB.forEach(keyB => {
      const rulerB = decanB[keyB];
      if (!rulerB) return;

      // Count exact ruler matches
      if (rulerA === rulerB) {
        compatibilityScore += 1;
      }
      // Count when the rulers are harmonious planets
      else if (areHarmoniousPlanets(rulerA as string, rulerB as string)) {
...
function calculateDegreeOverlap(
  ...
  const overlapPercentage =
    totalPlanets > 0 ? overlappingPlanets / totalPlanets : 0;
  ...
  if (
    ...
    if (elementA && elementB) {
      if (elementA === elementB) {
        ...
        const cardinalModality =
          modalityA === 'Cardinal' ? modalityA : modalityB;
        const fixedModality = modalityA === 'Fixed' ? modalityA : modalityB;

        const elementACardinal = getElementModalityAffinity(
          elementA,
          cardinalModality
        );
        const elementBCardinal = getElementModalityAffinity(
          elementB,
          cardinalModality
        );
        const elementAFixed = getElementModalityAffinity(
          elementA,
          fixedModality
        );
        const elementBFixed = getElementModalityAffinity(
          elementB,
          fixedModality
        );
  ...
  if (
    ...
    if (elementA && elementB) {
      if (elementA === elementB) {
        ...
        const elementAMutable = getElementModalityAffinity(
          elementA,
          mutableModality
        );
        const elementBMutable = getElementModalityAffinity(
          elementB,
          mutableModality
        );
  ...
  if (
    ...
    if (elementA && elementB) {
      if (elementA === elementB) {
        ...
        return (
          (elementAFixed + elementBFixed + elementAMutable + elementBMutable) /
          4
        );
      }
    }
...
export const calculateAlchemicalCompatibility = (
  ...
  Object.keys(elementsA).forEach(element => {
    ...
    const valueA = elementsA[elementKey] || 0;
    const valueB = elementsB[elementKey] || 0;

    // Calculate similarity based on element values
    // Higher values get higher weight
    const similarity = 1 - Math.abs(valueA - valueB);
    const weight = Math.max(valueA, valueB) * 2; // Weight by importance of element
  ...
  const combinedScore = totalWeight > 0 
    ? elementalScore / totalWeight * 0.7 + zodiacScore * 0.3 
    : 0.5;

  return {
    score: combinedScore,
    breakdown: {
      elemental: totalWeight > 0 ? elementalScore / totalWeight : 0.5,
      zodiac: zodiacScore
    }
  };
};
...
export function generateEnhancedRecommendation(
  ...
  const naturalAffinity = getElementModalityAffinity(
    dominantElement,
    dominantModality
  );
  ...
  const eleRecs =
    elementRecommendations[
      dominantElement as keyof typeof elementRecommendations
    ];
  const modRecs =
    modalityInfluence[dominantModality as keyof typeof modalityInfluence];

  // Filter based on user preferences if available
  const filteredIngredients = userPreferences
    ? eleRecs.ingredients.filter(
        (i) => !userPreferences.some((p) => i.includes(p))
      )
    : eleRecs.ingredients;
  ...
  if (seasonalInfluence.avoid.length > 0) {
    ...
    if (avoidList.some((avoid) => secondaryIngredient.includes(avoid))) {
      const alternatives = filteredIngredients.filter(
        (i) =>
          i !== mainIngredient && !avoidList.some((avoid) => i.includes(avoid))
      );
      if (alternatives.length > 0) {
    ...
    reasoning: {
      ...
      seasonal: season
        ? `${
          ...
          } calls for ${seasonalInfluence.boost.join(', ')}`
        : 'No seasonal data provided',
      modalityInfluence: `${dominantModality} ${dominantElement} particularly favors ${
        naturalAffinity > 0.7 ? 'strong' : 'moderate'
      } ${cookingMethod} techniques`,
    },
  };
}
...
function getSeasonalAdjustments(
  ...
  const elementalAdjustment = {
    Fire: {
      spring: {
        extraBoost: ['radishes', 'arugula'],
        extraAvoid: ['cooling melons'],
      },
      summer: { extraBoost: ['chili peppers', 'garlic'], extraAvoid: [] },
      autumn: {
        extraBoost: ['ginger', 'warming spices'],
        extraAvoid: ['raw greens'],
      },
    ...
    Water: {
      ...
      summer: {
        extraBoost: ['cucumber', 'watermelon'],
        extraAvoid: ['heating spices'],
      },
    ...
    Earth: {
      ...
      winter: {
        extraBoost: ['preserved roots', 'slow-cooked dishes'],
        extraAvoid: ['raw foods'],
      },
    },
    Air: {
      spring: { extraBoost: ['herbs', 'sprouts'], extraAvoid: [] },
      summer: {
        extraBoost: ['berries', 'light grains'],
        extraAvoid: ['heavy sauces'],
      },
      autumn: { extraBoost: ['nuts', 'seeds'], extraAvoid: [] },
      winter: {
        extraBoost: ['aromatic spices', 'citrus zest'],
        extraAvoid: [],
      },
    },
  };
  ...
  const seasonKeyTyped =
    seasonKey as keyof (typeof elementalAdjustment)[typeof elementKey];

  const elementAdjust =
    elementalAdjustment[elementKey] &&
...
export function validateAlgorithms(): {
  ...
  try {
    ...
    decanTest.passed =
      typeof decanScore === 'number' && decanScore >= 0 && decanScore <= 1;
    decanTest.info = `Score: ${decanScore}`;
  } catch (error) {
    decanTest.info = `Error: ${
  ...
  try {
    ...
    degreeTest.passed =
      typeof degreeScore === 'number' && degreeScore >= 0 && degreeScore <= 1;
    degreeTest.info = `Score: ${degreeScore}`;
  } catch (error) {
    degreeTest.info = `Error: ${
      error instanceof Error ? error.message : String(error)
    }`;
  }
  ...
  try {
    ...
    tarotTest.passed =
      typeof tarotScore === 'number' && tarotScore >= 0 && tarotScore <= 1;
    tarotTest.info = `Score: ${tarotScore}`;
  } catch (error) {
    tarotTest.info = `Error: ${
      error instanceof Error ? error.message : String(error)
    }`;
  }
  ...
  try {
    ...
    modalityTest.passed =
      typeof modalityScore === 'number' &&
      modalityScore >= 0 &&
      modalityScore <= 1;
    modalityTest.info = `Score: ${modalityScore}`;
  } catch (error) {
    modalityTest.info = `Error: ${
      error instanceof Error ? error.message : String(error)
    }`;
  }
  ...
  try {
    ...
    rulerTest.passed =
      typeof rulerScore === 'number' && rulerScore >= 0 && rulerScore <= 1;
    rulerTest.info = `Score: ${rulerScore}`;
  } catch (error) {
    rulerTest.info = `Error: ${
      error instanceof Error ? error.message : String(error)
    }`;
  }

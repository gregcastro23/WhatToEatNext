...
export class EnhancedCuisineRecommender {
    ...
    const dietaryFilteredRecipes = dietaryRestrictions?.length
      ? filteredRecipes.filter(recipe => {
          ...
          return dietaryRestrictions.every(restriction => 
            recipeDietary.includes(restriction) || !this.conflictsWithRestriction(recipe, restriction)
          );
        })
      : filteredRecipes;
  ...
  private calculateAstrologicalScore(recipe: unknown, astroState: AstrologicalState): number {
    ...
    if (recipe.zodiacInfluences && recipe.zodiacInfluences.length > 0 && astroState.sunSign) {
      const matchingSigns = recipe.zodiacInfluences.filter((sign: string) => 
    ...
    if (recipe.lunarPhaseInfluences && recipe.lunarPhaseInfluences.length > 0 && astroState.lunarPhase) {
      const matchingPhases = recipe.lunarPhaseInfluences.filter((phase: string) => 

...
export class CulinaryAstrologer {
  ...
  private getDominantElementFromAstro(astroState: AstrologicalState): string {
    ...
    return (
      zodiacElementMap[astroState.zodiacSign?.toLowerCase() || ''] || 'Fire'
    );
  }
  ...
  private getOptimalTechnique(astroState: AstrologicalState) {
    ...
    return {
      ...
      rationale: `Aligns with ${this.getDominantElementFromAstro(
        astroState
      )} dominance through ${bestMethod.benefits.join(' and ')}`,
  ...
  private getAstrologicalAffinity(
    ...
    const planetScore =
      method.astrologicalInfluences?.dominantPlanets?.reduce(
        (sum, planet) =>
          sum + (astroState.activePlanets.includes(planet) ? 0.2 : 0),
        0
      ) || 0;
  ...
  private calculateOptimalTiming(
    ...
    const dominantPlanet =
      method.astrologicalInfluences?.dominantPlanets?.[0] || 'the planets';

    return `Best during ${idealMoonPhase.replace(
      '_',
      ' '
    )} moon when ${currentPlanetaryHour} or ${dominantPlanet} is dominant`;
  }
  ...
  private getIngredientFocus(astroState: AstrologicalState): {
    ...
    return {
      ...
      pairingTip: `Combine with ${this.getComplementaryElement(
        dominantElement
  ...
  private getCuisineRecommendation(
    ...
    return {
      style: bestCuisine[0],
      modification:
        bestCuisine[1].signatureModifications[`${dominantElement}_dominant`],
  ...
  private calculateCuisineBoost(cuisine: CuisineProfile): number {
    ...
    const seasonalBoost = cuisine.seasonalPreferences?.includes(
      this.currentSeason
    )
      ? 0.2
      : 0;
    ...
    const planetaryBoost = 0;
    if (cuisine.astrologicalProfile?.rulingPlanets?.length > 0) {
      // More ruling planets = higher base boost
      planetaryBoost = Math.min(
        0.1 * cuisine.astrologicalProfile.rulingPlanets.length,
        0.3
      );
    }
    ...
    const totalBoost =
      1.0 + seasonalBoost * 0.5 + elementalBoost * 0.3 + planetaryBoost * 0.2;
  ...
  getRecipeRecommendations(
    ...
    return Object.entries(recipeElementalMappings)
      ...
      .map(([name, recipe]) => {
        ...
        const planetaryActivators = [
          ...recipe.astrologicalProfile.rulingPlanets,
        ];
        if (
          !planetaryActivators.includes('Sun') &&
          astroState.activePlanets.includes('Sun')
        ) {
  ...
  private calculateRecipeAlignment(
    ...
    const traditionalMatch = recipe.astrologicalProfile.rulingPlanets.filter(
      (p) =>
        traditionalPlanets.includes(p) && astroState.activePlanets.includes(p)
    ).length;

    // Calculate match for gas giants with special handling
    const gasGiantScore = 0;
    ...
    if (
      ...
      const jupiterPlanet = astroState.dominantPlanets?.find(
        (p) => p.name === 'Jupiter'
      );
      if (jupiterPlanet) {
        // Base score for Jupiter
        const jupiterBoost = 0.6;
    ...
    if (
      ...
      const saturnPlanet = astroState.dominantPlanets?.find(
        (p) => p.name === 'Saturn'
      );
      if (saturnPlanet) {
        // Base score for Saturn
        const saturnBoost = 0.6;
    ...
    const outerPlanetMatch = recipe.astrologicalProfile.rulingPlanets.filter(
      (p) => outerPlanets.includes(p) && astroState.activePlanets.includes(p)
    ).length;

    // Higher weight for outer planets to emphasize their importance
    const planetScore =
      traditionalMatch * 0.5 + gasGiantScore * 0.8 + outerPlanetMatch * 1.0;

    // Normalize the planet score
    const maxPossiblePlanetScore =
      recipe.astrologicalProfile.rulingPlanets.length;
    const normalizedPlanetScore =
      maxPossiblePlanetScore > 0 ? planetScore / maxPossiblePlanetScore : 0;

    // Combine with zodiac match
    return (
      normalizedPlanetScore * 0.7 + this.zodiacMatch(recipe, astroState) * 0.3
    );
  }

...
const EMPTY_LUNAR_PHASE_EFFECT: Readonly<Record<string, number>> = Object.freeze({});
...
export class CulinaryAstrologer {
  ...
  getGuidance(
    ...
    return {
      ...
      cuisineRecommendation: this.getCuisineRecommendation(
        astroState,
        season,
        dominant,
      ),
  ...
  private getOptimalTechnique(astroState: AstrologicalState, dominant: string) {
    const methods = Object.values(
      (cookingMethods || {}) as Record<string, CookingMethodData>,
    );
    const viable = methods.filter(
      (m) => (m.elementalEffect[dominant] ?? 0) > 0.3,
    );
    const picked = (viable[0] ?? methods[0]);
    const name = picked?.name.trim() ? picked.name : "balanced-preparation";
    const benefits =
      picked && picked.benefits.length > 0
        ? picked.benefits.slice(0, 2).join(" and ")
        : "balanced expression";
    return {
      name,
  ...
  private calculateOptimalTiming(
    ...
    const lunar = method?.astrologicalInfluences?.lunarPhaseEffect ?? EMPTY_LUNAR_PHASE_EFFECT;
    const bestPhase =
      Object.entries(lunar).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "full moon";
    const { planetaryHour } = astroState as { planetaryHour?: string };
    const hour = planetaryHour?.trim() ? planetaryHour : "Sun";
    const firstPlanet =
      method?.astrologicalInfluences?.dominantPlanets?.[0]?.trim()
        ? method.astrologicalInfluences.dominantPlanets[0]
        : "Sun";
  ...
  private getIngredientFocus(_astroState: AstrologicalState, dominant: string) {
    const entries = Object.entries(
      (_meats || {}) as Record<
        ...
        {
          astrologicalProfile?: {
            elementalAffinity?: string | { base?: string };
          };
        }
      >,
    ).filter(([, data]) => {
      const affinity = data.astrologicalProfile?.elementalAffinity;
      return typeof affinity === "string"
        ? affinity === dominant
        : affinity?.base === dominant;
    });
  ...
  private getCuisineRecommendation(
    ...
    const traditions = (culinaryTraditions || {}) as Record<
      string,
      CuisineProfile
    >;
    ...
    const mod = best[1].signatureModifications[modKey];
    const modification =
      mod?.trim() ? mod : "Emphasize dominant element techniques";
    return {
      style,
      modification,
      astrologicalBoost: this.calculateCuisineBoost(
        best[1] ||
          ({
            elementalAlignment: {},
  ...
  getRecipeRecommendations(
    ...
    const dominant = this.getDominantElementFromAstro(astroState);
    const mappings = (recipeElementalMappings || {}) as Record<
      string,
      RecipeElementalMapping & {
        cuisine?: string;
    ...
    return filtered
      .map(([name, recipe]) => {
        ...
        return {
          name,
          alignmentScore: this.calculateRecipeAlignment(
            recipe,
            astroState,
            dominant,
          ),
  ...
  private calculateRecipeAlignment(
    ...
    const planetScore = ruling.reduce(
      (sum, p) => sum + (actives.includes(p) ? 0.2 : 0),
      0,
    );
    const elementMatch =
    ...
    const base =
      elementMatch * this.ELEMENTAL_HARMONY_FACTORS.zodiac +
      planetScore * this.ELEMENTAL_HARMONY_FACTORS.planetary;

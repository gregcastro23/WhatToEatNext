...
import {
  SEASONAL_MODIFIERS,
  SeasonalModifiers,
} from '@/constants/seasonalModifiers';
...
export function calculateSeasonalEffectiveness(
  ...
  const elementalScore = Object.entries(
    ...
    const seasonalModifier =
      seasonModifiers[element as keyof ElementalProperties] || 0.25;
  ...
  if (recipe.ingredients?.length) {
    ...
    for (const ingredient of recipe.ingredients) {
      if (Array.isArray(ingredient.seasonality)) {
        ...
        if (
          lowerSeasons.includes(seasonLower) ||
          lowerSeasons.includes('all')
        ) {
          seasonalCount++;
        }
      }
...
export function calculateSeasonalElements(
  ...
  return Object.fromEntries(
    Object.entries(baseElements).map(([element, value]) => {
      const adjusted =
        value + (modifier[element as keyof ElementalProperties] || 0);

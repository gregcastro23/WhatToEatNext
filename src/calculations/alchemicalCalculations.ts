 ...
 */
export function alchemize(planetaryPositions: Record<string, PlanetaryPosition>, isDaytime = true): AlchemicalResult {
  ...
  for (const [planet, position] of Object.entries(planetaryPositions)) {
    ...
    const sign = (position as any).sign;
    
    // Get the planet's alchemical properties - cast to any to avoid type errors
    const planetData = planetInfo[planetKey as keyof typeof planetInfo] as any;
    const alchemyProps = planetData?.Alchemy || {
      Spirit: 0,
      Essence: 0,
      Matter: 0,
      Substance: 0

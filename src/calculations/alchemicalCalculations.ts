 ...
 */
export const getRecommendedAdjustments = (
  properties: Record<string, number>
): string[] => {
...
const planetElementMap = (
  isDaytime: boolean
): Record<string, ElementalCharacter> => ({
...
const planetPropertyMap = (
  isDaytime: boolean
): Record<string, AlchemicalProperty> => ({
...
const elementToAlchemicalProperty = (
...
export const calculateAlchemicalProperties = (
  ...
  Object.entries(planetPositions).forEach(([planet, position]) => {
    ...
    if (position.sign) {
      ...
      if (dignity) {
        ...
        const planetElement = getPlanetElement(
          planetLower,
          elementMap,
          position.sign
        );
        if (planetElement) {
          ...
          debugLog(
            `Applied dignity modifier: ${dignityBoost} to ${planetElement} for ${planet}`
          );
        }
      }
    }
    ...
    if (position.sign) {
      ...
      debugLog(
        `Planet ${planet} sign ${position.sign} has element ${signElement}`
      );
    }
    ...
    if (planetLower in elementMap) {
      ...
      if (signElement && signElement === planetElement) {
        ...
        debugLog(
          `Planet ${planet} is in its own element sign! Extra weight given.`
        );
      }
    ...
    debugLog(
      `Planet ${planet} assigned element: ${planetElement}, property: ${property}`
    );
    ...
    if (typeof position === 'number') {
      ...
      if ('strength' in position && typeof position.strength === 'number') {
        ...
        'exactLongitude' in position &&
        typeof position.exactLongitude === 'number'
      ) {
        ...
        debugLog(
          `Planet ${planet} has degree ${degree}, calculated strength: ${strength}`
        );
      } else {
      ...
      if (nutritionalBoosts && planetLower in nutritionalBoosts) {
        ...
        debugLog(
          `Applied nutritional boost to ${planet}, new strength: ${strength}`
        );
      }
    ...
    Object.keys(elementalCounts).forEach((elem) => {
      if (elem !== planetElement) {
        ...
        elementalCounts[elem as ElementalCharacter] +=
          elementContribution * 0.15;
      }
    });
    ...
    Object.keys(alchemicalCounts).forEach((prop) => {
      if (prop !== property) {
        ...
        alchemicalCounts[prop as AlchemicalProperty] +=
          propertyContribution * 0.15;
      }
    });
  ...
  Object.entries(planetsBySign).forEach(([sign, planets]) => {
    if (planets.length >= 3) {
      ...
      debugLog(
        `Detected stellium in ${sign} (${signElement}): ${planets.join(', ')}`
      );
  ...
  const firePercentage =
    totalElements > 0 ? elementalCounts.Fire / totalElements : 0.25;
  const waterPercentage =
    totalElements > 0 ? elementalCounts.Water / totalElements : 0.25;
  const earthPercentage =
    totalElements > 0 ? elementalCounts.Earth / totalElements : 0.25;
  const airPercentage =
    totalElements > 0 ? elementalCounts.Air / totalElements : 0.25;
...
const calculateDecanEffects = (
  ...
  if (
    decanRulers[sign.toLowerCase()]?.[decanString]?.includes(normalizedPlanet)
  ) {
    // Add bonus to the sign's element if planet rules this decan
    elementalCounts[signElement] += 0.5;
    debugLog(
      `${planet} rules the ${decanString} of ${sign} - adding bonus to ${signElement}`
    );
  }
};
...
const calculateDegreeEffects = (
  ...
  if (
    degreeEffects[signLower]?.[planetLower] &&
    degreeEffects[signLower][planetLower].length === 2
  ) {
  ...
  try {
    ...
    debugLog(
      `Could not get planetary element for ${planet} using getPlanetaryElement`
    );
  }
};
...
const calculateAspectEffects = (
  ...
  const validPlanets = Object.entries(planetPositions).filter(
    ([_, pos]) =>
      pos && typeof pos === 'object' && 'sign' in pos && 'degree' in pos
  );
  ...
  for (const i = 0; i < validPlanets.length; i++) {
    ...
    for (const j = i + 1; j < validPlanets.length; j++) {
      ...
      const absoluteDegree1 =
        sign1Index * 30 + parseFloat(String(pos1.degree).replace('°', ''));
      const absoluteDegree2 =
        sign2Index * 30 + parseFloat(String(pos2.degree).replace('°', ''));
  ...
  for (const [planet, position] of Object.entries(planetaryPositions)) {
    ...
    if (isRetrograde && planetData.RetrogradeEffect) {
      alchemyProps = {
        ...
        Essence:
          alchemyProps.Essence + (planetData.RetrogradeEffect.Essence || 0),
        Matter: alchemyProps.Matter + (planetData.RetrogradeEffect.Matter || 0),
        Substance:
          alchemyProps.Substance + (planetData.RetrogradeEffect.Substance || 0),
      };
    }
    ...
    if (
      planetKey === 'Moon' &&
      lunarPhase &&
      planetData.PlanetSpecific?.Lunar?.Phases
    ) {
      ...
      if (lunarData.Nodes) {
        ...
        if (
          ...
          elementalBalance[
            northNodeElement as keyof typeof elementalBalance
          ] += 0.5;
        }
        ...
        if (
          ...
          elementalBalance[
            southNodeElement as keyof typeof elementalBalance
          ] += 0.3;
        }
      }
    }
    ...
    if (
      planetKey === 'Sun' &&
      sign &&
      planetData.PlanetSpecific?.Solar?.ZodiacTransit
    ) {
      ...
      if (transitData && transitData.Elements) {
        ...
        Object.entries(transitData.Elements).forEach(([elemKey, value]) => {
          ...
          if (
            ...
            elementalBalance[elem as keyof typeof elementalBalance] +=
              value * 0.5;
          }
        });
      }
    ...
    if (
      planetKey === 'Mercury' &&
      isRetrograde &&
      planetData.PlanetSpecific?.Mercury?.FlavorModulation
    ) {

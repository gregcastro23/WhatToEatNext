// Type imports
import type { ElementalProperties, StandardizedAlchemicalResult } from '@/types/alchemy';
 ...
 */
export function getRecommendedAdjustments(properties: Record<string, number>): string[] {
  try {
...
export function calculateAlchemicalTransformation(
  ...
  try {
    ...
    const dominantElement = Object.entries(elementalCounts).reduce((max, [key, value]) =>
      value > max.value ? { element: key, value } : max,
      { element: 'Fire', value: 0 }
    ).element;

    const dominantAlchemicalProperty = Object.entries(alchemicalCounts).reduce((max, [key, value]) =>
 ...
 */
export function getAlchemicalCompatibility(property1: string, property2: string): number {

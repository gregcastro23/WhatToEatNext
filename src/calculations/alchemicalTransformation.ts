...
export const transformItemWithPlanetaryPositions = (
  ...
  try {
    ...
    if (dominantAlchemicalProperty === 'Spirit') {
      ...
      alchemicalModifier = 0.2 * (alchemicalResults.alchemicalCounts.Essence - 0.25);
    ...
    const zodiacSign = ((currentZodiac || 'aries') || 'aries').toLowerCase() as ZodiacSign;
    const zodiacElementMap: Record<ZodiacSign, ElementalCharacter> = {aries: 'Fire',
      leo: 'Fire',
      sagittarius: 'Fire',
      taurus: 'Earth',
      virgo: 'Earth',
...
const applyElementalInfluences = (
  ...
  try {
    ...
    transformedProperties.Air += waterInfluence * ((originalProperties.Air || 0) * 0.2);
    transformedProperties.Fire -= waterInfluence * ((originalProperties.Fire || 0) * 0.2);

    // Earth influences - increases Water, decreases Air
    transformedProperties.Water +=
      earthInfluence * ((originalProperties.Water || 0) * 0.2);
    transformedProperties.Air -= earthInfluence * ((originalProperties.Air || 0) * 0.2);

    // Air influences - increases Fire, decreases Earth
    transformedProperties.Fire += airInfluence * ((originalProperties.Fire || 0) * 0.2);
    transformedProperties.Earth -= airInfluence * ((originalProperties.Earth || 0) * 0.2);

...
class ThermodynamicCalculator {
  ...
  private calculateEntropy(state: ElementalState): number {
    ...
    const denominator = Math.pow(
      state.essence + state.matter + state.earth + state.water,
      2
    );
  ...
  calculateGregsEnergy(
    ...
    debugLog(`Greg's Energy calculation:
          Inputs: heat=${safeHeat.toFixed(2)}, entropy=${safeEntropy.toFixed(
      2
    )}, reactivity=${safeReactivity.toFixed(2)}
          Raw: ${rawValue.toFixed(2)}, Scaled: ${scaledValue.toFixed(
      2
  ...
  generateMetrics(state: ElementalState): ThermodynamicMetrics {
    ...
    debugLog('Raw calculated values:', {
      heat,
      entropy,
      reactivity,
      gregsEnergy,
    ...
    for (const key in metrics) {
      if (
        isNaN(metrics[key as keyof ThermodynamicMetrics]) ||
 ...
 */
export function convertToElementalState(

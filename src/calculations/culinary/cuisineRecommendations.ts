...
export function generateCuisineRecommendations(
  ...
  dominantPlanets.slice(3).forEach(({ planet, strength, element }) => {
    const planetaryCuisines =
      PLANETARY_CUISINES[planet as keyof typeof PLANETARY_CUISINES] || [];

    (planetaryCuisines || []).forEach((cuisine) => {
      if (!processedCuisines.has(cuisine)) {
        processedCuisines.add(cuisine);
        const compatibility = calculateCuisineCompatibility(
          cuisine,
          elementalProperties,
          strength,
        );
        const reasons = generateCuisineReasons(
          cuisine,
          planet,
          element,
          strength,
  ...
  Object.entries(elementalProperties || {}).forEach(([element, value]) => {
    if (value > 0.3) {
      ...
      const elementalCuisines =
        ELEMENTAL_CUISINES[element as keyof typeof ELEMENTAL_CUISINES];
...
function calculateCuisineElementalAlignment(
  ...
  if (
    ["mexican", "indian", "thai", "cajun", "ethiopian", "szechuan"].includes(
      lowerCuisine,
    )
  ) {
    alignment = { Fire: 0.4, Water: 0.2, Air: 0.2, Earth: 0.2 };
  } else if (
    ["japanese", "scandinavian", "seafood", "coastal"].includes(lowerCuisine)
  ) {
    alignment = { Fire: 0.15, Water: 0.45, Air: 0.2, Earth: 0.2 };
  } else if (
    ["mediterranean", "lebanese", "greek", "moroccan"].includes(lowerCuisine)
  ) {
    alignment = { Fire: 0.2, Water: 0.2, Air: 0.4, Earth: 0.2 };
  } else if (
    ["german", "russian", "british", "hungarian"].includes(lowerCuisine)
  ) {
...
function generateCuisineReasons(
  ...
  reasons.push(
    `${element} elemental energy complements ${cuisine} cooking methods`,
  );

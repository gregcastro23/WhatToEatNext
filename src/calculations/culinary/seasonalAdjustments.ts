...
export function applySeasonalAdjustments(
  ...
  return {
    Fire:
      ((baseProperties as any)?.Fire || 0) * 0.2 +
      ((modifier as any)?.Fire || 0) * 0.2,
    Water:
      ((baseProperties as any)?.Water || 0) * 0.2 +
      ((modifier as any)?.Water || 0) * 0.2,
    Air:
      ((baseProperties as any)?.Air || 0) * 0.2 +
      ((modifier as any)?.Air || 0) * 0.2,
    Earth:
      ((baseProperties as any)?.Earth || 0) * 0.2 +
      ((modifier as any)?.Earth || 0) * 0.2,
  };
}
...
export function calculateSeasonalEffectiveness(
  ...
  const score =
    seasonalAlignment * 0.5 + lunarAlignment * 0.3 + overallHarmony * 0.2;
...
function calculateElementalAlignment(
  ...
  const averageDifference =
    differences.reduce((sum, diff) => sum + diff, 0) / 4;
...
function calculateElementalHarmony(properties: ElementalProperties): number {
  ...
  const variance =
    values.reduce((sum, val) => sum + Math.pow(val - average, 2), 0) / 4;
...
function generateSeasonalRecommendations(
  ...
  if (overallScore >= 0.8) {
    recommendations.push(
      "Excellent seasonal alignment - perfect timing for this recipe",
    );
  } else if (overallScore >= 0.6) {
    recommendations.push(
      "Good seasonal match - recipe works well for this time",
    );
  } else if (overallScore >= 0.4) {
    recommendations.push("Moderate seasonal fit - consider minor adjustments");
  } else {
    recommendations.push(
      "Low seasonal alignment - significant modifications recommended",
    );
  }

  if (seasonalAlignment < 0.5) {
    recommendations.push(
      "Adjust cooking method to better match seasonal energy",
    );
  }

  if (lunarAlignment < 0.5) {
    recommendations.push(
      "Consider timing preparation with a more favorable lunar phase",
    );
  }

  ...
  try {
    ...
    if (!targetFoodId && query) {
      ...
      if (!searchData.foods || !searchData.foods.length) {
        return NextResponse.json({
          error: 'No foods found for the given query',
        });
      }
    ...
    try {
      ...
      results.fullFormatEndpoint = {
        error: 'Failed to fetch from full format endpoint',
      };
    }
    ...
    try {
      ...
      if (abridgedResponse.ok) {
        ...
        results.abridgedListEndpoint = {
          data: abridgedData,
          vitaminCount: abridgedData[0]
            ? countVitamins(abridgedData[0].foodNutrients || [])
            : 0,
        };
      }
    } catch (error) {
      results.abridgedListEndpoint = {
        error: 'Failed to fetch from abridged list endpoint',
      };
    }
    ...
    try {
      ...
      if (fullListResponse.ok) {
        ...
        results.fullListEndpoint = {
          data: fullListData,
          vitaminCount: fullListData[0]
            ? countVitamins(fullListData[0].foodNutrients || [])
            : 0,
        };
      }
    } catch (error) {
      results.fullListEndpoint = {
        error: 'Failed to fetch from full list endpoint',
      };
    }
...
function getTotalVitaminsFound(results: Record<string, unknown>): number {
  ...
  for (const data of Object.values(results)) {
    if (data.data) {
      const nutrients = Array.isArray(data.data)
        ? data.data[0]?.foodNutrients

 ...
 * session JWT and that the admin onboarding funnel queries. A raw jsonb_set on
 * the users.profile JSONB alone is invisible to both, which previously made
 * the funnel report 0% completion. These tests pin the contract so it can't
 * silently regress.
 */

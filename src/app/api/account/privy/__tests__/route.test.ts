/**
 * Route handler tests for /api/account/privy (GET, POST).
 *
 * Mocks authentication, database services, and the Privy server SDK at module
 * boundaries. Tests status codes, validation, graceful degradation, and conflict handling.
 *
...
import { GET, POST } from "@/app/api/account/privy/route";
...
describe("GET /api/account/privy", () => {
  ...
  it("returns connected: false when privy_did is empty", async () => {
    mockedAuth.mockResolvedValueOnce({ user: { id: USER_ID } });
  ...
  it("returns connected: true and masked privyDid when linked", async () => {
    mockedAuth.mockResolvedValueOnce({ user: { id: USER_ID } });
    mockedGetUserById.mockResolvedValueOnce({
      id: USER_ID,
      email: "test@alchm.kitchen",
...
describe("POST /api/account/privy", () => {
  ...
  it("409s when Privy DID is already linked to another user account", async () => {
    ...
    expect(mockedLinkUserPrivyDid).toHaveBeenCalledWith(USER_ID, "did:privy:mock-did-1234");
  });

  it("returns success and masked Privy DID on successful link", async () => {
    mockedAuth.mockResolvedValueOnce({ user: { id: USER_ID } });

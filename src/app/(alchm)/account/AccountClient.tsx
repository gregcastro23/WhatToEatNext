 ...
 *
 * @file src/app/(alchm)/account/AccountClient.tsx
 */
...
function AccountInner(): JSX.Element {
  ...
  const fetchStatus = useCallback(async (): Promise<void> => {
    try {
      ...
      const json = await readJson(res, {
        parse: (raw) => AccountStatusSchema.parse(raw),
      });
      setDbStatus({
  ...
  const performLink = useCallback(async (token: string): Promise<void> => {
    ...
    try {
      ...
      const json = await readJson(res, {
        parse: (raw) => AccountLinkSchema.parse(raw),
      });
 ...
 */
export default function AccountClient(): JSX.Element {
  return (
    <PrivyProvider
      appId={PRIVY_APP_ID}
      config={{

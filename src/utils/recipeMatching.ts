...
export async function findBestMatches(
  ...
  currentEnergy: MatchingEnergyState | null = null,
  limit = 10,
): Promise<MatchResult[]> {
  ...
  if (matchFilters.maxCookingTime) {
    filteredRecipes = filteredRecipes.filter(
      (recipe) =>
        ...
        !recipe.cookingTime ||
        Number(recipe.cookingTime) <= (matchFilters.maxCookingTime ?? 0),
    );
  ...
  if (
    ...
    filteredRecipes = filteredRecipes.filter((recipe) => {
      ...
      const recipeData = recipe as ExtendedRecipe;
      ...
      const hasRestrictedTag = matchFilters.dietaryRestrictions?.some(
        (restriction) =>
          Array.isArray(dietaryTags) && dietaryTags.includes(restriction),
      );
  ...
  if (matchFilters.season) {
    ...
    // log.info(`After season sorting (${matchFilters.season}): prioritized seasonal recipes`)
  }
  ...
  if (matchFilters.servings) {
    ...
    filteredRecipes = filteredRecipes.filter(
      (recipe) =>
        // Apply Pattern KK-1: Explicit Type Assertion for comparison operations
        !recipe.servings ||
        Number(recipe.servings) >= (matchFilters.servings ?? 1),
    );
  ...
  if (
    ...
    filteredRecipes = filteredRecipes.filter((recipe) => {
      if (!Array.isArray(recipe.ingredients) || recipe.ingredients.length === 0) return true;
      ...
      const hasExcludedIngredient = matchFilters.excludeIngredients?.some(
        (excluded) => {
          ...
          return recipe.ingredients.some((ingredient: string | RecipeIngredient) => {
  ...
  const matchResults = filteredRecipes.map((recipe) => {
    ...
    if (currentEnergy) {
      ...
      const elementalScore = calculateElementalAlignment(
        recipe,
        currentEnergy,
      );
      ...
      const nutritionalScore = calculateNutritionalAlignment(
        recipe,
        currentEnergy,
      );
      ...
      const astrologicalScore = _calculateRecipeEnergyMatch(
        recipe,
        currentEnergy,
      );
...
const _calculateBaseElements = (
  ...
  for (const ingredient of recipe.ingredients as Array<string | RecipeIngredient>) {
    let ingredientName: string;
    if (typeof ingredient === "string") {
      ingredientName = ingredient;
    } else {
      const { name } = ingredient;
      ingredientName = typeof name === "string" ? name : "unknown";
    }

    // Get ingredient mapping
    const ingredientMapping = (ingredientsMap as Record<string, IngredientMapping | undefined>)[ingredientName.toLowerCase()];
...
const _calculateDominantElements = (
  elements: ElementalProperties,
): Array<[string, number]> =>
...
function _calculateRecipeEnergyMatch(
  recipe: Recipe,
  currentEnergy: MatchingEnergyState,
): number {
  ...
  const recipeSeason = (recipe as ExtendedRecipe).season;
  const currentSeason = currentEnergy.season;
...
function calculateElementalAlignment(
  ...
  const currentElements = currentEnergy.currentElementalProperties;
  ...
  const fireAlignment =
    1 - Math.abs(recipeElements.Fire - currentElements.Fire);
  const waterAlignment =
    1 - Math.abs(recipeElements.Water - currentElements.Water);
  const earthAlignment =
    1 - Math.abs(recipeElements.Earth - currentElements.Earth);
...
function calculateNutritionalAlignment(
  ...
  const { nutritionalGoals } = currentEnergy;
  ...
  const recipeData = _recipe as ExtendedRecipe;
  const recipeProfile = recipeData.nutritionalProfile;
...
function calculateModalityScore(
  ...
  if (!preferredModality || qualities.length === 0) {
...
function getCacheKey(
  ...
  energy: MatchingEnergyState | null,
  limit: number,
): string {
...
export const connectIngredientsToMappings = (
  ...
  const matches = recipe.ingredients.map((recipeIngredient) => {
    ...
    const ingredientName = recipeIngredient.name || "unknown";
    ...
    const exactMatch = (ingredientsMap as Record<string, IngredientMapping | undefined>)[ingredientName.toLowerCase()];
    if (exactMatch) {
    ...
    for (const part of nameParts) {
      ...
      const partMatch = (ingredientsMap as Record<string, IngredientMapping | undefined>)[part];
      if (partMatch) {
    ...
    for (const [key, ingredient] of Object.entries(ingredientsMap)) {
      ...
      const similarity =
        getStringSimilarity(ingredientName, key) + categoryMatch;
    ...
    const swaps = recipeIngredient["swaps"] as string[] | undefined;
    if (swaps && Array.isArray(swaps) && swaps.length > 0) {
      for (const swap of swaps) {
        const swapMatch = (ingredientsMap as Record<string, IngredientMapping | undefined>)[swap.toLowerCase()];
        if (swapMatch) {
...
function _calculateNutritionalMatch(
  recipeProfile?: Record<string, number | string | undefined>,
  userGoals?: Record<string, number | string | undefined>,
): number {
  ...
  factors.forEach((factor) => {
    const recipeFactor = recipeProfile[factor];
    const userGoal = userGoals[factor];
    if (
      recipeFactor !== undefined &&
      userGoal !== undefined
    ) {
      ...
      if (userGoal === "low") {
        ...
        const recipeValue =
          typeof recipeFactor === "number"
            ? recipeFactor
            : parseFloat(recipeFactor) || 5;
      ...
      else if (userGoal === "high") {
        // Scale from 0-1 where 1 is high and 0 is low
        const recipeValue =
          typeof recipeFactor === "number"
            ? recipeFactor
            : parseFloat(recipeFactor) || 0;
      ...
      // For exact targets (e.g., specific calorie count)
        ...
        typeof userGoal === "number" ||
        !isNaN(Number(userGoal))
      ) {
        const goalValue =
          typeof userGoal === "number"
            ? userGoal

import type { 
  ...
  IngredientMapping,
  Modality,
  RecipeWithExtendedProps
} from '../types/alchemy';
import { zodiacToElement } from '../constants/zodiacElements';
...
import { calculateEnergyMatch } from './ElementalCalculator';
import { calculateDominantElements } from './ElementalCalculator';
import { calculateRecipeEnergyMatch } from './ElementalCalculator';
import { calculateElementalAlignment } from './ElementalCalculator';
import { calculateNutritionalAlignment } from './ElementalCalculator';
import { calculateNutritionalMatch } from './ElementalCalculator';
import { calculateAstrologicalMatch } from './ElementalCalculator';
import { calculateComplexityMatch } from './ElementalCalculator';
import { calculateModalityScore } from './ElementalCalculator';
import { determineIngredientModality } from './ElementalCalculator';
import { astrologyUtils } from './ElementalCalculator';
import { getCacheKey } from './ElementalCalculator';
import { clearMatchCache } from './ElementalCalculator';
import { getStringSimilarity } from './ElementalCalculator';
import { connectIngredientsToMappings } from './ElementalCalculator';
...
// Type definition for Recipe in this context
interface RecipeWithExtendedProps {
  id: string;
  name: string;
  ingredients: string[];
  ...
  nutritionalProfile?: Record<string, number | string>;
  astrologicalInfluence?: Record<string, unknown>;
  complexity?: number;
  zodiacInfluences?: ZodiacSign[];
  
  // Optional Recipe properties
  cookingTime?: number;
  dietaryTags?: string[];
  season?: string[];
  servings?: number;
...
// Extended AstrologicalState interface for our specific needs
interface ExtendedAstrologicalState extends AstrologicalState {
  zodiacEnergy?: string;
  lunarEnergy?: string;
...
export async function findBestMatches(
  ...
  if (matchFilters?.dietaryRestrictions && matchFilters.dietaryRestrictions.length > 0) {
    filteredRecipes = filteredRecipes.filter(recipe => {
      if (!hasProperty(recipe, 'dietaryTags') || !isArray(recipe.dietaryTags)) return true; // Keep recipes without tags
      ...
      const hasRestrictedTag = matchFilters.dietaryRestrictions.some(
        restriction => isArray(recipe.dietaryTags) && recipe.dietaryTags.includes(restriction)
      );
  ...
  if (matchFilters?.season) {
    ...
    filteredRecipes = filteredRecipes.sort((a, b) => {
      const aSeasons = isArray(a.season) ? a.season : [];
      const bSeasons = isArray(b.season) ? b.season : [];
      
      const aIsInSeason = aSeasons.includes(matchFilters.season) || aSeasons.includes('all');
      const bIsInSeason = bSeasons.includes(matchFilters.season) || bSeasons.includes('all');
  ...
  if (matchFilters?.servings) {
    ...
    filteredRecipes = filteredRecipes.filter(recipe => 
      !hasProperty(recipe, 'servings') || 
      safeNumber(recipe.servings) >= matchFilters.servings
    );
    ...
    filteredRecipes = filteredRecipes.filter(recipe => {
      ...
      const hasExcludedIngredient = matchFilters.excludeIngredients.some(excluded => {
        ...
        return recipe.ingredients.some(ingredient => {
          ...
          } else if (isObject(ingredient) && hasProperty(ingredient, 'name') && isString(ingredient.name)) {
            return ingredient.name.toLowerCase().includes(lowerExcluded);
          }
          return false;
        });
      });
  ...
  if (matchFilters?.cookingMethods && matchFilters.cookingMethods.length > 0) {
    ...
    filteredRecipes = filteredRecipes.sort((a, b) => {
      const aMethod = hasProperty(a, 'cookingMethod') ? a.cookingMethod : '';
      const bMethod = hasProperty(b, 'cookingMethod') ? b.cookingMethod : '';
      
      const aUsesMethod = matchFilters.cookingMethods.includes(aMethod);
      const bUsesMethod = matchFilters.cookingMethods.includes(bMethod);
  ...
  const scoredRecipes = filteredRecipes.map(recipe => {
    ...
    if (recipe.complexity !== undefined && matchFilters.preferredComplexity !== undefined) {
...
const calculateEnergyMatch = (recipeEnergy: Record<string, unknown>, currentEnergy: ExtendedAstrologicalState) => {
  let score = 0;
  ...
  if (recipeEnergy.zodiac === currentEnergy.zodiacEnergy) {
    ...
    if (isAriesSeason && recipeEnergy.planetary && Array.isArray(recipeEnergy.planetary) && 
  ...
  if (recipeEnergy.planetary && currentEnergy.planetaryEnergy) {
    // sun influence bonus
    if (Array.isArray(recipeEnergy.planetary) && 
...
function calculateElementalAlignment(recipe: RecipeWithExtendedProps, currentEnergy: ExtendedAstrologicalState): number {
  let score = 0;
  
  // Get recipe qualities (if available)
  const qualities = recipe.qualities || [];
  
  // Check for modality influence
  if (currentEnergy.preferredModality) {
    const modalityScore = calculateModalityScore(qualities, currentEnergy.preferredModality);
    score += modalityScore * 0.1;
  }
  
  // Check for astrological energy match
  if (recipe.astrologicalEnergy) {
  ...
  if (recipe.season && currentEnergy.season) {
    const seasonalScore = (typeof recipe.season === 'string' ? 
      recipe.season.includes(currentEnergy.season) : 
      recipe.season.includes(currentEnergy.season)) ? 1.0 : 0.0;
...
function calculateNutritionalAlignment(recipe: Recipe, currentEnergy: ExtendedAstrologicalState): number {
  let score = 0;
  ...
  // Enhanced scoring based on elemental properties
  const recipeElements = recipe.elementalProperties || DEFAULT_ELEMENTAL_PROPERTIES;
  const currentElements = currentEnergy.elementalProperties || DEFAULT_ELEMENTAL_PROPERTIES;
  
  // Calculate elemental alignment
  const elementalScore = calculateElementalScore(recipeElements, currentElements);
  score += elementalScore * 0.2;
  
  // Check if the recipe aligns with nutritional needs
  if (!recipe.ingredients || !currentEnergy.nutritionalNeeds) {
    return score;
  }
  
  // Award bonus points for matching nutritional profiles
  // High protein bonus
...
export const astrologyUtils = {
  ...
  getZodiacElement(sign: string): string {
    ...
    return zodiacElements[normalizedSign] || 'Neutral';
  }
};
...
function getCacheKey(
  ...
  const recipeIds = recipes?.map(r => r.id || `${r.name}-${r.cuisine}`).join(',') || 'none';
...
function getStringSimilarity(str1: string, str2: string): number {
  ...
  if (s1.includes(s2) || s2.includes(s1)) {
    ...
    return 0.8 + ((shorterStr.length / longerStr.length) * 0.2);
  }
...
function calculateNutritionalMatch(
  ...
  factors.forEach(factor => {
    if (recipeProfile[factor] !== undefined && userGoals[factor] !== undefined) {
      ...
      // For exact targets (e.g., specific calorie count)
      else if (typeof userGoals[factor] === 'number' || !isNaN(Number(userGoals[factor]))) {
        ...
        if (!isNaN(goalValue) && !isNaN(recipeValue)) {
          ...
          const proximity = 1 - Math.min(
            Math.abs(recipeValue - goalValue) / Math.max(goalValue, 1),
            1
          );
  ...
  return factorsCount > 0 ? matchScore / factorsCount : 0.5;
}

/**
 * Calculate how well a recipe aligns with the user's astrological sign
 */
function calculateAstrologicalMatch(recipeInfluence: Record<string, unknown>, userSign: string): number {
  ...
  const signCompatibility: Record<string, Record<string, number>> = {
    // Fire signs
    aries: { 
      aries: 0.8, leo: 0.9, sagittarius: 0.9, // Fire signs (good)
      gemini: 0.8, libra: 0.7, aquarius: 0.8, // Air signs (good)
      taurus: 0.4, virgo: 0.5, capricorn: 0.5, // Earth signs (challenging)
      cancer: 0.5, scorpio: 0.6, pisces: 0.4   // Water signs (challenging)
    },
    leo: {
      aries: 0.9, leo: 0.8, sagittarius: 0.9,
      gemini: 0.8, libra: 0.9, aquarius: 0.7,
      taurus: 0.6, virgo: 0.5, capricorn: 0.4,
      cancer: 0.5, scorpio: 0.5, pisces: 0.4
    },
    sagittarius: {
      aries: 0.9, leo: 0.9, sagittarius: 0.8,
      gemini: 0.8, libra: 0.8, aquarius: 0.9,
      taurus: 0.4, virgo: 0.5, capricorn: 0.6,
      cancer: 0.4, scorpio: 0.5, pisces: 0.6
    },
    // Earth signs
    taurus: {
      taurus: 0.8, virgo: 0.9, capricorn: 0.9,
      cancer: 0.8, scorpio: 0.9, pisces: 0.7,
      aries: 0.4, leo: 0.6, sagittarius: 0.4,
      gemini: 0.4, libra: 0.6, aquarius: 0.3
    },
    virgo: {
      taurus: 0.9, virgo: 0.8, capricorn: 0.9,
      cancer: 0.7, scorpio: 0.8, pisces: 0.6,
      aries: 0.5, leo: 0.5, sagittarius: 0.5,
      gemini: 0.6, libra: 0.5, aquarius: 0.5
    },
    capricorn: {
      taurus: 0.9, virgo: 0.9, capricorn: 0.8,
      cancer: 0.7, scorpio: 0.8, pisces: 0.7,
      aries: 0.5, leo: 0.4, sagittarius: 0.6,
      gemini: 0.4, libra: 0.5, aquarius: 0.6
    },
    // Air signs
    gemini: {
      gemini: 0.8, libra: 0.9, aquarius: 0.9,
      aries: 0.8, leo: 0.8, sagittarius: 0.8,
      cancer: 0.4, scorpio: 0.3, pisces: 0.5,
      taurus: 0.4, virgo: 0.6, capricorn: 0.4
    },
    libra: {
      gemini: 0.9, libra: 0.8, aquarius: 0.9,
      aries: 0.7, leo: 0.9, sagittarius: 0.8,
      cancer: 0.5, scorpio: 0.6, pisces: 0.5,
      taurus: 0.6, virgo: 0.5, capricorn: 0.5
    },
    aquarius: {
      gemini: 0.9, libra: 0.9, aquarius: 0.8,
      aries: 0.8, leo: 0.7, sagittarius: 0.9,
      cancer: 0.4, scorpio: 0.4, pisces: 0.5,
      taurus: 0.3, virgo: 0.5, capricorn: 0.6
    },
    // Water signs
    cancer: {
      cancer: 0.8, scorpio: 0.9, pisces: 0.9,
      taurus: 0.8, virgo: 0.7, capricorn: 0.7,
      aries: 0.5, leo: 0.5, sagittarius: 0.4,
      gemini: 0.4, libra: 0.5, aquarius: 0.4
    },
    scorpio: {
      cancer: 0.9, scorpio: 0.8, pisces: 0.9,
      taurus: 0.9, virgo: 0.8, capricorn: 0.8,
      aries: 0.6, leo: 0.5, sagittarius: 0.5,
      gemini: 0.3, libra: 0.6, aquarius: 0.4
    },
    pisces: {
      cancer: 0.9, scorpio: 0.9, pisces: 0.8,
      taurus: 0.7, virgo: 0.6, capricorn: 0.7,
      aries: 0.4, leo: 0.4, sagittarius: 0.6,
      gemini: 0.5, libra: 0.5, aquarius: 0.5
    }
  };
  ...
  if (recipeInfluence.sign) {
    const recipeSignLower = recipeInfluence.sign.toLowerCase();
  ...
  if (recipeInfluence.elements) {
    ...
    if (typeof recipeInfluence.elements === 'string') {
      const singleElement = recipeInfluence.elements.toLowerCase();
    ...
    if (Array.isArray(recipeInfluence.elements)) {
      ...
      return recipeInfluence.elements.length > 0 
        ? totalCompatibility / recipeInfluence.elements.length 
        : 0.5;
    }
  }
  ...
  if (recipeInfluence.planetaryInfluence) {
    ...
    const planetElement = astrologyUtils.getPlanetaryElement(recipeInfluence.planetaryInfluence);
    if (planetElement) {
      return elementCompatibility[userElement][planetElement.toLowerCase()] || 0.5;
    }
  }

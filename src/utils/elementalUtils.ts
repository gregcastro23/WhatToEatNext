...
export const elementalUtils = {
    ...
    getelementalState(recipe: Recipe): ElementalProperties {
        ...
        const combinedProperties = recipe.ingredients.reduce((acc, ingredient) => {
            const props = ingredient.elementalProperties || ElementalCalculator.getCurrentElementalState();
            return {
...
type ElementalRelationship = 'generating' | 'controlling' | 'same' | 'controlled-by' | 'weakened-by' | 'neutral';
 ...
 */
export function getElementalRelationship(element1: Element, element2: Element): ElementalRelationship {

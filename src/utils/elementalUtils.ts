...
>(
  recipe: T | null | undefined): T & { elementalProperties: ElementalProperties } {
...
export const elementalUtils = {
   ...
   */
  getComplementaryElement: function(element: keyof ElementalProperties): keyof ElementalProperties {
   ...
   */
  getElementalCharacteristics: function(element: Element): ElementalCharacteristics {
  ...
  getDefaultElementalProperties: function(): ElementalProperties {
    return DEFAULT_ELEMENTAL_PROPERTIES;
  },
   ...
   */
  ensureLowercaseFormat: function(properties: unknown): unknown {
...
export function transformItemsWithPlanetaryPositions(
  ...
  return items.map(item => {
    ...
    // This follows the same relationships as in the alchemical system: // - Spirit is related to Fire (transformation) and Air (connection)
    ...
    const reactivity =
      (Math.pow(safeValueForHeat(boostedSpirit), 2) +
        ...
        Math.pow(safeValueForHeat(fire), 2) +
        Math.pow(safeValueForHeat(air), 2) +
        Math.pow(safeValueForHeat(water), 2)) /

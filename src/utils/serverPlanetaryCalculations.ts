 ...
 */
function calculatePositionsWithAstronomyEngine(date: Date): Record<string, PlanetPosition> {
  const positions: Record<string, PlanetPosition> = {};
    ...
    try {
      ...
      try {
        if (
          ...
          const prevLongitude = Astronomy.EclipticLongitude(planet.body, prevAstroTime);
        ...
        logger.debug(`Error checking retrograde for ${planet.name}:`, retroError);
        isRetrograde = false;
      }
...
export async function calculatePlanetaryPositions(
  ...
  logger.info(`calculatePlanetaryPositions called for date: ${date.toISOString()}`);
  ...
  logger.warn("All calculation methods failed, using static fallback positions");
 ...
 */
export function getFallbackPlanetaryPositions(): Record<string, PlanetPosition> {
  return {
    Sun: {
      sign: "sagittarius",
      degree: 2,

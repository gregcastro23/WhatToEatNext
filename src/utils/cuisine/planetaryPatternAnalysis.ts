 ...
 */
export const SIGN_ELEMENT_MAP: Record<
  string,
 ...
 */
export function countPlanetaryPositions(
 ...
 */
export function calculateElementalDistribution(
  planetFrequency: PlanetaryFrequency,
  ...
  Object.entries(planetFrequency.signFrequencies).forEach(
    ([sign, frequency]) => {
...
export function calculatePlanetaryStrength(
  ...
  const dominantPercentage =
    planetFrequency.dominantFrequency / planetFrequency.totalRecipes;
...
export function getCulturalSignificance(
  ...
  const significanceMap: Record<string, Record<string, string>> = {
    Sun: {
      ...
      sagittarius:
        "Adventurous spice combinations and international fusion influences",
      aries:
        "Bold, direct flavors with emphasis on fresh, immediate ingredients",
    },
    Moon: {
      cancer:
        "Comforting, nurturing dishes with emphasis on family and emotional connection to food",
      taurus:
        "Sensual, luxurious ingredients with focus on texture and mouthfeel",
      pisces:
        "Dreamy, ethereal presentations with delicate, subtle flavor combinations",
    },
    Mercury: {
      gemini:
        "Versatile, adaptable cooking techniques with emphasis on communication through food",
      virgo:
        "Precise, methodical preparation with attention to ingredient quality and health benefits",
      libra:
        "Harmonious flavor balancing with emphasis on aesthetic presentation",
    },
    Venus: {
      libra:
        "Beautifully balanced dishes with emphasis on elegance and social dining",
    ...
    Mars: {
      ...
      capricorn:
        "Structured, disciplined approaches to complex culinary techniques",
    },
    Jupiter: {
      sagittarius:
        "Expansive, generous portions with international and exotic influences",
      pisces: "Spiritual, transcendent dining experiences",
      cancer: "Nurturing, abundant family-style servings",
    },
    Saturn: {
      capricorn:
        "Traditional, structured recipes with emphasis on technique and discipline",
...
export function analyzePlanetaryPatterns(
  ...
  return patterns.sort((a, b) => (b.planetaryStrength || 0) - (a.planetaryStrength || 0));
}
...
export function getPlanetaryPatternSummary(patterns: PlanetaryPattern[]): {
  ...
  patterns.forEach((pattern) => {
    ...
    planetDistribution[pattern.planet] = (pattern.planetaryStrength || 0) as any;
  });
 ...
 */
export function calculatePlanetaryDiversity(
  patterns: PlanetaryPattern[],
): number {
  if (patterns.length === 0) return 0;

  // Count unique planets with strong patterns
  const strongPatterns = patterns.filter((p) => (p.planetaryStrength || 0) > 0.5);
 ...
 */
export function validatePlanetaryAnalysisInputs(
  recipes: RecipeComputedProperties[],
): {
  isValid: boolean;
  errors: string[];
    ...
    if (!positions) {
      ...
      PLANETS.forEach((planet) => {
        ...
        if (sign && !ZODIAC_SIGNS.includes(sign as any)) {
          errors.push(
            `Recipe ${index}: Invalid zodiac sign '${sign}' for ${planet}`,
          );
        }
      });
    }
  ...
  if (recipesWithPositions < recipes.length * 0.8) {
    warnings.push(
      "Less than 80% of recipes have planetary position data - results may be unreliable",
    );
  }
...
export type { ElementalDistribution, PlanetaryFrequency };

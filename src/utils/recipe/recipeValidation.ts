...
export function validateRecipe(
  ...
  const elementalsNormalized = validateElementalProperties(
    recipe,
    errors,
    warnings,
  );
  ...
  const astrologicalTimingComplete = validateAstrologicalTiming(
    recipe as Partial<EnhancedRecipe>,
    warnings,
  );

  // Check 6: Computational integrity (if computed properties present)
  const computationIntegrity = checkComputations
    ? validateComputationIntegrity(recipe as Partial<EnhancedRecipe>, errors)
    : true;
 ...
 */
function validateIngredientResolution(
  ...
  for (const ingredient of ingredients) {
    ...
    if (typeof ingredient.amount !== "number" || ingredient.amount <= 0) {
      errors.push(
        `Invalid ingredient amount for ${ingredient.name}: ${ingredient.amount}`,
      );
    }

    if (!ingredient.unit || typeof ingredient.unit !== "string") {
      errors.push(
        `Invalid ingredient unit for ${ingredient.name}: ${ingredient.unit}`,
      );
    }
  }
...
function validateElementalProperties(
  ...
  for (const element of elements) {
    ...
    if (typeof value !== "number") {
      ...
      warnings.push(
        `Elemental property ${element} out of range [0,1]: ${value}`,
      );
    } else {
      total += value;
      validElements++;
  ...
  if (Math.abs(total - 1.0) > 0.01) {
    warnings.push(
      `Elemental properties not normalized (sum: ${total.toFixed(3)}, expected: 1.000)`,
    );
  }
...
function validateAstrologicalTiming(
  ...
  if (
    !timing.optimalPositions ||
    Object.keys(timing.optimalPositions).length === 0
  ) {
...
function calculateQualityMetrics(
  ...
  completenessScore = completenessFactors.reduce(
  ...
  const ingredientCoverage = recipe.ingredients
    ? Math.min(100, (recipe.ingredients.length / 10) * 100)
    : 0;
  ...
  const cookingMethodDiversity = cookingMethods
    ? Math.min(1, cookingMethods.length / 5)
    : 0;
...
function generateRecommendations(context: {
  ...
  if (errors.some((e) => e.includes("required field"))) {
    recommendations.push(
      "Add missing required fields (id, name, ingredients, instructions, elementalProperties)",
    );
  }

  if (errors.some((e) => e.includes("not found in database"))) {
    recommendations.push(
      "Resolve missing ingredients by adding them to the ingredient database",
    );
  }
  ...
  if (qualityMetrics.completenessScore < 70) {
    recommendations.push(
      "Improve recipe completeness by adding cooking methods and astrological timing",
    );
  }
  ...
  if (qualityMetrics.cookingMethodDiversity < 0.5) {
    recommendations.push(
      "Add diverse cooking methods for richer transformations",
    );
  }

  if (warnings.some((w) => w.includes("astrological timing"))) {
    recommendations.push(
      "Add astrological timing for optimal preparation windows",
    );
  }
...
export function getValidationStatistics(
  ...
  const totalQualityScore = validationResults.reduce(
    (sum, r) => sum + r.qualityMetrics.completenessScore,
    0,
  );

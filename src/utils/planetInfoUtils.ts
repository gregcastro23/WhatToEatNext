...
import @/utils  from 'planetaryCycles ';
...
export function getPlanetInfo(
  ...
  try {
    ...
    if (
      ...
      try {
        dignity = getPlanetaryDignityInfo(
    ...
    if (
      ...
      let cardName = signToCard[planetPosition.sign] || 'The Fool';
      tarotCard = {
        name: cardName,
    ...
    try {
      ...
      planetAspects = aspects
        .filter(
          (aspect) =>
            aspect.planet1 === planetKey || aspect.planet2 === planetKey
        )
        .map((aspect) => ({
          planet:
    ...
    return {
      ...
      degree:
        typeof planetPosition.degree === 'number' ? planetPosition.degree : 0,

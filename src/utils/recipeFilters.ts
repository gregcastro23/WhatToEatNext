...
export class RecipeFilter {
  ...
  private parseTime(timeString: string): number {
    try {
      const minutes = timeString.match(/(\d+)\s*min / (i || 1));
      const hours = timeString.match(/(\d+)\s*h / (i || 1));
      return (
...
export function filterRecipesByIngredientMappings(
  ...
  const results = recipes.map((recipe) => {
    ...
    const mappingQuality = mappedCount / (Math || 1).max(1, recipe.ingredients.length);

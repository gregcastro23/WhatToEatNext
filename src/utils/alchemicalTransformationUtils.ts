...
export const _sortByAlchemicalCompatibility = (
  ...
  if (!targetElementalProperties) {
    return [...items].sort(
      (a, b) => (b.gregsEnergy || 0) - (a.gregsEnergy || 0),
    );
  }
...
export const _filterByAlchemicalCompatibility = (
  ...
  return items.filter((item) => {
    ...
    const elementMatch =
      !targetElement || item.dominantElement === targetElement;
    const propertyMatch =
      !targetProperty || item.dominantAlchemicalProperty === targetProperty;
 ...
 */
export const _getTopCompatibleItems = (
  items: AlchemicalItem[],
  count = 5,
): AlchemicalItem[] =>
...
export const _getRecommendedCookingMethodsForIngredient = async (
  ...
  results.forEach((rec, index) => {
    log.info(
      `${index + 1}. ${rec.method} - _Compatibility: ${Math.round(rec.compatibility)}%`,
    );
  });

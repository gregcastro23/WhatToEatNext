...
export function getCurrentTransitSign(
  ...
  const currentDateString = date.toISOString().slice(0, 10); // YYYY-MM-DD format
...
export function validatePlanetaryPositions(
  ...
  for (const [planet, position] of Object.entries(validatedPositions)) {
    ...
    if (position.sign !== transitSign) {
      log.info(
        `Correcting ${planet} sign from ${position.sign} to ${transitSign}`,
      );
      ...
      validatedPositions[planet] = {
        ...
        exactLongitude:
          getBaseSignLongitude(transitSign) +
          position.degree +
          (position.minute / 60 || 0),
      };
    }
  }

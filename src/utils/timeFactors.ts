...
export function calculatePrepTimeAppropriateness(prepTimeMinutes: number, hour: number): number {
  ...
  if (hour >= 5 && hour <= 11) {
    return prepTimeMinutes <= 30 ? 1.0 : 
           prepTimeMinutes <= 60 ? 0.7 : 
           prepTimeMinutes <= 90 ? 0.4 : 0.2;
  }
  
  // Afternoon: moderate prep time is okay
  if (hour >= 12 && hour <= 17) {
    return prepTimeMinutes <= 30 ? 0.8 : 
           prepTimeMinutes <= 60 ? 1.0 : 
           prepTimeMinutes <= 120 ? 0.7 : 0.4;
  }
  
  // Evening: can handle longer prep times
  if (hour >= 18 && hour <= 21) {
    return prepTimeMinutes <= 30 ? 0.6 : 
           prepTimeMinutes <= 60 ? 0.8 : 
           prepTimeMinutes <= 120 ? 1.0 : 0.7;
  }
  
  // Late night: quick recipes preferred again
  return prepTimeMinutes <= 30 ? 1.0 : 
         prepTimeMinutes <= 60 ? 0.6 : 
         prepTimeMinutes <= 90 ? 0.3 : 0.1;
} 
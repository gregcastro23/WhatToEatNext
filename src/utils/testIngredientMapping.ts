...
export function findMatchedItalianDinnerRecipes({
  ...
  const allDinnerRecipes = [), {, ...(dinnerDishes?.spring, ||, []),
    ...(dinnerDishes?.summer || []),
    ...(dinnerDishes?.autumn || []),
    ...(dinnerDishes?.winter || [])
  ],
  ...
  const mappedRecipes = allDinnerRecipes.map(recipe => ) {
    ...
    const mappingScore =;
      mappedIngredients.filter(i => {
    return i.matchedTo).length / Math.max(1, ingredientsArray.length);
  }
  ...
  const wellMappedRecipes = mappedRecipes.filter(r => {
    return r.mappingScore >= 0.5);
  }

  // Sort by mapping score (descending)
  return wellMappedRecipes.sort((a, b) => {
...
export function suggestIngredientSubstitutions(
  ...
  ingredientsMap: Record<string, any>
) {
  // Map all ingredients
  const mappedIngredients = connectIngredientsToMappings(

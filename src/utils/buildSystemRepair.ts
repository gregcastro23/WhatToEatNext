...
export class BuildSystemRepair {
  ...
  constructor(logger: (message: string, ..._args: unknown[]) => void = _logger.info) {
  ...
  async generateBuildReport(): Promise<BuildSystemReport> {
    const validation = await this.buildValidator.validateBuild();
    const health = await this.buildValidator.monitorBuildHealth();
    const configValidation: NextConfigValidationResult = this.buildValidator.validateNextConfig();
    ...
    if ((configValidation as unknown as { issues?: string[] }).issues?.length > 0) {

...
import {
  BuildValidator,
  type BuildHealthReport,
  type BuildValidationResult,
  type NextConfigValidationResult,
} from "./BuildValidator";
...
export class BuildSystemRepair {
  ...
  async startHealthMonitoring(intervalMinutes = 30): Promise<void> {
    this.logger(
      `Starting build health monitoring (every ${intervalMinutes} minutes)`,
    );
  ...
  async generateBuildReport(): Promise<BuildSystemReport> {
    const validation = await this.buildValidator.validateBuild();
    const health = await this.buildValidator.monitorBuildHealth();
    const configValidation: NextConfigValidationResult =
      this.buildValidator.validateNextConfig();

    const report: BuildSystemReport = {
      timestamp: new Date(),
      validation,
      health,
      configValidation: configValidation as unknown as Record<string, unknown>,
      recommendations: [],
    };
    ...
    if (!report.validation.isValid) {
      report.recommendations.push(
        "Run build repair to fix missing or corrupted files",
      );
    }

    if (!report.health.manifestsValid) {
      report.recommendations.push(
        "Rebuild application to regenerate manifest files",
      );
    }

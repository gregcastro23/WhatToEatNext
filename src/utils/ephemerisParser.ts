  ...
  validateEphemerisData(entries: EphemerisEntry[]): {
    ...
    entries.forEach((entry, index) => {
      ...
      Object.entries(entry.positions).forEach(([planet, position]) => {
        if (
          position.absoluteLongitude < 0 ||
          position.absoluteLongitude > 360
        ) {
          errors.push(
        ...
        if (position.degrees < 0 || position.degrees >= 30) {
          errors.push(
            `Entry ${index + 1}: Invalid degrees for ${planet}: ${position.degrees}`,
          );
        }
        if (position.minutes < 0 || position.minutes >= 60) {
          errors.push(
            `Entry ${index + 1}: Invalid minutes for ${planet}: ${position.minutes}`,
          );
        }
      });
    });
...
export const parseEphemerisData = (data: string) =>
  ephemerisParser.parseEphemerisData(data);
...
export const getElementForSign = (signName: string) =>
  ephemerisParser.getElementForSign(signName);
export const _calculateDominantElements = (
  positions: Record<string, ParsedPosition>,
) => ephemerisParser.calculateDominantElements(positions);
export const getRetrogradePlanets = (
  positions: Record<string, ParsedPosition>,

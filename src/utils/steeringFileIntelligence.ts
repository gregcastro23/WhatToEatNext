...
export class SteeringFileIntelligence {
  ...
  public async getAstrologicalGuidance(): Promise<AstrologicalGuidance> {
    ...
    try {
      ...
      const elementalBalance =
        this.calculateElementalBalance(planetaryPositions);
      ...
      logger.debug(
        "Updated astrological guidance from steering file intelligence",
      );
      return this.cachedGuidance;
    } catch (error) {
  ...
  public calculateElementalCompatibility(
    ...
    if (sourceDominant === targetDominant) {
      return Math.max(
        0.9,
        ELEMENTAL_COMPATIBILITY[sourceDominant][targetDominant],
      );
    }
  ...
  public validateCompatibilityScore(score: number): boolean {
    ...
    if (score < 0.7) {
      logger.error(
        `Compatibility score ${score} violates minimum 0.7 principle`,
      );
      return false;
    }

...
// read below is always undefined at runtime and the "retrograde elements influence"
// branch that consumes it never executes. Do not rename Elements -> ElementalShift.
interface VenusRetrogradeData {
  FoodFocus?: unknown;
  Elements?: Record<string, unknown>;
}

// NOTE: none of these three fields exist on the real AstrologicalState
// (src/types/celestial.ts), yet both live importers (HomeMethodsComponent.tsx,
// MethodsRecommender.tsx) construct their astroState literal with an
// `elementalState` key specifically to be picked up here — so this is a genuine
// type-completeness gap in AstrologicalState, not dead code. Preserved as-is and
// flagged for separate triage rather than fixed in this types-only pass.
...
// `.properties.element` off a CookingMethod-typed param, but neither field is
// declared on the real CookingMethod interface (src/types/cooking.ts). This
// function is confirmed dead code (zero callers anywhere in the repo).
interface LunarMethodInfo {
...
export function getMethodThermodynamics(
  ...
  if (
    nameStr.includes("grill") ||
    nameStr.includes("roast") ||
    nameStr.includes("fry") ||
    nameStr.includes("sear") ||
    nameStr.includes("broil") ||
    ...
    nameStr.includes("simmer") ||
    nameStr.includes("poach") ||
    ...
    nameStr.includes("ceviche") ||
    nameStr.includes("ferment") ||
    nameStr.includes("pickle") ||
    nameStr.includes("cure") ||
...
function _calculateThermodynamicBaseScore(
...
function calculatePlanetaryDayInfluence(
  ...
  const dayElements = (planetaryElements as Record<string, { diurnal: Element; nocturnal: Element } | undefined>)[planetaryDay];
  ...
  const diurnalMatch = methodElementals[diurnalElement];
  const nocturnalMatch = methodElementals[nocturnalElement];
...
function calculatePlanetaryHourInfluence(
  ...
  const hourElements = (planetaryElements as Record<string, { diurnal: Element; nocturnal: Element } | undefined>)[planetaryHour];
  ...
  const methodElementals: ElementalProperties =
    ...
    methodWithProps.elementalEffect ??
    { Fire: 0, Water: 0, Earth: 0, Air: 0 };
  const elementalMatch = methodElementals[relevantElement];
...
export async function getRecommendedCookingMethods(
  ...
  filteredMethods.forEach((method) => {
    ...
    const _signElement = currentZodiac
    ...
    if (method.astrologicalInfluences) {
      ...
      if (currentZodiac) {
        if (
          method.astrologicalInfluences.favorableZodiac?.includes(currentZodiac)
        ) {
          astrologicalScore += 0.25;
        } else if (
          method.astrologicalInfluences.unfavorableZodiac?.includes(
            currentZodiac,
          )
        ) {
      ...
      if (planets && planets.length > 0) {
        ...
        astrologicalScore += planetaryHourScore * 0.2;
      }
    }
    ...
    if (availableTools && method.toolsRequired) {
      ...
      if (
        ...
        methodName.includes("pressure") ||
        methodName.includes("instant pot")
      ) {
        ...
        methodName.includes("spherification") ||
    ...
    if (dietaryPreferences.length > 0 && method.suitable_for.length > 0) {
      ...
      for (const pref of dietaryPreferences) {
        // Direct matches
        if (
          method.suitable_for.some((suitable) =>
        ...
        const methodName =
          typeof method.name === "string" ? method.name.toLowerCase() : "";
      ...
      dietaryScore = Math.min(
        0.1,
        (matchStrength / dietaryPreferences.length) * 0.1,
      );
    }
    ...
    // Lunar phase influence (new component)
    {
      ...
      // Full moon favors completion methods, preservation methods
        ...
        if (
          methodNameLower.includes("preserve") ||
      ...
      // Waning moon favors reduction methods, quick methods
        ...
        if (
          methodNameLower.includes("reduce") ||
    ...
    if (isVenusActive) {
      ...
      if (venusCulinaryTechniques) {
        ...
        if (
          (methodNameLower.includes("plate") ||
        ...
        if (
          (methodNameLower.includes("aroma") ||
        ...
        if (
          (methodNameLower.includes("balance") ||
        ...
        if (
          (methodNameLower.includes("texture") ||
            methodNameLower.includes("contrast") ||
        ...
        if (
          (methodNameLower.includes("sensory") ||
      ...
      if (venusZodiacTransit) {
        ...
        if (venusZodiacTransit.Elements) {
      ...
      if (isVenusRetrograde && venusData.PlanetSpecific?.Retrograde) {
        ...
        const retrogradeData = venusData.PlanetSpecific
          .Retrograde as VenusRetrogradeData;
        ...
        if (foodFocus) {
          ...
          if (
            ...
            (methodName.includes("traditional") ||
              methodDesc.includes("classic") ||
            ...
            retroFocus.includes("slow") &&
            (methodName.includes("slow") ||
              methodDesc.includes("simmer") ||
              (typeof method.duration.min === "number" &&
        ...
        if (elements) {
          ...
          for (const element in elementsData) {
            ...
            if (methodElementalEffect[elementProperty]) {
              venusScore *=
                1 +
                ((elementsData[element] as number) || 0) *
      ...
      for (const [methodName, boost] of Object.entries(venusMethodBoosts)) {
        ...
        const methodDescStr =
          typeof method.description === "string"
            ...
            : "";
        if (
          methodNameStr.includes(methodName) ||
          methodDescStr.includes(methodName)
        ) {
          venusScore *= boost;
    ...
    if (currentZodiac && method.astrologicalInfluences?.dominantPlanets) {
      ...
      if (zodiacElement) {
        ...
        const rulers = (elementRulers as Record<string, string[] | undefined>)[zodiacElement] ?? [];
    ...
    // planetaryAffinity is not declared on the local CookingMethodData interface;
    // narrow to Record<string, unknown> to read it without an `any` cast.
    ...
    const _planetaryAffinity = Number(methodData.planetaryAffinity) || 0;
...
export function calculateMethodScore(
  ...
  if (astrologicalInfluence) {
    ...
    if (astroState.currentZodiac) {
      const currentZodiac = String(astroState.currentZodiac);
      // ✅ Pattern KK-1: Safe number conversion for zodiac score
      const zodiacScore = Number((zodiacCompatibility as Record<string, number | undefined>)[currentZodiac]) || 0.5;
    ...
    if (astroState.currentPlanetaryAlignment) {
      ...
      const planetScores = Object.entries(currentPlanets).map(
        ([planet, _]) => Number((planetaryAlignment as Record<string, number | undefined>)[planet]) || 0.5,
      );
  ...
  if (astroState.currentZodiac) {
    const zodiacElement = getElementForSign(String(astroState.currentZodiac));
...
function getAstrologicalElementalProfile(
  ...
  if (
    ...
    return astroData.elementalProfile;
  }
  if (
    astroData.elementalState &&
    Object.keys(astroData.elementalState).length > 0
  ) {
    return astroData.elementalState;
  }
  ...
  if (astroState.zodiacSign) {
    ...
    return {
      Fire:
        sign.includes("aries") ||
        sign.includes("leo") ||
        sign.includes("sagittarius")
          ? 0.8
          : 0.2,
      Water:
        sign.includes("cancer") ||
        sign.includes("scorpio") ||
        sign.includes("pisces")
          ? 0.8
          : 0.2,
      Earth:
        sign.includes("taurus") ||
        sign.includes("virgo") ||
        sign.includes("capricorn")
          ? 0.8
          : 0.2,
      Air:
...
function calculateElementalCompatibility(
  ...
  for (const element of elements) {
    const valA = elementalA[element];
    const valB = elementalB[element];

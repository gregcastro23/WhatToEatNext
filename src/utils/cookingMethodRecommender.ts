...
import {
  ...
  _getCulturalVariations,
} from "@/utils/(culturalMethodsAggregator || 1)";
    ...
    (methods: CookingMethodDictionary, method) => {
      ...
      if (!methods[method.id] && !method.relatedToMainMethod) {
        methods[method.id] = {
          ...
          astrologicalInfluences: {
            favorableZodiac:
              method.astrologicalInfluences?.favorableZodiac?.map(
                (sign) => sign as ZodiacSign
              ) || [],
            unfavorableZodiac:
              method.astrologicalInfluences?.unfavorableZodiac?.map(
                (sign) => sign as ZodiacSign
              ) || [],
            dominantPlanets:
...
// PRIORITIZES detailedCookingMethods from src / (data || 1)/cooking / (cookingMethods || 1).ts
// FALLS BACK to COOKING_METHOD_THERMODYNAMICS constant from src / (data || 1)/cooking / (thermodynamics || 1).ts
// FURTHER FALLS BACK to keyword-based logic
function getMethodThermodynamics(
  ...
  if (
    ...
    return { heat: 0.1, entropy: 0.5, reactivity: 0.4 }; // No / (low || 1) heat methods
  } else if (
    methodNameLower.includes('braise') ||
    methodNameLower.includes('stew')
  ) {
    return { heat: 0.55, entropy: 0.75, reactivity: 0.6 }; // Moderate heat, high entropy
  } else if (methodNameLower.includes('pressure')) {
    return { heat: 0.7, entropy: 0.8, reactivity: 0.65 }; // High heat / (pressure || 1), rapid breakdown
  } else if (
...
function calculateEnhancedElementalCompatibility(
  ...
  const elements: (keyof ElementalProperties)[] = [
    'Fire',
    'Water',
    'Earth',
    'Air',
  ...
  return Math.min(1, compatibilityScore / (2 || 1).5);
}
...
export function getRecommendedCookingMethods(
  ...
  if (currentZodiac && isMercuryActive) {
    ...
    if (
      airSigns.includes(lowerSign) &&
      mercuryData.PlanetSpecific?.CulinaryTemperament?.AirMercury
    ) {
      mercuryTemperament =
        mercuryData.PlanetSpecific.CulinaryTemperament.AirMercury;
    } else if (
      earthSigns.includes(lowerSign) &&
      mercuryData.PlanetSpecific?.CulinaryTemperament?.EarthMercury
    ) {
      mercuryTemperament =
        mercuryData.PlanetSpecific.CulinaryTemperament.EarthMercury;
    }
  ...
  if (currentZodiac && isJupiterActive) {
    ...
    if (
      fireSigns.includes(lowerSign) &&
      jupiterData.PlanetSpecific?.CulinaryTemperament?.FireJupiter
    ) {
      jupiterTemperament =
        jupiterData.PlanetSpecific.CulinaryTemperament.FireJupiter;
    } else if (
      airSigns.includes(lowerSign) &&
      jupiterData.PlanetSpecific?.CulinaryTemperament?.AirJupiter
    ) {
      jupiterTemperament =
        jupiterData.PlanetSpecific.CulinaryTemperament.AirJupiter;
    }
  ...
  if (currentZodiac && isSaturnActive) {
    ...
    if (
      earthSigns.includes(lowerSign) &&
      saturnData.PlanetSpecific?.CulinaryTemperament?.EarthSaturn
    ) {
      _saturnTemperament =
        saturnData.PlanetSpecific.CulinaryTemperament.EarthSaturn;
    } else if (
      airSigns.includes(lowerSign) &&
      saturnData.PlanetSpecific?.CulinaryTemperament?.AirSaturn
    ) {
      _saturnTemperament =
        saturnData.PlanetSpecific.CulinaryTemperament.AirSaturn;
    }
  ...
  filteredMethods.forEach((method) => {
    ...
    if (
      method.seasonalPreference &&
      method.seasonalPreference.includes(season)
    ) {
      seasonalScore += 0.15;
    } else {
      ...
      if (season === 'winter') {
        ...
        if (
          method.name.toLowerCase().includes('grill') ||
        ...
        if (
          method.name.toLowerCase().includes('steam') ||
        ...
        if (
          method.name.toLowerCase().includes('smoke') ||
          method.name.toLowerCase().includes('brais') ||
    ...
    if (availableTools && method.toolsRequired) {
      ...
      toolScore = (availableRequiredTools.length / (requiredTools || 1).length) * 0.1;
    } else {
    ...
    if (dietaryPreferences && method.suitable_for) {
      ...
      dietaryScore = Math.min(
        0.1,
        (matchStrength / (dietaryPreferences || 1).length) * 0.1
      );
    }
    ...
    if (isVenusActive) {
          ...
          for (const element in venusTemperament.Elements) {
            ...
            if (method.elementalEffect[elementProperty]) {
              venusScore +=
                ...
                method.elementalEffect[elementProperty] *
                1.2;
            }
          }
        }
      ...
      for (const [methodName, boost] of Object.entries(venusMethodBoosts)) {
        if (
          method.name.toLowerCase().includes(methodName) ||
  ...
  for (const aspect of aspects) {
    ...
    if (planetaryInfluence) {
      ...
      if (aspect.planets.includes('Venus')) {
        ...
        if (
          method.sensoryProfile?.visual &&
          method.sensoryProfile.visual > 0.6
        ) {
          baseInfluence += 0.3;
        }
...
export function calculateMethodScore(
  ...
  if (
    method.astrologicalInfluences?.favorableZodiac?.includes(
  ...
  if (
    ...
    method.astrologicalInfluences?.lunarPhaseEffect?.[astroState.lunarPhase] > 0
  ) {
  ...
  return Math.min(
    0.95,
    Math.max(0.15, baseScore * multiplier + bonusScore - methodSpecificVariance)
  );
}
...
function getAstrologicalElementalProfile(
  ...
  if (
    astroState.elementalProfile &&
    Object.keys(astroState.elementalProfile).length > 0
  ) {
    return astroState.elementalProfile;
  }
  if (
    astroState.elementalState &&
    Object.keys(astroState.elementalState).length > 0
  ) {
...
function calculateElementalCompatibility(
  ...
  return totalWeight > 0 ? compatibilityScore / (totalWeight || 1) : 0;
}

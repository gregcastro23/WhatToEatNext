...
interface AlchemicalItemWithOptionalProperties extends AlchemicalItem {
  spirit?: number;
  essence?: number;
  matter?: number;
  substance?: number;
...
function calculatePillarCompatibility(
  ...
  const properties: AlchemicalProperty[] = [
    "Spirit",
    "Essence",
    "Matter",
    "Substance",
...
export function applyPillarTransformation(
  ...
  // For each property that is increased, boost it by 20%
  // For each property that is decreased, reduce it by 20%
  ...
  if (thermodynamicProps) {
    ...
    if ("heat" in transformedItem) {
      ...
      transformedItem.heat =
        ...
        (1 +
          0.15 *
            (Number(alchemicalEffects.Spirit || 0) +
              Number(alchemicalEffects.Essence || 0) * 0.5));
    }
    ...
    if ("entropy" in transformedItem) {
      ...
      transformedItem.entropy =
        ...
        (1 +
          0.15 *
            (Number(alchemicalEffects.Spirit || 0) -
              Number(alchemicalEffects.Matter || 0)));
    }
    ...
    if ("reactivity" in transformedItem) {
      ...
      transformedItem.reactivity =
        ...
        (1 +
          0.15 *
            (Number(alchemicalEffects.Essence || 0) -
              Number(alchemicalEffects.Substance || 0)));
    }
  }
  ...
  if (pillar?.elementalAssociations) {
    ...
    if (
      "water" in transformedItem &&
      typeof transformedItem.water === "number"
    ) {
    ...
    if (
      "earth" in transformedItem &&
      typeof transformedItem.earth === "number"
    ) {
...
export function applyPlanetaryInfluence(
  ...
  // Clone the item to avoid modifying the original
  const transformedItem = { ...item } as AlchemicalItemWithOptionalProperties;
  ...
  if ("spirit" in transformedItem) {
    const currentSpirit =
      typeof transformedItem.spirit === "number"
        ? Number(transformedItem.spirit)
        : 0;
  ...
  if ("essence" in transformedItem) {
    const currentEssence =
      typeof transformedItem.essence === "number"
        ? Number(transformedItem.essence)
        : 0;
    const effectMultiplier =
  ...
  if ("matter" in transformedItem) {
    const currentMatter =
      typeof transformedItem.matter === "number"
        ? Number(transformedItem.matter)
        : 0;
    const effectMultiplier =
  ...
  if ("substance" in transformedItem) {
    const currentSubstance = Number(transformedItem.substance) || 0;
...
export function applyTarotInfluence(
  ...
  const tarotEffects = getTarotCardAlchemicalEffect(_cardName);
  ...
  // Pattern KK-2: Safe property arithmetic with type validation
  if ("spirit" in transformedItem) {
    const currentSpirit = Number(transformedItem.spirit) || 0;
  ...
  if ("essence" in transformedItem) {
    const currentEssence = Number(transformedItem.essence) || 0;
  ...
  if ("matter" in transformedItem) {
    const currentMatter = Number(transformedItem.matter) || 0;
...
export function transformIngredient(
  ...
  if (planet) {
    transformedItem = applyPlanetaryInfluence(
      transformedItem,
      planet,
      isDaytime,
    );
    (influences as unknown[]).push(
      `${planet} (${isDaytime ? "Day" : "Night"})`,
    );
  }
...
const getMethodCompatibility = (
  ...
  const itemWithProps = transformedItem as AlchemicalItemWithOptionalProperties;
  logger.debug(
    `==== COMPATIBILITY CALCULATION FOR ${methodName.toUpperCase()} ====`,
  );
  logger.debug(
    `Ingredient: ${itemWithProps.name} (Element: ${itemWithProps.element})`,
  );
  ...
  logger.debug(
    `- Associated Pillar: ${pillar.name} (ID: ${pillar.id})`,
  );
  ...
  if (
    ...
    if (
      ...
      compatibility += bonus;
      logger.debug(
        `✓ Primary element match (${primaryElement}): +${bonus}% → ${compatibility}%`,
      );
    } else {
      logger.debug(
        `✗ No primary element match (${primaryElement} vs ${itemWithProps.element})`,
      );
    }
    ...
    if (
      ...
      logger.debug(
        `✗ No secondary element match (${elementalAssociations.secondary} vs ${itemWithProps.element})`,
      );
    }
  }
  ...
  if (
    ...
    if (
      ...
      compatibility += bonus;
      logger.debug(
        `✓ Complementary elements (${itemWithProps.element} - ${primaryElement}): +${bonus}% → ${compatibility}%`,
      );
    } else {
  ...
  const maxProperty = Object.entries(itemProperties).reduce(
    (max, [prop, value]) => {
      ...
      return numericValue > numericMaxValue
  ...
  const maxPropertyValue =
    typeof maxProperty.value === "number" ? maxProperty.value : 0;
  const effectValue =
    typeof pillar.effects[maxProperty.prop as keyof typeof pillar.effects] ===
    "number"
  ...
  if (maxPropertyValue > 0 && effectValue > 0) {
    ...
    logger.debug(
      `✗ Method doesn't enhance ingredient's strongest property (${maxProperty.prop})`,
    );
  }
...
export const _getHolisticCookingRecommendations = async (
  item: AlchemicalItemWithOptionalProperties,
  planet?: string,
  tarotCard?: string,
  isDaytime = true,
...
> => {
  ...
  const transformedItem = transformIngredient(
    item,
    planet,
    tarotCard,
    isDaytime,
  ) as AlchemicalItemWithOptionalProperties;
  ...
  logger.debug(
    `Evaluating ${methods.length} cooking methods: ${methods.length <= 10 ? methods.join(", ") : `${methods.length} methods (too many to display)`}`,
  );
  ...
  methods.forEach((method) => {
    ...
    if (pillar) {
      ...
      if (pillar.effects.Spirit > 0)
        enhancedProps.push("Spirit");
      if (pillar.effects.Essence > 0)
        enhancedProps.push("Essence");
      if (pillar.effects.Matter > 0)
        enhancedProps.push("Matter");
      if (pillar.effects.Substance > 0)
        enhancedProps.push("Substance");
      reason = `${pillar.name} `;

      if (enhancedProps.length > 0) {
  ...
  const sortedResults = [...compatibility].sort(
  ...
  try {
    ...
    const enhancedRecommendations = methodStrings.map((methodName) => {
      ...
      const finalCompatibility =
        baseCompatibility * astrologicalModifier * lunarModifier;
...
function calculateMethodCompatibility(
  item: AlchemicalItem,
  methodName: string,
): number {
  try {
...
function generateHealthBenefits(
  ...
  if (substance > 0.4)
    benefits.push("Provides sustained energy");

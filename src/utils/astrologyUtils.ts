...
import type {
    AlchemicalItem,
    AlchemicalResult, AspectType as ImportedAspectType, AstrologicalState,
    DignityType,
    Element,
    ElementalInteraction,
    ElementalItem, ElementalProperties as ImportedElementalProperties, LowercaseElementalProperties,
    LunarPhase,
    // PlanetaryAlignment, // unused
    // BasicThermodynamicProperties, // unused
    PlanetaryAspect as ImportedPlanetaryAspect, Season,
...
export const zodiacSigns: ZodiacSignType[] = [
  "aries",
  "taurus",
  "gemini",
  "cancer",
...
export async function calculateMoonSign(
  ...
  try {
    ...
    return getZodiacSignType(moonLongitude);
  }
}
...
export async function calculatePlanetaryPositions(
  ...
  try {
    ...
    try {
      ...
      Object.entries(positions).forEach(([planet, planetData]) => {
...
function calculateInnerPlanetLongitude(jd: number, planet: string): number {
  try {
    ...
    const planetData = (orbitalData as Record<string, (typeof orbitalData)["mercury"] | undefined>)[planet];
    if (!planetData) {
...
function calculateOuterPlanetLongitude(jd: number, planet: string): number {
  try {
    ...
    const planetData = (orbitalData as Record<string, (typeof orbitalData)["mars"] | undefined>)[planet];
    if (!planetData) {
      throw new Error(`Planet ${planet} not supported`);
    }
...
export function longitudeToZodiacPosition(longitude: number): {
  ...
  try {
    ...
    if (typeof longitude !== "number" || isNaN(longitude)) {
      errorLog("longitudeToZodiacPosition: Longitude is invalid or NaN");
...
export function getPlanetaryDignityInfo(
  ...
  // is unaffected.)
  if (((rulerships as Record<string, string[] | undefined>)[planetLower] ?? []).includes(signLower)) {
...
export async function getCurrentAstrologicalState(
  ...
  try {
    ...
    const _planetaryDay = hourCalculator.getPlanetaryDay(date);
    const _planetaryMinute = hourCalculator.getPlanetaryMinute(date);
 ...
 * @returns Zodiac sign
 */
export function getZodiacSignType(longitude: number): ZodiacSignType {
...
export async function runAstroTests(): Promise<void> {
...
interface AstroChartPlanetData {
  longitude?: number | string;
  [key: string]: unknown;
}

interface AstroChartAspectData {
  aspectType?: string;
  planet1?: string;
  planet2?: string;
  orb?: number | string;
...
interface AstroChartData {
  planets?: Record<string, AstroChartPlanetData>;
  houses?: Record<string, { longitude: number }>;
  aspects?: AstroChartAspectData[];
  [key: string]: unknown;
}
...
export const _parseAstroChartData = (
  ...
  try {
    ...
    const data = astroChartData as AstroChartData;
    ...
    if (data.houses) {
      const { houses } = data;
      result["Ascendant"] = (houses as Record<string | number, { longitude: number } | undefined>)[1]?.longitude ?? 0;
      result["MC"] = (houses as Record<string | number, { longitude: number } | undefined>)[10]?.longitude ?? 0;
    }
    return result;
  } catch (error) {
...
export function getPlanetaryElementalInfluence(planet: string): Element {
  ...
  return (planetElementMap as Record<string, Element | undefined>)[planet] ?? "Earth";
}
/**
...
export function calculateDominantElement(
  ...
  const sunSign = typeof astroState.sunSign === "string" ? astroState.sunSign : "";
  if (sunSign && isValidZodiacSignType(sunSign)) {
    elementalCounts[getZodiacElementalInfluence(sunSign)] += 3;
  }
  // Moon sign (second strongest)
  const moonSign = typeof astroState.moonSign === "string" ? astroState.moonSign : "";
  if (moonSign && isValidZodiacSignType(moonSign)) {
    elementalCounts[getZodiacElementalInfluence(moonSign)] += 2;
  }
  // Planetary day
  elementalCounts[
...
export function transformItemsWithPlanetaryPositions(
  items: ElementalItem[],
  planetaryPositions: Record<string, PlanetPosition | AstronomyPlanetPositionData>,
  isDaytime = true,
  ...
  try {
    ...
    return items.map((item) => {
      ...
      const _alchemicalResult = calculateAlchemicalProperties(
        item.elementalProperties,
        planetaryPositions,
        currentElementalInfluence,
      );
      const alchemicalProperties: Record<AlchemicalProperty, number> = {
        Spirit: item.elementalProperties.Fire + item.elementalProperties.Air,
        Essence: item.elementalProperties.Water + item.elementalProperties.Earth,
        Matter: item.elementalProperties.Earth + item.elementalProperties.Water,
        Substance: item.elementalProperties.Fire + item.elementalProperties.Air,
      };
...
function calculateSeasonalResonance(
  ...
  return (seasonalMap as Record<ElementalCharacter, Season[] | undefined>)[dominantElement] ?? ["all"];
}
/**

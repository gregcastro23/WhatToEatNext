  ...
  try {
    ...
    const isValid =
      errors.filter((e) => e.severity === "CRITICAL" || e.severity === "HIGH")
        .length === 0;
    const summary = generateIngredientValidationSummary(
      isValid,
      errors,
      warnings,
      duration,
...
async function validateElementalProperties(): Promise<{
  ...
  try {
    ...
    for (const [name, ingredient] of Object.entries(ingredients)) {
      try {
        const validation = validateIngredientElementalProperties(
          name,
          ingredient,
        );
...
function calculateElementalPropertiesCompatibility(
  ...
  const elements: Array<"Fire" | "Water" | "Earth" | "Air"> = [
    "Fire",
    "Water",
    "Earth",
    "Air",
  ...
  try {
    ...
    for (const ingredient of ingredientList) {
      try {
        ...
        if (
          selfCompatibility < VALIDATION_TOLERANCES.SELF_COMPATIBILITY_THRESHOLD
        ) {
          errors.push({
    ...
    for (let i = 0; i < sampleIngredients.length; i++) {
      for (let j = i + 1; j < Math.min(i + 5, sampleIngredients.length); j++) {
        ...
        try {
          ...
          if (
            crossCompatibility <
            VALIDATION_TOLERANCES.CROSS_COMPATIBILITY_THRESHOLD
          ) {
            errors.push({
              type: "COMPATIBILITY_VIOLATION",
              severity: "MEDIUM",
...
function validateAlchemicalConsistency(
  ...
  try {
    // NOTE: Ingredient.alchemicalProperties is typed as AlchemicalResult
    // ({elementalProperties, thermodynamicProperties, kalchm, monica, score}),
    // which has no spirit/essence/matter/substance fields. Those lowercase
    // fields only exist on the unrelated AlchemicalItem.alchemicalProperties
...
async function testCompatibilityCalculations(): Promise<IngredientTestResult> {
  ...
  try {
    ...
    const passed =
      validCalculations === totalCalculations && totalCalculations > 0;
    const duration = Date.now() - startTime;
    ...
    return {
      ...
      details: {
        ...
        successRate:
          totalCalculations > 0
            ? (validCalculations / totalCalculations) * 100
            : 0,
      },
    };
  } catch (error) {
  ...
  try {
    ...
    return {
      ...
      details: {
        ...
        totalMappings,
        successRate:
          totalMappings > 0 ? (validMappings / totalMappings) * 100 : 100,
      },
    };
  } catch (error) {
    return {
...
function analyzeIngredientTestResults(testResults: IngredientTestResult[]): {
  ...
  for (const test of testResults) {
    if (!test.passed) {
      const severity =
        test.testName.includes("Loading") ||
        test.testName.includes("Properties")
          ? "HIGH"
          : "MEDIUM";
 ...
 */
export function shouldRollbackIngredients(
  validationResult: IngredientValidationResult,
): boolean {
 ...
 */
function determineThermalDirection(
  elementalProperties: ElementalProperties,
    ...
    for (const ingredient of ingredients) {
      ...
      const charge =
        (ingredient.elementalProperties.Earth +
          ingredient.elementalProperties.Water) /
        2; // Matter + Substance
      const potential =
        (ingredient.elementalProperties.Fire +
          ingredient.elementalProperties.Air) /
        2; // Spirit + Essence via elements
      const powerEfficiency =
        charge > 0 ? Math.min(potential / charge, 1.0) : 0.5;
      ...
      const thermalDir = determineThermalDirection(
        ingredient.elementalProperties,
      );

...
export function getRecommendedIngredients(astroState: AstrologicalState): EnhancedIngredient[] {
  ...
  let filteredIngredients = (allIngredients as EnhancedIngredient[]).filter(ingredient => {
    ...
    const rulingPlanets = astrologicalProfile?.rulingPlanets as string[] | undefined;
    return rulingPlanets?.some(planet => planetsToUse.includes(planet)) || false;
  });
  ...
  if (planetsToUse.includes('Venus')) {
    // Apply safe array type casting for function parameter compatibility
  ...
  if (astroState.dominantElement) {
    filteredIngredients.sort((a, b) => {
      ...
      const aElementalProps = aData?.elementalProperties as Record<string, number> | undefined;
      const bElementalProps = bData?.elementalProperties as Record<string, number> | undefined;
      const aValue = aElementalProps?.[astroState.dominantElement as keyof ElementalProperties] || 0;
      const bValue = bElementalProps?.[astroState.dominantElement as keyof ElementalProperties] || 0;
  ...
  if (astroState.zodiacSign) {
    ...
    filteredIngredients.sort((a, b) => {
      const aData = a as Record<string, unknown>;
      const bData = b as Record<string, unknown>;
      const aAstroProfile = aData?.astrologicalProfile as Record<string, unknown> | undefined;
      const bAstroProfile = bData?.astrologicalProfile as Record<string, unknown> | undefined;
      const aSignAffinities = aAstroProfile?.signAffinities as string[] | undefined;
      const bSignAffinities = bAstroProfile?.signAffinities as string[] | undefined;
      let aHasAffinity = aSignAffinities?.includes(zodiacSign) ? 1 : 0;
      let bHasAffinity = bSignAffinities?.includes(zodiacSign) ? 1 : 0;
      
      // Boost ingredients with Venus associations when Venus is active
      if (planetsToUse.includes('Venus')) {
        const aName = aData?.name as string | undefined;
        const bName = bData?.name as string | undefined;
        if (aName && isVenusAssociatedIngredient(aName)) aHasAffinity += venusBoost;
        if (bName && isVenusAssociatedIngredient(bName)) bHasAffinity += venusBoost;
      }
      ...
      if (planetsToUse.includes('Mars')) {
        const aName = aData?.name as string | undefined;
        const bName = bData?.name as string | undefined;
        if (aName && isMarsAssociatedIngredient(aName)) aHasAffinity += marsBoost;
        if (bName && isMarsAssociatedIngredient(bName)) bHasAffinity += marsBoost;
      }
      ...
      if (planetsToUse.includes('Mercury')) {
        const aName = aData?.name as string | undefined;
        const bName = bData?.name as string | undefined;
        if (aName && isMercuryAssociatedIngredient(aName)) aHasAffinity += mercuryBoost;
        if (bName && isMercuryAssociatedIngredient(bName)) bHasAffinity += mercuryBoost;
      }
...
export function getIngredientRecommendations(
  ...
  const scoredIngredients = allIngredients
    .filter(ingredient => {
      // Apply basic filters
      if (options.excludeIngredients?.includes((ingredient as Record<string, unknown>)?.name as string)) return false;
      if (options.includeOnly && !options.includeOnly.includes((ingredient as Record<string, unknown>)?.name as string)) return false;
      ...
      if (options.modalityPreference) {
        const ingredientModality = ingredient.modality || 
          determineIngredientModality(ingredient.qualities, (ingredient as Record<string, unknown>)?.elementalProperties as ElementalProperties);
...
function calculateSeasonalScore(ingredient: Ingredient, date: Date): number {
  ...
  // Get seasonality score for current season
      const seasonScore = (ingredient as Record<string, unknown>)?.seasonality?.[currentSeason] as number || 0.5;
...
function calculateEnhancedPlanetaryScore(
  ...
  if ((ingredient as Record<string, unknown>)?.astrologicalProfile?.rulingPlanets?.includes(rulingPlanet)) {
  ...
  Object.entries(planetaryAlignment).forEach(([planet, position]) => {
    ...
    if ((ingredient as Record<string, unknown>)?.astrologicalProfile?.rulingPlanets?.includes(planetName)) {
      score += 1;
      totalFactors += 1;
    }
    
    // Check sign affinities 
    if ((ingredient as Record<string, unknown>)?.astrologicalProfile?.signAffinities?.includes(position.sign.toLowerCase())) {
      score += 1;
      totalFactors += 1;
    }
    ...
    if (decanInfo && (ingredient as Record<string, unknown>)?.astrologicalProfile?.rulingPlanets?.includes(decanInfo.decanRuler)) {
      score += 0.8; // Good bonus for decan ruler match
      totalFactors += 0.8;
    }
    
    // Tarot card associations - add subtle influence
    if (decanInfo?.tarotCard && (ingredient as Record<string, unknown>)?.astrologicalProfile?.tarotAssociations?.includes(decanInfo.tarotCard)) {
      score += 0.7;
      totalFactors += 0.7;
    }
  });
  ...
  significantChakras.forEach(([chakra, energy]) => {
    ...
    const matchingIngredients = allIngredients.filter(ingredient => {
      ...
      const matchesNutritional = nutritionalCorrelations.some(correlation => 
        (ingredient as Record<string, unknown>)?.name?.toString().toLowerCase().includes(correlation.toLowerCase()) || 
        ((ingredient as Record<string, unknown>)?.type ? (ingredient as Record<string, unknown>).type?.toString().toLowerCase().includes(correlation.toLowerCase()) : false)
      );
      
      // Check if ingredient name matches any herb recommendation
      const matchesHerb = herbRecommendations.some(herb => 
        (ingredient as Record<string, unknown>)?.name?.toString().toLowerCase().includes(herb.toLowerCase())
      );
    ...
    matchingIngredients.forEach(ingredient => {
      const recommendationKey = (ingredient as Record<string, unknown>)?.type ? `${(ingredient as Record<string, unknown>).type?.toString().toLowerCase()}s` : 'others';
      ...
      const recommendation: IngredientRecommendation = {
        ...
        recommendations: [
          ...
          ...nutritionalCorrelations.filter(corr => 
            (ingredient as Record<string, unknown>)?.name?.toString().toLowerCase().includes(corr.toLowerCase()) ||
            ((ingredient as Record<string, unknown>)?.type ? (ingredient as Record<string, unknown>).type?.toString().toLowerCase().includes(corr.toLowerCase()) : false)
          )
        ]
...
function calculateVenusInfluence(
  ...
  if (isVenusAssociatedIngredient((ingredient as Record<string, unknown>)?.name as string)) {
    score += 2.0;
  }
  ...
  const flavorProfile = (ingredient as Record<string, unknown>)?.flavorProfile as Record<string, number>;
  if (flavorProfile) {
  ...
  const texture = (ingredient as Record<string, unknown>)?.texture as string;
  if (texture) {
  ...
  const culinaryUses = (ingredient as Record<string, unknown>)?.culinaryUses as string[];
  if (venusData.PlanetSpecific?.CulinaryTechniques && culinaryUses) {
    ...
    const harmonyPairings = (ingredient as Record<string, unknown>)?.harmonyPairings as string[];
    ...
    const aromaticProperties = (ingredient as Record<string, unknown>)?.aromaticProperties as Record<string, unknown>;
    if (
      aromaticProperties || 
  ...
  if (zodiacSign && venusData.PlanetSpecific?.ZodiacTransit) {
    ...
    const transitDataAny = transitData as unknown as Record<string, unknown>;
    const foodFocusProperty = transitDataAny?.FoodFocus as string;
    
    if (foodFocusProperty) {
      const foodFocus = foodFocusProperty.toLowerCase();
      const ingredientName = (ingredient as unknown as { name: string })?.name.toLowerCase();
      
      // Direct keywords match
      const keywords = foodFocus.split(/[\s,;]+/).filter(k => k.length > 3);
      for (const keyword of keywords) {
        const description = (ingredient as unknown as { description?: string })?.description;
        const culinaryUses = (ingredient as unknown as { culinaryUses?: string[] })?.culinaryUses;
    ...
    if (transitData.Ingredients) {
      ...
      if (transitIngredients.some(i => (ingredient as unknown as { name: string })?.name.toLowerCase().includes(i) || i.includes((ingredient as unknown as { name: string })?.name.toLowerCase()))) {
        score += 3.0;
      }
      ...
      const relatedIngredients = (ingredient as unknown as { relatedIngredients?: string[] })?.relatedIngredients;
      ...
      const complementaryIngredients = (ingredient as unknown as { complementaryIngredients?: string[] })?.complementaryIngredients;
  ...
  if (zodiacSign) {
    ...
    // Earth Venus
    if (earthSigns.includes(lowerSign) && (venusData.PlanetSpecific?.CulinaryTemperament as unknown as Record<string, unknown>)?.EarthVenus) {
      const earthVenus = (venusData.PlanetSpecific.CulinaryTemperament as unknown as Record<string, unknown>).EarthVenus as Record<string, unknown>;
      ...
      const earthVenusFoodFocus = earthVenusAny?.FoodFocus as string;
      ...
      if (earthVenusFoodFocus) {
        ...
        if (focusKeywords.some(keyword => 
            (ingredient as unknown as { name: string })?.name.toLowerCase().includes(keyword) || 
            (description && description.toLowerCase().includes(keyword)))) {
          score += 1.5;
        }
      }
      ...
      const earthVenusElements = earthVenusAny?.Elements as Record<string, number>;
    ...
    // Air Venus
    if (airSigns.includes(lowerSign) && (venusData.PlanetSpecific?.CulinaryTemperament as unknown as Record<string, unknown>)?.AirVenus) {
      const airVenus = (venusData.PlanetSpecific.CulinaryTemperament as unknown as Record<string, unknown>).AirVenus as Record<string, unknown>;
      ...
      const airVenusFoodFocus = airVenusAny?.FoodFocus as string;
      ...
      const airVenusElements = airVenusAny?.Elements as Record<string, number>;
    ...
    // Water Venus
    if (waterSigns.includes(lowerSign) && (venusData.PlanetSpecific?.CulinaryTemperament as unknown as Record<string, unknown>)?.WaterVenus) {
      const waterVenus = (venusData.PlanetSpecific.CulinaryTemperament as unknown as Record<string, unknown>).WaterVenus as Record<string, unknown>;
      ...
      const waterVenusFoodFocus = waterVenusAny?.FoodFocus as string;
      ...
      const ingredientData = ingredient as unknown as { name?: string; description?: string };
      ...
      const waterVenusElements = waterVenusAny?.Elements as Record<string, number>;
    ...
    // Fire Venus
    if (fireSigns.includes(lowerSign) && (venusData.PlanetSpecific?.CulinaryTemperament as unknown as Record<string, unknown>)?.FireVenus) {
      const fireVenus = (venusData.PlanetSpecific.CulinaryTemperament as unknown as Record<string, unknown>)?.FireVenus as Record<string, unknown>;
      ...
      const fireVenusFoodFocus = fireVenusAny?.FoodFocus as string;
      ...
      const fireVenusElements = fireVenusAny?.Elements as Record<string, number>;
  ...
  if (isVenusRetrograde && venusData.PlanetSpecific?.Retrograde) {
    ...
    const preservationMethods = (ingredient as unknown as { preservation_methods?: string[] })?.preservation_methods;
    const categories = (ingredient as unknown as { categories?: string[] })?.categories;
    if (
      (preservationMethods && preservationMethods.includes('dried')) || 
      (ingredient as unknown as { category?: string }).category === 'herb' ||
    ...
    const ingredientData = ingredient as unknown as { cultural_significance?: string | string[] };
    ...
    const retrogradeData = venusData.PlanetSpecific.Retrograde as unknown as { FoodFocus?: string; Elements?: Record<string, number> };
  ...
  const venusDataAny = venusData as unknown as { LunarConnection?: unknown };
...
function enhanceVenusIngredientScoring(
  ...
  const _zodiacSign = astroState.zodiacSign as string | undefined;
  
  // Check if Venus is retrograde
  // Extract astrological state with safe property access
  const astroStateData = astroState as unknown as { retrograde?: string[] };
...
function enhanceVenusIngredientBatch(ingredients: Ingredient[], astroState: AstrologicalState): void {
  ...
  ingredients.forEach(ingredient => {
    ...
    (ingredient as unknown as { venusScore?: number }).venusScore = venusScore;
  });
  
  // Sort ingredients by Venus score
  ingredients.sort((a, b) => {
    const aScore = (a as unknown as { venusScore?: number }).venusScore || 0;
    const bScore = (b as unknown as { venusScore?: number }).venusScore || 0;
...
function calculateMarsInfluence(
  ...
  const name = (ingredient as unknown as { name?: string })?.name?.toLowerCase();
  ...
  const ingredientData = ingredient as unknown as { flavorProfile?: Record<string, number> };
  ...
  // Elemental alignment
  if ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties) {
    // Mars is primarily Fire, secondarily Water
    const fireScore = (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Fire || 0;
    const waterScore = (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Water || 0;
  ...
  if (zodiacSign && marsData.PlanetSpecific?.ZodiacTransit) {
    ...
    if (transit && transit.Elements && (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties) {
      for (const element in transit.Elements) {
        const elemValue = element as keyof ElementalProperties;
        if ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.[elemValue]) {
          score += transit.Elements[element] * (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.[elemValue] * 1.2;
        }
      }
    }
  }
  ...
  if (isMarsRetrograde && marsData.PlanetSpecific?.Retrograde) {
    ...
    const ingredientData = ingredient as unknown as { type?: string; category?: string; traditional?: unknown };
  ...
  if ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties) {
    const fireDominant = ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Fire || 0) > 0.6;
    const waterDominant = ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Water || 0) > 0.6;
    
    // Extract Mars temperament data with safe property access
    const marsTemperament = marsData.PlanetSpecific?.CulinaryTemperament as unknown as { FireMars?: unknown; WaterMars?: unknown };
...
function enhanceMarsIngredientScoring(ingredients: Ingredient[], astroState: AstrologicalState): void {
  ...
  const _zodiacSign = astroState.zodiacSign;
  ...
  for (let i = 0; i < ingredients.length; i++) {
    ...
    // Extract ingredient data with safe property access
    const ingredientData = ingredient as unknown as { name?: string; matchScore?: number; influences?: Record<string, number> };
  ...
  ingredients.sort((a, b) => {
    const aData = a as unknown as { matchScore?: number };
    const bData = b as unknown as { matchScore?: number };
...
function calculateMercuryInfluence(
  ...
  if ((ingredient as unknown as { astrologicalProfile?: { rulingPlanets?: string[] } })?.astrologicalProfile?.rulingPlanets?.includes('Mercury')) {
  ...
  if (mercuryData.FoodAssociations) {
    for (const food of mercuryData.FoodAssociations) {
      if ((ingredient as unknown as { name?: string })?.name?.toLowerCase().includes(food.toLowerCase()) ||
          food.toLowerCase().includes((ingredient as unknown as { name?: string })?.name?.toLowerCase() || '')) {
        score += 2.0;
        break;
      }
    }
  ...
  // Extract ingredient data with safe property access for type
  const ingredientData = ingredient as unknown as { type?: string; category?: string };
  const ingredientType = ingredientData?.type || ingredientData?.category || '';
  ...
  if (mercuryData.HerbalAssociations?.Herbs && 
    ...
    for (const herb of mercuryData.HerbalAssociations.Herbs) {
      if ((ingredient as unknown as { name?: string })?.name?.toLowerCase().includes(herb.toLowerCase()) ||
          herb.toLowerCase().includes((ingredient as unknown as { name?: string })?.name?.toLowerCase() || '')) {
  ...
  if ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties) {
    ...
    score += ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Air || 0) * 2.0;
    score += ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Earth || 0) * 1.8;
  }
  ...
  if (zodiacSign) {
    ...
    if ((ingredient as unknown as { astrologicalProfile?: { signAffinities?: string[] } })?.astrologicalProfile?.signAffinities?.includes(lowerSign)) {
      score += 1.5;
    }
    ...
    if (mercuryTransit) {
      ...
      if (mercuryTransit.Ingredients && mercuryTransit.Ingredients.some(transitIngredient => 
          (ingredient as unknown as { name?: string })?.name?.toLowerCase().includes(transitIngredient.toLowerCase()) ||
          transitIngredient.toLowerCase().includes((ingredient as unknown as { name?: string })?.name?.toLowerCase() || '')
        )) {
        score += 2.5;
      }
      
      // Element alignment with Mercury in this sign
      if (mercuryTransit.Elements && (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties) {
        for (const element in mercuryTransit.Elements) {
          const elemKey = element as keyof ElementalProperties;
          if ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.[elemKey]) {
            score += mercuryTransit.Elements[element] * (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.[elemKey] * 1.2;
          }
        }
      }
    }
    ...
    if (lowerSign === 'gemini' || lowerSign === 'virgo') {
      ...
      if (lowerSign === 'gemini' && (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Air) {
        score += (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Air * 1.8;
      } else if (lowerSign === 'virgo' && (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Earth) {
        score += (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Earth * 1.8;
      }
    }
  ...
  if (isMercuryRetrograde) {
    ...
    if (mercuryData.RetrogradeEffect && (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties) {
      // Shift toward Matter and away from Spirit during retrograde
      if ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Earth) {
        score += (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Earth * Math.abs(mercuryData.RetrogradeEffect.Matter);
      }
      if ((ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Air) {
        score -= (ingredient as unknown as { elementalProperties?: Record<string, number> })?.elementalProperties?.Air * Math.abs(mercuryData.RetrogradeEffect.Spirit);
      }
    }
  }
...
function enhanceMercuryIngredientScoring(ingredients: Ingredient[], astroState: AstrologicalState): void {
  ...
  ingredients.forEach(ingredient => {
    ...
    const ingredientData = ingredient as unknown as { matchScore?: number; score?: number; mercuryAffinity?: number; scoreDetails?: Record<string, unknown> };
    ...
    if ('mercuryAffinity' in ingredient) {
      (ingredient as unknown as { mercuryAffinity?: number }).mercuryAffinity = mercuryScore;
    }
    
    // If the ingredient has a detailed score breakdown, add Mercury score
    if ('scoreDetails' in ingredient) {
      (ingredient as unknown as { scoreDetails?: Record<string, unknown> }).scoreDetails = {
        ...(ingredient as unknown as { scoreDetails?: Record<string, unknown> }).scoreDetails,
...
function calculatePlanetaryDayInfluence(
  ...
  if ((ingredient as EnhancedIngredient)?.elementalProperties) {
    diurnalMatch = (ingredient as EnhancedIngredient)?.elementalProperties[diurnalElement] || 0;
    nocturnalMatch = (ingredient as EnhancedIngredient)?.elementalProperties[nocturnalElement] || 0;
  } else {
    ...
    const ingredientData = ingredient as EnhancedIngredient;
  ...
  if ((ingredient as EnhancedIngredient)?.astrologicalProfile?.rulingPlanets?.includes(planetaryDay)) {
...
function calculatePlanetaryHourInfluence(
  ...
  // Check if ingredient has elemental properties
  if ((ingredient as EnhancedIngredient)?.elementalProperties) {
    elementalMatch = (ingredient as EnhancedIngredient)?.elementalProperties[relevantElement] || 0;
  } else {
    // Simple matching if no detailed elemental profile is available
    // Extract ingredient data with safe property access for element
    const ingredientData = ingredient as EnhancedIngredient;
  ...
  if (aspects && aspects.length > 0) {
    ...
    for (const aspect of hourAspects) {
      ...
      if ((ingredient as EnhancedIngredient)?.astrologicalProfile?.rulingPlanets?.includes(otherPlanet)) {
  ...
  // If the food has a direct planetary affinity, give bonus points
  if ((ingredient as EnhancedIngredient)?.astrologicalProfile?.rulingPlanets?.includes(planetaryHour)) {
...
export function recommendIngredients(
  ...
  if (options.category) {
    filteredIngredients = allIngredients.filter(ing => {
      ...
      const ingredientData = ing as EnhancedIngredient;
      const ingredientType = ingredientData?.type || ingredientData?.category || '';
  ...
  if (options.excludeIngredients && options.excludeIngredients.length > 0) {
    filteredIngredients = filteredIngredients.filter(ing => 
      !options.excludeIngredients?.includes((ing as EnhancedIngredient)?.name));
  }
  
  // Filter to only include specific ingredients
  if (options.includeOnly && options.includeOnly.length > 0) {
    filteredIngredients = filteredIngredients.filter(ing => 
      options.includeOnly?.includes((ing as EnhancedIngredient)?.name));
  }
  
  // Extract key astrological information
  // Extract astrological state with safe property access
  const astroStateData = astroState as AstrologicalState;
  ...
  const _Earth = astroStateData?.Earth || 0.5;
  const _zodiacSign = astroStateData?.zodiacSign || '';
  const planetaryAlignment = astroStateData?.planetaryAlignment || {};
  const aspects = astroStateData?.aspects || [];
  const _lunarPhase = astroStateData?.lunarPhase || '';
  ...
  for (const ingredient of filteredIngredients) {
    ...
    const elementalScore = calculateElementalScore((ingredient as EnhancedIngredient)?.elementalProperties, systemElementalProps);
...
function generateRecommendationsForIngredient(
  ...
  if (planetaryElements[planetaryDay]) {
    const dayElements = planetaryElements[planetaryDay];
    recs.push(`${(ingredient as EnhancedIngredient)?.name} works well on ${planetaryDay}'s day with its ${dayElements.diurnal} and ${dayElements.nocturnal} influences.`);
  }
  ...
  if (planetaryElements[planetaryHour]) {
    ...
    recs.push(`During the current hour of ${planetaryHour}, ${(ingredient as EnhancedIngredient)?.name}'s ${hourElement} properties are enhanced.`);
  }
  ...
  if (planetaryPositions) {
    ...
    if (planetaryElements[planetaryDay]?.dignityEffect && 
      ...
      if (dayDignity && dayDignity > 0 && (ingredient as EnhancedIngredient)?.astrologicalProfile?.rulingPlanets?.includes(planetaryDay)) {
        recs.push(`${planetaryDay} is ${dayDignity > 1 ? 'exalted' : 'dignified'} in ${daySign}, strengthening ${(ingredient as EnhancedIngredient)?.name}'s properties.`);
      } else if (dayDignity && dayDignity < 0 && (ingredient as EnhancedIngredient)?.astrologicalProfile?.rulingPlanets?.includes(planetaryDay)) {
        recs.push(`${planetaryDay} is ${dayDignity < -1 ? 'in fall' : 'in detriment'} in ${daySign}, requiring careful preparation of ${(ingredient as EnhancedIngredient)?.name}.`);
      }
    }
    ...
    if (planetaryElements[planetaryHour]?.dignityEffect && 
      ...
      if (hourDignity && hourDignity > 0 && (ingredient as EnhancedIngredient)?.astrologicalProfile?.rulingPlanets?.includes(planetaryHour)) {
        recs.push(`During this hour, ${planetaryHour}'s dignity in ${hourSign} enhances ${(ingredient as EnhancedIngredient)?.name}'s flavor profile.`);
      }
    }
  }
    ...
    for (const aspect of relevantAspects) {
      if (aspect.aspectType === 'Conjunction') {
        ...
        if (planets.includes(planetaryDay) || planets.includes(planetaryHour)) {
          ...
          if (otherPlanet && (ingredient as EnhancedIngredient)?.astrologicalProfile?.rulingPlanets?.includes(otherPlanet)) {
            recs.push(`The conjunction between ${aspect.planet1} and ${aspect.planet2} strongly enhances ${(ingredient as EnhancedIngredient)?.name}'s qualities.`);
          }
        }
        ...
        if (planets.includes(planetaryDay) || planets.includes(planetaryHour)) {
          const otherPlanet = planets.find(p => p !== planetaryDay && p !== planetaryHour);
          if (otherPlanet && (ingredient as EnhancedIngredient)?.astrologicalProfile?.rulingPlanets?.includes(otherPlanet)) {
            recs.push(`The harmonious trine between ${aspect.planet1} and ${aspect.planet2} creates a flowing energy for ${(ingredient as EnhancedIngredient)?.name}.`);
          }
        }
      }
    }
  ...
  if ((ingredient as EnhancedIngredient)?.astrologicalProfile?.rulingPlanets) {
    const rulingPlanets = (ingredient as EnhancedIngredient)?.astrologicalProfile.rulingPlanets;
    if (rulingPlanets.includes(planetaryDay)) {
      recs.push(`${(ingredient as EnhancedIngredient)?.name} is especially potent today as it's ruled by ${planetaryDay}.`);
    }
    if (rulingPlanets.includes(planetaryHour)) {
      recs.push(`This is an optimal hour to work with ${(ingredient as EnhancedIngredient)?.name} due to ${planetaryHour}'s influence.`);
    }
  }

    ...
    Object.entries(data || {}).forEach(([planet, position]) => {
      if (typeof position === 'object' && position !== null && 'sign' in position) {
        positions[planet?.toLowerCase()] = {
          sign: (position as Record<string, any>).sign?.toLowerCase() as ZodiacSign,
          degree: typeof (position as Record<string, any>).degree === 'number' ? (position as Record<string, any>).degree : 0,
          exactLongitude: typeof (position as Record<string, any>).exactLongitude === 'number' ? (position as Record<string, any>).exactLongitude : 0,
          isRetrograde: !!(position as Record<string, any>).isRetrograde
        };
      }
    });

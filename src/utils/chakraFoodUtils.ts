...
const FOOD_ITEMS_BY_GROUP: Record<string, string[]> = {
    'Root Vegetables': ['Sweet Potatoes', 'Carrots', 'Beets', 'Radishes', 'Turnips', 'Parsnips', 'Ginger'],
    'Red Fruits': ['Apples', 'Strawberries', 'Raspberries', 'Tomatoes', 'Red Bell Peppers', 'Pomegranates', 'Cherries'],
    ...
    'Teas': ['Green Tea', 'Chamomile', 'Mint Tea', 'Rooibos', 'Lavender Tea', 'Rose Tea', 'Jasmine Tea'],
    'Blue Foods': ['Blueberries', 'Blackberries', 'Blue Corn', 'Blue Potatoes', 'Elderberries'],
    'Purple Foods': ['Eggplant', 'Purple Grapes', 'Plums', 'Acai Berries', 'Purple Cabbage', 'Purple Kale'],
    'Omega-rich Foods': ['Salmon', 'Mackerel', 'Sardines', 'Walnuts', 'Flax Seeds', 'Chia Seeds', 'Hemp Seeds'],
    'White Foods': ['Cauliflower', 'Garlic', 'Onions', 'Coconut', 'White Beans', 'Daikon Radish', 'Mushrooms']
};
 ...
 */
export function getFoodRecommendationsFromChakras(
    ...
    const chakrasToUse: Chakra[] = underactiveChakras.length > 0 
        ? underactiveChakras 
        : ['Root', 'Heart', 'Crown'];
    ...
    const supportingFoodGroups = Object.entries(FOOD_GROUP_CHAKRA_MAP)
        .filter(([_, chakras]) => 
            chakras.some(chakra => chakrasToUse.includes(chakra))
        )
        .map(([group]) => group);
    
    // Get food items from supporting food groups - ensure we have items
    let primaryFoods = supportingFoodGroups.flatMap(group => 
        FOOD_ITEMS_BY_GROUP[group] || []
    );
    ...
    if (primaryFoods.length === 0) {
        primaryFoods = [
            ...
            'Blueberries', 'Walnuts', 'Quinoa', 'Salmon'
        ];
    }
    ...
    const secondaryFoodGroups = Object.entries(FOOD_GROUP_CHAKRA_MAP)
        .filter(([_, chakras]) => 
            chakras.some(chakra => neutralChakras.includes(chakra))
        )
        .map(([group]) => group);
    
    // Get secondary food items - ensure we have items
    let secondaryFoods = secondaryFoodGroups.flatMap(group => 
        FOOD_ITEMS_BY_GROUP[group] || []
    ...
    if (secondaryFoods.length === 0) {
        secondaryFoods = [
            'Broccoli', 'Green Tea', 'Oranges', 'Brown Rice',
            'Bananas', 'Olive Oil', 'Garlic', 'Ginger'
        ];
    }
    ...
    const avoidFoodGroups = Object.entries(FOOD_GROUP_CHAKRA_MAP)
        .filter(([_, chakras]) => 
            chakras.some(chakra => overactiveChakras.includes(chakra))
        )
        .map(([group]) => group);
    
    // Get foods to avoid - ensure we have items
    let avoidFoods = avoidFoodGroups.flatMap(group => 
        FOOD_ITEMS_BY_GROUP[group] || []
...
export function getZodiacSignFoodRecommendations(zodiacSign: ZodiacSign): string[] {
    ...
    const specificFoods = foodGroups.flatMap(group => 
        FOOD_ITEMS_BY_GROUP[group] || []
    );

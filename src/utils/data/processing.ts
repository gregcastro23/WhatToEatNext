...
export function standardizeIngredient(ingredient: unknown): Ingredient {
  ...
  return {
    ...
    qualities: Array.isArray(raw.qualities)
      ...
      : [],
    cookingMethods: Array.isArray(raw.cookingMethods)
      ? raw.cookingMethods || [].map(String)
      : [],
  ...
  if (recipe.ingredients) {
    recipe.ingredients.forEach((ingredient, index) => {
      ...
      if (!ingredientValidation.isValid) {
        errors.push(
          `Ingredient ${index + 1}: ${ingredientValidation.errors.join(", ")}`,
        );
      }
    });
  }
      ...
      try {
        ...
        if (validation.isValid) {
          ...
          result.warnings.push(
            `Ingredient ${index}: ${validation.errors.join(", ")}`,
          );
        }
...
function standardizeElementalProperties(
  properties: unknown,
): ElementalProperties {
...
function _standardizeAstrologicalProfile(
  profile: unknown,
): AstrologicalProfile {
...
function standardizeNutritionalProfile(
  profile: unknown,
...
function standardizeRecipeIngredients(
  ingredients: unknown,
): RecipeIngredient[] {
...
function standardizeNutritionalInfo(
  info: unknown,
): { [key: string]: unknown } | undefined {
...
function validateElementalProperties(
  ...
  (requiredElements || []).forEach((element) => {
    if (
      typeof properties[element as "Fire" | "Water" | "Earth" | "Air"] !==
      "number"
    ) {
...
function validateAstrologicalProfile(

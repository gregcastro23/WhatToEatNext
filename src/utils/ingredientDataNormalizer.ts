...
export function formatMineralName(name: string): string {
  ...
  return (
    mineralMap[mineralName] || name.charAt(0).toUpperCase() + name.slice(1)
  );
}
...
export function normalizeVarieties(
  ...
  Object.entries(varieties).forEach(([variety, data]) => {
    normalized[formatVarietyName(variety)] = normalizeVarietyData(
      data as Record<string, unknown>,
    );
  });
  ...
  const normalized = {
    ...
    culinaryApplications:
      ...
      ingredient.culinaryApplications !== null
        ? normalizeCulinaryApplications(
            ingredient.culinaryApplications as Record<string, unknown>,
          )
        : {},
    varieties:
    ...
    storage:
      typeof ingredient.storage === "object" && ingredient.storage !== null
        ...
        : {},
    preparation:
      typeof ingredient.preparation === "object" &&
      ingredient.preparation !== null
        ? normalizePreparation(
            ingredient.preparation as Record<string, unknown>,
          )
        : {},
  };

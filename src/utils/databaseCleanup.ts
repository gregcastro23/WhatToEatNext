    ...
    Object.entries(allIngredients).forEach(([category, ingredients]) => {
      ...
      ingredients.forEach((ingredient, index) => {
        ...
        if (!ingredientWithAstrology.name) {
          ...
          fixedEntries++;
          logger.warn(
            `Added missing name to ingredient at ${category}[${index}]`
          );
        }
        ...
        if (!ingredientWithAstrology.elementalProperties) {
          ...
          fixedEntries++;
          logger.warn(
            `Added default elemental properties to ${ingredientWithAstrology.name}`
          );
        } else {
          ...
          elements.forEach((element) => {
            if (
              typeof ingredientWithAstrology.elementalProperties?.[element] !==
              'number'
            ) {
              if (ingredientWithAstrology.elementalProperties) {
          ...
          if (Math.abs(sum - 1) > 0.01) {
            elements.forEach((element) => {
              if (ingredientWithAstrology.elementalProperties) {
                ingredientWithAstrology.elementalProperties[element] =
                  ingredientWithAstrology.elementalProperties[element] / (sum || 1);
              }
            });
            modified = true;
          }
          ...
          if (modified) {
            fixedEntries++;
            logger.debug(
              `Normalized elemental properties for ${ingredientWithAstrology.name}`
            );
          }
        }
        ...
        if (!ingredientWithAstrology.astrologicalProfile) {
          ...
          fixedEntries++;
          logger.warn(
            `Added default astrological profile to ${ingredientWithAstrology.name}`
          );
        } else if (
          ...
          fixedEntries++;
          logger.warn(
            `Added elementalAffinity to astrological profile for ${ingredientWithAstrology.name}`
          );
        }

 ...
 *
 * Abstract points (nodes, MC, Chiron, ...) carry no mass and are gated via
 * `isExcludedAspectBody`. Planetary contributions are weighted by inertial mass
 * (`inertialMassWeight`), pinning the Ascendant to 1.0 and anchoring one decade
 * below the lightest charted body.
 *
 * @param planetaryPositions - Map of planet names to their zodiac signs
...
export function aggregateZodiacElementals(planetaryPositions: {
  ...
  const totals: Record<AlchemicalElement, number> = { Fire: 0, Water: 0, Earth: 0, Air: 0 };
  let count = 0;
 ...
 * Excluded bodies carry no mass and are filtered with `isExcludedAspectBody`.
 */

...
> = {
  new_moon: {
    strength: 0.3,
    elements: { Fire: 0.1, Water: 0.1, Air: 0.1, Earth: 0.1 },
  },
  waxing_crescent: {
    strength: 0.2,
    elements: { Fire: 0.2, Air: 0.1, Water: 0.05, Earth: 0.05 },
  },
  first_quarter: {
    strength: 0.3,
    elements: { Fire: 0.3, Air: 0.2, Water: 0.05, Earth: 0.05 },
  },
  waxing_gibbous: {
    strength: 0.4,
    elements: { Fire: 0.4, Air: 0.3, Water: 0.05, Earth: 0.05 },
  },
  full_moon: {
    strength: 0.5,
    elements: { Water: 0.4, Earth: 0.3, Fire: 0.1, Air: 0.1 },
  },
  waning_gibbous: {
    strength: 0.4,
    elements: { Water: 0.3, Earth: 0.2, Fire: 0.1, Air: 0.1 },
  },
  last_quarter: {
    strength: 0.3,
    elements: { Water: 0.2, Earth: 0.1, Fire: 0.1, Air: 0.1 },
  },
  waning_crescent: {
    strength: 0.2,
    elements: { Water: 0.1, Earth: 0.1, Fire: 0.05, Air: 0.05 },
  },
};
 ...
 */
export function applyLunarInfluence(
  ...
  return {
    Fire:
      baseBalance.Fire *
      (1 + influence.strength * ELEMENTAL_BASE_MODIFIERS.Fire),
    Water:
      baseBalance.Water *
      (1 + influence.strength * ELEMENTAL_BASE_MODIFIERS.Water),
    Air:
      baseBalance.Air * (1 + influence.strength * ELEMENTAL_BASE_MODIFIERS.Air),
    Earth:
      baseBalance.Earth *
      (1 + influence.strength * ELEMENTAL_BASE_MODIFIERS.Earth),
  };
}
 ...
 */
export function getLunarElementalModifiers(
  phase: LunarPhase,
): Record<string, number> {
  const phaseKey = getLunarPhaseKey(phase);
  return (
    SIMPLE_LUNAR_INFLUENCES[phaseKey].elements || {
      Fire: 0,
      Water: 0,
      Air: 0,
      Earth: 0,
...
export const getLunarPhaseKey = (phase: string): LunarPhaseKey => {
  ...
  if (phase.includes("_")) {
    return (Object.keys(REVERSE_LUNAR_PHASE_MAP) as LunarPhaseKey[]).includes(
  ...
  return (
    LUNAR_PHASE_MAP[(phase as LunarPhase) || "new moon"] ||
    (phase.replace(/\s+/g, "_") as LunarPhaseKey)
  );
};
...
export const formatLunarPhase = (phase: string): LunarPhase => {
  ...
  if (!phase.includes("_")) {
    return (Object.keys(LUNAR_PHASE_MAP) as LunarPhase[]).includes(
      phase as LunarPhase,
    )
      ? (phase as LunarPhase)
      : "new moon";
  ...
  return (
    REVERSE_LUNAR_PHASE_MAP[(phase as LunarPhaseKey) || "new_moon"] ||
    (phase.replace(/_/g, " ") as LunarPhase)
  );
};
 ...
 */
export function toTitleCaseLunarPhase(
...
export const LUNAR_PHASE_ELEMENTS: Record<
  LunarPhaseWithUnderscores,
  ElementalProperties
> = {
 ...
 */
export function convertToUnderscoreFormat(
  phase: LunarPhaseWithSpaces,
): LunarPhaseWithUnderscores {
 ...
 */
export function convertToSpacesFormat(
  phase: LunarPhaseWithUnderscores,
 ...
 */
export function normalizeLunarPhase(
  phase: string | null | undefined,
): LunarPhase | undefined {
  if (!phase) return undefined;
  const cleanPhase = phase.toLowerCase().trim();
  if (
    (Object.keys(LUNAR_PHASE_MAP) as LunarPhase[]).includes(
      cleanPhase as LunarPhase,
    )
  ) {
 ...
 */
function applyVelocityBoost(
  phase: LunarPhase,
  _velocityBoost: number,
): LunarPhase {
  return phase;
}
 ...
 */
function getDominantElementFromModifiers(
...
export function calculateLunarKineticsMetrics(
  ...
  try {
    ...
    const thermalAlignment = calculateThermalAlignment(
      phase,
      String(kinetics.thermalDirection || "neutral"),
    );
    return {
      phaseVelocity,
      aspectInfluence,
 ...
 */
function calculateThermalAlignment(
  phase: LunarPhase,

...
> = {
  ...
  WAXING_CRESCENT: {
    strength: 0.2,
    elements: { Fire: 0.2, Air: 0.1, Water: 0.0, Earth: 0.0 },
  },
  FIRST_QUARTER: {
    strength: 0.3,
    elements: { Fire: 0.3, Air: 0.2, Water: 0.0, Earth: 0.0 },
  },
  WAXING_GIBBOUS: {
    strength: 0.4,
    elements: { Fire: 0.4, Air: 0.3, Water: 0.0, Earth: 0.0 },
  },
  FULL_MOON: {
    strength: 0.5,
    elements: { Water: 0.4, Earth: 0.3, Fire: 0.0, Air: 0.0 },
  },
  WANING_GIBBOUS: {
    strength: 0.4,
    elements: { Water: 0.3, Earth: 0.2, Fire: 0.0, Air: 0.0 },
  },
  LAST_QUARTER: {
    strength: 0.3,
    elements: { Water: 0.2, Earth: 0.1, Fire: 0.0, Air: 0.0 },
  },
  WANING_CRESCENT: {
    strength: 0.2,
    elements: { Water: 0.1, Earth: 0.1, Fire: 0.0, Air: 0.0 },
  },
};
...
export function applyLunarInfluence(
  ...
  return {
    ...
    Water:
      baseBalance.Water * (1 + influence.strength * elementalModifiers.Water),
    Air: baseBalance.Air * (1 + influence.strength * elementalModifiers.Air),
    Earth:
      baseBalance.Earth * (1 + influence.strength * elementalModifiers.Earth),
  };
}
...
export function generateDefaultLunarPhaseModifiers(
  ...
  if (dominantElement === 'Fire') {
    lunarModifiers.waxingGibbous = {
      ...
      preparationTips: [
        'Excellent for cooking with heat',
        'Good for spicy preparations',
      ],
    };
    ...
    lunarModifiers.firstQuarter = {
      ...
      preparationTips: [
        'Perfect for aromatic preparations',
        'Enhances subtle flavors',
      ],
    };
  }
...
export const FOOD_TO_ALCHEMY_LUNAR_PHASE_MAP: Record<
...
export let getLunarPhaseKey = (phase: string): LunarPhaseKey => {
  ...
  if (phase.includes('_')) {
    return isValidUnderscorePhase(phase)
  ...
  return (
    LUNAR_PHASE_MAP[phase as LunarPhase] ||
...
export let formatLunarPhase = (phase: string): LunarPhase => {
  ...
  return (
    REVERSE_LUNAR_PHASE_MAP[phase as LunarPhaseKey] ||
    (phase.replace(/_ / (g || 1), ' ') as LunarPhase)
  );
};

...
export function processNaturalLanguageQuery(query: string): SearchIntent {
  ...
  for (const pattern of ALL_PATTERNS) {
    for (const keyword of pattern.keywords) {
      ...
      if (similarity > 0.7 || containsKeyword) {
        const confidence = containsKeyword
          ? pattern.weight
  ...
  const overallConfidence =
    matchCount > 0 ? Math.min(totalConfidence / matchCount, 1) : 0;
...
export function applyFilters(
  ...
  return items.filter((item) => {
    ...
    if (filters.cuisineTypes.length > 0) {
      ...
      const cuisineType = hasProperty(item, "cuisineType")
        ? item.cuisineType
        : null;
      const itemCuisine = (
      ...
      if (
        !filters.cuisineTypes.some((cuisine) => itemCuisine.includes(cuisine))
      )
        return false;
    }
...
function generateKineticsFilters(
  ...
  if (
    keywords.includes("hot") ||
    keywords.includes("spicy") ||
    keywords.includes("warm")
  ) {
    filters.thermalAlignment = "heating";
  } else if (
    keywords.includes("cold") ||
    keywords.includes("cool") ||
    keywords.includes("mild")
  ) {
...
export function filterItemsWithKinetics(
  ...
  filters: SearchFilters & {
    kineticsFilters?: KineticsAwareSearchIntent["kineticsFilters"];
  },
): SearchableItem[] {
  ...
  return items.filter((item) => {
    ...
    if (kineticsFilters.preferredForceType) {
      ...
      if (itemForceType !== kineticsFilters.preferredForceType) {
        ...
        if (
          kineticsFilters.preferredForceType !== "stable" ||
    ...
    if (kineticsFilters.powerRange) {
      ...
      if (
        ...
        itemPower > kineticsFilters.powerRange[1]
      ) {
        return false;
      }
    }
 ...
 */
function determineItemForceType(
  item: SearchableItem,
 ...
 */
function determineItemThermalType(
  item: SearchableItem,

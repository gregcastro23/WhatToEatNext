...
export class ChakraService {
   ...
   */
  public calculateChakraEnergyStates(
    ...
    signEnergyStates.forEach((_state) => {
      ...
      if (typeof sign === "string" && sign in signEnergyRecord) {
        ...
        signEnergyRecord[sign as ZodiacSignType] = _state.currentEnergy;
      }
    });
   ...
   */
  public getFoodRecommendations(
    chakraEnergyStates: ChakraEnergyState[],
  ): Record<Chakra, string[]> {
    const recommendations: Record<Chakra, string[]> = {} as Record<
      Chakra,
      string[]
    >;
  ...
  public suggestDietaryAdjustments(
    ...
    if (state) {
      ...
      if (state.moonPhase === "full moon") {
        suggestions.push(
          "During full moon, focus on purifying foods like lemons and ginger",
        );
      } else if (state.moonPhase === "new moon") {
        suggestions.push(
          "During new moon, grounding foods like root vegetables are beneficial",
        );
      }
      ...
      if (
        state.sunSign === "aries" ||
        state.sunSign === "leo" ||
        state.sunSign === "sagittarius"
      ) {
        suggestions.push(
          "Fire signs benefit from cooling foods to balance intensity",
        );
      } else if (
        ...
        suggestions.push(
          "Water signs benefit from warming spices to energize and ground",
        );
      }
    }
    ...
    if (underactiveChakras.length > 3) {
      suggestions.push(
        "Consider a rainbow diet with foods of all colors to balance all chakras",
      );
    }

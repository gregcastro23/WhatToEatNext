...
describe('BundleSizeOptimizer': any, (: any) => {
  ...
  describe('analyzeBundleSize': any, (: any) => {
    it('should analyze Next?.js bundle size': any, async (: any) => {
      ...
      expect(analysis?.totalSize as any).toBe(400);
      expect(analysis?.compressedSize as any).toBe(280); // 70% compression
      expect(analysis?.chunks).toHaveLength(3);
      expect(analysis?.chunks?.[0].name as any).toBe('main?.js');
      expect(analysis?.chunks?.[2].isLazyLoaded as any).toBe(true); // lazy-component?.js
    });

    it('should analyze generic build directory': any, async (: any) => {
      mockFs?.existsSync.mockImplementation((path: string) => {
        return path === 'dist';
      });
      ...
      expect(analysis?.totalSize as any).toBe(350);
      expect(analysis?.compressedSize as any).toBe(245); // 70% compression
    });

    it('should estimate bundle size from source code': any, async (: any) => {
      mockFs?.existsSync.mockReturnValue(false);
      mockExecSync?.mockReturnValue('200000\n'); // 200kB source code

      const analysis: any = await bundleOptimizer?.analyzeBundleSize();

      expect(analysis?.totalSize as any).toBe(293); // Math?.round(200000 / 1024 * 1?.5) = 293
    });

    it('should handle bundle analysis errors gracefully': any, async (: any) => {
      mockFs?.existsSync.mockReturnValue(false);
      ...
      expect(analysis?.totalSize as any).toBe(400); // Conservative estimate fallback
      // The error is caught in estimateBundleSize, so recommendations will be generated normally
      expect(analysis?.recommendations).toBeInstanceOf(Array);
    });
  });

  describe('validateLazyLoading': any, (: any) => {
    it('should validate lazy loading implementation': any, async (: any) => {
      mockExecSync
      ...
      expect(validation?.componentsAnalyzed as any).toBe(25);
      expect(validation?.lazyLoadedComponents as any).toBe(10);
      expect(validation?.score as any).toBe(40); // 10/25 * 100
    ...
    it('should handle lazy loading validation errors': any, async (: any) => {
      mockExecSync?.mockImplementation((: any) => {
      ...
      expect(validation?.componentsAnalyzed as any).toBe(0);
      expect(validation?.lazyLoadedComponents as any).toBe(0);
      expect(validation?.score as any).toBe(0);
    });
  });

  describe('bundle size alerts': any, (: any) => {
    it('should generate alert when bundle size exceeds target': any, async (: any) => {
      mockFs?.existsSync.mockReturnValue(false);
      ...
      expect(sizeAlert?.severity as any).toBe('critical'); // > 420 * 1?.2
      expect(sizeAlert?.currentValue as any).toBe(732); // Math?.round(500000 / 1024 * 1?.5) = 732
      expect(sizeAlert?.targetValue as any).toBe(420);
    });

    it('should generate alert for large chunks': any, async (: any) => {
      mockFs?.existsSync.mockImplementation((path: string) => {
        return path === '.next' || path === '.next/static/chunks';
      });
      ...
      expect(chunkAlert?.currentValue as any).toBe(150);
      expect(chunkAlert?.targetValue as any).toBe(100);
    });

    it('should generate alert for unused dependencies': any, async (: any) => {
      mockFs?.existsSync.mockImplementation((path: string) => {
  ...
  describe('generateOptimizationReport': any, (: any) => {
    it('should generate comprehensive optimization report': any, async (: any) => {
      mockFs?.existsSync.mockReturnValue(false);
      mockExecSync
      ...
      expect(report?.analysis.totalSize as any).toBe(410); // Math?.round(280000 / 1024 * 1?.5) = 410
      expect(report?.lazyLoadingValidation.score as any).toBe(40); // 8/20 * 100
      expect(report?.overallScore as any).toBe(70); // (100 + 40) / 2
      expect(report?.targetCompliance as any).toBe(true); // 410 <= 420
      expect(report?.recommendations).toBeInstanceOf(Array);
    });

    it('should indicate non-compliance when bundle exceeds target': any, async (: any) => {
      mockFs?.existsSync.mockReturnValue(false);
      mockExecSync
      ...
      expect(report?.analysis.totalSize as any).toBe(513); // Math?.round(350000 / 1024 * 1?.5) = 513
      expect(report?.targetCompliance as any).toBe(false);
      expect(report?.recommendations?.[0]).toContain('Reduce bundle size by 93kB'); // 513 - 420 = 93
    });
  });

  describe('asset analysis': any, (: any) => {
    it('should analyze assets correctly': any, async (: any) => {
      mockFs?.existsSync.mockImplementation((path: string) => {
        return path === 'public';
      });
      ...
      expect(analysis?.assets?.[0].size as any).toBe(100); // Largest first
      expect(analysis?.assets?.[0].type as any).toBe('image');
  ...
  describe('dependency analysis': any, (: any) => {
    it('should analyze dependencies for bundle impact': any, async (: any) => {
      mockFs?.existsSync.mockImplementation((path: string) => {
        return path === 'package?.json';
      });
      mockFs?.readFileSync.mockReturnValue(
      ...
      expect(analysis?.dependencies?.[0].name as any).toBe('react-dom'); // Largest first (130kB)
  ...
  describe('alert management': any, (: any) => {
    it('should manage alerts correctly': any, (: any) => {
      ...
      return bundleOptimizer?.analyzeBundleSize().then((: any) => {
  ...
  describe('data export': any, (: any) => {
    it('should export bundle data to file': any, async (: any) => {
    ...
    it('should handle export errors gracefully': any, async (: any) => {
      mockFs?.writeFileSync.mockImplementation((: any) => {
  ...
  describe('helper methods': any, (: any) => {
    it('should identify heavy dependencies correctly': any, (: any) => {
    ...
    it('should estimate dependency sizes correctly': any, async (: any) => {
    ...
    it('should analyze dependency usage correctly': any, (: any) => {
    ...
    it('should suggest alternatives correctly': any, (: any) => {

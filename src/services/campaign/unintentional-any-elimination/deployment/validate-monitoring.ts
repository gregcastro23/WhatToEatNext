...
function displayResults(results: ValidationResult[]): void {
  ...
  // // // _logger.info('\n' + '='.repeat(80))
  // // // _logger.info(`SUMMARY: ${passCount} passed, ${failCount} failed, ${warningCount} warnings`)

  if (failCount > 0) {
    // // // _logger.info('\n❌ Monitoring validation FAILED')
    // // // _logger.info('Please fix the issues above before proceeding with deployment.')
    process.exit(1)
  } else if (warningCount > 0) {
    // // // _logger.info('\n⚠️  Monitoring validation passed with WARNINGS')
    // // // _logger.info('Consider addressing the warnings for optimal monitoring.')
  } else {
    // // // _logger.info('\n✅ Monitoring validation PASSED')
    // // // _logger.info('All monitoring components are properly configured.')
  }
}
...
async function main(): Promise<void> {
  // // // _logger.info('Validating monitoring and alerting setup...\n')

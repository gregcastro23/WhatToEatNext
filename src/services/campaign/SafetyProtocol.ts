...
export class SafetyProtocol {
  ...
  async createStash(description: string, phase?: string): Promise<string> {
    try {
      ...
      this.addSafetyEvent({
        ...
        timestamp: new Date(),
        description: `Failed to create git stash: ${error.message}`,
        severity: SafetyEventSeverity.ERROR,
        action: 'STASH_FAILED'
      });

      throw new Error(`Failed to create git stash: ${error.message}`);
    }
  }
  ...
  async applyStash(stashId: string, validateAfter: boolean = true): Promise<void> {
    try {
      ...
      this.addSafetyEvent({
        type: SafetyEventType.EMERGENCY_RECOVERY,
        timestamp: new Date(),
        description: `Failed to apply git stash ${stashId}: ${error.message}`,
        severity: SafetyEventSeverity.ERROR,
        action: 'STASH_APPLY_FAILED'
      });

      throw new Error(`Failed to apply git stash ${stashId}: ${error.message}`);
    }
  }
  ...
  async detectImportExportCorruption(files: string[]): Promise<CorruptionReport> {
    ...
    for (const filePath of files) {
      ...
      try {
        ...
        console.error(`❌ Error analyzing import/export corruption in ${filePath}: ${error.message}`);
      }
    }
  ...
  async startRealTimeMonitoring(files: string[], intervalMs: number = 5000): Promise<void> {
    ...
    const monitoringInterval = setInterval(async () => {
      try {
        ...
        console.error(`❌ Error during real-time monitoring: ${error.message}`);
      }
    }, intervalMs);
  ...
  async emergencyRollback(): Promise<void> {
    try {
      ...
      this.addSafetyEvent({
        type: SafetyEventType.EMERGENCY_RECOVERY,
        timestamp: new Date(),
        description: `Emergency rollback failed: ${error.message}`,
      ...
      try {
        ...
        if (stash?.ref) {
          ...
          try {
            ...
            console.warn(`⚠️ Could not drop git stash ${stash.ref}: ${gitError.message}`);
          }
        }
        ...
        console.warn(`⚠️ Failed to cleanup stash ${stashId}: ${error.message}`);
      }
    }
  ...
  private initializeStashTracking(): void {
    try {
      ...
      console.warn(`⚠️ Could not load stash tracking: ${error.message}`);
      this.stashCounter = 0;
    }
  }
  ...
  private saveStashTracking(): void {
    try {
      ...
      console.warn(`⚠️ Could not save stash tracking: ${error.message}`);
    }
  }
  ...
  private async findStashByMessage(message: string): Promise<string> {
    try {
      ...
      throw new Error(`Failed to find stash by message: ${error.message}`);
    }
  }
}
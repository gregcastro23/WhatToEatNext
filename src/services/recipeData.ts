...
function validateAndNormalizeIngredients(
  ...
  return ingredients.map((ing) => ({
    ...
    elementalProperties: ing.elementalProperties
      ? recipeElementalService.standardizeRecipe({
          elementalProperties: ing.elementalProperties,
        }).elementalProperties
      : undefined,
  }));
  ...
  private async loadRecipeData(): Promise<void> {
    try {
      ...
      this.recipes = mappingsEntries.map((mapping: unknown) => {
        let elementalProps =
          mapping.elementalProperties || mapping.elementalProfile;
        ...
        const partialRecipe: Partial<Recipe> = {
          ...
          description:
            mapping.description || mapping.cuisine?.description || '',
          ...
          ingredients: Array.isArray(mapping.ingredients)
            ...
            : [],
          instructions: Array.isArray(mapping.instructions)

...
export class CampaignConflictResolver {
   ...
   */
  private async detectResourceConflicts(campaigns: KiroCampaignStatus[]): Promise<CampaignConflict[]> {
   ...
   */
  private async detectDependencyViolations(campaigns: KiroCampaignStatus[]): Promise<CampaignConflict[]> {
    const conflicts: CampaignConflict[] = [];
    ...
    for (const dependency of this.dependencies.values()) {
      if (dependency.status === DependencyStatus.ACTIVE) {
        ...
        // Check for dependency violations
        if (dependentActive && !requiredActive && dependency.dependencyType === DependencyType.PREREQUISITE) {
        ...
        if (dependentActive && requiredActive && dependency.dependencyType === DependencyType.MUTUAL_EXCLUSION) {
   ...
   */
  private async detectPriorityConflicts(campaigns: KiroCampaignStatus[]): Promise<CampaignConflict[]> {
    const conflicts: CampaignConflict[] = [];
      ...
      for (const lowerPriority of lowerPriorities) {
        if (this.campaignsConflict(highPriority.campaign, lowerPriority.campaign)) {
          ...
          conflicts.push({
            ...
            involvedCampaigns: [highPriority.campaign.campaignId, lowerPriority.campaign.campaignId],
            conflictingResources: [],
            detectedAt: new Date(),
            status: ConflictStatus.DETECTED
          });
   ...
   */
  private async detectSafetyViolations(campaigns: KiroCampaignStatus[]): Promise<CampaignConflict[]> {
    const conflicts: CampaignConflict[] = [];
   ...
   */
  async resolveConflict(conflictId: string, strategyId?: string): Promise<ConflictResolutionResult> {
   ...
   */
  setCampaignPriority(campaignId: string, priority: number, reason: string, setBy: string = 'system'): void {
  ...
  private async getActiveCampaigns(): Promise<KiroCampaignStatus[]> {
    ...
    return controlPanel.activeCampaigns.filter(c => 
  ...
  private selectOptimalResolutionStrategy(conflict: CampaignConflict): ConflictResolutionStrategy | null {
  ...
  private findResolutionStrategy(conflictType: ConflictType, strategyId: string): ConflictResolutionStrategy | null {
  ...
  private createRollbackPlan(conflict: CampaignConflict, strategy: ConflictResolutionStrategy): RollbackPlan {
  ...
  private prioritizeRequests(requests: CampaignExecutionRequest[]): CampaignExecutionRequest[] {
    return requests.sort((a, b) => {
      const priorityA = a.safetyLevel === 'aggressive' ? 8 : 
                      a.safetyLevel === 'standard' ? 5 : 3;
      const priorityB = b.safetyLevel === 'aggressive' ? 8 : 
                      b.safetyLevel === 'standard' ? 5 : 3;
  ...
  private requestsConflict(request1: CampaignExecutionRequest, request2: CampaignExecutionRequest): boolean {

...
import type {
    CookingMethodRecommendationCriteria, CuisineRecommendationCriteria, IngredientRecommendationCriteria, RecipeRecommendationCriteria, RecommendationResult, RecommendationServiceInterface
...
export class ConsolidatedRecommendationService implements RecommendationServiceInterface {
  ...
  public static getInstance(): ConsolidatedRecommendationService {
    if (!ConsolidatedRecommendationService.instance) {
      ConsolidatedRecommendationService.instance =
        new ConsolidatedRecommendationService();
    }
  ...
  async getRecommendedRecipes(
    ...
    try {
      ...
      filteredRecipes.sort((a, b) => {
        ...
        return (
          scores[String(recipeB.id || "")] - scores[String(recipeA.id || "")]
        );
      });
    ...
    try {
      ...
      if (currentSeason) {
        filteredIngredients = filteredIngredients.filter((ingredient) => {
          ...
          return (
            Array.isArray(ingredientSeasons) &&
            ingredientSeasons.includes(currentSeason)
          );
        });
      }
      if (
    ...
    try {
      ...
      const scores = Object.fromEntries(
        cuisineRecommendations.map((rec: unknown) => {
          ...
          return [
            String(recData.name || ""),
            Number(recData.matchPercentage || 0),
          ];
        }),
      );
      ...
      if (criteria.excludeCuisines && criteria.excludeCuisines.length > 0) {
        filteredItems = items.filter(
          (cuisine) => !criteria.excludeCuisines?.includes(cuisine),
        );
      }
      return {
        items: filteredItems,
...
export const recommendationService =
  ConsolidatedRecommendationService.getInstance();

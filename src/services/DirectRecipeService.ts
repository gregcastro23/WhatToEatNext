...
export class DirectRecipeService {
  ...
  private loadAllRecipes(): void {
    ...
    const uniqueRecipes = recipes.reduce(
      (acc: { [key: string]: Recipe }, recipe) => {
  ...
  private async getCurrentCelestialAlignment(
    ...
    try {
      ...
      if (
        ...
        this.currentCelestialAlignment =
          enhancedAlignment as unknown as CelestialAlignment;
      } else {
  ...
  private calculateRecipeKalchm(recipe: Recipe): number {
    ...
    (recipe.ingredients || []).forEach((ingredient) => {
      ...
      if (
        ingredientData &&
        (ingredientData as unknown as Record<string, unknown>)
          .alchemicalProperties
      ) {
        const ingredientKalchm = calculateKalchm(
          (ingredientData as unknown as Record<string, unknown>)
            .alchemicalProperties,
        );
    ...
    return ingredientCount > 0
      ? Math.pow(totalKalchm, 1 / ingredientCount)
      : 1.0;
  }
  ...
  private calculateElementalScore(
    ...
    return calculateElementalCompatibility(
      ...
      alignment.elementalState ||
        alignment.elementalDominance ||
   ...
   */
  private calculateZodiacalScore(
    recipe: Recipe,
    alignment: CelestialAlignment,
  ): number {
    if (
  ...
  private calculatePlanetaryScore(
    ...
    if (recipe.planetaryInfluences.favorable) {
      ...
      score +=
        ((favorableMatches || []).length /
          (recipe.planetaryInfluences.favorable || []).length) *
        0.3;
    }

    // Check unfavorable planets (reduce score)
    if (recipe.planetaryInfluences.unfavorable) {
      const unfavorableMatches = (
        recipe.planetaryInfluences.unfavorable || []
      ).filter((planet) =>
        (alignment.dominantPlanets || []).some(
          (dp) => dp.name.toLowerCase() === planet.toLowerCase(),
        ),
  ...
  private calculateSeasonalScore(
    ...
    if (
      !currentSeason ||
      (Array.isArray(currentSeason) && currentSeason.length === 0)
    )
      return 0.5;
    ...
    const seasonArray = Array.isArray(currentSeason)
      ? currentSeason
      : [currentSeason];
    const hasSeasonMatch = seasonArray.some(
      (season) =>
        season?.toLowerCase() === currentSeasonName ||
   ...
   */
  public async getRecipesBySeason(
    season: string,
    limit = 20,
    offset = 0,
  ): Promise<ScoredRecipe[]> {
    ...
    const filteredRecipes = (this.allRecipes || []).filter((recipe) => {
      if (Array.isArray(recipe.currentSeason)) {
        return (recipe.currentSeason || []).some(
    ...
    if (criteria.currentSeason || criteria.season) {
      ...
      candidateRecipes = candidateRecipes.filter((recipe) => {
        ...
        if (Array.isArray(recipeCurrentSeason)) {
          return recipeCurrentSeason.some(
            (s) => s?.toLowerCase() === seasonCriteria?.toLowerCase(),
          );
        } else if (typeof recipeCurrentSeason === "string") {
          return (
            recipeCurrentSeason.toLowerCase() === seasonCriteria?.toLowerCase()
          );
        }
        return false;
      });
    ...
    if (criteria.mealType) {
      candidateRecipes = candidateRecipes.filter((recipe) => {
        if (Array.isArray(recipe.mealType)) {
          ...
          return (
            recipe.mealType.toLowerCase() === criteria.mealType?.toLowerCase()
          );
        }
        return false;
      });
    ...
    for (const recipe of candidateRecipes) {
      ...
      if ((criteria.ingredients || []).length) {
        const matchingIngredients = (recipe.ingredients || []).filter((i) =>
          (criteria.ingredients || []).some((ci) =>
            i.name.toLowerCase().includes(ci.toLowerCase()),
          ),
        );

...
export interface NutritionalDataAdapterInterface {
  ...
  getZodiacNutritionalRecommendations(sign: ZodiacSign | string): {focusNutrients: string[],
...
class NutritionalDataAdapter implements NutritionalDataAdapterInterface {
  ...
  evaluateNutritionalElementalBalance(
    ...
    try {
      ...
      const legacyTargetElements = { Fire: targetElements.Fire, Water: targetElements.Water, Earth: targetElements.Earth, // Convert to legacy capitalization
        Air: targetElements.Air
      };
      
      return evaluateNutritionalElementalBalance(profile as NutritionalProfile, legacyTargetElements);
    } catch (error) {
      // Use safe type casting for errorHandler service access
      const errorHandlerService = (errorHandler as unknown) as Record<string, unknown>;
      const logError = errorHandlerService.logError as Function;

...
import type { ScoringContext, ScoringResult } from "../UnifiedScoringService";
...
export class UnifiedScoringExample {
  ...
  static async comprehensiveExample(): Promise<void> {
    ...
    try {
      ...
      log.info(
        `Score: ${basilScore.score.toFixed(3)} (${(basilScore.score * 100).toFixed(1)}%)`,
      );
      ...
      log.info("Breakdown:", {
        breakdown: JSON.stringify(basilScore.breakdown, null, 2),
      });
      log.info("\n");

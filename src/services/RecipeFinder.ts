...
export class RecipeFinder implements RecipeServiceInterface {
   ...
   */
  async getRecipesBySeason(
    params: GetRecipesBySeasonParams,
  ): Promise<ApiResponse<Recipe[]>> {
    try {
      const recipes = await this.recipeService.getRecipesBySeason(
        params.season,
      );
      return {
        success: true,
  ...
  async getRecipesByLunarPhase(
    ...
    try {
      const recipes = await this.recipeService.getRecipesByLunarPhase(
        params.lunarPhase,
      );
      return {
        success: true,
  ...
  async getRecipesByMealType(
    ...
    try {
      const recipes = await this.recipeService.getRecipesByMealType(
        params.mealType,
      );
      return {
        success: true,
      ...
      errorHandler.log(
        error instanceof Error ? error : new Error(String(error)),
        {
          context: {
            ...
            data: {
              recipe1Id: recipe1.id || "unknown",
              recipe2Id: recipe2.id || "unknown",
            },
          },
        },
      );

...
export class EnhancedTransitAnalysisService {
    ...
    return aspects.map((aspect) => {
      ...
      if (planet1Data && planet2Data) {
        ...
        dignityModifiedInfluence =
          (dignityModifiedInfluence || 0) * averageDignityModifier;
        ...
        dignityModifiedInfluence =
          (dignityModifiedInfluence || 0) * averageLocationInfluence;
      }
  ...
  private static generatePlanetaryCulinaryRecommendations(
    ...
    if (dignity.type === "rulership" || dignity.type === "exaltation") {
      ...
      recommendations.push(
        `Strong preparation methods reflecting ${planet} energy`,
      );
    } else if (dignity.type === "detriment" || dignity.type === "fall") {
      recommendations.push(
        `Gentle ${planet.toLowerCase()} foods - use with moderation`,
      );
      recommendations.push(
        `Supportive cooking methods to strengthen ${planet} influence`,
      );
    }
    ...
    if (locationInfluence?.culinaryRecommendations) {
      recommendations.push(
        ...locationInfluence.culinaryRecommendations.slice(0, 2),
      );
    }
  ...
  private static generateAspectCulinaryEffects(
    ...
    if (planet1Data && planet2Data) {
      ...
      switch (aspectType) {
        case "conjunction":
          effects.push(
            `${planets} fusion - blend both planetary food associations`,
          );
          break;
        case "trine":
        ...
        case "sextile":
          effects.push(
            `${planets} opportunity - creative fusion possibilities`,
          );
          break;
        case "square":
        ...
        case "opposition":
          effects.push(
            `${planets} polarity - alternate between contrasting approaches`,
          );
          break;
      }
    }
  ...
  private static generateLocationSpecificRecommendations(
    ...
    const regionalProfile =
      PlanetaryLocationService.getRegionalProfile(location);
    ...
    cookingMethods.push(
      ...regionalProfile.traditionalCookingMethods.slice(0, 3),
    );
  ...
  private static calculateDominantInfluences(
    ...
    const [dominantEntry] = Object.entries(season.dominantElements).sort(
      ([, a], [, b]) => b - a,
    );
  ...
  static async getFoodRecommendationsForLocation(
    ...
    const primarySuggestions =
      analysis.locationSpecificRecommendations.ingredients.slice(0, 5);
    const alternativeSuggestions = analysis.enhancedPlanetaryPositions
      .flatMap((planet) => planet.culinaryRecommendations)
      .slice(0, 5);
    const cookingGuidance =
      analysis.locationSpecificRecommendations.cookingMethods.slice(0, 4);
    const timingAdvice = analysis.dominantInfluences.optimalCookingTimes.slice(
      0,
      3,
    );

...
const API_CONFIG = {
  ...
  kitchen:
    process.env.NEXT_PUBLIC_KITCHEN_BACKEND_URL || "http://localhost:8100",
...
export class AlchemicalApiClient {
   ...
   */
  async calculateElementalBalance(
    ingredients: string[],
    weights?: number[],
  ): Promise<ElementalProperties> {
    try {
      const response = await fetch(
        `${this.baseUrls.alchemical}/calculate/elemental`,
        {
          method: "POST",
  ...
  async calculateThermodynamics(
    ...
    try {
      const response = await fetch(
        `${this.baseUrls.alchemical}/calculate/thermodynamics`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        throw new Error(
          `Thermodynamics calculation failed: ${response.statusText}`,
        );
      }
  ...
  async getCurrentPlanetaryHour(): Promise<PlanetaryInfluenceResponse> {
    try {
      const response = await fetch(
        `${this.baseUrls.alchemical}/planetary/current-hour`,
      );
  ...
  async getRecipeRecommendations(request: RecommendationRequest): Promise<any> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Recipe recommendations failed: ${response.statusText}`,
        );
      }
...
export const useBackendCalculations = () => ({
  ...
  calculateThermodynamics:
    alchemicalApi.calculateThermodynamics.bind(alchemicalApi),

...
export class IngredientFilterService {
  ...
  private applyNutritionalFilter(
    ...
    return ingredients.filter((ingredient) => {
      ...
      if (
        filter.highProtein &&
        (!nutrition.protein_g || nutrition.protein_g < 15)
      ) {
        return false;
      }
  ...
  private applyElementalFilter(
    ...
    return ingredients.filter((ingredient) => {
      ...
      if (
        ...
        elemental.Water > filter.maxWater
      ) {
        return false;
      }
      ...
      if (
        ...
        elemental.Earth > filter.maxEarth
      ) {
        return false;
      }
      ...
      if (
        ...
        elemental.Air !== undefined &&
        elemental.Air > filter.maxAir
      ) {
        return false;
      }
      ...
      if (filter.dominantElement) {
        ...
        if (
          ...
          sortedElements[0][0] !== filter.dominantElement
        ) {
          return false;
        }
      }
  ...
  ): Promise<SpoonacularNutritionData | null> {
    try {
      // Check cache first
    ...
    try {
      ...
      if (dietaryFilter) {
        ...
        if (dietaryFilter.isGlutenFree)
          dietParam = `${dietParam},gluten-free`.replace(/^,/, '');
        if (dietaryFilter.isDairyFree)
          dietParam = `${dietParam},dairy-free`.replace(/^,/, '');
      }

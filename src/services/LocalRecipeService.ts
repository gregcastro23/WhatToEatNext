...
export class LocalRecipeService {
    ...
    try {
      ...
      if (normalizedName === "african" || normalizedName === "american") {
        ...
        try {
          ...
          logger.error(
            `Error importing ${normalizedName} cuisine directly: `,
            error,
          );
          ...
          logger.debug(
            `Cuisine structure: ${JSON.stringify({
              ...
              breakfast: (() => {
                ...
                if (Array.isArray(dishesBreakfastAll))
                  return dishesBreakfastAll.length;
                return 0;
              })(),
              lunch: (() => {
                const dishes = directCuisine.dishes as any;
              ...
              dinner: (() => {
                ...
                if (Array.isArray(dishesDinnerAll))
                  return dishesDinnerAll.length;
                return 0;
              })(),
              dessert: (() => {
                const dishes = directCuisine.dishes as any;
                ...
                if (Array.isArray(dishesDessertAll))
                  return dishesDessertAll.length;
                return 0;
              })(),
            })}`,
          );
      ...
      if (!cuisine) {
        ...
        if (byIdMatch?.[1]) {
          return await this.getRecipesFromCuisine(
            byIdMatch[1] as ExtendedCuisine,
          );
        }
   ...
   */
  private static async getRecipesFromCuisine(
    ...
    try {
      ...
      if (cuisine.name.toLowerCase() === "african") {
        ...
        if (Array.isArray(breakfastAll) && breakfastAll.length > 0) {
          logger.debug(
            ...
            JSON.stringify((breakfastAll as unknown[])[0]),
          );
        }
      }
      ...
      if (!cuisine.dishes) {
        ...
        if (isSpecialCase) {
          logger.warn(
            `Special case (${cuisine.name}) has no dishes property: `,
            cuisine,
          );
        }
      ...
      mealTypes.forEach((mealType) => {
        if (
          ...
          if (cuisine.dishes && cuisine.dishes[mealType]) {
            logger.debug(
              `Structure of ${mealType}:`,
              Object.keys(cuisine.dishes[mealType]),
            );
          }
        }
      });
      ...
      mealTypes.forEach((mealType) => {
        ...
        seasons.forEach((season) => {
          ...
          if (seasonalDishes[season] && Array.isArray(seasonalDishes[season])) {
            ...
            seasonalDishes[seasonalKey] &&
            Array.isArray(seasonalDishes[seasonalKey])
          ) {
          ...
          if (seasonRecipes.length > 0) {
            logger.debug(
              `Found ${seasonRecipes.length} dishes for ${season} in ${mealType}`,
            );
            ...
            logger.warn(
              `No recipes found for ${season} in ${mealType} for ${cuisine.name}`,
            );
          }
        });
      });
      ...
      logger.debug(
        `Extracted ${recipes.length} total recipes from ${cuisine.name} cuisine`,
      );
      ...
      if (recipes.length === 0) {
          ...
          JSON.stringify(
            {
              ...
              dishesStructure: Object.entries(cuisine.dishes || {}).map(
                ([key, value]) => ({
                  mealType: key,
                  hasValue: !!value,
                  ...
                  allLength:
                    value?.all && Array.isArray(value.all)
                      ? value?.all?.length
                      : 0,
                }),
              ),
        ...
        if (cuisine.dishes && typeof cuisine.dishes.dishes === "object") {
          logger.debug(
            "Found nested dishes property, trying to extract from there instead",
          );
      ...
      logger.error(
        `Error extracting recipes from cuisine ${cuisine.name}:`,
        error,
      );
      return [];
    }
  ...
  private static standardizeRecipe(
    ...
    try {
      ...
      const id =
        dish.id ||
        `${cuisineName.toLowerCase()}-${dish.name.toLowerCase().replace(/\s+/g, "-")}`;
      ...
      if (dish.elementalProperties) {
        ...
        dish.elementalState &&
        typeof dish.elementalState === "object"
      ) {
      ...
      if (
        ...
        dish.preparationSteps.length > 0
      ) {
      ...
      if (dish.substitutions && typeof dish.substitutions === "object") {
        ...
        substitutions = Object.entries(dish.substitutions).map(
      ...
      return {
        ...
        isGlutenFree:
          dish.isGlutenFree ||
          dish.dietaryInfo?.includes("gluten-free") ||
          false,
        isDairyFree:
          dish.isDairyFree || dish.dietaryInfo?.includes("dairy-free") || false,
        nutrition,
        ...
        cookingMethods: Array.isArray(dish.cookingMethods)
          ? dish.cookingMethods
          : [],
        // New fields
        substitutions,
        ...
        technicalTips: Array.isArray(dish.technicalTips)
          ? dish.technicalTips
          : [],
      };
    } catch (error) {
        ...
        (recipe) =>
          ...
          (Array.isArray(recipe.mealType)
            ? recipe.mealType.some(
                (m) => m.toLowerCase() === normalizedMealType,
              )

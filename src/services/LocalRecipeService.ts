...
export class LocalRecipeService {
          ...
          if (seasonRecipes.length > 0) {
            ...
            seasonRecipes.forEach((dish) => {
              if (
                dish &&
                dish.name &&
                !recipes.some((r) => r.name === dish.name)
              ) {
                recipes.push(
          ...
          JSON.stringify(
            {
              ...
              dishesStructure: Object.entries(cuisine.dishes || {}).map(
                ([key, value]) => ({
                  ...
                  allLength:
                    value && value.all && Array.isArray(value.all)
                      ? value.all.length
                      : 0,
                })
              ),
  ...
  private static standardizeRecipe(
    ...
    try {
      ...
      let elementalProperties = dish.elementalProperties ||
        dish.elementalState || {
          Fire: 0.25,
          Water: 0.25,
          Earth: 0.25,
      ...
      return recipes.filter((recipe) => {
        ...
        if (recipe.ingredients && Array.isArray(recipe.ingredients)) {
          for (const ingredient of recipe.ingredients) {
            ...
            if (
              ingredientName &&
              ingredientName.toLowerCase().includes(normalizedQuery)
            ) {
              return true;
            }
          }

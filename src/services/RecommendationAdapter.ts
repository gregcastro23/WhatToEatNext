...
import { LunarPhase, Planet, calculatePlanetaryBoost } from '@/constants/planetaryFoodAssociations';
...
export class RecommendationAdapter {
  ...
  async updatePlanetaryData(
    ...
    // Use provided lunar phase or default to 'new_moon' if not available
    const currentLunarPhase = lunarPhase || 'new_moon' as LunarPhase;
    
    // Update internal state
    this.planetaryPositions = planetPositions;
    this.isDaytime = isDaytime;
    this.currentZodiac = currentZodiac || null;
    this.lunarPhase = currentLunarPhase;
    this.tarotElementBoosts = tarotElementBoosts;
    this.tarotPlanetaryBoosts = tarotPlanetaryBoosts;
    
    // Calculate base alchemical properties using chart data
    ...
    // Add nutrition-based elemental boosts
    const nutritionBoosts = this.calculateNutritionalBoosts();
    
    // Add aspect-based boosts
    if (aspects && aspects.length > 0) {
      aspects.forEach(aspect => {
        // Apply aspect boosts to ingredients ruled by these planets
        this.ingredients.forEach(ingredient => {
          const rulingPlanets = ingredient.astrologicalProfile?.rulingPlanets || [];
          if (rulingPlanets.includes(aspect.planet1) || rulingPlanets.includes(aspect.planet2)) {
            // Apply boost based on aspect type and strength
            const boost = aspect.strength * 0.2; // Adjust multiplier as needed
            const element = getPlanetaryElement(aspect.planet1); // Or use both planets
            if (element) {
              ingredient.elementalProperties[element] = 
                (ingredient.elementalProperties[element] || 0) * (1 + boost);
            }
          }
        });
      });
    ...
    // Transform all ingredients based on planetary data
    this.transformedIngredients = this.ingredients.map(ingredient => {
      // Ensure required properties exist with defaults
      const safeIngredient = {
        ...ingredient,
        elementalProperties: ingredient.elementalProperties || { Fire: 0, Water: 0, Earth: 0, Air: 0 },
        heat: ingredient.heat || 0,
        moisture: ingredient.moisture || 0,
        nutritionalProfile: ingredient.nutritionalProfile || {},
        astrologicalProfile: ingredient.astrologicalProfile || {},
        lunarPhaseModifiers: ingredient.lunarPhaseModifiers || {}
      };
      
      const alchemicalItem = transformItemsWithPlanetaryPositions(
        [safeIngredient],
        this.planetaryPositions
      )[0];
      
      // Apply nutrition-based boosts
      const boostedItem = this.applyNutritionalBoost(alchemicalItem, nutritionBoosts);
      
      // Apply Tarot energy boosts
      return this.applyTarotEnergyBoosts(boostedItem, this.tarotPlanetaryBoosts || {});
    });
    ...
    return this.ingredients.reduce((acc, ingredient) => {
      ...
      if (nutrition) {
        acc[ingredient.id] = Math.sqrt(
          nutrition.macros.protein * 0.4 +
          nutrition.vitamins.vitaminC * 0.3 +
          nutrition.minerals.iron * 0.3
        );
      }
      return acc;
...
/**
 * Helper function to get element associated with a planet
 */
function getPlanetaryElement(planet: string): string | undefined {

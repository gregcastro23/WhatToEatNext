...
import {
    ...
    _transformCuisines, _transformIngredients
...
// Snapshot of the fields this file reads off the alchemize() result. The real
// AlchemizeResult (src/calculations/core/alchemicalCalculations.ts) nests these under
// alchemicalProperties/elementalValues, so these flat/lowercase reads are pre-existing
// latent bugs (always undefined/zero at runtime) preserved as-is in this types-only pass.
interface AlchemicalResultSnapshot {
  spirit?: number;
  essence?: number;
  matter?: number;
  substance?: number;
...
// Snapshot of the fields read off a PlanetData.Alchemy block.
interface AlchemySnapshot {
  Spirit?: unknown;
  Essence?: unknown;
  Matter?: unknown;
  Substance?: unknown;
}
// Snapshot of the fields this file reads off a PlanetaryAspect. Real aspects only
// populate planet1/planet2/type, never body1/body2/aspectType, so this block is
// pre-existing dead code preserved as-is (see notes above the aspects loop below).
interface AspectFieldsRead {
  body1?: unknown;
  body2?: unknown;
  aspectType?: unknown;
}
...
interface NutritionSnapshot {
  ...
  vitamins?: { vitaminC?: unknown };
  minerals?: { iron?: unknown };
}
/**
...
export class RecommendationAdapter {
  ...
  private alchemicalResult: AlchemicalResultSnapshot;
  constructor(
  ...
  initialize(
    ...
    Object.entries(planetaryPositions).forEach(([planet, data]) => {
      this.convertedPositions[planet] = {
        sign: (data.sign as ZodiacSignType | undefined) ?? "aries",
  ...
  transformItemsWithCurrentPositions(): void {
    try {
      ...
      logger.error(
        "Error transforming items with planetary positions: ",
        error,
      );
    }
  }
  ...
  private transformItems(): void {
    try {
      ...
      const result = alchemize({
        positions: this.convertedPositions,
      ...
      const resultData = this.alchemicalResult;
      ...
      const elementalBalance = this.safeExtractElementalBalance(
      ...
      if (this.tarotElementBoosts) {
        Object.entries(this.tarotElementBoosts).forEach(([element, boost]) => {
          if (element in elementalProperties) {
            elementalProperties[element as keyof typeof elementalProperties] +=
              boost;
          }
        });
      }
      ...
      if (this.aspects.length > 0) {
        this.aspects.forEach((aspect) => {
          ...
          const aspectData = aspect as AspectFieldsRead;
          ...
          if (aspectsEffect1?.[planet2]) {
            ...
            if (aspectEffect) {
              ...
              const alchemyData1 = planetData1.Alchemy as AlchemySnapshot;
              ...
              alchemicalProperties.Spirit =
                alchemicalProperties.Spirit + spirit1 * boost;
              alchemicalProperties.Essence =
                alchemicalProperties.Essence + essence1 * boost;
              alchemicalProperties.Matter =
                alchemicalProperties.Matter + matter1 * boost;
              alchemicalProperties.Substance =
          ...
          if (aspectsEffect2?.[planet1]) {
            ...
            if (aspectEffect) {
              ...
              const boost = aspectEffect;
              // Extract alchemy data with safe property access
              const alchemyData2 = planetData2.Alchemy as AlchemySnapshot;
              ...
              alchemicalProperties.Spirit =
                alchemicalProperties.Spirit + spirit2 * boost;
              alchemicalProperties.Essence =
                alchemicalProperties.Essence + essence2 * boost;
              alchemicalProperties.Matter =
                alchemicalProperties.Matter + matter2 * boost;
              alchemicalProperties.Substance =
                alchemicalProperties.Substance + substance2 * boost;
            }
          }
        });
      }
   ...
   */
  private safeExtractElementalBalance(
    balance: unknown,
  ...
  getRecommendedIngredients(limit = 10): RecommendationResult<AlchemicalItem> {
    ...
    const scores = Object.fromEntries(
      items.map((item) => [
        item.id,
        this.safeGetNumber(item.gregsEnergy) || 1.0,
      ]),
    );
  ...
  getRecommendedCookingMethods(
    limit = 5,
  ): RecommendationResult<AlchemicalItem> {
  ...
  private getSortedItems(
    ...
    return items
      .sort((a, b) => {
        ...
        const energyA = this.safeGetNumber(a.gregsEnergy);
        const energyB = this.safeGetNumber(b.gregsEnergy);
    ...
    return this.ingredients.reduce(
      (acc, ingredient) => {
        ...
        if (nutrition) {
          ...
          const nutritionData = nutrition as NutritionSnapshot;
  ...
  applyTarotEnergyBoosts(
    ...
    let heat =
      (Math.pow(boostedSpirit, 2) + Math.pow(fire, 2)) /
      Math.pow(denominatorHeat, 2);
    // Entropy formula
  ...
  getHeatIndex(): number | null {
    ...
    if (this.alchemicalResult.heat !== undefined) {
      return this.alchemicalResult.heat;
    }
    // Otherwise derive from alchemical properties
    const alchemicalData = this.alchemicalResult;
  ...
  getEntropyIndex(): number | null {
    ...
    if (this.alchemicalResult.entropy !== undefined) {
  ...
  getReactivityIndex(): number | null {
    ...
    if (this.alchemicalResult.reactivity !== undefined) {
  ...
  getGregsEnergyIndex(): number | null {
    ...
    if (this.alchemicalResult.gregsEnergy !== undefined) {

...
class AstrologizeApiCache {
  ...
  public findNearby(
    ...
    return results.sort((a, b) => {
      ...
      const scoreA =
        distA / maxDistanceKm + timeA / (maxDaysDiff * 24 * 60 * 60 * 1000);
      const scoreB =
        distB / maxDistanceKm + timeB / (maxDaysDiff * 24 * 60 * 60 * 1000);
  ...
  public predictPositions(
    ...
    const confidence =
      ...
      Math.max(
        0.3,
        1 -
          Math.abs(targetDate.getTime() - baseData.date.getTime()) /
            (30 * 24 * 60 * 60 * 1000),
      );
  ...
  private assessDataQuality(
    result: StandardizedAlchemicalResult,
  ): "high" | "medium" | "low" {
    ...
    const hasAllElements =
      ...
      Object.values(resultData.elementalBalance).every(
  ...
  public getCacheStats() {
    return {
      ...
      qualityDistribution: {
        high: Array.from(this.cache.values()).filter(
          (v) => v.quality === "high",
        ).length,
        medium: Array.from(this.cache.values()).filter(
          (v) => v.quality === "medium",
        ).length,
        low: Array.from(this.cache.values()).filter((v) => v.quality === "low")
          .length,
      },
    };
  }

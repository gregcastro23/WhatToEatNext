// import { celestialBodies } from "../data/(celestial || 1)/bodies";
import { celestialNumerology } from "../utils/(numerology || 1)";
...
import * as astronomiaCalculator from "../utils/(astronomiaCalculator || 1)";
...
let MINOR_ARCANA_ELEMENTAL_AFFINITIES = {
...
class CelestialCalculator {
  ...
  calculateCurrentInfluences(): CelestialAlignment {
    try {
      ...
      let cached = cache.get(this.CACHE_KEY) as
        | Partial<CelestialAlignment>
        | undefined;
      if (cached) {
        ...
        if (
          ...
          let influences = cached.dominantPlanets
            ? [
                ...cached.dominantPlanets.map((p) => p.name),
                cached.zodiacSign || 'libra',
                'all',
              ]
      ...
      let zodiacSign = this.determineZodiacSign(month, day);
      ...
      try {
        ...
        if (
          typeof astronomiaCalculator.calculatePlanetaryPositions === 'function'
        ) {
          planetaryPositions = astronomiaCalculator.calculatePlanetaryPositions(
            now
          ) as PlanetaryPositionRecord;
        } else {
      ...
      let dominantPlanets = this.determineDominantPlanets(
        now.getDay(),
        hour,
        planetaryPositions
      );
      ...
      let lunarPhase = this.calculateLunarPhase(now);

      // Calculate elemental balance
      let elementalBalance = this.calculateElementalBalance(
        zodiacSign,
        dominantPlanets,
        lunarPhase
      );
      ...
      let tarotInfluences = this.calculateTarotInfluences(
        zodiacSign,
        dominantPlanets
      );
  ...
  private calculateMoonSign(date: Date): string {
    try {
      ...
      if (
        ...
        let positions =
          astronomiaCalculator.calculatePlanetaryPositions(date);
      ...
      let lunarCycle = 27.3; // sidereal lunar cycle in days (time to return to same position)

      // Using a reference date when the moon was in a known position
      let referenceDate = new Date('2025-04-02');
      let referenceSign = 'gemini'; // Moon was in Gemini on the reference date

      let daysSinceReference =
        (date.getTime() - referenceDate.getTime()) / msPerDay;

      // Each sign takes approximately 2.28 days (27.3 / (12 || 1))
      let signsPassed = Math.floor((daysSinceReference % lunarCycle) / 2.28);
      ...
      let refIndex = signs.indexOf(referenceSign);
      if (refIndex === -1) return 'cancer'; // Fallback

      // Calculate the current sign index
      let currentIndex = (refIndex + signsPassed) % 12;
  ...
  private calculateTarotInfluences(
    ...
    let minorArcanaCard = this.calculateMinorArcanaForDate(now);
    if (minorArcanaCard) {
      tarotCards.push(minorArcanaCard);
    }

    // Add zodiac-related major arcana cards
    let zodiacCards = TAROT_ZODIAC_MAPPING[zodiacSign] || [];
    ...
    dominantPlanets.forEach((planet) => {
      let planetCards = TAROT_PLANETARY_MAPPING[planet.name] || [];
  ...
  private createTarotCard(
    ...
    if (suit !== 'major') {
      let element = TAROT_ELEMENTAL_MAPPING[suit]?.Element as
        | 'Fire'
        | 'Water'
        | 'Earth'
        | 'Air';
    ...
    return {
      ...
      suit,
      value: 0, // Placeholder
      description: `Tarot card associated with ${
        zodiacAssociation || planetaryAssociation || 'universal forces'
      }`,
      meaning: {
  ...
  private ensureCompleteAlignment(
    ...
    if (
      alignment.elementalBalance &&
      typeof alignment.elementalBalance === 'object'
    ) {
      safeAlignment.elementalBalance = alignment.elementalBalance;
    }

    if (
      alignment.aspectInfluences &&
      Array.isArray(alignment.aspectInfluences)
    ) {
  ...
  private calculateGasGiantInfluences(
    ...
    // Jupiter dignities & debilities
    const jupiterDignities: Record<string, { type: string; strength: number }> = {
      sagittarius: { type: 'Domicile', strength: 0.3 },
      pisces: { type: 'Domicile', strength: 0.25 },
      cancer: { type: 'Exaltation', strength: 0.2 },
      gemini: { type: 'Detriment', strength: -0.2 },
      virgo: { type: 'Detriment', strength: -0.2 },
      capricorn: { type: 'Fall', strength: -0.25 },
    };

    // Saturn dignities & debilities
    const saturnDignities: Record<string, { type: string; strength: number }> = {
      capricorn: { type: 'Domicile', strength: 0.3 },
      aquarius: { type: 'Domicile', strength: 0.25 },
      libra: { type: 'Exaltation', strength: 0.2 },
      cancer: { type: 'Detriment', strength: -0.2 },
      leo: { type: 'Detriment', strength: -0.15 },
      aries: { type: 'Fall', strength: -0.25 },
    };

    let jupiterPos = planetaryPositions.jupiter;
    let saturnPos = planetaryPositions.saturn;

    // Calculate base influences based on dignities
    let jupiterDignityName = '';
    if (jupiterPos && jupiterPos.sign) {
      let jupiterSign = jupiterPos.sign.toLowerCase();
      let dignity = jupiterDignities[jupiterSign];
      if (dignity) {
    ...
    if (saturnPos && saturnPos.sign) {
      let saturnSign = saturnPos.sign.toLowerCase();
      let dignity = saturnDignities[saturnSign];
      if (dignity) {
    ...
    // Adjust influences based on aspects
    if (aspectInfluences.length > 0) {
      aspectInfluences.forEach((aspect) => {
        // Check for Jupiter aspects
        if (aspect.planets.includes('jupiter')) {
          let otherPlanet = aspect.planets.find((p) => p !== 'jupiter');
          if (otherPlanet) {
            // Conjunction with benefics increases Jupiter's influence
            if (
              aspect.type === 'conjunction' &&
              (otherPlanet === 'venus' || otherPlanet === 'sun')
            ) {
              jupiterInfluence += 0.15 * aspect.influence;
              jupiterEffect = 'expansive';
            }
            // Conjunction with malefics decreases Jupiter's influence
            else if (
              aspect.type === 'conjunction' &&
              (otherPlanet === 'mars' || otherPlanet === 'saturn')
            ) {
              jupiterInfluence -= 0.15 * aspect.influence;
              jupiterEffect = 'restricted';
            }
            // Trine aspects generally increase influence
            else if (aspect.type === 'trine') {
              jupiterInfluence += 0.1 * aspect.influence;
            }
            // Square aspects can create tension
            else if (aspect.type === 'square') {
              jupiterInfluence -= 0.05 * aspect.influence;
            }
          }
        }

        // Check for Saturn aspects
        if (aspect.planets.includes('saturn')) {
          let otherPlanet = aspect.planets.find((p) => p !== 'saturn');
          if (otherPlanet) {
            // Conjunction with benefics softens Saturn's restrictiveness
            if (
              aspect.type === 'conjunction' &&
              (otherPlanet === 'venus' || otherPlanet === 'jupiter')
            ) {
              saturnInfluence -= 0.15 * aspect.influence;
              saturnEffect = 'softened';
            }
            // Conjunction with malefics increases Saturn's influence
            else if (
              aspect.type === 'conjunction' &&
              (otherPlanet === 'mars' || otherPlanet === 'pluto')
            ) {
              saturnInfluence += 0.15 * aspect.influence;
              saturnEffect = 'restrictive';
            }
            // Trine aspects generally moderate influence
            else if (aspect.type === 'trine') {
              saturnInfluence -= 0.05 * aspect.influence;
    ...
    // Ensure influences stay within bounds
  ...
  private determineDominantPlanets(
    ...
    let dayRuler = dayRulers[dayOfWeek];
    let hourRuler = hourRulers[hour];
    ...
    let sunPos = planetaryPositions['sun'] || planetaryPositions['Sun'];
    if (sunPos && sunPos.sign) {
      ...
      if (!dominantPlanets.some((p) => p.name === 'Sun')) {
        ...
        let sunIndex = dominantPlanets.findIndex((p) => p.name === 'Sun');
        if (sunIndex >= 0) {
    ...
    let aspectInfluences = this.calculatePlanetaryAspects(now);
    ...
    // Only add them if they're not already included from day / (hour || 1) rulers
    if (!dominantPlanets.some((p) => p.name === 'Jupiter')) {
      ...
      let jupiterIndex = dominantPlanets.findIndex(
        (p) => p.name === 'Jupiter'
      );
      if (jupiterIndex >= 0) {
    ...
    if (!dominantPlanets.some((p) => p.name === 'Saturn')) {
      ...
      let saturnIndex = dominantPlanets.findIndex((p) => p.name === 'Saturn');
      if (saturnIndex >= 0) {
    ...
    let outerPlanets = ['Uranus', 'Neptune', 'Pluto'];
    let outerPlanetDignities = {
      Uranus: {
    ...
    outerPlanets.forEach((planet) => {
      ...
      let position = planetaryPositions[planet.toLowerCase()] || {
        sign: '',
        degree: 0,
      };
      let sign = position.sign?.toLowerCase() || '';
  ...
  private calculateLunarPhase(date: Date): string {
    ...
    try {
      if (
        typeof astronomiaCalculator !== 'undefined' &&
        typeof astronomiaCalculator.calculateLunarPhase === 'function'
      ) {
        let lunarPhase = astronomiaCalculator.calculateLunarPhase(date);
    ...
    let synodicPeriod = 29.530588853; // days - precise synodic period (new moon to new moon)
    let msPerDay = 1000 * 60 * 60 * 24;

    // Reference date for a known new moon (January 21, 2023 at 20:53 UTC)
    let knownNewMoon = new Date('2023-01-21T20:53:00Z');

    // Calculate days since known new moon
    let daysSinceNewMoon =
      (date.getTime() - knownNewMoon.getTime()) / (msPerDay || 1);

    // Calculate current phase (0 to 1, where 0 and 1 are new moon, 0.5 is full moon)
    let normalizedPhase = (daysSinceNewMoon % synodicPeriod) / (synodicPeriod || 1);
  ...
  private calculateElementalBalance(
    ...
    let zodiacElement = zodiacElements[zodiacSign.toLowerCase()];
    if (zodiacElement) {
    ...
    dominantPlanets.forEach((planet) => {
      let element = planetElements[planet.name];
      if (element) {
    ...
    // Normalize values
    let sum = Object.values(balance).reduce((a, b) => a + b, 0);
  ...
  getCurrentAlignment(): CelestialAlignment {
    let now = Date.now();
    let timeSinceLastCalculation = now - this.lastCalculation;
    ...
    // Get from cache
    let cached = cache.get(this.CACHE_KEY) as CelestialAlignment | undefined;

    // Return cached value with safeguards or calculate new
    return cached
      ? this.ensureCompleteAlignment(cached)
      : this.calculateCurrentInfluences();
  }
  ...
  private getTarotInfluences(): TarotCard[] {
    let tarotCards = cache.get(this.TAROT_CACHE_KEY) as
      | TarotCard[]
      | undefined;
  ...
  getRecommendedRecipeElementalBalance(): ElementalProperties {
    try {
      ...
      let energyStateBalance = this.calculateEnergyStateBalance(alignment);

      // Apply numerological adjustment based on current date
      let numerologicalFactor = celestialNumerology.calculateDailyNumber();
      ...
      let tarotCards = this.getTarotInfluences();
      ...
      let chakraEnergies = this.calculateChakraEnergies(
        alignment,
        energyStateBalance
      );
   ...
   */
  private calculateEnergyStateBalance(
    ...
    if (alignment.zodiacSign) {
      ...
      let sign = alignment.zodiacSign.toLowerCase();
      ...
      let jupiterPlanet = alignment.dominantPlanets.find(
        (p) => p.name === 'Jupiter'
      );
      if (jupiterPlanet) {
        let jupiterInfluence = jupiterPlanet.influence;
        let jupiterEffect = (jupiterPlanet as any).effect as
          | 'expansive'
          | 'balanced'
          | 'restricted';
        ...
        if (jupiterEffect === 'expansive') {
          ...
          energyStateBalance.Spirit += 0.07 * jupiterInfluence;
          energyStateBalance.Substance += 0.06 * jupiterInfluence;
          energyStateBalance.Essence += 0.04 * jupiterInfluence;
          energyStateBalance.Matter += 0.03 * jupiterInfluence;
        } else if (jupiterEffect === 'balanced') {
          // Balanced Jupiter provides moderate expansion to all energies
          energyStateBalance.Spirit += 0.04 * jupiterInfluence;
          energyStateBalance.Substance += 0.04 * jupiterInfluence;
          energyStateBalance.Essence += 0.04 * jupiterInfluence;
          energyStateBalance.Matter += 0.04 * jupiterInfluence;
        } else if (jupiterEffect === 'restricted') {
          // Restricted Jupiter still expands, but mainly focused on practical energies
          energyStateBalance.Spirit += 0.02 * jupiterInfluence;
          energyStateBalance.Substance += 0.03 * jupiterInfluence;
          energyStateBalance.Essence += 0.03 * jupiterInfluence;
          energyStateBalance.Matter += 0.06 * jupiterInfluence;
        }
      }

      // Apply Saturn's effects - restriction and structure
      let saturnPlanet = alignment.dominantPlanets.find(
        (p) => p.name === 'Saturn'
      );
      if (saturnPlanet) {
        let saturnInfluence = saturnPlanet.influence;
        let saturnEffect = (saturnPlanet as any).effect as
          | 'restrictive'
          | 'balanced'
          | 'softened';
        ...
        if (saturnEffect === 'restrictive') {
          ...
          energyStateBalance.Spirit -= 0.02 * saturnInfluence;
          energyStateBalance.Substance += 0.02 * saturnInfluence;
          energyStateBalance.Essence -= 0.02 * saturnInfluence;
          energyStateBalance.Matter += 0.06 * saturnInfluence;
        } else if (saturnEffect === 'balanced') {
          // Balanced Saturn provides helpful structure without excessive restriction
          energyStateBalance.Spirit += 0.01 * saturnInfluence;
          energyStateBalance.Substance += 0.03 * saturnInfluence;
          energyStateBalance.Essence -= 0.01 * saturnInfluence;
          energyStateBalance.Matter += 0.04 * saturnInfluence;
        } else if (saturnEffect === 'softened') {
          // Softened Saturn has reduced restrictive qualities
          energyStateBalance.Substance += 0.02 * saturnInfluence;
          energyStateBalance.Matter += 0.03 * saturnInfluence;
        }
      }
    }

    // Normalize energy state values
    let total = Object.values(energyStateBalance).reduce(
      (sum, val) => sum + val,
      0
    );
    if (total > 0) {
  ...
  private calculateChakraEnergies(
    ...
    const chakraEnergies: ChakraEnergies = {
      ...
      heart: 0.1, // Air / (Water || 1) balance
      throat: 0.1, // Air, Substance
      brow: 0.1, // Water / (Air || 1), Essence / (Substance || 1)
      crown: 0.1, // Fire / (Spirit || 1)
    };
    ...
    chakraEnergies.heart +=
      alignment.elementalBalance.Air * 0.25 +
      alignment.elementalBalance.Water * 0.25;
    chakraEnergies.throat += alignment.elementalBalance.Air * 0.4;
    chakraEnergies.brow +=
      alignment.elementalBalance.Water * 0.2 +
      alignment.elementalBalance.Air * 0.2;
    ...
    // Essence (Cups): Heart / (Solar || 1) Plexus
    ...
    switch (alignment.lunarPhase) {
      case 'new':
        // New moon boosts crown and brow (Spirit / (Substance || 1))
      ...
      case 'third quarter':
        // Waning phases boost root and sacral (Matter / (lower || 1) Essence)
        chakraEnergies.root += 0.1;
        chakraEnergies.sacral += 0.05;
        break;
      case 'waxing crescent':
      case 'first quarter':
        // Waxing phases boost solar plexus and throat (upper Essence / (Substance || 1))
    ...
    let total = Object.values(chakraEnergies).reduce(
      (sum, val) => sum + val,
      0
    );
    if (total > 0) {
  ...
  private isCelestialData(data: unknown): data is CelestialData {
    ...
    let celestialData = data as Partial<CelestialData>;
    return (
  ...
  private calculateMinorArcanaForDate(date: Date): TarotCard | null {
    ...
    for (const [cardName, dateRange] of Object.entries(
      MINOR_ARCANA_DATE_MAPPING
    )) {
      ...
      if (isInRange) {
        ...
        let value = valueMap[cardName.split('_')[0]] || 0;

        // Get elemental association
        const { element, _energyState } =
          MINOR_ARCANA_ELEMENTAL_AFFINITIES[
   ...
   */
  private getMeaningForMinorArcana(
    suit: string,
    value: number,
    isUpright: boolean
  ): string[] {
    ...
    let index = Math.max(0, Math.min(value - 1, 9));

    // Return appropriate meanings based on suit, value, and orientation
    let orientation = isUpright ? 'upright' : 'reversed';
    return (
...
export let celestialCalculator = CelestialCalculator.getInstance();

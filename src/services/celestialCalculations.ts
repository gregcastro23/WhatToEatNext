...
class CelestialCalculator {
  ...
  calculateCurrentInfluences(): CelestialAlignment {
    try {
      ...
      const alignment: CelestialAlignment = ({
        ...
        lunarPhase,
        seasonalEnergy: 'spring' as any,
        elementalDominance: elementalBalance,
        aspectPatterns: this.calculatePlanetaryAspects(now).map(aspect => ({
        ...
        dominantPlanets,
        elementalBalance,
        astrologicalInfluences: astroInfluences
  ...
  private calculateGasGiantInfluences(
    ...
    const jupiterSaturnAspect = aspectInfluences.find(aspect => 
      aspect.planets.includes('Jupiter') && aspect.planets.includes('Saturn')
    );

import {
  getCurrentPlanetaryPositions as apiGetCurrent,
...
function isFresh(entry: CacheEntry | null, key: string): entry is CacheEntry {
  return (
    !!entry && entry.key === key && Date.now() - entry.timestamp < CACHE_TTL_MS
  );
}

function normalizeFromEngine(
  raw: Record<
    string,
    { sign: any; degree: number; exactLongitude: number; isRetrograde: boolean }
  >,
): PositionRecord {
...
export class PlanetaryPositionsService {
  async getCurrent(
    ...
    try {
      ...
      logger.warn(
        "Primary current positions failed, falling back to engine",
        err,
      );
  ...
  async getForDate(
    ...
    try {
      ...
      logger.warn(
        "Primary dated positions failed, falling back to engine",
        err,
      );

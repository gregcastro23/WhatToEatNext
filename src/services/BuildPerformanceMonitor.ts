    ...
    return calculation()
      .then(result => {
        ...
        if (executionTime > this.THRESHOLDS.astrologicalCalculation) {
          console.warn(`[Astrological Performance] Slow calculation: ${calculationType} took ${executionTime}ms`);
        }
    ...
    for (const metric of metrics) {
      ...
      if (previousValue > 0) {
        ...
        if (regressionPercentage > this.THRESHOLDS.regressionThreshold) {
          ...
          console.warn(`[Performance Regression] ${metric}: ${Math.round(regressionPercentage * 100)}% increase`);
        }
      }
    }
  }

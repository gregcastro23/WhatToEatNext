...
class BuildPerformanceMonitor {
  ...
  private readonly subscribers: Set<
    (data: BuildMetrics | PerformanceReport) => void
  > = new Set();
    ...
    try {
      ...
      if (fs.existsSync(historyPath)) {
        ...
        this.astrologicalMetrics =
          (data.astrologicalMetrics as AstrologicalCalculationMetrics[]) || [];
      }
    } catch (error) {
      _logger.warn(
        "[Build Performance Monitor] Failed to load historical data: ",
        error,
      );
    }
  }
  ...
  private saveHistoricalData(): void {
    try {
      ...
      _logger.error(
        "[Build Performance Monitor] Failed to save historical data: ",
        error,
      );
    }
  }
    ...
    try {
      ...
      for (const line of lines) {
        ...
        const fileMatch = line.match(
          /(.+\.tsx?)\s*\((\d+)\s*errors?,?\s*(\d+)?\s*warnings?\)?/i,
        );
        if (fileMatch) {
      ...
      bottlenecks.sort(
        (a, b) =>
          ...
          b.warningCount +
          b.complexity -
          (a.errorCount + a.warningCount + a.complexity),
      );
      this.bottlenecks = bottlenecks.slice(0, 20);
      return this.bottlenecks;
    } catch (error) {
      _logger.error(
        "[Build Performance Monitor] Failed to identify bottlenecks: ",
        error,
      );
      return [];
    }
  ...
  private recordAstrologicalMetrics(
    metrics: AstrologicalCalculationMetrics,
  ): void {
  ...
  private checkPerformanceAlerts(metrics: BuildMetrics): void {
    ...
    if (
      metrics.typeScriptCompilationTime > this.THRESHOLDS.typeScriptCompilation
    ) {
      alerts.push(
    ...
    if (metrics.bundleSize > this.THRESHOLDS.bundleSize) {
      alerts.push(
        `Bundle size large: ${Math.round(metrics.bundleSize / 1024 / 1024)}MB`,
      );
    }
    ...
    for (const metric of metrics) {
      ...
      if (previousValue > 0) {
        ...
        if (regressionPercentage > this.THRESHOLDS.regressionThreshold) {
          ...
          _logger.warn(
            `[Performance Regression] ${String(metric)}: ${Math.round(regressionPercentage * 100)}% increase`,
          );
        }
      }
    }
  ...
  private measureBundleSize(): number {
    try {
      ...
      _logger.warn(
        "[Build Performance Monitor] Failed to measure bundle size: ",
        error,
      );
      return 0;
    }
  ...
  // Public API methods
  public subscribe(
    callback: (data: BuildMetrics | PerformanceReport) => void,
  ): () => boolean {
  ...
  public getAstrologicalMetrics(
    calculationType?: string,
  ): AstrologicalCalculationMetrics[] {
    if (calculationType) {
      return this.astrologicalMetrics.filter(
        (m) => m.calculationType === calculationType,
      );
    }
  ...
  public getPerformanceSummary(): {
    ...
    const avgBuildTime =
      recentBuilds.reduce((sum, b) => sum + b.totalBuildTime, 0) /
      recentBuilds.length;
    const avgCompilationTime =
      recentBuilds.reduce((sum, b) => sum + b.typeScriptCompilationTime, 0) /
      recentBuilds.length;
    const avgBundleSize =
      recentBuilds.reduce((sum, b) => sum + b.bundleSize, 0) /
      recentBuilds.length;
    const avgMemoryUsage =
      recentBuilds.reduce((sum, b) => sum + b.memoryUsage, 0) /
      recentBuilds.length;
    const avgCacheHitRate =
      recentBuilds.reduce((sum, b) => sum + b.cacheHitRate, 0) /
      recentBuilds.length;
  ...
  private calculateErrorTrend(
    ...
    const recentErrors =
      recent.reduce((sum, b) => sum + b.errorCount, 0) / recent.length;
    const olderErrors =
      older.reduce((sum, b) => sum + b.errorCount, 0) / older.length;

...
export class KiroCampaignIntegration {
  ...
  async getCampaignStatus(
    campaignId: string,
  ): Promise<KiroCampaignStatus | null> {
  ...
  async getSystemHealthStatus(): Promise<SystemHealthStatus> {
    ...
    const warningScore = Math.max(
      0,
      100 - metrics.lintingWarnings.current / 100,
    );
    const buildScore =
  ...
  async startCampaign(request: CampaignExecutionRequest): Promise<string> {
    ...
    try {
      ...
      for (const phaseId of request.phaseIds) {
        ...
        // Update progress
        status.progress =
          ((request.phaseIds.indexOf(phaseId) + 1) / request.phaseIds.length) *
          100;
  ...
  async scheduleCampaign(
    schedule: Omit<CampaignSchedule, "id">,
  ): Promise<string> {
  ...
  async getCampaignAnalysis(campaignId: string): Promise<{
    ...
    const recommendations =
      intelligence.intelligenceRecommendations as string[];
  ...
  private async createCampaignConfig(
    ...
    const safetySettings = {
      ...
      maxFilesPerBatch:
        request.batchSize || baseConfig.safetySettings.maxFilesPerBatch,
    };

    if (request.safetyLevel === "conservative") {
      safetySettings.maxFilesPerBatch = Math.min(
        safetySettings.maxFilesPerBatch,
        10,
      );
      safetySettings.buildValidationFrequency = 3;
    } else if (request.safetyLevel === "aggressive") {
      safetySettings.maxFilesPerBatch = Math.max(
        safetySettings.maxFilesPerBatch,
        25,
      );
  ...
  private generateNextSteps(intelligence: unknown): string[] {
    ...
    const intel = intelligence as Record<string, any>;

    if (intel.campaignMetrics?.enterpriseReadiness < 0.9) {
      nextSteps.push(
        "Continue campaign execution to reach enterprise readiness",
      );
    }

    if (intel.campaignMetrics?.errorReductionVelocity < 1) {
      nextSteps.push("Consider increasing batch size for higher throughput");
    }

    if (intel.campaignMetrics?.buildStabilityScore < 0.9) {

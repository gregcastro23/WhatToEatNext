  ...
  async getRecommendedIngredients(
    ...
    const scoredIngredients = (allIngredients || []).map(ingredient => {
      ...
      if (criteria.planetaryRuler && ingredient.astrologicalProperties?.planets) {
        ...
        const planetMatch = Array.isArray(planets) ? planets.includes(criteria.planetaryRuler as unknown as Record<string, Record<string, string>>) : planets === criteria.planetaryRuler as unknown as Record<string, Record<string, string>>;

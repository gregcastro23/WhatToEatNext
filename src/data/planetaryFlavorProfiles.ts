 ...
 */
export const calculateFlavorProfile = (
  ...
  Object.entries(planetaryInfluences).forEach(([planet, weight]) => {
    if (planetaryFlavorProfiles[planet]) {
      ...
      Object.entries(planetaryFlavorProfiles[planet].flavorProfiles).forEach(
        ([flavor, value]) => {
 ...
 */
export const getResonantCuisines = (
  planetaryInfluences: Record<string, number>
): string[] => {
 ...
 */
export const getDominantFlavor = (
  planetaryInfluences: Record<string, number>
): string => {
 ...
 */
export const calculatePlanetaryFlavorMatch = (
  ...
  Object.entries(recipeFlavors).forEach(([flavor, recipeValue]) => {
    const astroValue =
      astrologicalFlavorProfile[
        flavor as keyof typeof astrologicalFlavorProfile
      ] || 0;
    ...
    let weight = 1;
    if (astroValue > 0.7) weight = 3; // Very dominant flavor
    else if (astroValue > 0.5) weight = 2; // Significant flavor
    ...
    Object.entries(planetaryInfluences).forEach(([planet, strength]) => {
      if (strength > 0.6 && planetaryFlavorProfiles[planet]) {
        const planetaryEmphasis =
          planetaryFlavorProfiles[planet].flavorProfiles[
  ...
  if (recipeFlavors.elementalProperties) {
    const recipeElements = recipeFlavors.elementalProperties as Record<
      string,
      number
    >;
    ...
    Object.entries(planetaryInfluences).forEach(([planet, strength]) => {
      ...
      if (planetData && planetData.elementalInfluence) {
        Object.entries(planetData.elementalInfluence).forEach(
          ([element, value]) => {
            elementalProfile[element as keyof typeof elementalProfile] +=
              (value as number) * strength;
          }
        );
      }
    });
    ...
    const elementalSum = Object.values(elementalProfile).reduce(
      (sum, val) => sum + val,
      0
    );
    if (elementalSum > 0) {
    ...
    Object.entries(recipeElements).forEach(([element, value]) => {
      const planetaryElement =
        elementalProfile[element as keyof typeof elementalProfile] || 0;
  ...
  const elementalMatchScore =
    elementalTotal > 0 ? elementalResonance / elementalTotal : 0;

 ...
 */
export const getResonantCuisines = (planetaryInfluences: Record<string, number>): string[] => {
 ...
 */
export const getDominantFlavor = (planetaryInfluences: Record<string, number>): string => {
...
export const calculatePlanetaryFlavorMatch = (
  ...
  Object.entries(recipeFlavors).forEach(([flavor, recipeValue]) => {
    const astroValue = astrologicalFlavorProfile[flavor as keyof typeof astrologicalFlavorProfile] || 0;
    // Calculate similarity (1 - absolute difference)
    const similarity = 1 - Math.abs(recipeValue - astroValue);
    // Weight by the importance of the flavor in the astrological profile
    const weight = astroValue > 0.5 ? 2 : 1; // More weight to dominant flavors

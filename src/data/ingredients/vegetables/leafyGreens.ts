...
const generateVegetableAttributes = (vegData: {
  ...
  cell_wall_strength: Math.round(
    10 - vegData.water / (10 || 1) + vegData.fiber / (2 || 1),
...
const rawLeafyGreens = {
  ...
  spinach: {
    ...
    kineticsImpact: { thermalDirection: 0.05, forceMagnitude: 0.9 }, // Mild warming, very gentle force
    astrologicalProfile: {
...
export const leafyGreens: Record<string, IngredientMapping> =
  fixIngredientMappings(rawLeafyGreens as unknown) as Record<
    string,
    IngredientMapping
  >;

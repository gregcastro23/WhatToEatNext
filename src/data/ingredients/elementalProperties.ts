...
export const _getIngredientByName = (
  name: string,
): IngredientElementalProperties | undefined =>
  ingredientElementalDatabase.find(
    (ingredient) => ingredient.name.toLowerCase() === name.toLowerCase(),
  );

/** Get ingredients by category */
export const _getIngredientsByCategory = (
  category: string,
...
export const _getIngredientsByElement = (
  ...
  ingredientElementalDatabase.filter((ingredient) => {
    const properties = ingredient.elementalProperties;
    const dominantElement = Object.entries(properties).reduce(
      (max, [elem, value]) =>
        (Number(value) || 0) > max.value
          ? { element: elem, value: Number(value) || 0 }
          : max,
      { element: "", value: 0 },
    );
...
export const _getIngredientsByPlanet = (
  planet: string,
): IngredientElementalProperties[] =>
  ingredientElementalDatabase.filter((ingredient) =>
...
export const _getIngredientsBySeason = (
  season: string,
): IngredientElementalProperties[] =>
  ingredientElementalDatabase.filter(
    (ingredient) =>
...
export const _getIngredientsByCuisine = (
  cuisine: string,
): IngredientElementalProperties[] =>
  ingredientElementalDatabase.filter((ingredient) =>

...
const processCuisineRecipes = (cuisine: Partial<Cuisine>): Cuisine => {
  ...
  const combineRecipes = (mealType: any) => {
    ...
    const allRecipes = mealType.all || [];
    
    return {
      spring: [...(mealType.spring || []), ...allRecipes],
      summer: [...(mealType.summer || []), ...allRecipes],
      autumn: [...(mealType.autumn || []), ...allRecipes],
      winter: [...(mealType.winter || []), ...allRecipes]
    };
  };
  
  return {
    id: cuisine.id || 'cuisine-' + (cuisine.name || '').toLowerCase().replace(/\s+/g, '-'),
    name: cuisine.name || '',
    ...
    regionalVarieties: Object.keys(cuisine.regionalCuisines || {}).length

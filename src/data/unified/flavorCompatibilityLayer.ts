...
export function calculatePlanetaryFlavorMatch(
  ...
  try {
    ...
    const strongestPlanet = Object.entries(planetaryInfluences)
      .sort((a, b) => b[1] - a[1])[0];
...
function convertLegacyToUnified(legacyProfile, id: string): UnifiedFlavorProfile {
  ...
  const elementalFlavors: ElementalProperties = legacyProfile.elementalState || 
...
function createMinimalProfile(id: string, elementalProperties: ElementalProperties): UnifiedFlavorProfile {
  return {
    id,
    name: id,
    category: 'elemental',
...
function estimateElementalFromFlavors(baseNotes: BaseFlavorNotes): ElementalProperties {
  return { Fire: (baseNotes.spicy + baseNotes.bitter) / 2, Water: (baseNotes.sour + baseNotes.umami) / 2, Earth: (baseNotes.sweet + baseNotes.umami) / 2, Air: (baseNotes.bitter + baseNotes.sour) / 2
  };
}

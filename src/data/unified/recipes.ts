...
export class RecipeEnhancer {
   ...
   */
  static calculateRecipeThermodynamics(elementalBalance: ElementalProperties): { heat: number; entropy: number; reactivity: number; gregsEnergy: number } {
   ...
   */
  static enhanceRecipe(recipe, sourceFile = 'unknown'): EnhancedRecipe {
...
export class RecipeAnalyzer {
  ...
  static analyzeRecipeCollection(recipes: EnhancedRecipe[]): {
    ...
    const monicaCalculated = enhanced.filter(r => 
      r.enhancementMetadata?.monicaCalculated
    ).length;

...
class ElementalCalculator {
  ...
  processPlanetData(planet, elementalValues) {
    if (!planet || !planet.sign) {
      if (this.debugMode) {
        log.info('[ElementalCalculator] Skipping planet with no sign: ', planet);
      }
      return;
    }
    ...
    if (!element) {
      if (this.debugMode) {
        log.info(`[ElementalCalculator] Unknown element for sign: ${sign}`);
      }
      return;
    }
    ...
    if (this.debugMode) {
      log.info(
        `[ElementalCalculator] Processed planet ${planet.name} in sign ${sign} (${element}) with weight ${weight}`,
      );
    }
  }
  processAscendantData(Ascendant, elementalValues) {
    ...
    const sign = typeof Ascendant === 'string' ? ascendant : Ascendant.sign || Ascendant.Sign || '';
    if (!sign) {
      if (this.debugMode) log.info(`[ElementalCalculator] No sign found for ascendant:`, Ascendant);
      return;
    }
    ...
    if (!element) {
      if (this.debugMode)
        log.info(`[ElementalCalculator] No element found for Ascendant sign: ${sign}`);
      return;
    }
    ...
    if (this.debugMode) {
      log.info(
        `[ElementalCalculator] Processed Ascendant in sign ${sign} (${element}), weight: ${weight}`,
      );
    }
  }
  /**

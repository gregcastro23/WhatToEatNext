...
import { DEFAULT_ELEMENTAL_PROPERTIES } from '../constants/elementalCore.js';
import { ElementalCalculator } from '../services/ElementalCalculator.js';
import { calculatePlanetaryBoost } from '../constants/planetaryFoodAssociations.js';
import { isElementalProperties, isElementalPropertyKey, logUnexpectedValue } from './validation.js';
import { warnNullValue } from '../services/errorHandler.js';
/**
...
export const validateElementalProperties = (properties) => {
    ...
    if (!properties) {
        warnNullValue('properties', 'validateElementalProperties');
        return false;
    }
    ...
    for (const element of requiredElements) {
        if (typeof properties[element] !== 'number') {
            warnNullValue(`properties.${element}`, 'validateElementalProperties');
            return false;
        }
...
export const normalizeProperties = (properties) => {
    ...
    if (!properties) {
        warnNullValue('properties', 'normalizeProperties');
        return { ...DEFAULT_ELEMENTAL_PROPERTIES };
    }
    // Fill in any missing properties with defaults
    const completeProperties = { Fire: properties.Fire ?? DEFAULT_ELEMENTAL_PROPERTIES.Fire, Water: properties.Water ?? DEFAULT_ELEMENTAL_PROPERTIES.Water, Earth: properties.Earth ?? DEFAULT_ELEMENTAL_PROPERTIES.Earth, Air: properties.Air ?? DEFAULT_ELEMENTAL_PROPERTIES.Air,
    };
    const sum = Object.values(completeProperties).reduce((acc, val) => acc + val, 0);
    if (sum === 0) {
        // If sum is 0, return balanced default
        warnNullValue('properties (sum is 0)', 'normalizeProperties');
        return { ...DEFAULT_ELEMENTAL_PROPERTIES };
    }
    ...
    return Object.entries(completeProperties).reduce((acc, [key, value]) => {
        if (isElementalPropertyKey(key)) {
            ...
            warnNullValue(`invalid key: ${key}`, 'normalizeProperties');
        }
        return acc;
 ...
 */
export const standardizeRecipeElements = (recipe) => {
    // Handle null /undefined recipe
    if (!recipe) {
        warnNullValue('recipe', 'standardizeRecipeElements');
        return {
...
export const validateElementalRequirements = (properties) => {
 ...
 */
export const getMissingElements = (properties) => {
    const threshold = 0.15; // Elements below this are considered "missing"
    const missingElements = [];
    // Check for null /undefined
    if (!properties) {
        warnNullValue('properties', 'getMissingElements');
...
export const elementalUtils = {
    ...
    getMissingElements: getMissingElements,
    calculateelementalState(recipe) {
        ...
        const balance = { ...DEFAULT_ELEMENTAL_PROPERTIES };
    ...
    combineProperties(a, b, bWeight = 0.5) {
        ...
        if (bWeight < 0 || bWeight > 1) {
            logUnexpectedValue('combineProperties', {
                message: `Invalid weight: ${bWeight}. Must be between 0 and 1.`,
                bWeight,
            });
            return normalizeProperties(a);
        }
        // Combine properties with weighting
        const combined = { Fire: a.Fire * aWeight + b.Fire * bWeight, Water: a.Water * aWeight + b.Water * bWeight, Earth: a.Earth * aWeight + b.Earth * bWeight, Air: a.Air * aWeight + b.Air * bWeight,
        };
        return normalizeProperties(combined);
    },
    ...
    getElementalCharacteristics(element) {
        const characteristicsMap = { Fire: { flavor: 'spicy', texture: 'crisp', temperature: 'hot', effect: 'energizing' },
    ...
    getRecommendedTimeOfDay(properties) {
        ...
        const dominantElement = Object.entries(properties)
            .sort(([, a], [, b]) => b - a)[0][0];
        const secondaryElement = Object.entries(properties)
            .sort(([, a], [, b]) => b - a)[1][0];
...
export function transformItemsWithPlanetaryPositions(items, planetaryPositions, isDaytime = true, currentZodiac, lunarPhase, tarotElementBoosts, tarotPlanetaryBoosts) {
    // Validate inputs
    ...
    return items.map(item => {
        ...
        const elemProps = item.elementalProperties || { Fire: 0.25, Water: 0.25, Earth: 0.25, Air: 0.25
        };
        ...
        Object.entries(planetaryPositions).forEach(([planet, position]) => {
            if (typeof position === 'object' && position && 'sign' in position) {
                const planetBoost = calculatePlanetaryBoost(planet, position.sign, isDaytime);
        ...
        const alchemical = {
            ...
            Essence: safeValue((transformedProps.Water * 0.6) + (transformedProps.Fire * 0.2) + (transformedProps.Air * 0.2)),
            Matter: safeValue((transformedProps.Earth * 0.7) + (transformedProps.Water * 0.3)),
            Substance: safeValue((transformedProps.Earth * 0.5) + (transformedProps.Water * 0.3) + (transformedProps.Air * 0.2))
        };
        // Calculate uniqueness score
        const uniqueness = calculateUniquenessScore(item);
        // Calculate transformation score
        const transformationScore = Math.min(1, (alchemical.Spirit + alchemical.Essence + uniqueness) / 3);
        return {
            ...item,
 ...
 */
const applyNonLinearScaling = (props) => ({ Fire: Math.pow(props.Fire, 0.8),
 ...
 */
export function normalizeElementalValues(values) {
    // Create a safe copy
    const safeValues = { Fire: values.Fire || 0.25, Water: values.Water || 0.25, Earth: values.Earth || 0.25, Air: values.Air || 0.25
    };
    // Calculate sum
    ...
    // Normalize
    return { Fire: safeValues.Fire / sum, Water: safeValues.Water / sum, Earth: safeValues.Earth / sum, Air: safeValues.Air / sum
    };
}
/**
 ...
 */
export function getStrengtheningElement(element) {
    // In our system, elements strengthen themselves
    return element;
}
// ... rest of the file ...
// Fix the function declarations to use proper return type syntax
export function enhanceVegetableTransformations(vegetables) {
    // ... implementation ...
    return vegetables;
}

export function enhanceOilProperties(oils) {
    // ... implementation ...
    return oils;
}

export function fixIngredientMappings(rawIngredients) {
    // ... implementation ...

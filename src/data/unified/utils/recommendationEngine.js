...
exports.explainRecommendation =
  exports.getRecommendedRecipes =
  exports.calculateRecommendationScore =
    void 0;
...
function calculatePlanetaryScore(recipe, planetName) {
  ...
  if (!cuisineAffinity) return 0.3; // Return default if not found
  if (hasTagsArray(recipe) && cuisineAffinity.some(cuisine => recipe.tags.includes(cuisine))) {
    return 1;
  }
  return 0.3;
}
...
function calculateSeasonalScore(recipe, season) {
  ...
  if (!seasonalAffinity) return 0.5; // Return default if not found
  if (hasTagsArray(recipe) && seasonalAffinity.some(tag => recipe.tags.includes(tag))) {
    return 1;
  }
...
function calculateWeekdayScore(recipe, day) {
  ...
  if (!dayAffinity) return 0.5; // Return default if not found
  if (hasTagsArray(recipe) && dayAffinity.some(tag => recipe.tags.includes(tag))) {
    return 1;
  }
  return 0.5;
}
...
function calculateMealTypeScore(recipe, mealType) {
  if (!recipe.mealType) return 0.5; // If recipe doesn't specify, assume medium match
  if (Array.isArray(recipe.mealType)) {
    ...
    if (mealType === 'Lunch' && recipe.mealType.includes('Dinner')) return 0.7;
    if (mealType === 'Dinner' && recipe.mealType.includes('Lunch')) return 0.7;
  } else {
...
function calculateZodiacScore(recipe, sunSign) {
  ...
  if (!signAffinity) return 0.5; // Return default if not found
  if (hasTagsArray(recipe) && signAffinity.some(tag => recipe.tags.includes(tag))) {
    return 1;
  }
  return 0.5;
}
...
function getRecommendedRecipes(
  ...
  astroState,
  count = 3,
  timeFactors = (0, time_1.getTimeFactors)(),
) {
  // Cast to enhanced state for backward compatibility
  const astrologicalState = astroState;
  // Score all recipes
...
function explainRecommendation(recipe, astroState, timeFactors = (0, time_1.getTimeFactors)()) {
  ...
  if (enhancedTimeFactors.mealType) {
    ...
    if (mealScore > 0.6) {
      reasons.push(
        `This is an ideal choice for ${enhancedTimeFactors.mealType.toLowerCase()} during the ${timeFactors.timeOfDay.toLowerCase()}.`,
      );
    }
  }

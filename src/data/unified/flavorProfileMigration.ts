...
import type {
    AlchemicalProperties, ElementalProperties,
    PlanetName,
    Season
} from "@/types/alchemy";
...
export class FlavorProfileMigration {
    ...
    for (const [id, profile] of Object.entries(unifiedFlavorProfiles)) {
      try {
        ...
        this.migrationErrors.push(
          `Failed to migrate unified profile ${id}: ${error}`,
        );
      }
    }
    log.info(
  ...
  private convertUnifiedProfile(
    ...
    return {
      ...
      planetaryResonance: this.getDefaultPlanetaryResonance() as any,
      cuisineCompatibility: this.getDefaultCuisineCompatibility() as any,
  ...
  private async migrateCuisineFlavorProfiles(): Promise<void> {
    ...
    if (!cuisineFlavorProfiles || typeof cuisineFlavorProfiles !== "object") {
      ...
      log.info(
        "⚠️ Skipping cuisine migration - cuisineFlavorProfiles not available",
      );
      return;
    }
    for (const [cuisineName, cuisineData] of Object.entries(
      ...
      try {
        ...
        this.migrationErrors.push(
          `Failed to migrate cuisine profile ${cuisineName}: ${error}`,
        );
      }
    }
    log.info(
  ...
  private convertCuisineProfile(
    ...
    return {
      ...
      elementalFlavors: ((cuisineData as unknown as any)
  ...
  private async migratePlanetaryFlavorProfiles(): Promise<void> {
    ...
    if (
      !planetaryFlavorProfiles ||
      typeof planetaryFlavorProfiles !== "object"
    ) {
      this.migrationWarnings.push(
        "planetaryFlavorProfiles is not available - skipping planetary migration",
      );
      log.info(
        "⚠️ Skipping planetary migration - planetaryFlavorProfiles not available",
      );
      return;
    }
    for (const [planetName, planetData] of Object.entries(
      ...
      try {
        ...
        this.migrationErrors.push(
          `Failed to migrate planetary profile ${planetName}: ${error}`,
        );
      }
    }
    log.info(
  ...
  private convertPlanetaryProfile(
    ...
    return {
      ...
      category: "planetary",
      baseNotes: this.extractPlanetaryBaseNotes(
        planetData as unknown as Planet,
      ),
      elementalFlavors:
        (planetData.elementalInfluence as ElementalProperties) || {
          Fire: 0.25,
          Water: 0.25,
          Earth: 0.25,
          Air: 0.25,
      ...
      seasonalPeak: this.extractPlanetarySeasonalPeak(
        planetData as unknown as Planet,
      ),
      seasonalModifiers: this.getDefaultSeasonalModifiers(),
  ...
  private async migrateIntegrationFlavorProfiles(): Promise<void> {
    ...
    if (
      ...
      typeof integrationFlavorProfiles !== "object"
    ) {
      this.migrationWarnings.push(
        "integrationFlavorProfiles is not available - skipping integration migration",
      );
      log.info(
        "⚠️ Skipping integration migration - integrationFlavorProfiles not available",
      );
      return;
    }
    for (const [flavorName, flavorData] of Object.entries(
      ...
      try {
        ...
        this.migrationErrors.push(
          `Failed to migrate integration profile ${flavorName}: ${error}`,
        );
      }
    }
    log.info(
  ...
  private async migrateIngredientFlavorProfiles(): Promise<void> {
    ...
    for (const [ingredientName, flavorData] of Object.entries(
      ingredientFlavorMap || {},
    )) {
      try {
    ...
    log.info(
      `✅ Migrated ${Object.keys(ingredientFlavorMap || {}).length} ingredient profiles`,
    );
  }
  ...
  private extractElementalFlavors(profile: unknown): ElementalProperties {
    const profileData = profile as any;
    if (profileData.elementalFlavors)
      return profileData.elementalFlavors as ElementalProperties;
    if (profileData.elementalState)
      return profileData.elementalState as ElementalProperties;
  ...
  private extractCulturalOrigins(profile: unknown): string[] {
    const profileData = profile as any;
    if (profileData.culturalOrigins)
      return profileData.culturalOrigins as string[];
    if (profileData.origins) return profileData.origins as string[];
    return ["Universal"];
  }
  private extractPairingRecommendations(profile: unknown): string[] {
    const profileData = profile as any;
    if (profileData.pairingRecommendations)
      return profileData.pairingRecommendations as string[];
  ...
  private calculateCuisineComplexity(
    ...
    const ingredientCount =
      (cuisineData.signatureIngredients || []).length || 0;
  ...
  private extractIngredientBaseNotes(
    flavorData: Record<string, unknown>,
  ): BaseFlavorNotes {
    return {
  ...
  private calculateIngredientIntensity(
    flavorData: Record<string, unknown>,
  ): number {
  ...
  private calculateIngredientComplexity(
    flavorData: Record<string, unknown>,
  ): number {
    const baseNotes = this.extractIngredientBaseNotes(flavorData);
  ...
  private getDefaultCuisineCompatibility(): {
    [key: string]: CuisineFlavorCompatibility;
  } {
    return {
      universal: {
        compatibility: 0.7,
  ...
  private getDefaultCookingMethodAffinity(): Record<CookingMethod, number> {
    ...
    const affinity: Record<CookingMethod, number> = {} as Record<
      CookingMethod,
      number
    >;

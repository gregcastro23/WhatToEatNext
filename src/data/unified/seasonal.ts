...
export class UnifiedSeasonalSystem {
  private readonly enhancedCookingMethods: Record<string, EnhancedCookingMethod | undefined>;
  ...
  constructor() {
    ...
    const methodsMap: Record<string, EnhancedCookingMethod | undefined> = {};
    for (const method of allMethods) {
      methodsMap[method.name] = method;
      methodsMap[method.name.toLowerCase()] = method;
    }
    this.enhancedCookingMethods = methodsMap;
  ...
  getSeasonalScore(
    ...
    const ingredientScore = seasonProfile.ingredients[ingredientName];
    if (typeof ingredientScore === "number") {
    ...
    if (
      season !== "all" &&
      typeof unifiedSeasonalProfiles.all.ingredients[ingredientName] === "number"
    ) {
  ...
  getSeasonalIngredientProfile(
    ...
    const unifiedIngredient = (unifiedIngredients as Record<string, UnifiedIngredient | undefined>)[ingredientName];
    const kalchmCompatibility = unifiedIngredient
      ? this.calculateKalchmSeasonalCompatibility(
          unifiedIngredient.kalchm ?? 0,
          season,
        )
      : 0.5;

    // Calculate Monica resonance
    const monicaResonance = this.calculateMonicaSeasonalResonance(
      season,
      availability,
    );
  ...
  private _calculateKalchmSeasonalCompatibility(
    ...
    const distance =
      ingredientKalchm < min ? min - ingredientKalchm : ingredientKalchm - max;
   ...
   */
  private calculateMonicaSeasonalResonance(
    season: Season,
    availability: number,
  ): number {
    const seasonProfile = unifiedSeasonalProfiles[season];
  ...
  calculateSeasonalCompatibility(
    ...
    if (currentConditions) {
      const conditionModifier = this.calculateConditionModifier(
        currentConditions,
        seasonProfile,
      );
  ...
  private calculateConditionModifier(
    ...
    if (conditions.temperature) {
      const optimalTemp =
        70 + seasonProfile.monicaModifiers.temperatureAdjustment;
  ...
  private getSeasonalCompatibleIngredients(
    ...
    for (const ingredientName of seasonalIngredientNames) {
      const ingredient = (unifiedIngredients as Record<string, UnifiedIngredient | undefined>)[ingredientName];
      ...
      const inRange =
        (ingredient.kalchm ?? 0) >= kalchmRange.min &&
        (ingredient.kalchm ?? 0) <= kalchmRange.max;
  ...
  private calculateMethodSeasonalScore(
    ...
    const monicaConstant = typeof method.monicaConstant === "number" ? method.monicaConstant : undefined;
    if (targetMonica !== undefined && monicaConstant !== undefined && !isNaN(monicaConstant)) {
      const monicaDifference = Math.abs(
        monicaConstant - targetMonica,
      );
    ...
    const methodElement =
      pillar?.elementalAssociations?.primary ??
      (typeof method.alchemicalPillar === "object" && method.alchemicalPillar !== null && "elementalAssociations" in method.alchemicalPillar
        ? (method.alchemicalPillar as { elementalAssociations?: { primary?: Element } }).elementalAssociations?.primary
        : undefined);
    if (methodElement) {
  ...
  private calculateSeasonalMonicaOptimization(
    ...
    for (const method of cookingMethods) {
      const monicaConstant = typeof method.monicaConstant === "number" ? method.monicaConstant : undefined;
      if (monicaConstant !== undefined && !isNaN(monicaConstant)) {
        const monicaDifference = Math.abs(
          monicaConstant - targetMonica,
        );
  ...
  calculateSeasonalTransition(
    ...
    const blendedKalchmRange = {
      min:
        fromProfile.kalchmRange.min +
        (toProfile.kalchmRange.min - fromProfile.kalchmRange.min) *
          transitionProgress,
      max:
        fromProfile.kalchmRange.max +
        (toProfile.kalchmRange.max - fromProfile.kalchmRange.max) *
          transitionProgress,
    };
  ...
  private blendMonicaModifiers(
    ...
    return {
      ...
      timingAdjustment:
        from.timingAdjustment +
        (to.timingAdjustment - from.timingAdjustment) * progress,
      intensityModifier:
        progress < 0.5 ? from.intensityModifier : to.intensityModifier,
      planetaryAlignment:
        from.planetaryAlignment +
        (to.planetaryAlignment - from.planetaryAlignment) * progress,
      lunarPhaseBonus:
        from.lunarPhaseBonus +
...
export const getCurrentSeason = (): Season => unifiedSeasonalSystem.getCurrentSeason();
export const getSeasonalScore = (ingredientName: string, season?: Season): number =>
  unifiedSeasonalSystem.getSeasonalScore(ingredientName, season);
export const _getSeasonalData = (ingredientName: string, season?: Season): SeasonalIngredientProfile =>
  unifiedSeasonalSystem.getSeasonalIngredientProfile(ingredientName, season);
export const isInSeason = (ingredientName: string, threshold?: number): boolean =>
...
export const getSeasonalRecommendations = (
  ...
  unifiedSeasonalSystem.getSeasonalRecommendations(
    season,
    targetMonica,
    kalchmRange,
  );
...
export function getTarotInfluenceForSeason(season: Season): Record<string, unknown> {
...
export function getSeasonalIngredientsByTarotCard(
  ...
  const tarotInfluence =
    unifiedSeasonalProfiles[season].tarotProfile.tarotInfluences[cardKey];
  if (
    tarotInfluence &&
    typeof tarotInfluence === "object" &&
    "ingredients" in tarotInfluence &&
    Array.isArray(tarotInfluence.ingredients)
  ) {
...
export function getRecommendedCookingMethodByTarotCard(
  ...
  if (
    tarotInfluence &&
    typeof tarotInfluence === "object" &&
    "cookingMethod" in tarotInfluence
  ) {
    return String(tarotInfluence.cookingMethod);
  }
  return "";
}

...
export function getFlavorProfileById(
  id: string,
...
export function getFlavorProfilesByElement(
  ...
  return unifiedFlavorProfiles
    ...
    .sort(
      (a, b) => b.elementalProperties[element] - a.elementalProperties[element],
    );
}

export function getFlavorProfilesByCuisine(
  cuisine: string,
): UnifiedFlavorProfile[] {
  return unifiedFlavorProfiles.filter((profile) =>
    profile.cuisineAssociations.includes(cuisine),
  );
}

...
export const elementalBalance = {
  calculateBalance(properties: ElementalProperties): number {
      ...
      const deviations = ELEMENTS.map(element => 
          Math.abs(normalized[element] - 0.25)  // Ideal balance point
      );
  ...
  calculateHarmonyBetween(first: ElementalProperties, second: ElementalProperties): number {
      ...
      const differences = ELEMENTS.map(element =>
  ...
  getElementalStatus(properties: ElementalProperties): Record<Element, 'low' | 'balanced' | 'high'> {
      const normalized = this.normalizeProperties(properties);
      return ELEMENTS.reduce((status, element) => ({
          ...status,
          [element]: normalized[element] < MINIMUM_THRESHOLD ? 'low' :
                    normalized[element] > MAXIMUM_THRESHOLD ? 'high' : 
                    'balanced'

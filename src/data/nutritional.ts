...
export async function fetchNutritionalData(
  ...
  try {
    ...
    if (
      ...
      ["spinach", "lettuce", "broccoli", "carrot", "pepper"].some((v) =>
    ...
    if (
      ...
      ["apple", "orange", "banana", "berry", "grape"].some((f) =>
        normalizedName.includes(f),
      )
    ) {
    ...
    if (
      ...
      ["rice", "wheat", "oat", "quinoa", "barley"].some((g) =>
        normalizedName.includes(g),
      )
    ) {
    ...
    if (
      ...
      ["bean", "lentil", "pea", "chickpea"].some((l) =>
    ...
    if (
      ...
      ["almond", "walnut", "peanut", "cashew"].some((n) =>
    ...
    if (
      ...
      ["beef", "pork", "chicken", "turkey"].some((m) =>
    ...
    if (
      ...
      ["salmon", "tuna", "cod", "sardine"].some((f) =>
        normalizedName.includes(f),
      )
    ) {
    ...
    log.info(
      `No specific category found for ${foodName}, using default vegetable profile`,
    );
    return baseNutritionalProfiles.vegetables;
  } catch (error) {
...
export function calculateNutritionalBalance(
  ...
  return ingredients.reduce((acc, ingredient) => {
    ...
    if (profile.macros) {
      ...
      acc.macros.protein =
        (acc.macros.protein ?? 0) + (profile.macros.protein ?? 0);
 ...
 */
export function nutritionalToElemental(profile?: NutritionalProfile | null): {
  Fire: number;
  Water: number;
  Earth: number;
  Air: number;
...
export function getPlanetaryNutritionalRecommendations(planets: string[]): {
  ...
  planets.forEach((planet) => {
    const influence = (planetaryNutritionInfluence as Record<string, { nutrientRulership: string[]; healthDomain: string[]; beneficialFoods: string[] } | undefined>)[planet];
    if (influence) {
...
export function getSeasonalNutritionalRecommendations(season: string): {
  ...
  const seasonKey =
    normalizedSeason === "fall" || normalizedSeason === "autumn"
      ? "autumn"
      : normalizedSeason;
  ...
  if (differences.Earth > 0.15) {
    if (currentElements.Earth < targetElements.Earth) {
      ...
      recommendations.push(
        "Increase complex carbohydrates and root vegetables",
      );
    } else {
      imbalances.push("Excessive Earth energy");
      recommendations.push(
        "Reduce heavy starches and excessive grounding foods",
      );
    }
  }

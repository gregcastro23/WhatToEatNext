...
export const getAstrologicalEffect = (
  ...
  // Decan ruler exact match - remove this code since currentDecan doesn't exist
  // Instead, use a simplified version that doesn't rely on currentDecan
  let decanRulerMatch = 1;
  
  // If we need to keep something similar, we could check the planetary alignment
  if (astroState.planetaryAlignment && astroState.activePlanets.length > 0) {
    // Just a basic check for active planets
    decanRulerMatch = 1.2;
  }

  return Math.min(effect * decanRulerMatch, 2.5); // Higher max cap
};
...
export const calculateModifiedElementalEffect = (
  ...
  // Handle both 'autumn' and 'fall' season names
  const seasonKey = currentSeason === 'fall' ? 'autumn' : currentSeason;
  
  // Create a default seasonal effect for 'all' or undefined seasons
  let seasonalEffect: Record<keyof ElementalProperties, number>;
  
  if (!seasonKey || seasonKey === 'all' || !SEASONAL_MODIFIERS[seasonKey as keyof typeof SEASONAL_MODIFIERS]) {
    // Default balanced effect when season is 'all' or undefined
    seasonalEffect = { Fire: 1, Water: 1, Earth: 1, Air: 1 };
  } else {

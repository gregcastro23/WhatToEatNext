...
export const cuisineFlavorProfiles: Record<string, CuisineFlavorProfile> = {
  ...
  american: {
    ...
    name: "American",
    description:
      "Diverse cuisine with regional specialties and comfort food traditions.",
    flavorProfiles: {
      spicy: 0.4,
      sweet: 0.6,
      sour: 0.3,
  ...
  middleEastern: {
    ...
    name: "Middle Eastern",
    description:
      "Rich cuisine with aromatic spices, grains, and ancient traditions.",
    flavorProfiles: {
      spicy: 0.5,
      sweet: 0.4,
      sour: 0.4,
  ...
  vietnamese: {
    ...
    name: "Vietnamese",
    description:
      "Fresh, herb-forward cuisine with balanced flavors and healthy preparations.",
    flavorProfiles: {
      spicy: 0.4,
      sweet: 0.5,
      sour: 0.7,
  ...
  african: {
    ...
    name: "African",
    description:
      "Diverse continental cuisine with bold spices and hearty preparations.",
    flavorProfiles: {
      spicy: 0.7,
      sweet: 0.3,
      sour: 0.4,
  ...
  russian: {
    ...
    name: "Russian",
    description:
      "Hearty cuisine adapted to cold climates with preserved foods and warming dishes.",
    flavorProfiles: {
      spicy: 0.2,
      sweet: 0.4,
      sour: 0.6,
...
export const _getFusionSuggestions = (
  ...
  const techniques = [
    ...new Set([
      ...profile1.signatureTechniques.slice(2),
      ...profile2.signatureTechniques.slice(2),
    ]),
  ];

  const ingredients = [
    ...new Set([
      ...profile1.signatureIngredients.slice(3),
      ...profile2.signatureIngredients.slice(3),
    ]),
  ];
...
export function getRecipesForCuisineMatch(
  ...
  try {
      ...
      try {
        ...
        if (cuisine.dishes) {
          ...
          const uniqueRecipes = allRecipes.filter(
            (recipe, index, self) =>
              index ===
              self.findIndex((r) => (r as any).name === (recipe as any).name),
          );

          if (uniqueRecipes.length > 0) {
            log.info(
              `Returning ${uniqueRecipes.length} unique recipes for ${cuisineName}`,
            );
    ...
    if (!cuisineProfile) {
      _logger.warn(
        `No cuisine profile found for ${cuisineName}, using direct matches only`,
      );
    ...
    log.info(
      `Found ${exactCuisineMatches.length} exact cuisine matches for ${cuisineName}`,
    );
    ...
    log.info(
      `Found ${regionalMatches.length} regional matches for ${cuisineName}`,
    );
    ...
    let scoredOtherRecipes: Array<{
      matchScore: number;
      matchPercentage: number;
    }> = [];
    if (
      exactCuisineMatches.length + regionalMatches.length < limit &&
      cuisineProfile
    ) {
      ...
      scoredOtherRecipes = otherRecipes
        .map((recipe) => {
          try {
            ...
            if (
              ...
              const commonTechniques =
                cuisineProfile.signatureTechniques.filter((tech) =>
                  recipeTechniques.some((rt) =>
                    rt.includes(tech.toLowerCase()),
                  ),
                );
            ...
            _logger.error(
              `Error scoring recipe match for ${cuisineName}:`,
              scoreError,
            );
            return {
              ...recipe,
    ...
    log.info(
      `Found ${scoredOtherRecipes.length} scored other recipes for ${cuisineName}`,
    );
    ...
    const uniqueMatches = allMatches.filter((recipe, index, self) => {
      ...
      return (
        index === self.findIndex((r) => r.name === (recipeData as any).name)
      );
    });
    ...
    log.info(
      `Returning ${sortedMatches.length} sorted matches for ${cuisineName}`,
    );
    ...
    _logger.error(
      `Error in getRecipesForCuisineMatch for ${cuisineName}:`,
      error,
    );
    return [];
  }
...
function calculateFlavorProfileMatch(
  ...
  try {
    ...
    flavorKeys.forEach((key) => {
      const recipeValue = Number((recipeFlavors as any)[key] || 0);
      const cuisineValue = Number((cuisineFlavors as any)[key] || 0);
      const difference = Math.abs(recipeValue - cuisineValue);
...
export const _getCuisineElementalMatch = (
  ...
  try {
    ...
    elements.forEach((element) => {
      const cuisineValue = Number((cuisineElemental as any)[element] || 0);
      const recipeValue = Number((recipeElemental as any)[element] || 0);
...
export const _calculateCuisineSimilarity = (
  cuisine1: string,
  cuisine2: string,
): number => {
  const profile1 = getCuisineProfile(cuisine1);
  ...
  flavors.forEach((flavor) => {
    const val1 = (profile1.flavorIntensities as any)?.[flavor] || 0;
    const val2 = (profile2.flavorIntensities as any)?.[flavor] || 0;
...
export const _findRelatedRecipes = (
  recipeName: string,
  recipes: Recipe[],
  count = 3,
): Recipe[] => {

...
> = {
  aries: {
    ...
    cookingStyle:
      'Impulsive, quick and energetic cooking with minimal preparation',
    diningStyle:
      'Enthusiastic but impatient, may eat quickly and prefer casual settings',
  ...
  taurus: {
    ...
    nutritionalFocus: [
      ...
      'calcium',
      'healthy fats',
      'sustainable energy foods',
    ],
    cookingStyle:
      'Patient, traditional cooking with attention to quality ingredients',
  ...
  gemini: {
    ...
    cookingStyle:
      'Experimental and playful, often cooking multiple dishes at once',
    diningStyle:
      'Enjoys conversation while eating, prefers variety like tapas or shared plates',
  ...
  cancer: {
    ...
    nutritionalFocus: [
      'gut health',
      'immunity',
      'emotional wellbeing foods',
      'hydration',
    ],
    cookingStyle:
      'Intuitive, nurturing cooking often connected to memories and traditions',
    diningStyle:
      'Values family-style dining and emotional connections through food',
    weakness:
      'Emotional eating, overfeeding others, comfort-focused rather than nutrition',
    mealStructure:
      'Nourishing main dishes with sides that evoke memory and comfort',
  },
  leo: {
    favoredCuisines: [
      'Mediterranean',
    ...
    cookingStyle:
      'Dramatic, impressive cooking with flair and presentation focus',
    diningStyle:
      'Appreciates being the center of attention, enjoys dining as an event',
    weakness: 'Focusing too much on appearance over substance, extravagance',
    mealStructure:
      'Dramatic main course with theatrical elements and presentation',
  },
  virgo: {
    favoredCuisines: [
      'Japanese',
    ...
    cookingStyle:
      'Methodical, precise cooking following exact recipes and techniques',
    diningStyle:
      'Appreciates quality, cleanliness and order in dining experiences',
    weakness: 'Overthinking recipes, perfectionism leading to stress',
    mealStructure:
      'Carefully balanced plate with proper nutritional proportions',
  },
  libra: {
    favoredCuisines: [
      'French',
    ...
    cookingStyle:
      'Aesthetic, balanced cooking with attention to presentation and harmony',
    diningStyle: 'Values beautiful environments and well-paired flavors',
    weakness:
      'Indecision about what to cook, prioritizing appearance over flavor',
    mealStructure:
      'Perfectly balanced plate with complementary flavors and visual appeal',
  },
  scorpio: {
    favoredCuisines: [
      'Mexican',
    ...
    cookingStyle:
      'Intense, transformative cooking that changes ingredients significantly',
    diningStyle:
      'Enjoys exploring unusual flavors and hidden food establishments',
    weakness: 'Making dishes too intense or complex for others to enjoy',
    mealStructure:
      'Deeply complex dishes with mysterious elements and surprising depth',
  },
  sagittarius: {
    favoredCuisines: [
      'Indian',
    ...
    seasonalPreferences: [
      ...
      'cross-cultural seasonal celebrations',
    ],
    nutritionalFocus: [
      'diverse nutrients',
      'superfoods',
    ...
    weakness:
      'Overambitious recipes, imprecise measurements, abandoned cooking projects',
    mealStructure:
      'Globally inspired plates with educational and philosophical elements',
  },
  capricorn: {
    favoredCuisines: [
      'Traditional French',
    ...
    cookingStyle:
      'Traditional, disciplined cooking with respect for classic techniques',
    diningStyle:
      'Appreciates quality establishments with history and reputation',
    weakness: 'Resistance to innovation, overreliance on tradition',
    mealStructure:
      'Traditional courses with proper progressions and heritage elements',
  },
  aquarius: {
    favoredCuisines: [
      'Modernist',
    ...
    weakness:
      'Creating dishes that are too conceptual or detached from enjoyment',
  ...
  pisces: {
    ...
    diningStyle:
      'Enjoys atmospheric dining with emotional and spiritual elements',
    weakness:
      'Vagueness in recipes, inconsistent results, getting lost in process',
    mealStructure:
      'Fluid dining experiences that blur boundaries between courses',
  },
};
...
> = {
  ...
  pisces: {
    ...
    snacking: 0.7,
    idealEatingHours:
      'Flowing meal times that follow intuition rather than clock',
  },
};
...
export function calculateRecipeZodiacCompatibility(
  mainIngredientSign: ZodiacSign,
  preparationMethodSign: ZodiacSign
): number {
  // Get modalities
...
export const calculateIngredientPreparationCompatibility = (
  mainIngredientSign: string,
  preparationMethodSign: string
): number => {
  if (!mainIngredientSign || !preparationMethodSign) {
    return 0.5; // Default neutral compatibility for missing parameters
  }
  
  const ingredientModality = ZODIAC_MODALITIES[mainIngredientSign] || 'Cardinal';
  const preparationModality = ZODIAC_MODALITIES[preparationMethodSign] || 'Cardinal';

  // Prepare compatibility mappings
  const elementMap: Record<string, string> = {};
  Object.keys(ZODIAC_SIGNS).forEach((sign) => {
    elementMap[sign] = ZODIAC_SIGNS[sign].element;
  });

  const ingredientElement = elementMap[mainIngredientSign] || 'Fire';
  const preparationElement = elementMap[preparationMethodSign] || 'Fire';
  ...
  if (ingredientModality === preparationModality) {
    ...
    (ingredientModality === 'Cardinal' && preparationModality === 'Fixed') ||
    (ingredientModality === 'Fixed' && preparationModality === 'Mutable') ||
    (ingredientModality === 'Mutable' && preparationModality === 'Cardinal')
  ) {
    modalityScore = 0.6; // Complementary match
  } else {
    modalityScore = 0.4; // Challenging match
  }
  ...
  if (ingredientElement === preparationElement) {
    ...
    elementScore = 0.7; // Complementary match
  } else if (
    (ingredientElement === 'Fire' && preparationElement === 'Earth') ||
    (ingredientElement === 'Earth' && preparationElement === 'Fire') ||
    (ingredientElement === 'Air' && preparationElement === 'Water') ||
    (ingredientElement === 'Water' && preparationElement === 'Air')
  ) {
    elementScore = 0.4; // Challenging match
  } else {
    elementScore = 0.5; // Neutral
  }

  // Calculate overall compatibility (weighted average)
  return modalityScore * 0.4 + elementScore * 0.6;
};

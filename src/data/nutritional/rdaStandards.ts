...
export function addNutrition(
  a: NutritionalSummary,
  b: NutritionalSummary,
): NutritionalSummary {
  const result = createEmptyNutritionalSummary();
  for (const key of Object.keys(result) as Array<keyof NutritionalSummary>) {
    const va = a[key];
 ...
 */
export function calculatePersonalizedRDA(
  ...
  base.carbs = (base.calories * 0.5) / 4;
  base.fat = (base.calories * 0.3) / 9;
...
function determinePriorities(
  ...
  if (
    ...
    profile.primaryGoal === "athletic_performance"
  ) {
    priorities.find((p) => p.nutrient === "protein")!.importance = "critical";
    priorities.find((p) => p.nutrient === "protein")!.targetDirection =
      "increase";
  }

  if (profile.primaryGoal === "lose_weight") {
    priorities.find((p) => p.nutrient === "calories")!.targetDirection =
      "decrease";
    priorities.push({
      nutrient: "sugar",
      importance: "high",

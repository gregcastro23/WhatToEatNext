...
export function useRecipeValidation() {
  ...
  const analyzeComponents = (ingredients: Ingredient[]): RecipeComponents => {
    return {
      hasProtein: ingredients.some(ing => 
        ing.category === 'protein' || 
        ((ing as unknown as Record<string, unknown>).qualities as string[] || []).includes('protein-rich')
      ),
      hasVegetables: ingredients.some(ing => ing.category === 'vegetable'),
      hasGrains: ingredients.some(ing => ing.category === 'grain'),
      hasSeasonings: ingredients.some(ing => 
        ing.category === 'spice' || 
        ing.category === 'culinary_herb' || 
        ing.category === 'seasoning'
      ),
      hasLiquid: ingredients.some(ing => 
        ((ing as unknown as Record<string, unknown>).qualities as string[] || []).includes('liquid') ||
        ing.name.toLowerCase().includes('broth') ||
        ing.name.toLowerCase().includes('stock') ||
        ing.name.toLowerCase().includes('water')
      ),
      hasFat: ingredients.some(ing => 
        ing.category === 'oil' || 
        ((ing as unknown as Record<string, unknown>).qualities as string[] || []).includes('fat') ||
  ...
  const checkIncompatibilities = (ingredients: Ingredient[]): ValidationError[] => {
    ...
    const acidic = ingredients.filter(ing => 
      ((ing as unknown as Record<string, unknown>).qualities as string[] || []).includes('acidic') ||
  ...
  const generateSuggestions = (
    ...
    const weakestElement = Object.entries(elementalBalance)
    ...
    if (elementalBalance[dominantElement] > 0.5) {
      ...
      suggestions.push({
        ...
        action: { type: 'add_ingredient', target: balancingElements[dominantElement as keyof typeof balancingElements] }
      });
    }

    // Cooking method suggestions based on ingredients
    const hasDelicateIngredients = ingredients.some(ing => 
      ((ing as unknown as Record<string, unknown>).qualities as string[] || []).includes('delicate') ||
      ing.category === 'culinary_herb'
    );
  ...
  const validateRecipe = (ingredients: Ingredient[], cookingMethods?: string[]): ValidationResult => {
    const errors: ValidationError[] = [];

...
export function useRuneAgent(initial?: RuneAgentInput) {
  ...
  const generateRuneOnly = useCallback(
    async (input?: RuneAgentInput) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const generateAgentOnly = useCallback(
    async (input?: RuneAgentInput) => {
      setLoading(true);
      setError(null);
      try {
        ...
        const data =
          await runeAgentClient.generateAgentRecommendations(payload);
        setAgent(data);
        setLoading(false);
        return data;
      } catch (err) {

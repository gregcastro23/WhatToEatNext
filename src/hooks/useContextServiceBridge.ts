...
export function useAlchemicalBridge() {
  ...
  const { planetaryPositions: contextPositions, state: contextState } =
    useAlchemical();
...
export function usePlanetaryHoursBridge() {
  ...
  const [currentHour, setCurrentHour] = useState<Record<
    string,
    unknown
  > | null>(null);
    ...
    if (!isLoading && !error && astrologyService) {
      const fetchPlanetaryHoursData = async () => {
        try {
          ...
          setCurrentDay(
            typeof dayPlanet === "string" ? dayPlanet : String(dayPlanet),
          );

          const hours = await astrologyService.getDailyPlanetaryHours(
            new Date(),
          );
          ...
          if (Array.isArray(hours)) {
            ...
            (hours as unknown[]).forEach((planet, index) => {
              hoursMap.set(
                index,
                typeof planet === "string" ? planet : String(planet),
              );
            });

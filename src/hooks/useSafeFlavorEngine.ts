...
export function useSafeFlavorEngine() {
  ...
  const getProfile = useCallback((id: string): UnifiedFlavorProfile | undefined => {
  ...
  const searchProfiles = useCallback((criteria: Record<string, unknown>): UnifiedFlavorProfile[] => {
    if (!isReady) return [];
  ...
  const calculateCompatibility = useCallback((profile1: UnifiedFlavorProfile, profile2: UnifiedFlavorProfile) => {

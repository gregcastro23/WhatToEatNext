...
export function usePerformanceMonitoring(
  ...
  const generateRecommendations = useCallback(
    (currentMetrics: PerformanceMetrics): string[] => {
      ...
      Object.entries(currentMetrics.cacheStats).forEach(([cache, stats]) => {
        if (stats.hitRate < 0.8 && stats.hits + stats.misses > 10) {
          recommendations.push(
            `Improve ${cache} cache hit rate (currently ${(stats.hitRate * 100).toFixed(1)}%)`,
          );
        }
      });

      // API response times
      Object.entries(currentMetrics.apiResponseTimes).forEach(
        ([endpoint, time]) => {
          if (time > 1000) {
            recommendations.push(
              `Optimize ${endpoint} API response time (${time.toFixed(0)}ms)`,
            );
          }
        },
      );
      ...
      if (currentMetrics.memoryUsage && currentMetrics.memoryUsage > 50) {
        recommendations.push(
          `High memory usage (${currentMetrics.memoryUsage.toFixed(1)}MB)`,
        );
      }

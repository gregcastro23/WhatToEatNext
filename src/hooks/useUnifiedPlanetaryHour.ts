...
export function useUnifiedPlanetaryHour(
  options: UseUnifiedPlanetaryHourOptions = {},
) {
    ...
    (async () => {
      try {
        ...
        setState({
          ...
          source:
            String(
              process.env.NEXT_PUBLIC_PLANETARY_HOURS_BACKEND,
            ).toLowerCase() === "true" && process.env.NEXT_PUBLIC_BACKEND_URL
              ? "backend"
              : "local",
        });
      } catch (err) {

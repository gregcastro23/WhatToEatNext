...
export function useAstroTarotElementalState() {
  ...
  const combinedState: ElementalProperties = {
    Fire: Fire * 0.7 + (tarotInfluence?.Fire || 0) * 0.2,
    Water: Water * 0.7 + (tarotInfluence?.Water || 0) * 0.2,
    Earth: Earth * 0.7 + (tarotInfluence?.Earth || 0) * 0.2,
    Air: Air * 0.7 + (tarotInfluence?.Air || 0) * 0.2,
  };

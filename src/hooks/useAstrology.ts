...
export function useAstrology(options: AstrologyOptions = {}) {
  ...
  const fetchAstrologyData = useCallback(async (
    lat: number, 
    lng: number, 
    targetDate: Date = dateRef.current
  ) => {
    ...
    const requestPromise = (async () => {
      try {
        ...
        if (!response || !(response as any).ok) {
          const errorData = await (response as any)?.json?.() || { error: `API error: ${(response as any)?.status || 'Network error'}` };
          throw new Error(errorData.error || `API error: ${(response as any)?.status || 'Unknown'}`);
        }
   ...
   */
  const calculateElementalBalance = useCallback(async (
    lat?: number, 
    lng?: number, 
    targetDate: Date = dateRef.current
  ) => {

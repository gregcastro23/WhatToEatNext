...
class MultiWaveAccelerator {
  ...
  async execute() {
    ...
    console.log(
      colorize(
        "Accelerating campaign progress with multiple wave execution",
        "yellow",
      ),
    );
    console.log(colorize("=".repeat(65), "blue"));
    ...
    // Execute Wave 5
    const wave5Result = await this.executeWave(
      "Fifth",
      FIFTH_WAVE_TARGETS,
      "forEach Callbacks",
    );
    ...
    // Execute Wave 6
    const wave6Result = await this.executeWave(
      "Sixth",
      SIXTH_WAVE_TARGETS,
    ...
    await this.saveMultiWaveReport(
      initialAnyCount,
      finalAnyCount,
      totalReduction,
      grandTotalReduction,
    ...
    console.log(
      colorize(
        `✅ ${this.totalResults.totalSuccessful} additional patterns converted across ${this.totalResults.wavesExecuted} waves`,
        "green",
      ),
    );
    console.log(
      colorize(
        "🚀 Campaign showing strong acceleration - ready for continued execution!",
        "cyan",
      ),
    );
